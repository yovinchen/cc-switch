...
const FORBIDDEN_PROXY_CORE_ADAPTER_SMALL_HELPER_FACADE_MARKERS: &[&str] = &[
    ...
    "pub(crate) use crate::proxy_core::api::transforms::normalize_anthropic_tool_thinking_history",
    "pub(crate) use crate::proxy_core::api::transforms::normalize_deepseek_thinking_disabled_strip_effort",
    "pub(crate) use crate::proxy_core::api::transforms::should_normalize_anthropic_tool_thinking_history",
    ...
    "claude_env_credentials_from_settings,",
    "gemini_env_map_from_settings,",
    "openclaw_credential_parts_from_settings,",
...
fn is_allowed_http_server_test_core_import(relative: &str, code: &str) -> bool {
...
fn proxy_core_adapter_does_not_export_proxy_core_event_alias() {
    ...
    assert!(
        !host_adapter_import
            .iter()
            .any(|identifier| identifier == "ProxyCoreEvent"),
        "proxy_core_host must not import ProxyCoreEvent through proxy_core_adapter"
    );
}
...
fn proxy_core_adapter_does_not_export_response_transport_aliases() {
    ...
    for alias in [
        ...
        assert!(
            !adapter_source.contains(alias),
            "proxy_core_adapter should not expose response transport contract alias `{alias}`; callers and adapter internals should use proxy_core::api::transport directly"
        );
    }
...
fn proxy_core_adapter_does_not_export_proxy_runtime_status_alias() {
    ...
    assert!(
        !host_adapter_import
            .iter()
            .any(|identifier| identifier == "ProxyRuntimeStatus"),
        "proxy_core_host must not import ProxyRuntimeStatus through proxy_core_adapter"
    );
}
...
fn proxy_core_adapter_excludes_small_helper_facades() {
    ...
    let config_reexport_blocks: Vec<&str> = source
        ...
        .map(|tail| tail.split("};").next().unwrap_or_default())
        .collect();
    let auth_reexport_blocks: Vec<&str> = source
        .split("pub(crate) use crate::proxy_core::api::auth::{")
        .skip(1)
        .map(|tail| tail.split("};").next().unwrap_or_default())
        .collect();
    ...
    for marker in [
        ...
        }) || auth_reexport_blocks
            .iter()
            .any(|block| block.contains(marker));
        assert!(
            !reexport_marker,
            "proxy_core_adapter should not re-export adapter-only auth helper `{marker}`"
        );
    }
    for marker in [
...
fn http_server_tests_import_route_contracts_directly() {
    ...
    let source = fs::read_to_string(manifest_dir.join(relative)).expect("read HTTP server source");
    let test_imports = function_slice(
        &source,
        "#[cfg(test)]\nmod tests {",
    ...
    for forbidden in ["CurrentRouteTarget", "RouteResolveRequest"] {
        if adapter_import_identifiers
            .iter()
            .any(|identifier| identifier == forbidden)
        {
            violations.push(format!(
                "{relative} tests import route contract `{forbidden}` through proxy_core_adapter"
            ));
        }
    }
    ...
    assert!(
        violations.is_empty(),
        "HTTP server tests should not route pure route/status contracts through proxy_core_adapter:\n{}",
        violations.join("\n")
    );
}
...
fn proxy_core_adapter_delegates_codex_responses_to_chat_gate_to_core() {
    ...
    for marker in [
        ...
        assert!(
            !reexport_marker,
            "proxy_core_adapter should not re-export pure Codex Responses to Chat policy helper `{marker}`"
        );
    }
    for marker in [
...
#[test]
fn proxy_dao_imports_config_contracts_directly() {
    let manifest_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    ...
    for forbidden in [
        ...
        {
            violations.push(format!(
                "{relative} imports config contract `{forbidden}` through proxy_core_adapter"
            ));
        }
    }
...
fn proxy_core_adapter_uses_channel_key_runtime_source_for_auth_profile_lookup() {
    ...
    assert!(
        core_ports_source.contains("fn load_channel_key_candidate(")
            ...
            && core_ports_source.contains("pub const DEFAULT_CHANNEL_KEY_FAILURE_COOLDOWN_MS"),
        "ChannelKeyRuntimeSource should return the selected runtime candidate and core should own key-ref candidate selection plus failure cooldown/weighted policy"
    );
    assert!(
...
fn production_channel_health_store_reads_channel_breaker_stats_through_core_port() {
    ...
    let adapter_reexport_blocks: Vec<&str> = adapter_source
        ...
        .map(|tail| tail.split("};").next().unwrap_or_default())
        .collect();
    for marker in ["channel_breaker_stats_from_parts", "ChannelBreakerStats"] {
        let reexport_marker = adapter_source.lines().any(|line| {
            line.contains("pub(crate) use crate::proxy_core::api::") && line.contains(marker)
        }) || adapter_reexport_blocks
            .iter()
        ...
        assert!(
            !reexport_marker,
            "proxy_core_adapter should not re-export channel breaker stats helper/type `{marker}`"
        );
    }
