...
/// Host-neutral cross-request history for Codex Responses -> Chat bridging.
///
/// Chat-oriented upstreams need the original assistant tool call immediately
/// before a tool result. Codex may send only `previous_response_id` plus
/// `function_call_output`, so this state restores missing function/custom/tool
/// search calls before the request is converted to Chat messages.
#[derive(Debug, Default)]
pub struct CodexChatHistoryState {
    responses: HashMap<String, CachedResponse>,
    response_order: VecDeque<String>,
    call_index: HashMap<String, VecDeque<String>>,
}

impl CodexChatHistoryState {
    pub fn record_response(&mut self, response: &Value) -> usize {
    ...
    pub fn record_call_item(&mut self, response_id: Option<&str>, item: &Value) -> bool {
        let Some(call) = cached_call_item(item) else {
            return false;
        };

        if let Some(response_id) = response_id.filter(|value| !value.is_empty()) {
            self.insert_calls(response_id, vec![call]) > 0
        } else {
            false
        }
    }
    ...
    fn lookup(
        ...
        let previous = previous_response_id.and_then(|id| self.responses.get(id).cloned());
        let fallback = self.unique_fallback_calls(requested_call_ids, previous.as_ref());
    ...
    fn prune(&mut self) {
        while self.response_order.len() > CODEX_CHAT_HISTORY_MAX_CACHED_RESPONSES {
...
mod tests {
    ...
    fn enriches_tool_output_with_cached_function_call_from_previous_response() {
        let mut history = CodexChatHistoryState::default();
    ...
    #[test]
    fn restores_unique_call_id_without_matching_previous_response() {
        let mut history = CodexChatHistoryState::default();
        history.record_response(&json!({
            "id": "resp_1",
            "output": [{
    ...
    #[test]
    fn enriches_existing_function_call_missing_name_arguments_and_reasoning() {
        let mut history = CodexChatHistoryState::default();
        history.record_response(&json!({
            "id": "resp_1",
            "output": [{
    ...
    #[test]
    fn restores_custom_and_tool_search_calls_from_previous_response() {
        let mut history = CodexChatHistoryState::default();
        history.record_response(&json!({
            "id": "resp_1",
            "output": [
        ...
        assert_eq!(history.enrich_request(&mut request), 2);
    ...
    #[test]
    fn record_call_item_uses_current_response_id() {
        let mut history = CodexChatHistoryState::default();

        assert!(history.record_call_item(
            Some("resp_stream"),
            &json!({
                "type": "function_call",
                "call_id": "call_1",
