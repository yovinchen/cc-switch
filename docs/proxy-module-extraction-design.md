...
1172. `CcSwitchForwarderAttemptRuntimeSource` 已迁入 `proxy/host/cc_switch/forwarder_attempt_runtime_source.rs`：该 host 模块拥有 ProviderRouter-backed attempt allow、max-attempt/circuit-bypass 默认兼容决策、成功/失败健康记录和 neutral permit 释放，`proxy_core_adapter` 只保留 `ForwarderAttemptRuntimeSource` trait/input/type alias 与 ProviderRouter permit/health 副作用 helper，并在 runtime 装配中私有引用 host factory。
...
1201. `CcSwitchForwarderAttemptRuntimeSource` 的 max-attempt/circuit-bypass 决策 helper/input 已改为直接引用 `proxy_core::api::transport`；`proxy_core_adapter` 不再为这些纯 attempt runtime decision helper/input 提供生产 re-export，仍保留 ProviderRouter permit/health 副作用 helper 和 source trait/input。
...
1204. forwarder transport/response/failover/protocol/runtime source 的默认 factory 与测试 helper 已从 adapter re-export 收口为私有 use 或直接 host 模块引用：transport/response/failover scheduler/protocol state/runtime state contract 已归入 `proxy/engine/forward_pipeline.rs`，`proxy_core_adapter` 仅继续保留尚未迁出的 attempt/auth/request source trait/input/type alias，且不再暴露 cc-switch host source factory。
...
| forwarder attempt runtime source | `host/cc_switch/forwarder_attempt_runtime_source.rs` | `CcSwitchForwarderAttemptRuntimeSource` 默认实现已迁到 `proxy/host/cc_switch/forwarder_attempt_runtime_source.rs`，负责 ProviderRouter-backed attempt allow、max-attempt/circuit-bypass 默认决策、成功/失败健康记录和 neutral permit 释放；max-attempt/circuit-bypass decision helper/input 已直接引用 `proxy_core::api::transport`，adapter 仅保留 source trait/input、ProviderRouter permit/health 副作用 helper，并在 runtime 装配中私有引用 host factory |
