...
本轮继续收窄 `proxy_core_adapter` 中的 provider switch dispatch fixture：dispatch、takeover lock 与 live token label 纯策略已由 `proxy-core::ports` owning tests 覆盖，adapter 只保留 core live takeover app kind 到 CC Switch `AppType` 的兼容解析断言。
