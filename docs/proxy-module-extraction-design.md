...
本轮继续收窄 `proxy_core_adapter` 的 live token sync policy facade：`provider_settings_with_live_token_sync` 改为 adapter 私有；host live takeover 仍只消费 `sync_provider_settings_with_live_token`，preserved-auth Codex live config projection 已由 host live takeover 私有实现承接。
