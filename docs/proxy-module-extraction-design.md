...
869. Host Codex live snapshot/sync 写入参数中的 provider category 投影已拆分：legacy `ConfigService` 仍通过 `proxy_core_adapter::provider_codex_live_settings_parts` 读取 sync 所需的 settings parts，`provider/live` 的 snapshot 路径则本地调用 `proxy-core::ports::codex_live_snapshot_parts_from_settings`，不再通过 adapter 中转。
870. Host Codex provider backfill 的 switch-away wrapper 已回到 `services/provider/live.rs` owning module：provider live 本地组合 `proxy-core::ports::codex_provider_backfill_parts_from_settings`、低层 token restore、official unified-session bucket strip 和 DB `modelCatalog` overlay；`proxy_core_adapter` 只保留 `restore_codex_settings_for_provider_backfill` 供 legacy `ConfigService` backfill 复用。
...
896. Provider switch-away backfill 的 Codex settings 恢复、统一会话 bucket 清理和 DB `modelCatalog` 优先保留已改为 `services/provider/live.rs` 本地 wrapper：`provider/live` 负责 DB snippet 读取、warning 日志、Codex-only backfill projection 和诊断事件；`proxy_core_adapter` 只保留低层 `restore_codex_settings_for_provider_backfill` mutation 给 legacy `ConfigService` 调用。
