...
8. `CcSwitchAuthProvider` 作为 host-owned source 包装 provider config auth profile 到 `AuthInfo` 的默认投影，并由 adapter re-export；`CcSwitchManagedAccountRuntimeSource` 作为 host-owned source 包装 Codex/Copilot OAuth token runtime；`ForwarderAdapterContext` 作为 host-owned context 包装 provider adapter trait 调用；`CcSwitchForwarderAuthSource` 作为 host-owned source 包装默认上游 auth header 组装；token 缓存/刷新、channel-key 轮询/随机和失败回退策略继续由 forward runtime adapter/source 渐进收敛。
