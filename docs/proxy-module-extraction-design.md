...
本轮后续已把 `CcSwitchProviderSource` 的 provider list/get/current-provider DB 读取和 `ProviderSpec` 投影收敛到 host-owned source wrapper，adapter 只保留 projection helper。
...
| ProviderRouter provider source | `host/cc_switch/provider_router_provider_source.rs` | `CcSwitchProviderRouterProviderSource` 默认实现已迁到 `proxy/host/cc_switch/provider_router_provider_source.rs`，负责包装 DB provider source 与 `CcSwitchRoutePolicySource` 并生成 ProviderRouter provider/current/failover inputs；adapter 仅保留 provider/policy projection helper，默认装配由 `provider_router_sources.rs` 完成 |
...
2. `CcSwitchProviderSource` 作为 host-owned source 包装 provider/current provider、active route target 和 route candidate provider ids，并由 adapter re-export/装配。
3. `CcSwitchChannelSource` 作为 adapter-owned 端口包装 provider 主 URL、`provider_endpoints` 和未来 channel 表读取。
4. `CcSwitchRoutePolicySource` 作为 host-owned source 直接消费 core route-policy/source/result contract，包装 failover queue 到 core `RoutePolicy` 的默认桥接，并由 adapter re-export 默认 source/装配；`CcSwitchRouteResolver` 作为 host-owned resolver 直接调用 core `build_route_plan` 并仅通过 adapter helper 包装 management dry-run route response，由 adapter re-export/装配。
...
11. `CcSwitchProviderRouterProviderSource` 作为 host-owned source 包装 DB provider source 与 `CcSwitchRoutePolicySource` 到 ProviderRouter provider/current/failover inputs 的默认桥接，并由 host-owned `CcSwitchProviderRouterSources` 装配。
...
- host-owned `CcSwitchProviderSource` 与现有 DB provider 表兼容，并通过 adapter helper 暴露 current provider、active route target 和 route candidate provider ids。
...
- host-owned `CcSwitchRoutePolicySource` 可以直接依赖 core route-policy/source/result contract，读取 failover queue 并通过 adapter helper 生成 core-facing `RoutePolicy`；adapter 只保留 projection helper、默认 source re-export 和装配，不再 re-export `RoutePolicySource` port trait。
...
- host-owned `CcSwitchProviderRouterProviderSource` 可以通过 core-facing `ProviderSource` 与 `RoutePolicySource` 生成 ProviderRouter provider/current/failover inputs，adapter 只保留 projection helper。
