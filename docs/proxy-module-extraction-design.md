...
本轮继续把 `ProxyService` 中构建有效 provider settings 所需的 common config snippet 读取收敛到 host service 边界：`live_takeover.rs` 本地 helper 直接读取 DB snippet，继续复用 `proxy_core_adapter::build_effective_settings_with_common_config` 的纯 settings 合成和 core common-config warning 文案；adapter 不再持有 effective-settings DB wrapper，实际 `write_live_with_common_config` 文件写入边界留作后续独立切片。
