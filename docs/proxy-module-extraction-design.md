...
662a. `ProxyServer::start` 级 runtime smoke 已覆盖真实上游转发闭环：真实本机端口创建 materialized channel、写入 `channel-key:<keyRef>`，再通过 `/v1/messages` 客户端入口转发到本机临时 upstream，验证 channel key 生成的 `x-api-key`、public model 到 upstream model 的改写、上游 JSON 响应透传和 active route target 归因。
