...
91. `ProxyCoreResponse -> Axum Response` 与 `ProxyCoreResponse -> hyper_client::ProxyResponse` 的宿主 transport 桥接已从 `handlers` 抽到 `proxy::response_adapter`；handler 只保留协议入口编排和错误语义映射。Axum response builder 失败时的 Claude/Codex/通用 tag 诊断上下文已收敛到 adapter-owned `AxumResponseBuildErrorContext`，生产 handler/response processor 不再散落 build-error 文案。
