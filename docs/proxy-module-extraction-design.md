...
128. Codex Chat history/inline-think 兼容所需的 response item call_id 提取、空 JSON 值判断和 `<think>` 前缀拆分规则已迁入 `proxy-core::response_transform`；host common 模块仅保留 Responses item 组装 helper。
129. Gemini Native base URL/full URL 规范化、opaque relay 保护、Google/Vertex host 判定和模型 ID `models/` 前缀裁剪已整体迁入 `proxy-core::gemini_url`；host `proxy::gemini_url` 仅保留兼容 re-export。
...
131. Provider env 模型映射（haiku/sonnet/opus/fable/default）与 Claude Code 本地 `[1M]` 上游剥离规则已迁入 `proxy-core::model_mapping`；host `model_mapper` 只负责从 `Provider.settings_config.env` 投影配置并保留日志。
132. cache-sensitive JSON canonical string、tool arguments 空值/结构化规范化和短 SHA-256 trace hash 已迁入 `proxy-core::json_canonical`；host `json_canonical` 仅保留旧 import path re-export。
133. Anthropic thinking `budget_tokens` 1024 约束错误触发判定、budget snapshot/result 和请求体整流规则已迁入 `proxy-core::thinking_budget_rectifier`；host 只负责把 `RectifierConfig` 投影成 core 开关。
134. Bedrock prompt cache 断点注入、既有 cache_control TTL 升级、system string 转 block 和 thinking block 跳过规则已迁入 `proxy-core::cache_injector`；host `cache_injector` 只负责 `OptimizerConfig` 投影和原日志格式。
135. Anthropic thinking signature 错误触发判定、thinking/redacted_thinking block 清理、非 thinking signature 字段剥离和顶层 thinking 兜底删除规则已迁入 `proxy-core::thinking_rectifier`；host 只负责把 `RectifierConfig` 投影成 core 开关。
...
137. 客户端请求格式识别（Claude/Codex/OpenAI/Gemini/Gemini CLI）和 Claude/Codex session id 提取优先级已迁入 `proxy-core::session`；host `session` 只保留 `ProxySession` 生命周期对象和 UUID 生成适配。
138. 代理日志错误码契约（CB/SRV/FWD/FO/RSP/USG）已迁入 `proxy-core::log_codes`；host `log_codes` 仅保留旧 import path re-export。
139. 媒体图片块检测/替换、text-only 模型启发式、channel/provider model catalog 显式图片能力判断和 unsupported image 错误文本识别已迁入 `proxy-core::request_media`；host `media_sanitizer` 仅保留 `Provider.settings_config` 与 `ProxyError` 适配。
140. 请求体私有字段递归过滤和白名单过滤已迁入 `proxy-core::request_body`；host `body_filter` 仅保留旧 import path re-export。
141. Claude/OpenAI/Codex/Gemini usage parser 配置表、流式模型提取器绑定和 SSE usage 事件预过滤器绑定已迁入 `proxy-core::usage_config`；host `handler_config` 只保留 `AppType` handler 绑定和旧 import path re-export。
142. Codex Chat/Responses 兼容所需的 reasoning_content 拼接、Responses function_call item 组装和 namespace/reasoning 附加规则已迁入 `proxy-core::response_transform`；host `codex_chat_common` 仅保留旧 import path re-export。
143. Provider 认证 header value 构造和非法 credential 字节校验已迁入 `proxy-core::request_headers::auth_header_value`；host adapter 只保留 `ProxyCoreError::Auth` 到 `ProxyError::AuthError` 的语义适配。
