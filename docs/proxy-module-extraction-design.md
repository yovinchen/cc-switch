...
本轮继续把 Codex Chat/Responses/Compact 的 dispatch outcome match、forward error response early-return、Chat passthrough 和 Responses transform-or-passthrough 分支收敛到 `proxy::response_adapter::{codex_chat_proxy_request_to_axum_response,codex_responses_proxy_request_to_axum_response}`；`handlers` 不再直接引用 `CodexProxyDispatchResponse`、`dispatch_codex_proxy_request_to_proxy_response`、`codex_response_needs_chat_transform` 或 Codex passthrough/transform response helper。
...
1. HTTP handler 只负责触发 transport adapter 读取 body、保留路由常量/少量上下文选择并选择协议响应分支；endpoint path/query bridge、`RequestContext` 初始化、`ProxyRequest` 构造、`ProxyEngine::handle` dispatch 与 `ProxyResult` 到 HTTP response 的宿主 bridge 由 adapter 承接；Gemini handler 已先降为 `response_adapter` 编排入口调用，Codex handlers 已移除 dispatch outcome match 与 response branch 编排。
