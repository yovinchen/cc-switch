...
1385. Claude transform gate 与 streaming decision 的 provider 事实投影已迁入 host-owned `proxy/host/cc_switch/provider_projection.rs`：response adapter 与 Claude provider adapter 直接消费 `provider_needs_claude_transform` / `provider_claude_transform_streaming_decision`，`proxy_core_adapter` 不再暴露这两个 façade，边界测试防止 adapter 回填 provider transform gate/streaming decision wrapper。
