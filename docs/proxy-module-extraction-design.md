...
511. Codex Chat 上游错误体归一化后的非 JSON body 诊断文案已迁入 `proxy-core::CodexChatErrorNormalization::non_json_body_log_message`，host warn 输出和 Responses 错误体 neutral response 构造已收敛到 `proxy_core_adapter::codex_chat_error_proxy_response`；上游错误 body read 与 Axum bridge 已收敛到 `proxy::response_adapter::codex_chat_upstream_error_response_to_axum_response`。
...
540. Codex Responses handler 的 proxy request 构造与 tool context 提取已合并到 `proxy_core_adapter::codex_responses_proxy_request_from_input`，Chat error body normalization 已迁入 `proxy_core_adapter::codex_chat_error_proxy_response`，Chat upstream error body read 与 Axum bridge 已迁入 `proxy::response_adapter::codex_chat_upstream_error_response_to_axum_response`；handler 只负责 Axum transport、`RequestContext` 和 transform 调度。
...
980. `RequestForwarder` 生产路径不再直接 import 或传递 provider adapter trait；provider adapter trait 被包进 `ForwarderAdapterContext`，forwarder 只消费 context/facts，底层 `ProviderAdapter` 调用继续集中在 `proxy/host/cc_switch/provider_adapter_context.rs` 并由 `proxy_core_adapter` re-export。
...
1167. `ForwarderAdapterContext`、provider adapter trait object、provider auth info/header fallback 调用、base URL facts、transform gate/action 和 upstream URL assembly 已迁入 `proxy/host/cc_switch/provider_adapter_context.rs`：该 host 模块拥有 CC Switch provider adapter registry/trait 调用，`proxy_core_adapter` 只 re-export context/factory，剩余鉴权迁移焦点转向 token 缓存/刷新、channel-key 轮询/随机和失败回退行为。
1168. `CcSwitchForwarderAuthSource`、上游 auth header finalization、AuthProvider 显式 header/fallback 分支、managed-account auth resolution 和 Copilot auth optimization 准备已迁入 `proxy/host/cc_switch/forwarder_auth_source.rs`：该 host 模块拥有 CC Switch 默认 forwarder auth source，`proxy_core_adapter` 只保留 `ForwarderAuthSource` trait/input/type alias 并 re-export factory，剩余鉴权迁移焦点进一步收窄到 token 缓存/刷新、channel-key 轮询/随机和失败回退行为。
1169. `CcSwitchForwarderTransportSource` 已迁入 `proxy/host/cc_switch/forwarder_transport_source.rs`：该 host 模块拥有 CC Switch 默认上游 transport delegate，`proxy_core_adapter` 只保留 `ForwarderTransportSource` trait/request/type alias 并 re-export factory，forwarder 继续只依赖可替换 transport source contract。
1170. `CcSwitchForwarderResponseSource` 已迁入 `proxy/host/cc_switch/forwarder_response_source.rs`：该 host 模块拥有 CC Switch 默认响应成功就绪、流式首包预读、非成功 upstream error 投影和 channel response status mapping，`proxy_core_adapter` 只保留 `ForwarderResponseSource` trait/input/type alias 并 re-export factory。
1171. `CcSwitchForwarderProtocolStateSource` 已迁入 `proxy/host/cc_switch/forwarder_protocol_state_source.rs`：该 host 模块拥有 Gemini shadow store、Codex Chat history store、Codex Chat request enrich 和 Claude/Gemini transform replay 的默认协议状态副作用，`proxy_core_adapter` 只保留 `ForwarderProtocolStateSource` trait/input/type alias 并 re-export factory。
