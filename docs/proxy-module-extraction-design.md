...
默认 `CcSwitchProviderSource` 已迁入 `proxy/host/cc_switch/provider_source.rs`：host source 持有 DB、`ProviderRouter` 与 active-route runtime map，直接消费 core provider/source/result contract，继续调用 adapter-owned `provider_specs_from_db_source`、`current_provider_id_from_db_source`、`active_route_target_from_runtime_source` 和 `route_candidate_provider_ids_from_router_source`；adapter 保留 provider/current/active-route/route-candidate 的 shared projection helper 和默认 source type re-export/装配入口，不再 re-export `ProviderSource` port trait。
