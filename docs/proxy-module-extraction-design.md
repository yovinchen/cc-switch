...
45. `/proxy/v1/route/resolve` 的 dry-run 过滤、淘汰原因生成、兼容接口匹配和候选排序算法已迁入 `proxy-core::route_resolve::resolve_channel_route`；host `channel_routing` 缩小为 `ProxyChannelRecord -> RouteResolveChannelInput` 适配层和 `AppError` 映射，source kind 字符串使用 DAO enum 的唯一出口。
