...
451. Codex Chat 上游错误体归一化后的非 JSON body 诊断文案已迁入 `proxy-core::CodexChatErrorNormalization::non_json_body_log_message`，host warn 输出已收敛到 `proxy_core_adapter::log_codex_chat_error_normalization`；handler 只负责重建 Responses 错误体。
...
本轮继续把 Codex Chat 错误体归一化后的非 JSON warning 输出收敛到 adapter，`handlers` 不再直接调用 `non_json_body_log_message`。
