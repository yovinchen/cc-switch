...
本轮继续把 `FailoverSwitchManager` 的 proxy_config enabled 读取收敛到 `proxy_core_adapter::failover_switch_app_enabled_from_db`：manager 只保留 Tauri emit、托盘刷新和 `hot_switch_provider` 宿主副作用，配置读取失败时跳过切换的策略集中在 adapter；底层 config-result helper 已收为 adapter 私有。
