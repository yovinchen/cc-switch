...
431. attempt started/failed phase 选择已收敛到 `ForwarderRuntimeStateSource::{record_attempt_started,record_failed_attempt}`，successful attempt 的 `attempt_succeeded` 事件、active route target 写入和 `route_selected` 事件已合并为 `ForwarderRuntimeStateSource::record_successful_attempt`；host forwarder 不再导入/传递 `AttemptEventPhase`，也不再暴露 `emit_*` 事件方法或拆分成功事件与 route target 副作用顺序。
