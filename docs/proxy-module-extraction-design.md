...
11. Codex `/v1/chat/completions` handler 已改为构造 neutral `ProxyRequest` 并经 `response_adapter` 进入 `ProxyEngine::handle`；返回的 `ProxyResult` 由 `response_adapter` 回填为 host `ProxyResponse` 后再进入 passthrough/transform 分支，保留现有 usage 解析、流式处理和响应构造。
...
934. handler 请求体 Axum request parts 拆包、body collection、strict JSON 解析、Gemini empty-body null fallback 与 `stream` 标志组合已收敛到 `proxy::response_adapter::{collect_json_proxy_request,collect_json_or_null_proxy_request}`，并通过 `ParsedAxumJsonProxyRequest` 返回 method/uri/headers/extensions/body/is_stream；handler 不再直接调用 `BodyExt::collect`、`parse_json_proxy_request_body*` 或 `request_body_read_error_message`。
...
本轮继续把 Codex Responses/Compact 的 `ProxyRequest` 构造与 `CodexToolContext` 提取合并到 `proxy_core_adapter::codex_responses_proxy_request_from_input`；`handlers` 不再直接调用 `codex_tool_context_from_request`，也不需要为了迁移额外 clone 请求 body。
...
本轮继续把协议 handler 的 Axum request parts 拆包、body collection、strict JSON/Gemini null JSON 解析和 stream 标志读取收敛到 `proxy::response_adapter::{collect_json_proxy_request,collect_json_or_null_proxy_request}`；`handlers` 只解构 `ParsedAxumJsonProxyRequest`，不再直接持有 body collection 或 JSON parse helper。
