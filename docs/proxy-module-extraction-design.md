...
516. forward error 的失败请求 usage record 构造与 `UsageSink` 异步调度已从 `proxy::usage_sink_bridge::record_forward_error_usage` 继续上移到 `proxy/engine/response_pipeline.rs`，`proxy_core_adapter::record_forward_error_usage` 仅保留兼容 re-export；协议 handler 的 `ProxyEngine::handle` dispatch、`ProxyCoreError -> ProxyError` 映射与 forward error usage 记录已继续收敛到 `proxy::response_adapter::dispatch_proxy_request`。
...
本轮继续把 response processor 的 SSE usage collector、finish guard 和流式 first-byte/idle timeout loop 迁入 `proxy/engine/response_pipeline.rs::create_logged_passthrough_stream`，`proxy_core_adapter` 只保留兼容 re-export；chunk-level SSE passthrough policy 仍委托 `proxy-core::SsePassthroughStreamState`，host response processor/handlers 只传入 stream、tag、collector、timeout config 与 active-connection guard。
...
本轮继续把通用 passthrough streaming 的 upstream response receive log、usage runtime source 和 `ProxyCoreResponse` stream response 构造迁入 `proxy/engine/response_pipeline.rs`，`proxy_core_adapter::passthrough_stream_proxy_response_from_context` 仅保留兼容 re-export；host `response_processor` 只拆出 upstream status、headers 和 `bytes_stream`，再统一交给 response adapter 桥接 Axum。
本轮继续把非流式 passthrough 的 body log、usage runtime source 和 `ProxyCoreResponse` bytes response 构造迁入 `proxy/engine/response_pipeline.rs`，`proxy_core_adapter::passthrough_non_stream_proxy_response_from_context` 仅保留兼容 re-export；host `response_processor` 只保留 decoded response 读取和 Axum bridge。
本轮继续把 Claude transformed SSE 的 provider stream transform、Gemini shadow/provider/session/tool hints 注入和 transformed logged stream 包装迁入 `proxy/engine/response_pipeline.rs::claude_transformed_sse_stream_from_context`；`proxy_core_adapter` 只保留兼容 re-export，`handlers` 只保留 streaming branch 选择与 Axum response bridge。
本轮继续把 Codex Chat→Responses transformed SSE 的 history-recording stream transform 和 transformed logged stream 包装迁入 `proxy/engine/response_pipeline.rs::codex_auto_transformed_sse_stream_from_context`；`proxy_core_adapter` 只保留兼容 re-export，`handlers` 只保留 streaming branch 选择与 Axum response bridge。
本轮继续把 Claude transformed JSON 的 provider response transform、Gemini shadow/provider/session/tool hints 注入和 transformed usage runtime source 包装迁入 `proxy/engine/response_pipeline.rs::claude_transformed_json_response_from_context`；`proxy_core_adapter` 只保留兼容 re-export，非流式 upstream body read、JSON/SSE fallback parse、transform error 映射和 Axum bridge 由 `proxy::response_adapter::claude_transformed_upstream_json_response_to_axum_response` 承接。
本轮继续把 Claude transformed JSON/SSE 的 Anthropic tool schema hints 提取迁入 `proxy/engine/response_pipeline.rs::claude_transform_tool_schema_hints` 并由 `proxy_core_adapter` 兼容 re-export；`handlers` 只把原始请求 body 作为转换上下文传入，不再直接调用 `extract_anthropic_tool_schema_hints`。
本轮继续把 Codex Chat→Responses transformed JSON 的 history-recording response transform 和 transformed usage runtime source 包装迁入 `proxy/engine/response_pipeline.rs::codex_auto_transformed_json_response_from_context`；`proxy_core_adapter` 只保留兼容 re-export，非流式 upstream body read、Chat JSON/SSE fallback parse、transform error 映射和 Axum bridge 由 `proxy::response_adapter::codex_transformed_upstream_json_response_to_axum_response` 承接。
...
1156. 通用 passthrough 响应 usage 的运行态 source 选择已迁入 `proxy/engine/response_pipeline.rs`，`proxy_core_adapter::{passthrough_streaming_usage_collector,record_non_streaming_response_usage}` 仅保留兼容 re-export；host `response_processor` 不再直接读取 `ProxyState.config` 或拼装 `StreamingUsageCollectorContext` / `NonStreamingUsageRecordContext`，只负责传递 transport body、status 和 parser config。
1157. 非流式 upstream response 的整包读取、body timeout 包装和 decode bridge 已迁入 `proxy/engine/response_pipeline.rs`，`proxy_core_adapter::read_decoded_proxy_response_body` 仅保留兼容 re-export；host `handlers` 与 `response_processor` 不再直接调用 `ProxyResponse::bytes()` 或维护 `read_decoded_body` 兼容函数。
1158. Claude/Codex transformed streaming 的 usage collector 选择、timeout config 和 logged passthrough wrapper 已迁入 `proxy/engine/response_pipeline.rs`，`proxy_core_adapter::{create_claude_transformed_logged_stream,create_codex_auto_transformed_logged_stream}` 仅保留兼容 re-export；host `handlers` 只生成协议转换后的 SSE stream 并交给 response adapter。
1159. 通用 passthrough streaming 的 usage collector 选择、timeout config 和 logged passthrough wrapper 已迁入 `proxy/engine/response_pipeline.rs`，`proxy_core_adapter::create_passthrough_logged_stream` 仅保留兼容 re-export；host `response_processor` 只保留 upstream `bytes_stream` 提取、headers/status 保存和 Axum response bridge。
