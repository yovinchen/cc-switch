...
1156. 通用 passthrough 响应 usage 的运行态 source 选择已收敛到 `proxy_core_adapter::{passthrough_streaming_usage_collector,record_non_streaming_response_usage}`；host `response_processor` 不再直接读取 `ProxyState.config` 或拼装 `StreamingUsageCollectorContext` / `NonStreamingUsageRecordContext`，只负责传递 transport body、status 和 parser config。
...
| `response_processor.rs` | `engine/response_pipeline.rs` | response pipeline 实现已迁到 `proxy/engine/response_pipeline.rs`，非流式 decoded body bridge、response log event dispatch、passthrough usage runtime source 与 streaming/non-streaming passthrough response construction 已归入该模块，旧 `proxy/response_processor.rs` 仅保留兼容 re-export；后续继续把 transformed streaming usage runtime source 收敛到 pipeline 端口 |
