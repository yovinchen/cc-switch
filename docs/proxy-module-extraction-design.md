...
127. Codex Chat 中转配置中的 reasoning 请求开关判定与 effort 值映射已迁入 `proxy-core::request_body::{codex_chat_reasoning_requested,map_codex_chat_reasoning_effort}`；host 只负责根据 provider config 写入目标参数形状。
