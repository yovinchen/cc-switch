...
299. route dry-run 的 channel/model input 投影已迁入 `proxy-core::RouteResolveChannelInput::from_channel_spec`；host `channel_routing` 只负责 DB record 到 `ChannelSpec` 的边界适配和 source kind 注入。
