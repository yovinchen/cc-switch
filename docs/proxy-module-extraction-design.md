...
801. proxy service 的接管策略 provider 判定已改为消费 `proxy_core_adapter` 的高层 managed-account/Codex OAuth 事实；GitHub Copilot raw 判定已收成 adapter 私有 helper，公开入口保留 `provider_github_copilot_managed_account_id` 与 `provider_is_github_copilot_stream_check_target` 等具体业务事实，不再直接调用 `Provider::is_github_copilot()`/`Provider::is_codex_oauth()`。
