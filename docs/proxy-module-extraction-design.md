...
573. error mapper 使用的 core error/status/display contract 已直接经 `proxy_core::api::errors` 引用，response parse 与 upstream send error contract 已直接经 `proxy_core::api::transport` 引用；Codex proxy error response 等 response 构造桥接 helper 仍经 `proxy_core_adapter` 消费，host error mapper 继续负责 `ProxyError`/`ForwardError` 与 core error category 的双向桥接。
