...
813. `provider_kind_from_app_type_and_config` 的真实实现已收敛到 host-owned `proxy/host/cc_switch/provider_projection.rs`，Gemini/Claude kind 推断作为 CC Switch provider projection 事实保留：`proxy/providers` 只保留兼容 facade，`proxy_core_adapter` 不再拥有该 helper 或 provider spec conversion。
