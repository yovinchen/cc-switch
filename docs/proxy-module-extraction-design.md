...
28. `TokenUsage` 到 `UsageTokens` 的映射、success/error usage record 的 request/outbound/response model 归因规则已迁入 `proxy-core::usage`；host `usage_sink_bridge` 只保留 UUID 生成器注入、provider meta 到 `ProviderKind` 的适配和错误记录外壳。
