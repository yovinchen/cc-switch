...
540. Codex Responses handler 的 tool context 提取与 Chat error body normalization 已迁入 `proxy_core_adapter::{codex_tool_context_from_request,codex_chat_error_proxy_response}`，Chat upstream error body read 与 Axum bridge 已迁入 `proxy::response_adapter::codex_chat_upstream_error_response_to_axum_response`；handler 只负责 Axum transport、`RequestContext` 和 transform 调度。
