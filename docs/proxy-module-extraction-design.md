...
319. `/health` 与 `/status` 的 runtime response envelope 已收敛到 `proxy-core::HealthCheckRequest` / `ProxyStatusRequest` helper；HTTP handler 直接为 health 注入当前时间并返回 typed JSON，status 仍通过 `response_adapter` 读取 runtime 状态快照。
