...
539. route attempt 的 `RouteSelection -> ChannelRouteCandidate -> ResolvedChannelAttempt` 投影已改为由 `route_attempt.rs` 作为 approved routing adapter 直接消费 `proxy_core::api::routing`；channel provider override 仍经 `proxy_core_adapter` 投影宿主 `Provider` mutation，request body model override 继续由 `ForwardAttempt` 持有日志语义。
