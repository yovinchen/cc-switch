...
504. handler context 使用的 app config DTO、proxy result/services trait、response runtime policy DTO、Gemini path model 提取、Claude metadata API-format 提取与 session id 提取入口已迁入 `proxy_core_adapter`；host `RequestContext` 继续负责 Provider 选择、DB provider 回填与 `RequestForwarder` 构造。
...
574. `/proxy/v1/apps/{app}/providers` 的 provider list 投影已收窄为 `ProviderSummaryInput`：host adapter 只传入展示字段，core 继续负责 current/failover/routeCandidate 标记和 response envelope。
575. `/proxy/v1/apps/{app}/routes/current` 的 configured provider summary 投影已收窄为 `CurrentRouteProviderSummaryInput::new(id,name,category)`：host adapter 不再为了当前路由摘要构造完整 `ProviderSpec`。
...
后续管理 API 收敛应优先从 `list_proxy_providers`、`list_proxy_channels` 的 route-aware 分支、`list_proxy_groups` 和 `test_proxy_channel` 的 reachability 结果映射继续做小步迁移；这些入口的剩余逻辑主要是 response shape/投影，仍可在不引入 DB 或 Axum 依赖的前提下迁入 core helper。
