...
本轮继续把 `ProxyState` 运行态数据结构与 `proxy_engine` helper 移入 `proxy_core_adapter`：`proxy/server.rs` 只保留旧路径 re-export，HTTP listener/router 生命周期已迁到 `proxy/transport/http/server.rs`，运行态 state shape 由 adapter 维护。
