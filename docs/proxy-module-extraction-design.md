...
1169. `CcSwitchForwarderTransportSource` 已迁入 `proxy/host/cc_switch/forwarder_transport_source.rs`：该 host 模块拥有 CC Switch 默认上游 transport delegate，`proxy_core_adapter` 只保留 `ForwarderTransportSource` trait/request/type alias，forwarder/测试构造器直接引用 host factory。
