...
805. Codex provider adapter 的裸 API key getter 已收成 adapter 私有 helper，公开入口保留 `proxy_core_adapter::{provider_codex_auth_info,provider_codex_base_url}`：Codex adapter 继续负责 auth header 与 upstream URL 构造，不再在 `extract_auth`/`extract_base_url` 中直接穿透 `Provider.settings_config`。
