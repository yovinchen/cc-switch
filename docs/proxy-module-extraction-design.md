...
671. route policy 的 failover queue provider id 到 `RoutePolicy` raw contract 投影已新增 `proxy-core::route_policy_from_failover_provider_ids` 并经 host adapter `route_policy_from_failover_queue` 接入：当前默认 `CcSwitchRoutePolicySource` 已迁到 host-owned source，adapter 只保留 DB 队列错误映射与 policy 投影 helper。
...
默认 `CcSwitchRoutePolicySource` 已迁入 `proxy/host/cc_switch/route_policy_source.rs`：host source 持有 DB，直接消费 core route-policy/source/result contract，并调用 adapter-owned `route_policy_from_db_source`；adapter 继续保留 failover queue 到 core `RoutePolicy` raw contract 的投影 helper，但不再 re-export `RoutePolicySource` port trait 或 `CcSwitchRoutePolicySource` host type，默认装配由 host services / ProviderRouter sources 直接引用 host 模块。默认 `CcSwitchRouteResolver` 也已迁入 `proxy/host/cc_switch/route_resolver.rs`：host resolver 持有 `ProviderRouter`，直接导入 core `RouteResolver` / `RouteRequest` / `RoutePlan` contract 并调用 `build_route_plan`，仅通过 adapter-owned `management_route_response_from_router_source` 处理 ProviderRouter-backed management dry-run response；adapter 不再保留 route plan facade。
