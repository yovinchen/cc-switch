...
801. proxy service 的接管策略 provider 判定已改为消费 `proxy_core_adapter` 的高层 managed-account/Codex OAuth 事实；GitHub Copilot raw 判定已收成 adapter 私有 helper，公开入口保留 `provider_github_copilot_managed_account_id` 等 managed-account 业务事实，stream-check-only target 判定已回到命令层本地投影，不再通过 adapter 暴露。
