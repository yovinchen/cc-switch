...
本轮继续把 Copilot dynamic endpoint 到 base URL 的选择收敛到 `ManagedAccountRuntimeSource::resolve_copilot_dynamic_base_url_for_provider`：`RequestForwarder` 不再直接读取 runtime endpoint 或调用 dynamic base URL 决策 helper，只接收可替换 source 返回的最终上游 base URL。
本轮继续把 Copilot model vendor 读取与 Claude API format 决策收敛到 `ManagedAccountRuntimeSource::resolve_claude_api_format_for_provider`：`RequestForwarder` 不再直接读取 Copilot vendor 或调用 Claude format 决策 helper，只消费 source 返回的 API format。
本轮继续把 Copilot live model 列表读取与 model ID fallback 解析收敛到 `ManagedAccountRuntimeSource::resolve_copilot_live_model_for_provider`：`RequestForwarder` 不再直接读取 live models 或调用模型 ID 匹配 helper，只在 source 返回 resolved model 时写回请求体。
...
本轮继续把 provider/adapter request facts 收敛到 `ForwarderRequestSource`：provider base URL 提取、full-url 标记和 GitHub Copilot upstream 判定合并为 `ForwarderProviderUrlFacts`，adapter name 与 Claude adapter 判定合并为 `ForwarderAdapterFacts`，`RequestForwarder` 不再直接消费这些地址/adapter 事实 helper，后续地址级中转配置可在 request source 边界扩展 base URL/interface/model 组合。
...
本轮继续把 thinking signature/budget rectifier 的错误触发判断与请求体整流收敛到 `ForwarderRequestSource`：`RequestForwarder` 不再直接抽取 rectifier 错误文本、调用 signature/budget 判定或 mutation helper，只根据 request source 返回的 rectifier plan 编排同 provider 重试与失败归因。
