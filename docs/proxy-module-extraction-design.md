...
本轮继续把 `hot_switch_provider_inner` 的 current provider 双写收敛到 host service 边界：provider 读取、官方供应商拦截、当前目标判断和 Live 备份存在性读取仍由 `proxy_core_adapter::proxy_hot_switch_target_state_from_db` 组合，`live_takeover.rs` 本地 helper 直接执行 DB current provider 与 settings current provider 双写并保留原错误文案；adapter 不再持有 hot-switch persistence wrapper。
