...
1240. channel-key failure cooldown 已支持 per-channel health policy：core 新增 `effective_channel_key_failure_cooldown_ms`，从 `healthPolicy.keyFailureCooldownMs`/`key_failure_cooldown_ms` 解析正数毫秒值；CC Switch DB-backed `ChannelKeyRuntimeSource` 选择 key 前读取当前 channel policy，把默认 60 秒冷却窗口替换为 channel 专属窗口后再调用 core weighted selector。这样同一组 key pool 可以按每个中转地址独立配置失败回退恢复速度，不需要新增 schema 或让 DAO 解析策略 JSON。
