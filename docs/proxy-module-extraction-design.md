...
226. Claude provider adapter 的非流式 OpenAI/Responses 响应转换已直接调用 `proxy-core::{openai_chat_to_anthropic_message,openai_responses_to_anthropic_message}`；provider transform wrapper 在生产构建中只承接请求转换，响应转换 wrapper 仅保留测试路径。
