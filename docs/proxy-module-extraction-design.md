...
576. response processor 使用的非流式 decoded body bridge 与 response log event dispatch 已迁入 `proxy/engine/response_pipeline.rs`，并继续只消费 `proxy-core` 的 response body decode/log 规则；passthrough response builder、SSE scanner/usage accumulator、timeout phase、usage parser config、provider usage facts 与 streaming/non-streaming usage record helper 入口仍由 `proxy_core_adapter` 兼容承接；host 继续负责 Axum response 转换、connection guard 生命周期与 `ProxyState` 调度。
...
| `response_processor.rs` | `engine/response_pipeline.rs` | response pipeline 实现已迁到 `proxy/engine/response_pipeline.rs`，非流式 decoded body bridge 与 response log event dispatch 已归入该模块，旧 `proxy/response_processor.rs` 仅保留兼容 re-export；后续继续把 passthrough/streaming usage runtime source 收敛到 pipeline 端口 |
