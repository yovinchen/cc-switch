...
813. `provider_kind_from_app_type_and_config` 的真实实现已收敛到 `proxy_core_adapter`，Gemini kind 推断仅作为 adapter 内部实现细节保留：`proxy/providers` 只保留兼容 facade，`proxy_core_adapter` 不再反向依赖 provider adapter 模块来构造 `ProviderSpec` 或 rectifier 判定。
