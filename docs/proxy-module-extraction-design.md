...
303. `handlers` 对 OpenAI Chat/Responses 流式转换 wrapper 的生产调用已改为直接引用 `proxy-core`；host `providers::streaming` 已删除，`providers::streaming_responses` 模块仍暂作 `#[cfg(test)]` 兼容测试入口，不再作为生产 re-export 面参与编译。
