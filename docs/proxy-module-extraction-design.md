...
本轮继续把 `stop_with_restore` / `stop_with_restore_keep_state` 的 legacy active flag 清理收敛到 host service 边界：`live_takeover.rs` 本地 strict/non-strict helper 分别维护手动停止和 keep-state 退出语义；全量 Live 备份删除和全量 provider health reset 仍复用 `proxy_core_adapter::{delete_all_live_backups_in_db,clear_all_provider_health_in_db}`。
本轮继续把 `recover_from_crash` 的 legacy active flag 清理收敛到 host service 边界：`live_takeover.rs` 本地 strict helper 直接清理旧版接管状态并保留“清除接管状态失败”错误文案；全量 Live 备份删除仍复用 `proxy_core_adapter::delete_all_live_backups_in_db`，异常恢复路径继续按旧语义中断返回。
