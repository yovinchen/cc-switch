...
592. runtime route planning 的 channel 过滤、模型匹配、候选排序和 attempt plan 构造已下沉到 `proxy-core::build_route_plan`，当前默认 `CcSwitchRouteResolver` 已迁到 host-owned resolver，adapter 只保留 `RouteRequest`/management dry-run helper；host 只装配 resolver。
...
此前曾把 `CcSwitchForwardPipeline` 本身迁为 adapter-owned optional runtime wrapper，runtime 缺失判断和 host forward runtime 调度都在 adapter wrapper 内完成；该方向已由后续 host forward pipeline split 修正，wrapper 数据结构和 `ForwardPipeline` impl 回到 `proxy/host/cc_switch/forward_pipeline.rs`，adapter 只保留 `HostForwardRuntime` trait、optional-runtime dispatch helper 和 re-export。
此前曾把 `CcSwitchProxyServices` 迁为 adapter-owned generic `ProxyServices` 容器；该方向已由后续 host service container split 修正，服务容器结构、runtime-backed source 装配、test-only fixture constructor 与 `ProxyServices` impl 回到 `proxy/host/cc_switch/proxy_services.rs`，adapter 只保留 runtime/service traits、`CcSwitchProxyRuntimeServices` alias 和兼容 re-export。
...
本轮后续已把 `CcSwitchConfigSource` 的 global/app/summary/runtime 配置 DB 读取和 core DTO 投影收敛到 host-owned source wrapper，adapter 只保留 config helper 和默认 source re-export。
...
此前曾把 `CcSwitchProxyRuntime` 数据结构和具体 `CcSwitchProxyServices<CcSwitchProxyRuntime>` alias 移入 `proxy_core_adapter`，让生产版 host 文件不再定义 runtime 数据形状；该方向已由后续 host runtime split 修正，runtime 数据结构回到 host 模块，adapter 仅保留 type alias/re-export。
本轮继续把 `CcSwitchProxyRuntime` 数据结构及其 `ProxyServiceRuntimeResources` / `HostForwardRuntime` impl 从 `proxy_core_adapter` 迁回 `proxy/host/cc_switch/proxy_runtime.rs`：host 模块重新拥有 CC Switch DB、router、status、event、history、failover、current-target、runtime-source 与 Tauri handle 资源字段，adapter 只保留 runtime/service traits、forward helper、`CcSwitchProxyRuntimeServices` alias 和 runtime type re-export。
...
1178. `CcSwitchManagementAuthSource` 已迁入 `proxy/host/cc_switch/management_auth_source.rs`：该 host 模块拥有 `ProxyConfig` 读锁、`CC_SWITCH_PROXY_MANAGEMENT_TOKEN` fallback 读取和 `ManagementAuthRuntimeConfig` 组装，`proxy_core_adapter` 只保留 management auth contract 并 re-export 默认 source。
1179. `CcSwitchRuntimeStatusSource` 已迁入 `proxy/host/cc_switch/runtime_status_source.rs`：该 host 模块拥有 runtime status/start-time/current route target 读锁、uptime 回填和 active target projection，`proxy_core_adapter` 只保留 `RuntimeStatusSource` contract、test-only `DefaultRuntimeStatusSource` 和默认 source re-export。
1180. `CcSwitchClaudeDesktopGatewayAuthSource` 已迁入 `proxy/host/cc_switch/claude_desktop_gateway_auth_source.rs`：该 host 模块拥有默认 gateway token source 的 `ClaudeDesktopGatewayAuthSource` 实现，继续调用 adapter-owned DB token helper 和 core error projection，`proxy_core_adapter` 只保留 token helper/contract 并 re-export 默认 source。
...
1185. `CcSwitchRoutePolicySource` 已迁入 `proxy/host/cc_switch/route_policy_source.rs`：该 host 模块拥有 failover queue 到 `RoutePolicySource` 的默认 DB-backed source，继续复用 adapter 的 queue-to-policy projection helper，`proxy_core_adapter` 只保留 helper 和默认 source re-export/装配。
1186. `CcSwitchRouteResolver` 已迁入 `proxy/host/cc_switch/route_resolver.rs`：该 host 模块拥有默认 `RouteResolver` 实现，包装 `ProviderRouter` 并复用 adapter 的 route plan 与 management dry-run projection helper，`proxy_core_adapter` 只保留 helper 和默认 resolver re-export/装配。
...
1196. `CcSwitchChannelKeyRuntimeSource` 的 runtime candidate 投影、key-ref selection 和 test-only enabled-key selection 已改为直接引用 `proxy_core::api::management`；`proxy_core_adapter` 不再为这些纯 channel-key runtime helper/input 提供生产 re-export，仅保留 DAO 测试便利函数仍需的 test-only candidate alias。
1197. `channel_auth_profile_attempts` 的 auth-profile provider application plan 与 missing channel-key error 已改为直接引用 `proxy_core::api::{domain,auth}`；`proxy_core_adapter` 不再为这些纯 auth-profile rule helper/type 提供生产 re-export，host source 继续负责具体 `Provider` mutation 与 channel-key 值注入。
...
默认 `CcSwitchRoutePolicySource` 已迁入 `proxy/host/cc_switch/route_policy_source.rs`：host source 持有 DB 并调用 adapter-owned `route_policy_from_db_source`，adapter 继续保留 failover queue 到 core `RoutePolicy` raw contract 的投影 helper 和 re-export 装配入口。默认 `CcSwitchRouteResolver` 也已迁入 `proxy/host/cc_switch/route_resolver.rs`：host resolver 持有 `ProviderRouter` 并调用 adapter-owned `route_plan_from_request` 与 `management_route_response_from_router_source` helper，adapter 继续保留 route plan / management dry-run response 的 shared projection helper 和 re-export 装配入口。
...
默认 `CcSwitchProxyRuntime` 已迁入 `proxy/host/cc_switch/proxy_runtime.rs`：host runtime 持有 DB、`ProviderRouter`、`ProxyStatus`、current target map、event bus、shadow/history store、failover manager、runtime source 与 `AppHandle`，并在 host 模块内实现 `ProxyServiceRuntimeResources` 和 `HostForwardRuntime`；adapter 继续保留 runtime/service trait contract、`forward_proxy_request_with_cc_switch_runtime` helper、`CcSwitchProxyRuntimeServices` alias 和 runtime type re-export。

默认 `ProxyState` 数据形状已迁入 `proxy/host/cc_switch/proxy_state.rs`：host state 持有 HTTP server 实际读写的 DB、config/status/start_time、current target map、ProviderRouter、core services、Gemini shadow、Codex history 和 event bus；adapter 继续保留 `ProxyState` re-export 以及 `ProxyEngine::new` 所在的 `proxy_engine()` impl，确保生产 host 代码仍通过 adapter 构造 engine。

默认 `CcSwitchForwardPipeline` wrapper 已迁入 `proxy/host/cc_switch/forward_pipeline.rs`：host pipeline 持有可选 `CcSwitchProxyRuntime` 与 DB-backed `CcSwitchChannelKeyRuntimeSource`，实现 core `ForwardPipeline` 端口并把 channel-key runtime source 注入到 adapter-owned optional-runtime dispatch helper；adapter 继续保留 `HostForwardRuntime` trait、缺 runtime 错误包装和 `forward_with_optional_host_runtime` helper。

默认 `CcSwitchProxyServices<R>` 已迁入 `proxy/host/cc_switch/proxy_services.rs`：host services container 直接引用 core `ProxyServices` 与 config/provider/channel/route/health/reachability/auth/model catalog/runtime status/usage/event/forward pipeline port traits，持有对应端口默认实现，并保留 test-only no-runtime fixture；adapter 继续保留 `ProxyServiceRuntimeResources`、`HostForwardRuntime`、`CcSwitchProxyRuntimeServices` alias 和兼容 re-export。
...
| management auth source | `host/cc_switch/management_auth_source.rs` | `CcSwitchManagementAuthSource` 默认实现已迁到 `proxy/host/cc_switch/management_auth_source.rs`，负责读取 `ProxyConfig` 与 `CC_SWITCH_PROXY_MANAGEMENT_TOKEN` fallback 并组装 `ManagementAuthRuntimeConfig`；adapter 仅保留 contract 并 re-export source |
| runtime status source | `host/cc_switch/runtime_status_source.rs` | `CcSwitchRuntimeStatusSource` 默认实现已迁到 `proxy/host/cc_switch/runtime_status_source.rs`，负责读取 runtime status、start time 和 current route targets 并投影 uptime/active target；adapter 仅保留 contract、test-only default source 与 re-export |
| Claude Desktop gateway auth source | `host/cc_switch/claude_desktop_gateway_auth_source.rs` | `CcSwitchClaudeDesktopGatewayAuthSource` 默认实现已迁到 `proxy/host/cc_switch/claude_desktop_gateway_auth_source.rs`，负责通过 adapter-owned DB token helper 加载 gateway bearer token 并映射 core auth 错误；adapter 仅保留 token helper/contract 并 re-export source |
...
| route resolver | `host/cc_switch/route_resolver.rs` | `CcSwitchRouteResolver` 默认实现已迁到 `proxy/host/cc_switch/route_resolver.rs`，负责包装 core route plan helper 与 ProviderRouter-backed management dry-run route response；adapter 仅保留 route projection helper、factory 装配和 re-export |
...
1. `CcSwitchConfigSource` 作为 host-owned 端口包装现有 DB/settings 读取，并由 adapter re-export/装配；config source/projection helper 继续留在 adapter。
...
4. `CcSwitchRoutePolicySource` 作为 host-owned source 包装 failover queue 到 core `RoutePolicy` 的默认桥接，并由 adapter re-export/装配；`CcSwitchRouteResolver` 作为 host-owned resolver 包装 core route plan 与 management dry-run route resolution，并由 adapter re-export/装配。
...
8. `CcSwitchManagementAuthSource` 作为 host-owned source 包装 `ProxyConfig` 读锁、管理 token env fallback 和 runtime auth config 组装，并由 adapter re-export。
9. `CcSwitchRuntimeStatusSource` 作为 host-owned source 包装 runtime status/start time/current route target 读取与 uptime/active target projection，并由 adapter re-export；test-only default source 留在 adapter。
...
- host-owned `CcSwitchConfigSource` 与现有 DB/settings 配置兼容，并通过 adapter helper 暴露 app catalog、global/app/app-summary/runtime config。
...
- host-owned `CcSwitchManagementAuthSource` 可以读取热更新后的 `ProxyConfig` 与 env fallback，adapter 只 re-export source。
- host-owned `CcSwitchRuntimeStatusSource` 可以读取 runtime status/start time/current route target，并生成与 `ProxyEngine::runtime_status` 一致的 uptime/active target。
...
- host-owned `CcSwitchRoutePolicySource` 可以读取 failover queue 并通过 adapter helper 生成 core-facing `RoutePolicy`，adapter 只保留 projection helper 和装配。
- host-owned `CcSwitchRouteResolver` 可以通过 adapter helper 生成 route plan 与 management dry-run route response，adapter 只保留 projection helper 和装配。
