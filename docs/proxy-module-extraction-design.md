...
1181. `CcSwitchProviderRouterChannelSource` 已迁入 `proxy/host/cc_switch/provider_router_channel_source.rs`：该 host 模块拥有 ProviderRouter 的 channel route input source，包装 `CcSwitchChannelSource` 并复用 adapter 的 route-input projection helper，`proxy_core_adapter` 只保留 helper。
...
- host-owned `CcSwitchProviderRouterChannelSource` 可以通过 core-facing `ChannelSource` 生成 ProviderRouter route inputs，adapter 只保留 projection helper。
