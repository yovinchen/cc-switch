...
本轮继续把 `CcSwitchProxyRuntime` 数据结构及其 `ProxyServiceRuntimeResources` / `HostForwardRuntime` contract 与 impl 从 `proxy_core_adapter` 迁回 `proxy/host/cc_switch/proxy_runtime.rs`：host 模块重新拥有 CC Switch DB、router、status、event、history、failover、current-target、runtime-source 与 Tauri handle 资源字段，adapter 只保留 forwarding bridge helper、`CcSwitchProxyRuntimeServices` alias 和 runtime type re-export。
