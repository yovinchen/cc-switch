...
744. channel reachability probe 已新增 `ChannelReachabilityProbe` core 端口，probe request 的 app 解析、provider 缺失、DB provider/config 读取、`StreamCheckService` 副作用和 probe 错误包装均由 `proxy/host/cc_switch/channel_reachability_probe.rs` 的 DB-backed `CcSwitchChannelReachabilityProbe` 包装；`proxy_core_adapter` 只 re-export/装配 probe，`proxy_core_host` 只装配 services，core 负责 channel test 编排、preflight failure 和最终 response envelope。
