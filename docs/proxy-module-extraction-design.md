...
1. HTTP handler 只负责触发 transport adapter 读取 body、鉴权、事件流入口转发和管理 API path/query/state 转发；协议入口的 app/tag/prefix/endpoint metadata、endpoint path/query bridge、`RequestContext` 初始化、`ProxyRequest` 构造、`ProxyEngine::handle` dispatch 与 `ProxyResult` 到 HTTP response 的宿主 bridge 由 adapter 承接；Claude/Codex/Gemini protocol handlers 已先降为 `response_adapter` 编排入口调用，`/proxy/v1/events` 的 SSE transport bridge 与 apps/providers/app-models/client-model-catalog 读查询的 typed JSON bridge 也由 `response_adapter` 承接。
