...
87. Codex Chat 上游错误体的 JSON/文本解析、非 JSON 预览截断和 Responses 风格 error envelope 归一化已迁入 `proxy-core::codex_error::normalize_codex_chat_error_body`；非 JSON warning、neutral error response 构造、上游错误 body read 与 Axum response bridge 由 `proxy::response_adapter::codex_chat_upstream_error_response_to_axum_response` 承接，host handler 只负责判定非成功 Chat 响应需要转换。
