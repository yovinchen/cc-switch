...
542. `provider:{app}:{providerId}` auth profile 已接入 host runtime：materialized channel 仍使用自身 provider/channel 的 base URL、interface、模型映射、熔断与健康统计，但认证提取和托管账号 token 选择会读取被引用 provider 的配置；跨 app、缺失 provider 与 `channel-key:*` 等未实现 profile 保持 fallback，不误覆盖认证来源。
