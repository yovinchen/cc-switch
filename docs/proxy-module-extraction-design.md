...
934. handler 请求体 Axum request parts 拆包、body collection、strict JSON 解析、Gemini empty-body null fallback、`stream` 标志组合、path/query 到 proxy endpoint 的 bridge，以及 app/interface/body/headers/extensions 到 `ProxyRequest` 的组装，已收敛到 `proxy::response_adapter::{collect_json_proxy_request,collect_json_or_null_proxy_request,endpoint_from_uri}` 和 `ParsedAxumJsonProxyRequest::{endpoint_from_request_uri_stripping_prefix,endpoint_for_path,into_*_proxy_request}`；handler 不再直接调用 `BodyExt::collect`、`parse_json_proxy_request_body*`、`append_query_to_endpoint_path`、`strip_endpoint_prefix`、`JsonProxyRequestInput`、`InterfaceKind::*` 或 `request_body_read_error_message`。
...
本轮继续把协议 handler 的 Axum request parts 拆包、body collection、strict JSON/Gemini null JSON 解析、stream 标志读取和 endpoint path/query bridge 收敛到 `proxy::response_adapter::{collect_json_proxy_request,collect_json_or_null_proxy_request,endpoint_from_uri}` 与 `ParsedAxumJsonProxyRequest` endpoint helper；`handlers` 只保留 parsed request 用于 context 选择与协议 request 构造，不再直接持有 body collection、JSON parse helper、endpoint bridge 或 proxy request DTO 组装。
...
1. HTTP handler 只负责触发 transport adapter 读取 body、保留路由常量/少量上下文选择并选择协议响应分支；endpoint path/query bridge、`ProxyRequest` 构造、`ProxyEngine::handle` dispatch 与 `ProxyResult` 到 HTTP response 的宿主 bridge 由 adapter 承接。
