...
本轮继续收窄 `proxy_core_adapter` 的 live/preserve-auth policy facade：`provider_settings_with_live_token_sync`、`preserve_codex_oauth_auth_in_backup_if_present`、`codex_preserved_auth_live_config_text_if_proxy_placeholder` 和 `codex_preserved_auth_live_config_text_for_policy` 都改为 adapter 私有；host live takeover 仍只消费 `sync_provider_settings_with_live_token` 与 configured-policy 入口。
