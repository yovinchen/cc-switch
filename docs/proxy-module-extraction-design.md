...
1184. `CcSwitchProviderRouterProviderSource` 已迁入 `proxy/host/cc_switch/provider_router_provider_source.rs`：该 host 模块拥有 ProviderRouter 的 provider id/current provider/failover source，直接引用 host `CcSwitchRoutePolicySource` 与 core `AppKind` / `ProviderSpec` / `ProviderSource` / `ProxyCoreResult` contract，并复用 adapter 的 provider/policy projection helper；`proxy_core_adapter` 不再为该模块 re-export `ProviderSource` 或 `CcSwitchRoutePolicySource`，只保留 helper。
...
| ProviderRouter provider source | `host/cc_switch/provider_router_provider_source.rs` | `CcSwitchProviderRouterProviderSource` 默认实现已迁到 `proxy/host/cc_switch/provider_router_provider_source.rs`，直接引用 core provider/source/result contract 与 host `CcSwitchRoutePolicySource`，负责包装 DB provider source 并生成 ProviderRouter provider/current/failover inputs；adapter 仅保留 provider/policy projection helper，不再 re-export `ProviderSource` 或 `CcSwitchRoutePolicySource`，默认装配由 `provider_router_sources.rs` 完成 |
...
11. `CcSwitchProviderRouterProviderSource` 作为 host-owned source 直接消费 core provider/source/result contract，并直接引用 host `CcSwitchRoutePolicySource`，包装 DB provider source 到 ProviderRouter provider/current/failover inputs 的默认桥接，由 host-owned `CcSwitchProviderRouterSources` 装配。
...
- host-owned `CcSwitchProviderRouterProviderSource` 可以通过 core-facing `ProviderSource` 与 `RoutePolicySource` 生成 ProviderRouter provider/current/failover inputs，并直接引用 host `CcSwitchRoutePolicySource`；adapter 只保留 projection helper，不再 re-export `ProviderSource` 或 `CcSwitchRoutePolicySource`。
