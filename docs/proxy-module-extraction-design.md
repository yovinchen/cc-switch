...
本轮继续收窄 `proxy_core_adapter` 的 provider/Claude Desktop 内层 projection facade：`apply_provider_model_mapping_from_provider`、`provider_claude_models_are_claude_safe`、`provider_claude_desktop_direct_inference_model_specs` 和 `provider_claude_desktop_proxy_provider_validation` 都改为 adapter 私有；host/command 调用方仍只消费 forward mapping、import decision、direct validation 和 gateway/profile 等高层入口。
...
1155. Claude Desktop forward request body mapping 已改为在 `proxy_core_adapter::apply_forward_request_model_mapping_from_provider` 内直接调用 `provider_claude_desktop_proxy_request_body`；production forward request path 不再回调 host `claude_desktop_config::map_proxy_request_model`，host wrapper 仅保留给本模块单测使用，adapter 内部负责把 route/body issue 投影为 `ProxyError::InvalidRequest` 文案，边界测试防止 forward mapping 重新形成 adapter 到 host config 的反向依赖。
