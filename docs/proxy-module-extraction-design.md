...
本轮继续把 Copilot live model 列表读取、model ID fallback 解析和请求体 `model` 写回收敛到 `ManagedAccountRuntimeSource::apply_copilot_live_model_for_provider`：`RequestForwarder` 不再直接读取 live models、调用模型 ID 匹配 helper 或维护 JSON 写回细节，只触发 source 行为。
