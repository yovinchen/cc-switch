...
821. OpenClaw live 写入 fallback 的 raw provider shape 检测与 write plan 已收成 adapter 私有 helper，公开入口保留 `proxy_core_adapter::{provider_openclaw_live_write_projection,provider_from_openclaw_live_config}`：provider live 继续负责 typed parse/raw write 策略，`baseUrl`/`api`/`models` 字段存在性规则集中在 core/domain 与 adapter。
...
826. provider live snapshot 的 OpenCode provider fragment 投影与 write plan 已收成 adapter 私有 helper，公开入口保留 `proxy_core_adapter::{provider_opencode_live_write_projection,provider_from_opencode_live_config}`：live 写入继续负责 typed/raw 写入和日志，整份 opencode config 到单 provider fragment 的兼容提取集中在 adapter。
...
899. OpenCode additive provider live 写入计划已收成 adapter 私有实现，公开入口保留 `proxy_core_adapter::provider_opencode_live_write_projection`：`provider/live` 继续负责 `opencode_config` typed/raw 写入与日志，完整 config fragment 提取、typed parse、raw fallback 与 invalid 分类集中在 adapter。
900. OpenClaw additive provider live 写入计划已收成 adapter 私有实现，公开入口保留 `proxy_core_adapter::provider_openclaw_live_write_projection`：`provider/live` 继续负责 `openclaw_config` typed/raw 写入与日志，typed parse、raw fallback 与 invalid 分类集中在 adapter。
