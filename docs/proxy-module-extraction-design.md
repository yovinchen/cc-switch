...
576. response processor 使用的非流式 decoded body bridge、response log event dispatch、passthrough streaming usage runtime source、非流式 usage runtime source、transformed streaming usage runtime source、transformed non-streaming usage runtime source、streaming/non-streaming passthrough response construction、Claude/Codex transformed SSE stream wrappers、Claude/Codex transformed JSON wrappers 与 shared logged-stream runtime loop 已迁入 `proxy/engine/response_pipeline.rs`，并继续只消费 `proxy-core` 的 response body decode/log/SSE passthrough policy 规则、adapter-owned usage record helper、provider transform/history helper 与 core response builder；SSE scanner/usage accumulator、provider usage facts 与 streaming/non-streaming usage record 构造入口仍由 `proxy_core_adapter` 兼容承接；host 继续负责 Axum response 转换、connection guard 生命周期与 `ProxyState` 调度。
...
| `response_processor.rs` | `engine/response_pipeline.rs` | response pipeline 实现已迁到 `proxy/engine/response_pipeline.rs`，非流式 decoded body bridge、response log event dispatch、passthrough usage runtime source、transformed streaming/non-streaming usage runtime source、streaming/non-streaming passthrough response construction、Claude/Codex transformed SSE stream wrappers、Claude/Codex transformed JSON wrappers 与 shared logged-stream runtime loop 已归入该模块，旧 `proxy/response_processor.rs` 仅保留兼容 re-export；后续继续把 provider usage facts 与底层 usage record helper 收敛到 pipeline/core 边界 |
