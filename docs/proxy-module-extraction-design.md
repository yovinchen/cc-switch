...
默认 `CcSwitchChannelHealthStore` 已迁入 `proxy/host/cc_switch/channel_health_store.rs`：host store 持有 DB 与 `ProviderRouter`，继续调用 adapter-owned `record_channel_attempt_in_db_source`、`reset_channel_health_with_router_source` 和 `channel_breaker_stats_with_router_source`，adapter 保留 channel health attempt/reset/stats fact 的 shared projection helper 和 re-export 装配入口。
...
默认 `CcSwitchProxyServices<R>` 已迁入 `proxy/host/cc_switch/proxy_services.rs`：host services container 持有 config/provider/channel/route/health/reachability/auth/model catalog/runtime status/usage/event/forward pipeline 端口默认实现，并保留 test-only no-runtime fixture；adapter 继续保留 `ProxyServiceRuntimeResources`、`HostForwardRuntime`、`CcSwitchProxyRuntimeServices` alias 和兼容 re-export。
