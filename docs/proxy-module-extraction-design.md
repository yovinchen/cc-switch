...
本轮继续移除 `proxy_core_adapter` 对 client model catalog source selection helper/type 的 re-export：`client_model_catalog_from_app_source` 仍在 adapter 内部调用 `proxy_core::api::model_catalog::{client_model_catalog_source_for_app, ClientModelCatalogSource}`，但这些纯选择契约不再作为 adapter 出口。
