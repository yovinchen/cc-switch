...
798. saved usage script 查询服务的 usage script 读取已回到 `Provider::usage_script()`，script/provider credential 覆盖策略已改为直接调用 `proxy_core::api::ports::usage_script_credentials_from_parts`；custom endpoints 服务的列表排序与 last-used mutation 已回到 `Provider::{custom_endpoint_list,mark_custom_endpoint_last_used}`，URL 归一化仍走 `proxy_core_adapter::normalize_custom_endpoint_url` 和 `proxy_core::api::management::custom_endpoint_url_key`：`services/provider` 继续负责执行脚本、credential fallback、DB 读写与结果格式化，普通 provider meta accessor 不再经 adapter 中转。
