...
264c. Claude Desktop 配置测试不再通过 `proxy::response_adapter` 间接取得 model-list response DTO，改为直接引用 `proxy-core::api::auth::ClaudeDesktopModelListResponse`；HTTP handler 签名仍按既有边界经 `response_adapter` 取得 DTO，避免 handler 直接依赖 `proxy-core`。
...
732. health/status/Claude Desktop model-list/app list 基础管理路由已继续收敛：health 只在 HTTP handler 内消费 core `HealthCheckRequest` 并注入当前时间，status 的 request/response helper 与 runtime status engine 调用已由 HTTP handler 直接承接且仍禁止直接读取 runtime status lock，Claude Desktop model-list typed JSON bridge 仍由 `proxy::response_adapter` 承接，app list 走 `ProxyEngine::app_list_response`，handler 不再直接调用这些 response source DTO，只保留 Axum 提取和 Claude Desktop gateway auth。
