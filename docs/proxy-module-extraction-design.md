...
本轮继续把 forwarder 的 media prevention text-only provider 图片替换 fact 收敛到 `proxy-core::request_media::apply_forwarder_media_prevention_from_facts`；media 预防式降级不再直接消费 provider 级模型能力投影 helper，相关策略 fixture 由 `proxy-core::request_media` owning tests 覆盖，adapter 不再保留重复测试。Codex Chat streaming decision 的 SSE/fallback 策略 fixture 也由 `proxy-core::response_transform` owning tests 覆盖，adapter 不再重复直接调用 core policy。
