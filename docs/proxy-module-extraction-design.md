...
本轮继续把 Codex Chat->Responses transform 的 streaming/错标 SSE 聚合决策收敛到 `proxy-core::response_transform::codex_chat_transform_streaming_decision`，host adapter 只传入请求 stream flag 和响应 header，handler 不再直接判断上游 SSE header 或指定 ChatCompletions 聚合策略。
