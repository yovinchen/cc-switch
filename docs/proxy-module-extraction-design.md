...
本轮继续移除 `proxy_core_adapter` 对 Claude takeover 纯策略 helper 的 re-export：`proxy/host/cc_switch/live_takeover.rs` 直接引用 `proxy-core::ports::{apply_claude_takeover_fields_with_policy,ClaudeTakeoverAuthPolicy}`，adapter 只保留 `Provider` 到 `ClaudeTakeoverProviderFacts` 的宿主投影入口。
...
1265. Claude takeover model/display helper 的 test-only adapter re-export 已删除：adapter 自测直接调用 `proxy-core::api::model_catalog` 与 `proxy-core::api::ports::apply_claude_takeover_fields_with_policy_and_models` owning helper；live takeover 生产路径直接引用 `proxy-core::ports::{apply_claude_takeover_fields_with_policy,ClaudeTakeoverAuthPolicy}`，`proxy_core_adapter` 仅保留 `Provider` facts 投影 wrapper；边界测试防止 pure model/policy helper 重新暴露。
