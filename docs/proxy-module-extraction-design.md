...
256. `/proxy/v1/channels` POST、`/proxy/v1/channels/{channel_id}` GET/PATCH 的单条 channel record response contract 已迁入透明 `proxy-core::ChannelRecordResponse<T>`；host 继续负责 DB CRUD，但外部 JSON 仍保持原始 channel record 形状。
...
258. `/proxy/v1/status` 的运行态 status response contract 已迁入透明 `proxy-core::ProxyStatusResponse<T>`；host 继续持有运行态字段，外部 JSON 仍保持原始 `ProxyStatus` 形状。
