...
| `response_processor.rs` | `engine/response_pipeline.rs` | response pipeline 实现已迁到 `proxy/engine/response_pipeline.rs`，非流式 decoded body bridge、response log event dispatch、passthrough usage runtime source 与非流式 passthrough response construction 已归入该模块，旧 `proxy/response_processor.rs` 仅保留兼容 re-export；后续继续把 passthrough streaming response construction 与 transformed streaming usage runtime source 收敛到 pipeline 端口 |
