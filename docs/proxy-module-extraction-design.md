...
1. HTTP handler 只负责触发 transport adapter 读取 body、保留路由常量/少量上下文选择并选择协议响应分支；endpoint path/query bridge、`RequestContext` 初始化、`ProxyRequest` 构造、`ProxyEngine::handle` dispatch 与 `ProxyResult` 到 HTTP response 的宿主 bridge 由 adapter 承接；Gemini 与 Codex handlers 已先降为 `response_adapter` 编排入口调用。
