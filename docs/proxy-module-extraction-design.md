...
565. proxy server 使用的 runtime config/status/info、current route target、Gemini shadow store、ProxyEngine、route resolve request 与 server log code 入口已迁入 `proxy_core_adapter`；server 启动/停止时的 started/stopped event、监听端口记录、runtime status/start_time 写入和 `ProxyServerInfo` 组装继续由 adapter 组合 helper 承接，HTTP lifecycle 具体实现已回到 `proxy/transport/http/server.rs`：Axum route tree、管理 API middleware、body limit、listener 地址解析/bind/local address/error mapping、Hyper HTTP/1.1 accept loop、raw header-case capture、connection serving、accept-loop stop wait 超时、日志和 `StopFailed`/`StopTimeout` 映射、shutdown sender/server handle storage、AlreadyRunning/NotRunning gate、start/stop 编排均由该 transport 模块拥有；`proxy_core_adapter` 仅保留这些 HTTP lifecycle helper 的兼容 re-export 和运行态副作用 helper，`ProxyServer` 实现已迁到 `proxy/transport/http/server.rs`，HTTP handlers 实现已迁到 `proxy/transport/http/handlers.rs`，旧 `proxy/server.rs` 与 `proxy/handlers.rs` 已删除，后续重点转为 handler 业务处理继续下沉 engine。
...
| 现文件 | 目标位置 | 处理方式 |
| --- | --- | --- |
| `server.rs` | `transport/http/server.rs` | `ProxyServer` 实现已迁到 `proxy/transport/http/server.rs`，旧 `proxy/server.rs` 已删除；listener binding、route assembly、Hyper accept loop/header-case capture、stop wait/error mapping、shutdown sender/server handle storage、running-state gate、start 编排与 stop 编排已由 `proxy/transport/http/server.rs` 拥有，`proxy_core_adapter` 只保留 runtime side effects 与 lifecycle helper 兼容 re-export |
