...
267. Tauri settings 中与代理请求处理相关的 `RectifierConfig`、`OptimizerConfig`、`CopilotOptimizerConfig` 已迁入 `proxy-core`，并保留到 thinking/cache/rectifier core config 的投影 helper；host settings DAO 继续负责 settings 表 JSON 读写，`LogConfig` 仍留在 host 因其直接依赖应用日志初始化；`RectifierConfig` 与 `OptimizerConfig` 调用方已直接引用 core DTO。
