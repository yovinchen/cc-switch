...
228. Claude provider adapter 的 Anthropic->OpenAI Chat/Responses 请求转换已直接调用 `proxy-core::{anthropic_to_openai_chat_request,anthropic_to_openai_responses_request}`；OpenAI Chat provider transform wrapper 已删除，Responses wrapper 暂保留为测试入口。
229. `providers::transform_responses` 模块声明仍限制为 `#[cfg(test)]`；生产构建不再编译 OpenAI Responses 的旧 provider facade，协议回归继续向 core 测试迁移。
