...
871. Provider live 删除目标映射已改为 `proxy_core_adapter::{provider_live_removal_target,ProviderLiveRemovalTarget}`：`ProviderService::delete/remove_from_live_config/switch_normal` 继续负责 live 存在性检查、OMO current 处理、DB 删除/保存和失败回滚，OpenCode/OpenClaw/Hermes 分别映射到对应 live 删除后端、非 additive live config app 不映射删除目标的判定集中在 adapter。
