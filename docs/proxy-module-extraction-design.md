...
本轮继续把 provider URL facts 收敛到 `ForwarderRequestSource`：provider base URL 提取、full-url 标记和 GitHub Copilot upstream 判定合并为 `ForwarderProviderUrlFacts`，`RequestForwarder` 不再直接消费这些地址事实 helper，后续地址级中转配置可在 request source 边界扩展 base URL/interface/model 组合。
