...
659. `ProxyRequest` 到 host forward runtime 输入的 app/body/method/header 投影已移入 `proxy_core_adapter::forward_runtime_request_from_proxy_request`：`proxy_core_host` 不再直接拆 `ProxyRequest` 或调用 `ProxyBody::into_json`。
...
本轮还把 `ProxyRequest` 到 host forward 输入的投影收敛到 adapter，host forward runtime 不再直接处理 app kind 解析和 body JSON 转换。
