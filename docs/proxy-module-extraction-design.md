...
默认 `CcSwitchRoutePolicySource` 已迁入 `proxy/host/cc_switch/route_policy_source.rs`：host source 持有 DB 并调用 adapter-owned `route_policy_from_db_source`，adapter 继续保留 failover queue 到 core `RoutePolicy` raw contract 的投影 helper 和 re-export 装配入口。
...
4. `CcSwitchRoutePolicySource` 作为 host-owned source 包装 failover queue 到 core `RoutePolicy` 的默认桥接，并由 adapter re-export/装配；`CcSwitchRouteResolver` 包装 core route plan 与 management dry-run route resolution。
