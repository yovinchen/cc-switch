...
本轮继续把批量 stream check 的 `proxy_targets_only` 过滤 DB 读取移回命令边界：命令层读取当前 provider 与故障转移队列，仍把集合归并交给 `proxy_core_adapter::stream_check_proxy_target_ids_from_sources` 这个纯 helper；adapter 不再持有 `stream_check_proxy_target_ids_from_db` 这种命令专用 DB wrapper。
