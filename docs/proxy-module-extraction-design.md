...
536. core `UsageRecord` 到 host `RequestLog` 的 usage/cost/request-id/missing-pricing 投影已迁入 `proxy_core_adapter::{usage_record_pricing_model,usage_record_to_request_log}`；host-owned `CcSwitchUsageSink` 负责 pricing DB 查询、warning 输出和日志落库，adapter 仅 re-export 默认 source。
...
713. UsageSink 的 provider/app 计费配置 lookup 输入已移入 `proxy_core_adapter::usage_pricing_config_lookup_from_record`：host-owned `CcSwitchUsageSink` 负责调用 usage logger 读取配置、定价和落库，`proxy_core_host` 只装配 sink。
...
741. `ProxyResult` 回填 `RequestContext` 的 outbound model、usage route context 与 selected provider hydration 已迁回 `proxy/engine/context.rs` owning module：context 直接消费 core route/usage/error helper 与 host `ForwardAttempt` 映射；`UsageSink` 的 missing pricing warning emission 已收敛到 `proxy_core_adapter::log_usage_request_projection_warnings`；host sink 只保留定价查询和落库字段赋值。
...
1177. `CcSwitchUsageSink` 已迁入 `proxy/host/cc_switch/database_usage_sink.rs`：该 host 模块拥有 `UsageLogger`、pricing lookup、缺失定价 warning、`RequestLog` 写入和 DB 持久化副作用，并直接引用 core `UsageSink` / `ProxyCoreResult` / usage DTO contract；`proxy_core_adapter` 不再 re-export `UsageSink`，只保留 usage projection helper 和默认 usage sink source re-export。
