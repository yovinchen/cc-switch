...
Channel 管理 API 的部分 response envelope 也已开始收敛到 core：`ChannelHealthResetResponse` 表示 breaker reset 结果，`ChannelDeleteResponse` 表示 channel 删除结果，`ChannelModelsResponse<T>` 表示 channel model 列表/替换结果。host handler 仍负责 HTTP path/query 解析、DB CRUD 和 host record 类型，但外部 response shape 不再散落在 `json!` 调用中。
