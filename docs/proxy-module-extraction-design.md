...
本轮继续移除 `proxy_core_adapter` 对 server stopped/status info helper 的 re-export：`live_takeover` 直接从 `proxy_core::api::ports` 导入 `proxy_server_info_from_parts`，adapter 内部私有调用 `proxy_server_info_from_parts` 与 `record_proxy_server_stopped_status`，不再把这些纯 runtime DTO/status helper 暴露为 crate-wide 出口。
...
1374. forward runtime status 与 current-route target 的低层 mutation/projection helper 已收窄为 `proxy_core_adapter` 私有函数；外部模块继续只调用语义化 runtime-source API，避免把 `record_forward_*_status`、`record_proxy_server_started_status` 和 `current_route_target_from_*` 这类内部组合 helper 当作 adapter 合约。
