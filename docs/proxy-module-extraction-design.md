...
| `provider_router.rs` | `engine/routing.rs` | `ProviderRouter` 与 routing source/store traits 已迁到 `proxy/engine/routing.rs`，旧 `proxy/provider_router.rs` 仅保留兼容 re-export；当前已通过 router 端 provider/channel/config/health 四个 focused port 注入 source/store，channel route 输入已切到 core `RouteResolveChannelInput`，config/channel source 与 health store 默认实现已迁到 `host/cc_switch/provider_router_*`，并分别通过 `CcSwitchConfigSource` / `CcSwitchChannelSource` / core attempt facts 对齐 core `ProxyConfigSource` / `ChannelSource` / `ProviderHealthStore`，provider/route-policy source 仍由 adapter glue 对齐 core `ProviderSource` / `RoutePolicySource`；live breaker map 已由 `ProviderRoutingCircuitRuntime` 承接，下一步评估是否可在不引入 router/store 环的前提下完全实现 core `ChannelHealthStore` |
