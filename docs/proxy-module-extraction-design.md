...
561. response adapter 使用的 pure core response/transport DTO、response builder helper、endpoint path/query bridge、Gemini path model extraction、event envelope、channel key/model record DTO 与 parser config 已改为直接经 `proxy_core::api::{transport,events,management,usage}` 引用；`proxy_core_adapter` 只保留其它宿主仍需的 request/result/core response alias、runtime/host 投影 helper、response build context 投影和 HTTP handler typed signature 兼容别名，不再作为 response adapter 的 pure transport/parser-config/channel key-model DTO 中转入口。host response adapter 继续只负责映射到内部 `ProxyResponse` 与 Axum response/SSE event，response pipeline 实现已迁到 `proxy/engine/response_pipeline.rs`，旧 `proxy/response_processor.rs` 仅保留兼容 re-export。
...
本轮继续把 `response_adapter` 使用的 `ChannelKeyRecord`/`ChannelModelRecord` 响应 DTO 改为直接经 `proxy_core::api::management` 引用；`proxy_core_adapter` 不再作为 response adapter 的 channel key/model record DTO 中转入口，HTTP handlers 仍按当前 host boundary 通过 adapter 兼容别名声明 typed JSON signature。
