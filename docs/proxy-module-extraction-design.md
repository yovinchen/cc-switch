...
| model catalog provider | `host/cc_switch/model_catalog_provider.rs` | `CcSwitchModelCatalogProvider` 默认实现已迁到 `proxy/host/cc_switch/model_catalog_provider.rs`，直接引用 core model catalog port/DTO/request/result contract，负责 provider/client/Claude Desktop model catalog 端口实现、Claude Desktop provider selection 错误映射和 route input 转换；adapter 仅保留 Claude Desktop proxy model route projection helper 与默认 provider 装配入口，不再 re-export `ModelCatalogProvider` port trait |
