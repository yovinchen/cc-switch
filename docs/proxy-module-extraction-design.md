...
本轮继续移除 `proxy_core_adapter` 对 Codex/provider/client raw 模型目录 helper 的 re-export：`codex_config` 直接从 `proxy_core::api::model_catalog` 导入 Codex 模型目录构造、spec 检测和简化 helper，adapter 仅保留使用这些 helper 组合 DB/provider/client catalog 的宿主投影函数。
