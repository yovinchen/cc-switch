...
233. OpenAI-compatible `/models` 单次候选请求的 Bearer auth header 契约已迁入 `proxy-core::model_fetch::build_openai_compatible_models_request`；host `services::model_fetch` 继续负责候选循环、User-Agent 注入、timeout、HTTP 发送和状态码分支。
