...
670. `CcSwitchProviderSource` 的 DB provider 到 `ProviderSpec` 投影已收敛到 host adapter `proxy_provider_to_core_spec` / `proxy_providers_to_core_specs`：当前默认 source 已迁到 host-owned `provider_source.rs`，负责 provider list/get/current 查询与 active-route runtime map 读取，metadata 脱敏和 provider kind 推断继续由 adapter/core 投影规则生成。
...
813. `provider_kind_from_app_type_and_config` 的真实实现已收敛为 `proxy_core_adapter` 内部 helper，Gemini/Claude kind 推断仅作为 adapter 实现细节保留：`proxy/providers` 只保留兼容 facade，`proxy_core_adapter` 不再反向依赖 provider adapter 模块来构造 `ProviderSpec` 或 rectifier 判定。
...
本轮后续已把 `CcSwitchProviderSource` 的 provider list/get/current-provider DB 读取、active route runtime map 读取和 `ProviderSpec` 投影收敛到 host-owned source wrapper；该模块直接消费 core `AppKind` / `ProviderSpec` / `ProviderSource` / `CurrentRouteTarget` / `ProxyCoreResult` contract，adapter 只保留 projection helper 和默认 source type re-export，不再为该 source re-export `ProviderSource` port facade。
...
1188. `CcSwitchProviderSource` 已迁入 `proxy/host/cc_switch/provider_source.rs`：该 host 模块拥有默认 `ProviderSource` 实现，直接引用 core `AppKind` / `ProviderSpec` / `ProviderSource` / `CurrentRouteTarget` / `ProxyCoreResult` contract，包装 DB、`ProviderRouter` 与 active-route runtime map 并复用 adapter 的 provider/current/active-route/route-candidate projection helper；`proxy_core_adapter` 不再 re-export `ProviderSource` port trait，只保留 helper 和默认 source type re-export/装配。
