...
934. handler 请求体 Axum request parts 拆包、body collection、strict JSON 解析、Gemini empty-body null fallback、`stream` 标志组合，以及 app/interface/body/headers/extensions 到 `ProxyRequest` 的组装，已收敛到 `proxy::response_adapter::{collect_json_proxy_request,collect_json_or_null_proxy_request}` 和 `ParsedAxumJsonProxyRequest::into_*_proxy_request`；handler 不再直接调用 `BodyExt::collect`、`parse_json_proxy_request_body*`、`JsonProxyRequestInput`、`InterfaceKind::*` 或 `request_body_read_error_message`。
...
本轮继续把协议 handler 的 Axum request parts 拆包、body collection、strict JSON/Gemini null JSON 解析和 stream 标志读取收敛到 `proxy::response_adapter::{collect_json_proxy_request,collect_json_or_null_proxy_request}`；`handlers` 只保留 `ParsedAxumJsonProxyRequest` 用于 context/endpoint 选择与协议 request 构造，不再直接持有 body collection、JSON parse helper 或 proxy request DTO 组装。
...
1. HTTP handler 只负责触发 transport adapter 读取 body、解析 endpoint 并选择协议响应分支；`ProxyRequest` 构造、`ProxyEngine::handle` dispatch 与 `ProxyResult` 到 HTTP response 的宿主 bridge 由 adapter 承接。
