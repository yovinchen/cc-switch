...
1161. ChannelSource 的 record CRUD/list、key/model 管理和 migration preview/materialize DB helper 已继续迁入 `proxy/host/cc_switch/database_channel_source.rs`：该模块现在完整拥有 DB-backed `ChannelSource` 管理面和 record/key/model DTO 投影，`proxy_core_adapter` 只保留宿主错误包装和旧 HTTP/response 层仍需的少量兼容别名。
...
3. `CcSwitchChannelSource` 作为 host-owned 端口直接消费 core channel contract，包装 provider 主 URL、`provider_endpoints` 和未来 channel 表读取；adapter 只保留宿主错误包装和旧兼容别名。
...
- host-owned `CcSwitchChannelSource` 可以从 provider 主 URL、`provider_endpoints` 和新 channel 表生成一致候选，并直接依赖 core channel contract；adapter 只保留宿主错误包装和旧兼容别名。
