...
565. proxy server 使用的 runtime config/status/info、current route target、Gemini shadow store、ProxyEngine、route resolve request 与 server log code 入口已迁入 `proxy_core_adapter`；server 启动/停止时的 started/stopped event、监听端口记录、runtime status/start_time 写入和 `ProxyServerInfo` 组装也已收敛到 adapter 组合 helper，Axum route tree、管理 API middleware 和 body limit 已由 `proxy_http_router_from_state` 承接，listener 地址解析/bind/local address/error mapping 已由 `bind_proxy_http_listener` 接管，Hyper HTTP/1.1 accept loop、raw header-case capture、connection serving 与 stopped runtime side effect 已由 `spawn_proxy_http_accept_loop` 接管，accept-loop stop wait 超时、日志和 `StopFailed`/`StopTimeout` 映射已由 `await_proxy_http_accept_loop_stop` 接管，shutdown sender/server handle storage 与 AlreadyRunning/NotRunning gate 已由 `ProxyHttpServerHandles` 接管，stop shutdown/handle/wait 编排已由 `stop_proxy_http_server` 接管，start running gate/shutdown channel/router/bind/start event/accept-loop/handle storage 顺序编排已由 `start_proxy_http_server` 接管，`ProxyServer` 实现已迁到 `proxy/transport/http/server.rs`，HTTP handlers 实现已迁到 `proxy/transport/http/handlers.rs`，旧 `proxy/server.rs` 与 `proxy/handlers.rs` 仅保留兼容 re-export，后续重点转为 handler 业务处理继续下沉 engine。
