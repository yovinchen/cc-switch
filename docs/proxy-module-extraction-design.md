...
91a-7. 原 `proxy::response_adapter` 已重命名为 `proxy::protocol_adapter`，职责名与当前代码实际一致：只承接 Claude/Codex/Gemini 协议 HTTP 入口编排、调用 HTTP request-body helper、request adapter 和 response pipeline 高层出口；旧 `src/proxy/response_adapter.rs` 由边界测试禁止恢复，避免后续把 response-specific bridge 重新塞回协议入口层。
