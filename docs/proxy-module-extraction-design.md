...
89. 转换后 JSON 响应的 header 重建、body 序列化和 neutral `ProxyCoreResponse` 构造已迁入 `proxy-core::response_build::rebuilt_json_proxy_response`；host `handlers` 不再直接调用 core builder，而是通过 `proxy::response_adapter::rebuilt_json_proxy_response_to_axum_response` 完成 build-error 映射和 Axum bridge。
90. 转换后 SSE 响应的固定 header、OK status 和 stream body neutral `ProxyCoreResponse` 构造已迁入 `proxy-core::response_build::transformed_sse_proxy_response`；host `handlers` 只保留 transform stream 生成、usage/timeout 包装，并通过 `proxy::response_adapter::transformed_sse_proxy_response_to_axum_response` 桥接 Axum。
...
93. `ProxyCoreError -> ProxyError` 的宿主错误适配已从 `handlers` 抽到 `proxy::error_mapper::proxy_core_error_to_proxy_error`；handler 不再承载 core/host 错误类型映射表。转换后 JSON/Codex 错误响应构造失败的日志上下文与 core-error 映射已进一步收敛到 `proxy::error_mapper::response_build_error_to_proxy_error`，Claude/Codex 响应转换失败的日志上下文与 `TransformError` 包装已收敛到 `proxy::error_mapper::response_transform_error_to_proxy_error`。
...
225. Claude/Codex response 转换已收敛到 adapter/core helper：Claude handler 通过 `proxy_core_adapter::provider_claude_transform_response_for_api_format` 分发 OpenAI Responses、OpenAI Chat 与 Gemini Native 响应转换，Codex handler 通过 `proxy_core_adapter::{transform_codex_chat_response_with_history,transform_codex_chat_sse_with_history}` 统一执行 Chat->Responses 非流/流式转换与 history 记录；host handler 只保留 usage 与 transport 编排，转换失败日志与 `ProxyError::TransformError` 包装由 `error_mapper` 承接。
