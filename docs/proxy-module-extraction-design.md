...
| 现文件 | 目标位置 | 处理方式 |
| --- | --- | --- |
| `server.rs` | `transport/http/server.rs` | 去掉 `Database`/`tauri`，只持有 `ProxyEngine`；当前启动/停止 runtime side effects、listener binding、route assembly、Hyper accept loop/header-case capture 与 stop wait/error mapping 已通过 `proxy_core_adapter` 组合 helper 收口，剩余重点是 shutdown sender/server handle storage、AlreadyRunning/NotRunning gate 与 transport server 边界 |
