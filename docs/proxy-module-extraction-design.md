...
1174. `CcSwitchForwarderRequestSource` 已迁入 `proxy/host/cc_switch/forwarder_request_source.rs`：该 host 模块拥有默认请求体预处理、provider transform/Codex Responses→Chat 转换、Copilot request optimizer、upstream URL planning、media fallback/rectifier plan、上游 body/header 组装和请求日志副作用，`proxy_core_adapter` 只保留 `ForwarderRequestSource` trait/input/type alias 与 adapter-local 测试私有导入，不再 re-export factory/test helper。
...
1204. forwarder transport/response/failover/protocol/attempt/auth/runtime source 的默认 factory 与测试 helper 已从 adapter re-export 收口为私有 use 或直接 host 模块引用：transport/response/failover scheduler/protocol state/attempt runtime/auth source/runtime state contract 已归入 `proxy/engine/forward_pipeline.rs`，`proxy_core_adapter` 仅继续保留尚未迁出的 request source trait/input/type alias，且不再暴露 cc-switch host source factory。
