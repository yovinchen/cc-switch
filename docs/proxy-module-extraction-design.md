...
本轮继续把 `set_auto_failover_enabled` 的开关 plan source 读取收敛到 `proxy_core_adapter::auto_failover_toggle_plan_from_db`：命令层保留队列写入、目标切换、proxy_config 写回、事件和托盘刷新，proxy_config、故障转移队列、当前 provider 和 core plan 输入组装由 adapter 负责；底层 source plan helper 已收为 adapter 私有。
