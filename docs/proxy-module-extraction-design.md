...
715. forward runtime 的 `RoutePlan` + host providers 到 `ForwardAttempt` 列表构造入口已迁入 `proxy/host/cc_switch/channel_auth_profile_attempts.rs`：`proxy_core_adapter` 仅 re-export `forward_attempts_from_plan` 兼容测试。
716. forward runtime 的 auth profile provider/channel-key 应用循环已迁入 `proxy/host/cc_switch/channel_auth_profile_attempts.rs`：`proxy_core_adapter` 只 re-export helper，`proxy_core_host` 只通过 adapter 兼容面测试该路径。
...
718. forward runtime 的 required attempts 空结果错误判断已迁入 `proxy/host/cc_switch/channel_auth_profile_attempts.rs`：`proxy_core_adapter` 仅 re-export `required_forward_attempts_from_plan` 兼容测试。
...
本轮继续把 `CcSwitchProxyRuntime` 数据结构及其 `ProxyServiceRuntimeResources` / `HostForwardRuntime` contract 与 impl 从 `proxy_core_adapter` 迁回 `proxy/host/cc_switch/proxy_runtime.rs`：host 模块重新拥有 CC Switch DB、router、status、event、history、failover、current-target、runtime-source 与 Tauri handle 资源字段，adapter 只保留 forwarding bridge helper 和 runtime type re-export。
