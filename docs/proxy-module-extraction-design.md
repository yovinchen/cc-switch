...
561. response adapter 使用的 pure core response/transport DTO、response builder helper、endpoint path/query bridge、Gemini path model extraction、event envelope 与 parser config 已改为直接经 `proxy_core::api::{transport,events,usage}` 引用；`proxy_core_adapter` 只保留其它宿主仍需的 request/result/core response alias、runtime/host 投影 helper 和 response build context 投影，不再作为 response adapter 的 pure transport/parser-config 中转入口。host response adapter 继续只负责映射到内部 `ProxyResponse` 与 Axum response/SSE event，response pipeline 实现已迁到 `proxy/engine/response_pipeline.rs`，旧 `proxy/response_processor.rs` 仅保留兼容 re-export。
