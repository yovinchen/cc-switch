...
98. `/proxy/v1/channels` POST 与 `/proxy/v1/channels/{channel_id}` GET/PATCH 已迁入透明 `proxy-core::ChannelRecordResponse<T>`；外部 JSON shape 不变，handler 不再手写单条 channel record 序列化。
