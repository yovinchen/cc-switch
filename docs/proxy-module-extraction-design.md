...
本轮继续把 `ProxyResult` 到 `RequestContext` 的回填、Claude api_format 投影和 `ProxyCoreResponse -> hyper_client::ProxyResponse` transport bridge 收敛到 `proxy::response_adapter::{proxy_result_to_proxy_response,claude_proxy_result_to_proxy_response}`；`handlers` 不再直接调用 `apply_proxy_result`、`claude_api_format_for_proxy_result` 或 `proxy_core_response_to_proxy_response`。
...
本轮继续把协议入口的 `ProxyEngine::handle` dispatch、`ProxyCoreError -> ProxyError` 映射与 forward error usage 记录收敛到 `proxy::response_adapter::dispatch_proxy_request`；Claude/Codex/Gemini handler 不再直接持有 `state.proxy_engine().handle`、`proxy_core_error_to_proxy_error` 或 `record_forward_core_error_usage` 调用。
