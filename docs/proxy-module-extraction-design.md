...
7. `UsageSink` 已升级为完整 `UsageRecord` 并可通过 `CcSwitchUsageSink` 写入现有 `proxy_request_logs`；response pipeline 与 handler 转换路径的成功 usage、forward error 日志都已改为走 `UsageSink`，协议入口的 `ProxyEngine::handle` dispatch、core error 映射与 forward error usage 记录已收敛到 `proxy::response_adapter::dispatch_proxy_request`（内部调用 `proxy_core_adapter::record_forward_core_error_usage`），app-specific 响应转换仍在 host 层。
...
更新：`proxy::response_adapter` 现在直接经 `proxy_core::api::{auth,domain,transport,events,management,model_catalog,ports,routing,transforms,usage}` 消费纯 core DTO/context，并作为 HTTP handler typed JSON 签名 DTO 的本地出口；上方 response pipeline 长段中提到的 `proxy_core_adapter::CoreResponseBuildFailureContext` 与 `proxy_core_adapter::AxumResponseBuildErrorContext` 均已过期，response pipeline、response adapter 和 error mapper 直接从 core transport API 引用 response build context。HTTP handler 侧仅保留 `ProxyState` 来自 `proxy_core_adapter`，其它管理/model/auth response 签名 DTO 不再经宽 adapter 导入。
...
| `handlers.rs` | `transport/http/handlers.rs` + `engine` | HTTP handler 实现已迁到 `proxy/transport/http/handlers.rs`，route builder 已导入新路径，typed JSON request/response 签名 DTO 由 `proxy::response_adapter` re-export，handler 对 `proxy_core_adapter` 只保留 `ProxyState` 运行态依赖；旧 `proxy/handlers.rs` 已删除，后续继续把非 HTTP 解析/鉴权/桥接的业务处理下沉到 engine |
