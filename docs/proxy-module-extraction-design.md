...
本轮继续把生产 `ProxyServer` 构造入口改为 `from_runtime_state(config, state)`：`proxy_core_adapter::proxy_server_from_runtime_config` 仍保留现有 service 调用入口，但 `Database/AppHandle` 到 `ProxyState` 的装配已委托 host-owned `proxy_state_from_runtime_sources`，旧 `ProxyServer::new(config, db, app_handle)` 仅保留为 server 单元测试兼容入口。
