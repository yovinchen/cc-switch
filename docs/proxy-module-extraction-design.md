...
| `provider_router.rs` | `engine/routing.rs` | 当前已通过 router 端 provider/channel/config/health 四个 focused port 注入 source/store，channel route 输入已切到 core `RouteResolveChannelInput`，config/channel source adapter 已分别通过 `CcSwitchConfigSource` / `CcSwitchChannelSource` 对齐 core `ProxyConfigSource` / `ChannelSource`，channel health 写入已复用 core `ChannelAttemptResult` 投影且保留 router failure threshold，DB-backed 构造统一在 host adapter factory；live breaker map 已由 `ProviderRoutingCircuitRuntime` 承接，下一步继续把 provider health 端口对齐到 core-facing `ProviderSource`/`HealthStore` |
