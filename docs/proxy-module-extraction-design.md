...
1181. `CcSwitchProviderRouterChannelSource` 已迁入 `proxy/host/cc_switch/provider_router_channel_source.rs`：该 host 模块拥有 ProviderRouter 的 channel route input source，直接引用 core `ChannelRouteSource` / `RouteResolveChannelInput` contract，包装 `CcSwitchChannelSource` 并复用 adapter 的 route-input projection helper；`proxy_core_adapter` 只保留 helper，不再作为该 source 的 route DTO facade。
...
| ProviderRouter channel source | `host/cc_switch/provider_router_channel_source.rs` | `CcSwitchProviderRouterChannelSource` 默认实现已迁到 `proxy/host/cc_switch/provider_router_channel_source.rs`，直接引用 core `ChannelRouteSource` / `RouteResolveChannelInput` contract，负责包装 `CcSwitchChannelSource` 并生成 ProviderRouter route inputs；adapter 仅保留 route-input projection helper，不再作为该 source 的 route DTO facade，默认装配由 `provider_router_sources.rs` 完成 |
...
- host-owned `CcSwitchProviderRouterChannelSource` 可以通过 core-facing `ChannelSource` 生成 ProviderRouter route inputs，并直接引用 core `ChannelRouteSource` / `RouteResolveChannelInput`；adapter 只保留 projection helper，不再作为该 source 的 route DTO facade。
