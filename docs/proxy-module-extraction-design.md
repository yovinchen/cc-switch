...
本轮继续移除 `proxy_core_adapter` 对 active connection status helper 的 re-export：`ActiveConnectionGuard` 仍通过 `ForwarderRuntimeStateSource` 执行 acquire/release 生命周期，adapter 内部私有调用 `proxy_core::api::ports::{record_active_connection_acquired_status,record_active_connection_released_status}`，不再把连接计数 helper 暴露为 crate-wide 出口。
...
1033c. Request/server lifecycle 纯事件构造已改为 `proxy-core::events::{request_started_event,server_started_event,server_stopped_event}`：runtime state source 继续通过 adapter 触发 event bus 副作用，`request_started`、`server_started` 和 `server_stopped` 的事件名与 payload shape 集中在 core，adapter 不再暴露对应 message facade。
