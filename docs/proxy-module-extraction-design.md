...
808. Claude provider adapter 的裸 auth key/base URL getter 已收成 adapter 私有 helper，公开入口保留 `proxy_core_adapter::{provider_claude_api_format,provider_claude_auth_info,required_claude_provider_base_url}`：Claude adapter 保留鉴权策略选择、auth header 构造与 upstream URL 构造，不再在这些入口直接穿透 `Provider.settings_config`/`meta`。
