...
当前 `forwarder.rs` 已不再直接依赖 Codex OAuth/Copilot token 刷新 manager；这一步先用 `proxy::managed_account_auth` 把 refresh/read state 副作用收口。动态 Copilot endpoint 和 live model 读取仍短期保留在 forwarder，独立模块最终应把“获取/刷新可用 token”抽象成宿主可替换端口。
...
| OAuth token 刷新被移出 forwarder 后时序改变 | Copilot/Codex OAuth 请求失败 | `managed_account_auth` 覆盖非托管透传和缺少 AppHandle 的错误路径；后续 `AuthProvider` 继续补 token 缓存、刷新、失败回退测试 |
