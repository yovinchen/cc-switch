...
- failover switch 的 DB、Tauri `AppHandle`、托盘菜单与前端事件副作用实现已迁到 `proxy/host/cc_switch/failover_switch.rs`，旧 `proxy/failover_switch.rs` 仅保留兼容 re-export；adapter 和 command wiring 已优先引用 host 路径。
...
| 故障转移 | `engine/routing.rs`, `circuit_breaker.rs`, `failover_switch.rs` | provider 选择、熔断状态、故障转移后切换当前 provider；旧 `provider_router.rs` 仅为兼容 re-export |
...
| 现文件 | 目标位置 | 处理方式 |
| --- | --- | --- |
| `server.rs` | `transport/http/server.rs` | `ProxyServer` 实现已迁到 `proxy/transport/http/server.rs`，旧 `proxy/server.rs` 仅保留兼容 re-export；listener binding、route assembly、Hyper accept loop/header-case capture、stop wait/error mapping、shutdown sender/server handle storage、running-state gate、start 编排与 stop 编排已由 `proxy/transport/http/server.rs` 拥有，`proxy_core_adapter` 只保留 runtime side effects 与 lifecycle helper 兼容 re-export |
| `handlers.rs` | `transport/http/handlers.rs` + `engine` | HTTP handler 实现已迁到 `proxy/transport/http/handlers.rs`，route builder 已导入新路径，typed JSON request/response 签名 DTO 由 `proxy::response_adapter` re-export，handler 对 `proxy_core_adapter` 只保留 `ProxyState` 运行态依赖；旧 `proxy/handlers.rs` 仅保留兼容 re-export，后续继续把非 HTTP 解析/鉴权/桥接的业务处理下沉到 engine |
...
| `provider_router.rs` | `engine/routing.rs` | `ProviderRouter` 与 routing source/store traits 已迁到 `proxy/engine/routing.rs`，旧 `proxy/provider_router.rs` 仅保留兼容 re-export；当前已通过 router 端 provider/channel/config/health 四个 focused port 注入 source/store，channel route 输入已切到 core `RouteResolveChannelInput`，provider/config/channel source 与 health store 默认实现已迁到 `host/cc_switch/provider_router_*`，并分别通过 `ProviderSource` + `RoutePolicySource` / `CcSwitchConfigSource` / `CcSwitchChannelSource` / core attempt facts 对齐 core `ProviderSource` / `RoutePolicySource` / `ProxyConfigSource` / `ChannelSource` / `ProviderHealthStore`；live breaker map 已由 `ProviderRoutingCircuitRuntime` 承接，下一步评估是否可在不引入 router/store 环的前提下完全实现 core `ChannelHealthStore` |
| `failover_switch.rs` | `host/cc_switch` | `FailoverSwitchManager` 实现已迁到 `proxy/host/cc_switch/failover_switch.rs`，旧 `proxy/failover_switch.rs` 仅保留兼容 re-export；核心/engine 继续只通过 failover scheduler resource 触发宿主副作用 |
| `response_processor.rs` | `engine/response_pipeline.rs` | response pipeline 实现已迁到 `proxy/engine/response_pipeline.rs`，非流式 decoded body bridge、response log event dispatch、passthrough usage runtime source、transformed streaming/non-streaming usage runtime source、passthrough/transformed usage runtime orchestration、streaming/non-streaming/transformed response usage record builder、forward-error usage helper、reusable sink-spawn helper、streaming/non-streaming passthrough response construction、Claude/Codex transformed SSE stream wrappers、Claude/Codex transformed JSON wrappers、shared logged-stream runtime loop 与 provider usage facts projection 已归入该模块，旧 `proxy/response_processor.rs` 仅保留兼容 re-export；后续继续收敛 response adapter/error mapper 中仍依赖 adapter 投影的响应错误与 transport 边界 |
| `usage/logger.rs` | `host/cc_switch/database_usage_sink.rs` | DB-backed `UsageLogger`/`RequestLog` 已迁到 `proxy/host/cc_switch/database_usage_sink.rs`，旧 `proxy/usage/logger.rs` 与 `proxy::usage` re-export 仅保留兼容入口；token/cost DTO 与纯计价策略继续通过 core/adapter 入口 |
| `providers/*` | `provider/*` | provider adapter 实现已迁到 `proxy/provider/*`，`proxy_core_adapter` 已优先引用 `proxy::provider`，旧 `proxy/providers/mod.rs` 仅保留兼容 re-export；后续继续把 adapter 内仍依赖 host `Provider` 存储形态的策略下沉到 adapter/core 边界 |
