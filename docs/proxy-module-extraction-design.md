...
7. `UsageSink` 已升级为完整 `UsageRecord` 并可通过 host-owned `CcSwitchUsageSink` 写入现有 `proxy_request_logs`；response pipeline 与 handler 转换路径的成功 usage、forward error 日志都已改为走 `UsageSink`，协议入口的 `ProxyEngine::handle` dispatch、core error 映射与 forward error usage 记录已收敛到 `proxy::response_adapter::dispatch_proxy_request`，并通过 owning `proxy/engine/response_pipeline.rs::record_forward_core_error_usage` 调度落库；app-specific 响应转换仍在 host 层。
...
11. Codex `/v1/chat/completions` handler 已改为把 parsed Axum JSON request 交给 `response_adapter` 构造 neutral `ProxyRequest` 并进入 `ProxyEngine::handle`；返回的 `ProxyResult` 由 `response_adapter` 回填为 host `ProxyResponse` 后再进入 passthrough/transform 分支，保留现有 usage 解析、流式处理和响应构造。
...
91a-3. `ProxyResult` 回填 `RequestContext`、Claude api_format 回读和 `ProxyCoreResponse -> ProxyResponse` 的 host bridge 已从 `proxy::response_adapter` 迁入 `proxy/engine/response_pipeline.rs`；`response_adapter` 只在协议 dispatch 后调用 response pipeline 出口获取 host `ProxyResponse`。
