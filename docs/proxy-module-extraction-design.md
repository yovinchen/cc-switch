...
101. Claude Desktop gateway 的 bearer header/token 校验和错误文案已迁入 `proxy-core::claude_desktop_gateway_auth`；gateway token 读取/生成通过 adapter-owned DB source 注入 `ProxyEngine`，host auth adapter 只负责委托 engine 并映射 core auth 错误。
...
此前曾把 `ProxyState` 运行态数据结构与 `proxy_engine` helper 移入 `proxy_core_adapter`，让 `proxy/server.rs` 只保留旧路径 re-export；该方向已由后续 host state split 修正，运行态 state shape 回到 `proxy/host/cc_switch/proxy_state.rs`，adapter 仅保留 `ProxyState` re-export 和 `ProxyEngine::new` 的边界内 impl。
...
Claude Desktop gateway token DB helper 已保留在 `proxy_core_adapter::get_or_create_claude_desktop_gateway_token_from_db_source` 和 `claude_desktop_gateway_token_configured_from_db_source`；host `get_or_create_gateway_token` wrapper 已删除，profile apply 与 status token configured 直接消费 adapter token helper，`CcSwitchClaudeDesktopGatewayAuthSource` 已迁入 `proxy/host/cc_switch/claude_desktop_gateway_auth_source.rs` 并通过 adapter re-export，边界测试防止 gateway token setting key、DB read/write 和 token 生成逻辑回流到 host config。
...
1180. `CcSwitchClaudeDesktopGatewayAuthSource` 已迁入 `proxy/host/cc_switch/claude_desktop_gateway_auth_source.rs`：该 host 模块拥有默认 gateway token source 的 `ClaudeDesktopGatewayAuthSource` 实现，直接引用 core gateway auth contract，并继续调用 adapter-owned DB token helper 和 core error projection；`proxy_core_adapter` 不再 re-export `ClaudeDesktopGatewayAuthSource`，只保留 token helper 和默认 source re-export。
...
10. `CcSwitchClaudeDesktopGatewayAuthSource` 作为 host-owned source 包装默认 Claude Desktop gateway bearer token source，直接消费 core gateway auth contract，并由 adapter re-export 默认 source；DB token helper 仍留在 adapter 供 profile apply/status/server smoke 复用。
...
- host-owned `CcSwitchClaudeDesktopGatewayAuthSource` 可以直接消费 core gateway auth contract，并通过 adapter token helper 加载/创建 gateway bearer token，adapter 只 re-export source。
