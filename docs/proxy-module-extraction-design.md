...
本轮后续已把 `CcSwitchConfigSource` 的 global/app/summary/runtime 配置 DB 读取和 core DTO 投影收敛到 host-owned source wrapper；该模块直接消费 core config/source/result contract，adapter 只保留 config helper 和默认 source re-export。
...
1182. `CcSwitchProviderRouterConfigSource` 已迁入 `proxy/host/cc_switch/provider_router_config_source.rs`：该 host 模块拥有 ProviderRouter 的 auto-failover、circuit breaker config 和 failure threshold source，包装 `CcSwitchConfigSource` 并复用 adapter 的 config projection helper，`proxy_core_adapter` 只保留 helper。
...
| ProviderRouter config source | `host/cc_switch/provider_router_config_source.rs` | `CcSwitchProviderRouterConfigSource` 默认实现已迁到 `proxy/host/cc_switch/provider_router_config_source.rs`，负责包装 `CcSwitchConfigSource` 并生成 ProviderRouter auto-failover、circuit breaker 和 failure threshold config；adapter 仅保留 config projection helper，默认装配由 `provider_router_sources.rs` 完成 |
...
1. `CcSwitchConfigSource` 作为 host-owned 端口直接消费 core config/source/result contract，包装现有 DB/settings 读取，并由 adapter re-export 默认 source/装配；config projection helper 继续留在 adapter。
...
- host-owned `CcSwitchConfigSource` 与现有 DB/settings 配置兼容，直接依赖 core config/source/result contract，并通过 adapter helper 暴露 app catalog、global/app/app-summary/runtime config。
