...
400. Copilot 托管账号的动态 API endpoint 与 live model id 解析编排已迁入 `proxy-core::managed_account_auth`，host adapter 只负责把 Provider account binding 投影为 account id，并保留 base URL 日志与请求体 model 写回副作用。
