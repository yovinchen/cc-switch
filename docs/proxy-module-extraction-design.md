...
411. host `ProxyStatus` 已收敛为 `proxy-core::ProxyRuntimeStatus` 别名，并删除 host status 到 core status 的字段拷贝适配器；运行态 status 结构只在 core 维护。
412. `proxy::types` 兼容壳模块已删除；host `proxy::ProxyStatus` 直接 re-export core runtime status，调用点不再依赖代理 host 本地 DTO 模块。
...
414. provider auth key/token 遮蔽算法已迁入 `proxy-core::mask_secret`；host `providers::AuthInfo` 只保留认证字段与策略，日志安全展示规则由 core 维护。
415. 全局代理 URL 日志遮蔽规则已迁入 `proxy-core::mask_url_for_log`；host `proxy::http_client::mask_url` 仅保留兼容 wrapper，外部中转集成可复用同一 URL 脱敏规则。
416. `proxy-core::ProviderKind` 已补齐 Display 与 known-only FromStr 契约，host `ProviderType::from_str` 复用 core provider kind 解析；后续替换本地 ProviderType 枚举时不会再复制字符串别名规则。
417. `CircuitState`、`AllowResult` 与 `CircuitBreakerStats` 已迁入 `proxy-core::circuit_breaker_config`；host `proxy::circuit_breaker` 保留 Tokio 运行态实现并 re-export core 熔断状态契约。
...
419. host `ProviderType` 重复枚举已删除并改为 `proxy-core::ProviderKind` 兼容别名；provider 模块仅保留从 desktop `Provider`/`AppType` 投影到 core provider kind 的适配函数。
420. provider adapter 的认证 DTO 与策略枚举已迁入 `proxy-core::{ProviderAuthInfo, ProviderAuthStrategy}`；host provider 模块继续保留 `AuthInfo`/`AuthStrategy` 兼容名称。
421. host `providers::auth` 兼容壳已删除；`providers::{AuthInfo, AuthStrategy}` 现在直接 re-export `proxy-core::{ProviderAuthInfo, ProviderAuthStrategy}`，provider adapter 不再保留本地认证 DTO 模块。
