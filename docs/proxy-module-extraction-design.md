...
610. Channel breaker stats 的 core fact builder 与 `ChannelBreakerStats` 类型已从 `proxy_core_adapter` re-export 降为 adapter 私有 import；host `CcSwitchChannelHealthStore` 继续直接引用 `proxy_core::api::ports` 的 stats type，并只通过 adapter helper 获取 DB/router projection 结果。
