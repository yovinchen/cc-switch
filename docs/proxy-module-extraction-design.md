...
550. Copilot OAuth/API URL 构造、Copilot API base fallback 与模型/usage 响应解析入口仍由 host 适配层承接；`CopilotModel` 与 `CopilotUsageResponse` DTO 入口已从 `proxy_core_adapter` 二次出口改为 Copilot 命令和 `copilot_auth` 直接引用/转出 `proxy_core::api::model_catalog`，Copilot auth 模块只负责 HTTP 调用、token/OAuth 状态和 endpoint/model 缓存。
...
607. legacy managed-auth command DTO 入口已从 `proxy_core_adapter` 二次出口改为 `proxy/copilot_auth.rs` 与 `proxy/codex_oauth_auth.rs` 直接引用/转出 `proxy_core::api::auth::{CopilotAuthStatus, GitHubAccount, GitHubDeviceCodeResponse, CodexOAuthStatus}`；命令层仍通过既有 `proxy::copilot_auth` 兼容路径消费，boundary 测试只允许这些精确 DTO direct-core 行。
