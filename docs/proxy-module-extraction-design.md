...
776. Gemini live 写入的 provider settings 到 `.env` map 投影与 strict validation 入口已改为 `proxy_core_adapter::{provider_gemini_env_map_for_live,validate_provider_gemini_settings_strict}`：provider live 继续负责 auth mode 分支、原子写 `.env`、settings.json 合并与安全标记写入，Gemini settings 转换/校验入口集中在 adapter。
