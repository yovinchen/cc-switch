...
本轮继续移除 `proxy_core_adapter` 对 proxy takeover status enabled-options helper 的 re-export：`proxy_takeover_status_from_db` 在 adapter 内部私有调用 `proxy_core::api::ports::proxy_takeover_status_from_enabled_options`，adapter 不再把接管状态矩阵 helper 暴露为 crate-wide 出口。
