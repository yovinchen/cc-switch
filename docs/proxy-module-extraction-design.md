...
180. Claude takeover service 对 `[1M]` 模型后缀的字符串剥离已直接引用 `proxy-core::model_mapping`；host `model_mapper` 不再为该纯字符串 helper 提供二次出口。
...
187. Provider `settings_config.env` 到 `ModelMapping` 的字段投影已迁入 `proxy-core::ModelMapping::from_settings_config`；host `model_mapper` 只负责传入当前 provider settings、保留日志和调用 core 映射算法。
188. Claude/Kimi/DeepSeek/MiMo 兼容所需的 reasoning vendor hint 判定、OpenAI Chat `reasoning_content` 保留 gate、Anthropic tool-use history thinking block 规范化和占位符常量已迁入 `proxy-core::response_transform`；host Claude adapter 只负责传入 provider settings/api_format 并执行协议转换编排。
189. DeepSeek 官方 Anthropic endpoint 的 `thinking: disabled` 与 `output_config.effort`/`reasoning_effort` 冲突清理、endpoint 候选字段判定和官方 URL 常量已迁入 `proxy-core::response_transform`；host Claude adapter 只保留 `Provider.settings_config` 到 core 的薄适配。
...
224. `OptimizerConfig` 到 thinking optimizer/cache injector core config 的字段投影已收敛到 host 配置类型本身；`thinking_optimizer`/`cache_injector` 只保留 body mutation 调用和原日志格式。
...
228. `providers::transform` 与 `providers::transform_responses` 模块声明已限制为 `#[cfg(test)]`；生产构建不再编译 OpenAI Chat/Responses 的旧 provider facade，协议回归由测试模块继续覆盖。
229. Codex Responses->Chat 上游请求转换已从 forwarder 直接调用 `proxy-core::responses_to_chat_completions_with_options`，模型能力判定也直接使用 core helper；`providers::transform_codex_chat` 已限制为 `#[cfg(test)]` fixture 模块。
230. Gemini Native endpoint rewrite 的 request model 读取已在 forwarder 直接从请求体提取，并删除 `transform_gemini::extract_gemini_model` 纯 passthrough helper；Gemini transform 模块继续只保留实际协议转换与 shadow 状态维护。
231. Codex OAuth/ChatGPT 后端模型目录的请求契约已迁入 `proxy-core::model_fetch::build_codex_oauth_models_request`，包括 backend URL、`client_version` query、Bearer header、`originator` 和 `chatgpt-account-id` header；host `services::codex_oauth_models` 只负责 HTTP 发送、timeout 和响应 JSON 解析入口。
232. OpenAI-compatible `/models` 单次候选请求的 Bearer auth header 契约已迁入 `proxy-core::model_fetch::build_openai_compatible_models_request`；host `services::model_fetch` 继续负责候选循环、User-Agent 注入、timeout、HTTP 发送和状态码分支。
