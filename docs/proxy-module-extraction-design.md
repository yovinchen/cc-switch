...
5. `ProxyEngine::handle` 已拥有编排骨架：请求计数、route selected 事件、`ForwardPipeline` 端口调用、`UsageSink` 调用；route result 后的 host `Provider` 回填已通过 `RequestContextProviderSource` source 注入，`RequestContext` 不再直接读 CC Switch DB。
6. `CcSwitchEventSink` 已桥接到现有 `ProxyEventBus`，核心事件可以进入 `/proxy/v1/events` 的 SSE 流。
7. `UsageSink` 已升级为完整 `UsageRecord` 并可通过 host-owned `CcSwitchUsageSink` 写入现有 `proxy_request_logs`；response pipeline 与 handler 转换路径的成功 usage、forward error 日志都已改为走 `UsageSink`，协议入口的 `ProxyEngine::handle` dispatch、core error 映射与 forward error usage 记录已收敛到 owning `proxy/engine/response_pipeline.rs` 调度落库；app-specific 响应转换仍在 host 层。
...
10. `CcSwitchForwardPipeline` 已迁回 host-owned optional-runtime wrapper，`CcSwitchProxyServices<R>` generic `ProxyServices` 容器也已迁回 host-owned `proxy/host/cc_switch/proxy_services.rs`，并接入运行中 `ProxyServer` 的共享 router/status/event/history/failover 运行态；`CcSwitchProxyRuntime` 及其 `ProxyServiceRuntimeResources` / `HostForwardRuntime` impl 已迁回 host-owned `proxy/host/cc_switch/proxy_runtime.rs`，`ProxyState` 数据形状也已迁回 host-owned `proxy/host/cc_switch/proxy_state.rs`，负责装配 DB、router、Tauri handle 等不可移植资源，把 `ProxyEngine::handle` 的 `RoutePlan` 映射为 host `ForwardAttempt` 并复用现有 HTTP 转发链。后续重点转为迁移 response pipeline、模型列表接口和外部管理 API。
11. Codex `/v1/chat/completions` handler 已改为把 parsed Axum JSON request 交给 `response_adapter` 调度；request/context/endpoint 构造由 `request_adapter` 与 HTTP request-body helper 拥有，`ProxyEngine::handle` dispatch、`ProxyResult` 回填和 host `ProxyResponse` 桥接由 `response_pipeline` 拥有，再进入 passthrough/transform 分支，保留现有 usage 解析、流式处理和响应构造。
12. Gemini handler 已改为进入 `ProxyEngine::handle`；模型名继续从 URI 提取，无模型的 `/models` 类端点不会把 `unknown` 写入 route filter，避免误过滤 channel。
13. Codex `/v1/responses` 与 `/v1/responses/compact` handler 已进入 `ProxyEngine::handle`；Responses -> Chat 转发请求转换由 core transform helper 与 host request source 编排，Chat -> Responses 响应/history 处理由 `proxy::codex_chat_history` 与 response pipeline owning module 承接，handler 只保留 Axum 入口调度。
14. Claude 与 Claude Desktop `/v1/messages` handler 已经由 `response_adapter` 调度进入 `ProxyEngine::handle`；核心 `ProxyResult` 会带回 `claudeApiFormat` 等宿主 metadata，并由 `response_pipeline` 桥接为 host `ProxyResponse` 与 api_format，host 侧继续复用现有格式转换、SSE/非流式响应处理和用量解析。
...
29. `UsageRecord` 到 `TokenUsage` 的 host 回填转换、pricing model override/request/response 选择规则已迁入 `proxy-core::usage`；host-owned `CcSwitchUsageSink` 负责读取 DB 计价配置、查询定价、执行 Decimal 成本计算、映射 host `RequestLog` 并写入 `UsageLogger`，adapter 不再保留 usage request-log projection helper。
...
43. `/proxy/v1/apps/{app}/channels/migration/preview` 与 `/materialize` 的 response envelope 和 preview channel payload 已迁入 `proxy-core::ChannelMigrationPreviewResponse<ChannelRecord>`/`ChannelMigrationMaterializeResponse`；旧 provider/endpoint 到 channel 的兼容迁移规划已由 `proxy-core::build_legacy_channel_migration_plan` 负责，DAO 负责装配本地 provider facts、调用 core plan，并映射回 `ProxyChannelRecord`/model rows。
44. `/proxy/v1/route/resolve` 的 request/response/candidate/rejected/source API contract 已迁入 `proxy-core::{RouteResolveRequest, RouteResolveResponse, ChannelRouteCandidate, ChannelRouteRejected, ChannelRouteSource}`；host 不再保留 `channel_routing` 桥接模块，DB channel record 到 route input 的投影由 host-owned ProviderRouter channel source 承接。
...
50. 管理 API 鉴权策略已迁入 `proxy-core::management_auth`；token-source 决策由 host-owned `CcSwitchManagementAuthSource` 读取热更新 `ProxyConfig` 与 `CC_SWITCH_PROXY_MANAGEMENT_TOKEN` fallback 后注入 `ProxyEngine::validate_management_auth`，host middleware 只负责把 headers 交给 `proxy::auth_adapter::validate_proxy_management_auth` 并继续 Axum middleware 链路，engine/auth adapter 负责校验 header 并把 core 鉴权错误映射为现有 `ProxyError::AuthError`。
...
71. provider 可重试失败、单 provider 失败、全部 provider 失败的日志 code/message 策略、上游错误摘要规则和 forward failure message 选择策略已迁入 `proxy-core::forward_failure`；`ProxyError` 到中立 `ForwardFailureKind` 的 host-only 事实投影已收敛到 `proxy::error_mapper`，host forwarder 只消费分类结果并执行实际日志输出。
72. provider failover eligibility 与健康度污染边界已迁入 `proxy-core::forward_failure::categorize_forward_failure`；400/405/406/413/414/415/422/501 等客户端请求错误不再由 host forwarder 本地判定，`ProxyError -> ForwardFailureKind` 由 `error_mapper` 持有。
...
87. Codex Chat 上游错误体的 JSON/文本解析、非 JSON 预览截断、Responses 风格 error envelope 归一化和 host-neutral error `ProxyCoreResponse` 构造已迁入 `proxy-core::codex_error::{normalize_codex_chat_error_body,codex_chat_error_proxy_response}`；host `response_pipeline` 负责非 JSON warning、上游错误 body read 与 Axum bridge，handler/adapter 只负责判定非成功 Chat 响应需要转换。
...
91. `ProxyCoreResponse -> Axum Response` 的宿主 transport bridge 已从 `handlers`/`response_adapter` 收敛到 `proxy/engine/response_pipeline.rs`，`ProxyCoreResponse -> hyper_client::ProxyResponse` 仍由 upstream transport bridge 承接；`ProxyEngine::handle` dispatch、`ProxyResult` 成功回填 context 后转成 host `ProxyResponse` 的路径、Claude transform gate、Codex Responses→Chat gate、transform streaming decision 调用、transformed response orchestration 与 passthrough response parser-config/process_response 入口已继续向 request/response pipeline 下沉。handler 只保留请求事实抽取、协议分支调度和错误语义映射，`response_adapter` 主要保留协议入口调度。Axum response builder 失败时的 Claude/Codex/通用 tag 诊断上下文已收敛到 core-owned `ProxyResponseBuildErrorContext`，转换后 JSON/SSE 的 core response builder 调用经 response pipeline bridge 统一落到 Axum，生产 handler/response processor 不再散落 build-error 文案。
91a. HTTP JSON 请求体解析与 `stream` flag 事实投影已收敛到 `proxy-core::request_body::{parse_json_proxy_request_body,parse_json_proxy_request_body_or_null}`：`proxy::response_adapter` 只负责收集 Axum body bytes、映射 host `ProxyError`，不再本地组合 `parse_json_request_body*` 与 `request_body_stream_flag`，避免协议入口重复维护 stream 判定。
91a-1. Axum body 收集、body read error 映射、JSON/null JSON body parse 调用与 URI endpoint 拼接已从 `proxy::response_adapter` 迁入 HTTP transport owning helper `proxy/transport/http/request_body.rs`；`response_adapter` 只消费 `ParsedHttpJsonProxyRequest` 并继续负责协议上下文、`ProxyRequest` 构造和响应编排。
91a-2. `ParsedHttpJsonProxyRequest` 到 `RequestContext`、协议 endpoint、`ProxyRequest` 与 Codex tool context 的构造已从 `proxy::response_adapter` 迁入 `proxy/request_adapter.rs`；`response_adapter` 不再持有 `AppKind`/`InterfaceKind`/`ProxyBody::Json` 等请求构造细节，只调用 request adapter 提供的协议请求方法后进入 dispatch/响应编排。
91a-3. `ProxyResult` 回填 `RequestContext`、Claude api_format 回读和 `ProxyCoreResponse -> ProxyResponse` 的 host bridge 已从 `proxy::response_adapter` 迁入 `proxy/engine/response_pipeline.rs`；`response_adapter` 只调用 response pipeline 出口获取 host `ProxyResponse` 或协议 Axum response。
91a-4. Claude/OpenAI Chat/Codex/Gemini passthrough response 的 parser-config 选择与 `process_response` 调用已从 `proxy::response_adapter` 迁入 `proxy/engine/response_pipeline.rs`；`response_adapter` 不再直接引用 `CLAUDE_PARSER_CONFIG`/`OPENAI_PARSER_CONFIG`/`CODEX_PARSER_CONFIG`/`GEMINI_PARSER_CONFIG`。
91a-5. Claude transform gate、Codex Responses→Chat response gate、Claude/Codex transform streaming decision、未标记 SSE 聚合事实、转换后 JSON/SSE 上游响应解析与高层 Axum response 编排已从 `proxy::response_adapter` 迁入 `proxy/engine/response_pipeline.rs`；`response_adapter` 不再直接引用 `provider_projection`、`Provider`、`HeaderMap`、transform streaming decision 或 response body decode helper，只调用 response pipeline 的高层协议响应出口。
91a-6. `ProxyEngine::handle` dispatch、`ProxyCoreError -> ProxyError` 映射、forward-error usage 调度、`ProxyRequest`/`ProxyResult` dispatch contract 和 Codex forward-error response outcome 已从 `proxy::response_adapter` 迁入 `proxy/engine/response_pipeline.rs`；`response_adapter` 不再直接引用 `proxy-core::api`、`ProxyRequest`、`ProxyResult`、`CodexToolContext` 或 `ProxyResponse`，只消费 request adapter 产出的 opaque request value 并调用 response pipeline 高层出口。
91a-7. 原 `proxy::response_adapter` 已重命名并下沉为 `proxy::transport::http::protocol_adapter`，职责名和模块归属与当前代码实际一致：只承接 Claude/Codex/Gemini 协议 HTTP 入口编排、调用 HTTP request-body helper、request adapter 和 response pipeline 高层出口；旧 `src/proxy/response_adapter.rs` 与根层 `src/proxy/protocol_adapter.rs` 由边界测试禁止恢复，避免后续把 response-specific bridge 或 HTTP transport 细节重新塞回根层协议入口。
91a-8. forward-error usage record 构造、`UsageSink` 异步调度、provider/app usage facts 投影与 usage logging 开关读取已从 `proxy/engine/response_pipeline.rs` 抽到 `proxy/engine/response_usage.rs`；`response_pipeline` 只在 `ProxyEngine::handle` error path 调用 `record_forward_core_error_usage`，流式/非流式 response usage 编排继续消费 `response_usage` 的 facts/sink helper，避免主响应编排文件继续承载通用 usage 调度细节。
91a-9. passthrough 与 transformed response usage 的纯 record 构造、request-id fallback、route context 回填、全 0 usage 跳过和 test-only success usage 记录 helper 已从 `proxy/engine/response_pipeline.rs` 迁入 `proxy/engine/response_usage.rs`；`response_pipeline` 继续拥有 `SseUsageCollector`、logged stream、非流式响应构造和 transformed response 的运行时编排，只通过 context wrapper 调用 `response_usage`，避免主响应编排文件继续直接依赖 core usage record builder。
91a-10. response body decode/log bridge、`ProxyCoreResponse -> Axum Response` bridge、转换后 JSON/SSE response assembly、Codex Chat upstream error assembly 与 Codex proxy error assembly 已从 `proxy/engine/response_pipeline.rs` 抽到 `proxy/engine/response_assembly.rs`；`response_pipeline` 只消费 assembly helper 并保留协议 dispatch、transform 分支选择、stream wrapper 和 usage runtime 编排，避免主响应编排文件继续拥有低层响应组装细节。
91a-11. 流式 response usage collector context、`SseUsageCollector`、finish guard、logged passthrough stream runtime loop，以及 passthrough/transformed streaming usage collector helper 已从 `proxy/engine/response_pipeline.rs` 抽到 `proxy/engine/response_stream.rs`；`response_pipeline` 只保留协议 dispatch、transform 分支选择、passthrough/transformed stream response 构造和对 stream helper 的调用，避免主响应编排文件继续拥有 SSE runtime 生命周期与 finish guard 细节。
91a-12. Claude/Codex transformed SSE wrapper、transformed JSON wrapper、Anthropic tool schema hints 提取和 transformed response usage 调度已从 `proxy/engine/response_pipeline.rs` 抽到 `proxy/engine/response_transform.rs`；`response_pipeline` 只保留是否转换、是否走流式、上游 JSON/SSE fallback parse 和 Axum response 出口编排，避免主响应编排文件继续拥有 app-specific response transform wrapper 细节。
91a-13. passthrough response 的流式/非流式处理入口、stream/non-stream `ProxyCoreResponse` 构造、响应日志调用和非流式 usage runtime wrapper 已从 `proxy/engine/response_pipeline.rs` 抽到 `proxy/engine/response_passthrough.rs`；`response_pipeline` 只保留 Claude/OpenAI Chat/Codex/Gemini 协议命名出口并调用 passthrough `process_response`，避免主响应编排文件继续承载低层透传响应构造。
91b. Codex Responses→Chat 响应分支 gate 的 Provider-shaped facts 已从 `proxy::provider::codex` 直接调用改为通过 host-owned `provider_projection` 获取，并随高层响应编排迁入 `proxy/engine/response_pipeline.rs`；`provider_projection` 负责从 CC Switch `Provider`/TOML/settings 投影 api_format、wire_api、base_url 和 config_base_url facts，并继续委托 `proxy-core::request_url` 的纯 conversion policy。
...
95. Codex 代理错误响应的 `ProxyError -> Responses error body` 宿主分类已从 `handlers` 抽到 `proxy::error_mapper::codex_proxy_error_json`，JSON body 的 neutral response 构造已迁入 `proxy-core::response_build::json_proxy_response`；Codex dispatch helper 从 `RequestContext`/endpoint/error 提取 provider/model/endpoint facts，neutral response 构造与 Axum bridge 由 `proxy::engine::response_pipeline::codex_proxy_error_to_axum_response` 承接，`response_adapter` 只调度该出口。
...
101. Claude Desktop gateway 的 bearer header/token 校验和错误文案已迁入 `proxy-core::claude_desktop_gateway_auth`；gateway token 读取/生成由 host-owned `CcSwitchClaudeDesktopGatewayAuthSource` 注入 `ProxyEngine`，host auth adapter 只负责委托 engine 并映射 core auth 错误。
102. Claude 非流响应转换中未标记 SSE body 的 api_format -> 聚合策略映射已迁入 `proxy-core::response_transform::claude_transform_unlabeled_sse_aggregation`；当前由 `proxy-core::response_transform::claude_transform_streaming_decision` 统一携带给 JSON/SSE parse helper，host adapter 只投影 provider 是否 Codex OAuth，handler 不再直接选择聚合策略。
...
109a. CC Switch `Provider` 到 Claude `api_format` 的字段投影已由 host-owned `provider_projection` 拥有，并直接委托 `proxy-core::response_transform::resolve_claude_api_format_from_settings`；`proxy/provider/claude.rs::claude_provider_api_format` 仅保留为兼容薄壳，不再让 `provider_projection` 反向依赖 provider adapter。
...
204. provider settings 中的模型目录摘要策略已迁入 `proxy-core::model_fetch::provider_model_catalog_from_settings`，统一处理顶层 `model`、Anthropic/Gemini env 默认模型和 `modelCatalog.models` 的 `model/id/name` 字段；host-owned `CcSwitchModelCatalogProvider::load_catalog` 直接读取 provider settings 并调用 core catalog helper。
...
248. Claude Desktop provider 的导入 direct/proxy/skip 决策、直连/本地路由校验、gateway profile model spec 投影、proxy request body 映射和 status facts 已迁入 `proxy/host/cc_switch/claude_desktop_provider.rs`；该 host-owned 模块直接消费 `proxy-core::api::auth` 纯策略，`commands/provider.rs`、`claude_desktop_config.rs`、`forwarder_request_source.rs` 与 `model_catalog_provider.rs` 不再经 `proxy_core_adapter` 访问这批 Provider 投影，`proxy_core_adapter.rs` 当前不再暴露 `pub(crate)` facade。
...
264a. `CcSwitchProxyRuntime` 的 host forward bridge 已迁回 `proxy/host/cc_switch/proxy_runtime.rs`：`HostForwardRuntime::forward_host` 直接读取 DB app config/current provider、生成 `ForwardRuntimeRequest`/`ForwarderRuntimeHostResources`、调用 `required_forward_attempts_from_sources` 并启动 `RequestForwarder::new_preplanned`；`proxy_core_adapter.rs` 不再保留 `forward_proxy_request_with_cc_switch_runtime`、preplanned launcher、runtime config/source helper 或 session id bridge。
264b. `src/lib.rs` 已完全移除 `proxy_core_adapter` 模块声明，空的 `src/proxy_core_adapter.rs` tombstone 也已删除；历史边界测试通过缺失即空的 `proxy_core_adapter_source` helper 保留负向断言，并新增断言禁止恢复该 tombstone。生产集成点应继续落在 `proxy-core::api`、`proxy/engine/*`、`proxy/host/cc_switch/*` 或 `proxy::response_adapter` 的 owning module。
264c. Claude Desktop 配置测试和 HTTP handler 签名不再通过 `proxy::response_adapter` 间接取得 model-list response DTO，均改为直接引用 `proxy-core::api::auth::ClaudeDesktopModelListResponse`；model-list handler 只保留 gateway auth、`ProxyEngine::claude_desktop_model_list_response` 调用和 typed JSON bridge。
...
295. usage 成本计算领域对象与公式已迁入 `proxy-core::cost`（`CostCalculator`、`ModelPricing`、`CostBreakdown`）；host-owned `database_usage_sink` 只保留数据库日志写入与配置读取桥接。
...
299. route dry-run 的 channel/model input 合约与 spec 入口已迁入 `proxy-core::RouteResolveChannelInput::from_channel_spec` / `resolve_channel_route_from_specs`；host 侧 DB record 到 route input 的投影由 `proxy/host/cc_switch/provider_router_channel_source.rs` 承接，以便保留存储态 status/source kind 语义。
...
310. 全局 HTTP client 避免系统代理回指 CC Switch 自身监听端口的递归保护已迁入 `proxy-core::request_transport`；host-owned `global_http_client` 只负责读取环境变量和当前监听端口，并把 proxy URL 判定交给 core policy。
...
319. `/health` 与 `/status` 的 runtime response envelope 已收敛到 `proxy-core::HealthCheckRequest` / `ProxyStatusRequest` helper；HTTP handler 直接为 health 注入当前时间，status 通过 `ProxyEngine::proxy_status_response` 读取 runtime 状态快照并返回 typed JSON，`response_adapter` 不再承接基础 status JSON bridge。
...
324. route dry-run 使用的 DB channel record 到 `RouteResolveChannelInput` 投影已收敛到 host-owned `proxy/host/cc_switch/provider_router_channel_source.rs`；router channel source 直接返回 core route input，保留 DB 原始 status/source kind 文本用于 rejected reason。
...
403. `proxy::channel_routing` 桥接模块已删除；management dry-run route resolution 已收敛到 host-owned `proxy/host/cc_switch/route_resolver.rs::management_route_response_from_router_source`，route resolver 负责向 router 读取 core route channel input、调用 `proxy-core::resolve_channel_route` 并应用 circuit-open rejection；`ProviderRouter` 只提供 route channel inputs/source 和 live circuit availability 查询。
...
416. ordinary forward failure 的 retryable error message 与日志投影已并入 `ForwarderRuntimeStateSource::forward_failure_decision`，non-retryable 分支改为直接调用 `record_forward_error_status(ForwarderForwardErrorStatusInput)`；host forwarder 不再对普通 forward 错误执行 `to_string()` 来填充熔断记录或 runtime status。
...
420. rectifier client-side failure 的 runtime status 记录已收敛到 `ForwarderRuntimeStateSource::record_forward_error_status(ForwarderForwardErrorStatusInput)`；signature/budget rectifier 客户端失败分支不再直接对 `ProxyError` 执行 `to_string()` 或处理中间状态文案，只把错误交给 runtime source 写入状态。
...
423. per-attempt current provider 状态写入已收敛到 `ForwarderRuntimeStateSource::record_current_provider(ForwarderCurrentProviderInput)`；host forwarder 不再拆 `provider.id/name` 写 runtime status，只传递当前 provider 事实。
424. provider failure 名称投影已收敛到 `ForwarderRuntimeStateSource::{forward_failure_decision,record_provider_failure(ForwarderProviderFailureInput),record_provider_rectifier_retry_failure(ForwarderProviderRectifierRetryFailureInput)}`；host forwarder 不再散传 provider/error/kind 或拆 `provider.name` 构造重试日志与 runtime failure status。
425. 客户端侧 rectifier retry failure 与 ordinary non-retryable failure 的 runtime status 记录已统一走 `ForwarderRuntimeStateSource::record_forward_error_status(ForwarderForwardErrorStatusInput)`；对应决策枚举不再携带只用于状态写入的 `error_message` 字段，host forwarder 不再保留 message-only failure status helper。
426. ordinary/terminal forward failure 的 warning log 行格式已收敛到 `ForwarderRuntimeStateSource::{forward_failure_decision,log_retryable_forward_failure(ForwarderRetryableFailureLogInput),log_terminal_forward_failure(ForwarderTerminalFailureLogInput)}`；host forwarder 不再拼接 `[app] [code] message` 或散传 app/provider/attempt/error 日志事实，只触发 source 日志行为。
...
428. provider-scoped rectifier retry failure 状态记录已改为 `ForwarderRuntimeStateSource::record_provider_rectifier_retry_failure(ForwarderProviderRectifierRetryFailureInput)`；host forwarder 不再取回 label、错误字符串或散传 provider/kind/error 再回传 source，label 与错误消息投影完全保留在 runtime source 内部。
...
431. attempt started/failed phase 选择已收敛到 `ForwarderRuntimeStateSource::{record_attempt_started(ForwarderAttemptStartedInput),record_failed_attempt(ForwarderAttemptFailedInput)}`，successful attempt 的 `attempt_succeeded` 事件、active route target 写入和 `route_selected` 事件已合并为 `ForwarderRuntimeStateSource::record_successful_attempt(ForwarderSuccessfulAttemptInput)`；host forwarder 不再散传 request/app/attempt/error、导入/传递 `AttemptEventPhase`，也不再暴露 `emit_*` 事件方法或拆分成功事件与 route target 副作用顺序。
432. request-started、attempt 事件和 active route target 的 host forwarder 薄 wrapper 已删除；`RequestForwarder` 直接调用 `ForwarderRuntimeStateSource` 语义方法并传入结构化 lifecycle input，channel route target 状态写入与 `route_selected` 事件回归测试下移到 source 层。
432a. `ForwarderRuntimeStateSource` 的 status、request id、provider failure、attempt event 和 active route target 行为测试已从 `proxy_core_adapter` 迁到 owning `proxy/host/cc_switch/forwarder_runtime_state_source.rs`；边界测试继续约束 runtime-state source 行为 fixture 不再回流到 adapter。
433. request-started event 与 runtime status timestamp 生成已收敛到 `ForwarderRuntimeStateSource::record_request_started(ForwarderRequestStartedInput)`；host forwarder 不再拆分事件发送、`chrono::Utc` 时间戳生成或中间时间字符串传递。
...
444a. channel authProfileRef 的 provider/key 应用行为测试与 `TestChannelKeyRuntimeSource` fixture 已从 `proxy_core_adapter` 迁到 owning `proxy/host/cc_switch/channel_auth_profile_attempts.rs`；边界测试继续约束 adapter 不再托管 channel-key runtime fixture，auth-profile source 自身覆盖 provider auth 替换、channel-key lookup 和缺失 key 错误投影。
...
445a. `ForwarderResponseSource` 的成功响应 finalization 与非成功 upstream error 投影行为测试已从 `proxy_core_adapter` 迁到 owning `proxy/host/cc_switch/forwarder_response_source.rs`；边界测试继续约束 response source 行为 fixture 不再回流到 adapter。
...
452a. `ForwarderAuthSource` 的 Copilot auth optimization、AuthProvider route-context header、Codex OAuth session header gate 和 Copilot override header 行为测试已从 `proxy_core_adapter` 迁到 owning `proxy/host/cc_switch/forwarder_auth_source.rs`；边界测试继续约束 `ChannelHeaderAuthProvider` fixture 与 auth-source 行为测试不再回流到 adapter。
...
454. `ForwarderAttemptRuntimeSource` trait 不再暴露 `attempt_limit_reached` max-attempt helper；默认 attempt runtime source 在 `allow` 决策内先执行尝试上限判定，再进入 circuit breaker permit，外部替换 source 只返回结构化 `ForwarderAttemptAllowDecision`；成功/失败健康记录和 neutral permit 释放也改为消费 `ForwarderAttemptSuccessInput` / `ForwarderAttemptFailureInput` / `ForwarderAttemptNeutralReleaseInput`，host forwarder 不再向 attempt runtime source 散传 attempt、app、half-open permit 和 error 事实。
455. `ForwarderRuntimeStateSource` trait 不再暴露 `rectifier_retry_success_log_line` / `rectifier_retry_failure_log_line` 字符串 helper；默认 runtime state source 仍保留日志行投影，并通过 `log_rectifier_retry_success(ForwarderRectifierRetrySuccessLogInput)` / `log_rectifier_retry_failure(ForwarderRectifierRetryFailureLogInput)` 执行日志副作用，host forwarder 不再散传 app/kind/error 日志事实。
456. `ForwarderRuntimeStateSource` trait 不再暴露 `terminal_forward_failure_log_line_for_error` 字符串 helper；默认 runtime state source 仍保留 terminal failure 日志行投影并通过 `log_terminal_forward_failure(ForwarderTerminalFailureLogInput)` 执行日志副作用。
457. `ForwarderFailureDecision::Retryable` 不再携带 `log_line` 字符串；默认 runtime state source 仍保留 retryable forward failure 日志行投影并通过 `log_retryable_forward_failure(ForwarderRetryableFailureLogInput)` 执行日志副作用。
...
473. `proxy::session` host UUID adapter 已迁入 `proxy/host/cc_switch/proxy_runtime.rs::extract_proxy_session_id` 内部 helper；session 提取策略继续由 `proxy-core::session` 维护，host runtime 只负责注入 UUID 生成器，adapter 不再保留 session 壳文件或 crate-wide API。
...
501. `/proxy/v1/channels/{channel_id}/test` 管理 API 已落地；DB-backed host reachability probe 负责读取 channel/provider 并执行现有 stream-check，core 负责 `ProxyChannelTestRequest`/`ChannelTestResponse` 对外契约。
...
513. `/proxy/v1/events` 的 `ProxyEventEnvelope` 到 Axum `Event` transport 适配、event bus subscription 和 keepalive 编排已迁入 `proxy::transport::http::handlers`；HTTP transport handler 只消费 core `ProxyEventSseSpec` 与 keep-alive contract，不再经 `response_adapter` 中转事件 SSE。
...
532. Codex settings/model catalog text 到 core catalog response 的 host-to-core 投影已迁入 `proxy_core_adapter::{codex_model_catalog_from_settings,simplify_codex_model_catalog}`；Codex config 模块只负责 TOML、模板和文件路径处理，并直接引用 `proxy_core::api::model_catalog::DEFAULT_CODEX_MODEL_CONTEXT_WINDOW` 作为默认上下文窗口常量，不再经 `proxy_core_adapter` 二次别名。
...
534. provider settings/client raw catalog 到 `ModelCatalog` 的 core 投影由 `proxy-core` 维护；host-owned model catalog provider 负责默认端口实现并本地读取 provider DB/source facts，`proxy_core_host` 只装配 provider。
...
536. core `UsageRecord` 到 host `RequestLog` 的 usage/cost/request-id/missing-pricing 投影已迁入 host-owned `proxy/host/cc_switch/database_usage_sink.rs::usage_record_to_request_log`；`CcSwitchUsageSink` 负责 pricing DB 查询、pricing lookup/model source 选择、warning 输出和日志落库，adapter 不再保留 request-log 投影 helper。
...
539. route attempt 的 `RouteSelection -> ChannelRouteCandidate -> ResolvedChannelAttempt` 投影已改为由 `route_attempt.rs` 作为 approved routing adapter 直接消费 `proxy_core::api::routing`；channel provider override 也由 route attempt 本地投影宿主 `Provider` mutation，request body model override 继续由 `ForwardAttempt` 持有日志语义。
540. Codex Responses handler 的 proxy request 构造与 tool context 提取已合并到 `proxy_core_adapter::codex_responses_proxy_request_from_input`，Chat error body normalization、warning 输出、neutral response 构造、Chat upstream error body read 与 Axum bridge 已迁入 `proxy::response_adapter::codex_chat_upstream_error_response_to_axum_response`；handler 只负责 Axum transport、`RequestContext` 和 transform 调度。
541. provider settings 与 Claude Desktop route 到 request body model mapping 的投影已收敛到 host-owned `proxy/host/cc_switch/forwarder_request_source.rs`；`RequestForwarder` 只负责 transport 准备和可选 debug 日志输出，`proxy_core_adapter` 不再暴露 forward request model mapping façade。
542. Codex Responses -> Chat endpoint rewrite 与 Gemini Native URL build/resolve 投影已迁入 `proxy-core::request_url`；forwarder 的 Codex app gate 与 Responses->Chat provider predicate 已收进 request source transform plan；`RequestForwarder` 保留 provider adapter URL fallback、full-url query 拼接和 transport 分支，`proxy_core_adapter` 不再承载 Codex/Gemini URL planning 聚合 fixture。
...
544. 上游请求 `anthropic-beta` 组装、ordered request headers 构建与 method-aware body serialization 已迁入 `proxy-core::request_headers` / `proxy-core::request_body`；`RequestForwarder` 只负责收集 upstream host、auth/session headers 和 managed-account 校验，`proxy_core_adapter` 不再承载 header/body serialization 聚合 fixture。
...
546. 上游请求 transport/streaming/SOCKS 发送策略已迁入 `proxy-core::request_transport`，发送分派由 `proxy/transport/upstream` owning module 消费 core policy；`RequestForwarder` 只保留 transport source 调用与 response priming，`proxy_core_adapter` 不再承载 request/send transport policy 聚合 fixture；forward pipeline 实现已迁到 `proxy/engine/forward_pipeline.rs`，旧 `proxy/forwarder.rs` 已删除，`proxy/mod.rs` 只保留面向现有调用的 `RequestForwarder`/`ForwardResult`/`ForwardError` 精确 re-export。
547. Codex 官方客户端 User-Agent 判定已迁入 `proxy_core::api::transport::is_official_codex_client_user_agent`；Codex provider 测试不再经 adapter 中转该 pure transport helper。
...
550. Copilot OAuth/API URL 构造、Copilot API base fallback 与模型/usage 响应解析入口仍由 host 适配层承接；`CopilotModel` 与 `CopilotUsageResponse` DTO 入口已从 `proxy_core_adapter` 二次出口改为 Copilot 命令和 `copilot_auth` 直接引用 `proxy_core::api::model_catalog`，Copilot auth 模块只负责 HTTP 调用、token/OAuth 状态和 endpoint/model 缓存，不再作为这些 core DTO 的二次出口。
551. Claude Desktop gateway bearer token 校验与 auth error 类型入口已迁入 core/auth adapter 边界；host auth adapter 只负责委托 `ProxyEngine` 并映射为 `ProxyError`，gateway token DB source 归 `proxy/host/cc_switch/claude_desktop_gateway_auth_source.rs` 维护。
...
554. host `ProxyError` 到 HTTP status/display 的 `ProxyErrorStatusKind`、状态码和展示文案投影入口已迁入 `proxy::error_mapper`；host error 模块直接经 `proxy_core::api::errors` 消费 HTTP status/body helper，`proxy_core_adapter` 不再作为这些 pure error response contract 的中转 re-export，也不再承载 HTTP status/body/error-message contract fixture。
555. settings command/DAO 与 forward pipeline 使用的 `RectifierConfig`、`OptimizerConfig`、`CopilotOptimizerConfig` DTO 入口已从 `proxy_core_adapter` 二次出口改为直接引用 `proxy_core::api::ports`；host settings DAO 继续负责 settings key、JSON 持久化与 `AppError` 映射，frontend JSON/default contract fixture 已归 `proxy-core::ports` owning tests，boundary 测试只允许这些精确 config DTO direct-core 行。
...
561. response adapter 使用的 pure core response/transport DTO、response builder helper、endpoint path/query bridge、Gemini path model extraction、event envelope、management request/response DTO、model catalog response DTO、auth/domain/ports/routing/transforms DTO 与 parser config 已改为直接经 `proxy_core::api::{auth,domain,transport,events,management,model_catalog,ports,routing,transforms,usage}` 引用；`proxy_core_adapter` 只保留其它宿主仍需的 request/result/core response alias 与 runtime/host 投影 helper，不再作为 response adapter 的 pure transport/parser-config/management/model catalog/auth/domain/ports/routing/transforms DTO、HTTP handler typed signature DTO 或 response build context 中转入口。HTTP handler 的 typed JSON 签名类型已改由 `proxy::response_adapter` re-export，handler 只从 `proxy_core_adapter` 导入 `ProxyState`；response pipeline、response adapter 和 error mapper 直接引用 core response build context。host response adapter 继续只负责映射到内部 `ProxyResponse` 与 Axum response/SSE event，response pipeline 实现已迁到 `proxy/engine/response_pipeline.rs`，旧 `proxy/response_processor.rs` 已删除，调用方直接使用 owning engine/adapter 路径。
562. route attempt 使用的 route candidate、resolved channel attempt、route plan 与 route selection DTO 已改为直接经 `proxy_core::api::routing` 引用；`route_attempt.rs` 是 host/core routing 的批准适配文件，`proxy_core_adapter` 不再 re-export route selection candidate/attempt helper。
563. session usage 与 usage stats 服务使用的 cost calculator、pricing、token usage 与 session request id prefix 已改为直接经 `proxy_core::api::usage` 引用；usage sink/database bridge 由 host-owned `database_usage_sink` 承接 `UsageRecord` 到 host request log/cost breakdown 的投影，host 继续负责 DB 查询、日志文件解析、dedup 与 request log 落库。
564. handler context 使用的 response runtime policy/timeout DTO、`ProxyResult`、`UsageRouteContext`、Gemini path model 提取、Claude metadata API-format 提取、session id 提取与 selected-provider fallback 文案已改为直接经 `proxy_core::api::{config,domain,errors,session,transport,transforms,usage}` 引用；app config raw 投影、host `AppType -> AppKind`、provider Claude API-format fact 与 `ProxyState`/`ProxyServices` 仍由 host 边界承接。`RequestContext` 实现已迁到 `proxy/engine/context.rs`，旧 `proxy/handler_context.rs` 已删除；route result 后的 DB provider hydration 已改为 `RequestContextProviderSource` source 注入，CC Switch 默认 DB 实现位于 `proxy/host/cc_switch/request_context_provider_source.rs`。
565. proxy server 使用的 runtime config/status/info、current route target、Gemini shadow store、ProxyEngine、route resolve request 与 server log code 入口曾阶段性迁入 `proxy_core_adapter`；后续已继续收敛到 owning module：HTTP lifecycle 具体实现已回到 `proxy/transport/http/server.rs`，server 启动/停止时的 started/stopped event、监听端口记录、runtime status/start_time 写入和 `ProxyServerInfo` 组装由 `proxy/host/cc_switch/proxy_server.rs` 承接。Axum route tree、管理 API middleware、body limit、listener 地址解析/bind/local address/error mapping、Hyper HTTP/1.1 accept loop、raw header-case capture、connection serving、accept-loop stop wait 超时、日志和 `StopFailed`/`StopTimeout` 映射、shutdown sender/server handle storage、AlreadyRunning/NotRunning gate、start/stop 编排均由 transport/host 模块拥有；`proxy_core_adapter` 不再保留 HTTP lifecycle helper 或 server runtime side-effect re-export，旧 `proxy/server.rs` 与 `proxy/handlers.rs` 已删除，后续重点转为 handler 业务处理继续下沉 engine。
566. provider adapters 使用的 provider auth info 与 auth strategy DTO 初期由 `proxy_core_adapter` 承接；后续已逐步改为 provider owning module 直接引用 `proxy_core::api::auth`，host provider adapters 继续负责从 desktop Provider 配置提取凭证、构建上游 URL 与 header。
...
571. Claude provider 使用的 API-format resolution、auth-key/base-url 提取、upstream URL builder、static/Copilot auth header builder 与 prompt-cache key helper 初期迁入 `proxy_core_adapter`；后续 API-format 与 auth info/header projection 已回到 `proxy/provider/claude.rs` owning module，Claude 大请求/响应 transform 仍保留为下一组独立迁移切片。
...
574. route attempt 测试继续复用 provider/channel DTO alias，但 route candidate/attempt 转换改为直接 import core routing owning module；host 继续负责 provider cloning、AppType-specific override 与 ForwardAttempt 编排，边界白名单仅放行 `src/proxy/route_attempt.rs` 这一处 direct routing adapter。
575. usage sink bridge 使用的 usage record helper、stream event filter、transformed usage format、`UsageRecord` 与 `ProxyServices` 入口已收敛到 response pipeline / adapter 边界；host-owned usage sink 继续负责 `UsageRecord` 到 request log 的投影、usage logging 开关后的异步落库目标与 DB 持久化。
576. response processor 使用的非流式 decoded body bridge、response log event dispatch、passthrough streaming usage runtime source、非流式 usage runtime source、transformed streaming usage runtime source、transformed non-streaming usage runtime source、passthrough/transformed usage runtime orchestration、streaming/non-streaming/transformed response usage record builder、forward-error usage record helper、reusable sink-spawn helper、streaming/non-streaming passthrough response construction、Claude/Codex transformed SSE stream wrappers、Claude/Codex transformed JSON wrappers、shared logged-stream runtime loop 与 provider usage facts projection 已迁入 `proxy/engine/response_pipeline.rs`，并继续只消费 `proxy-core` 的 response body decode/log/SSE passthrough policy 规则、core usage builder、provider transform/history helper 与 core response builder；`response_pipeline` 使用的 streaming timeout config、pure response header predicate、core response DTO、usage parser/config/format/failure context、selected-provider-missing 文案与 route context 已改为直接经 `proxy_core::api::{config,transport,usage}` 引用，`proxy_core_adapter` 不再作为这些 config/usage/transport contract 的中转 alias，也不再为自身测试 re-export response pipeline helper；host 继续负责 Axum response 转换、connection guard 生命周期与 `ProxyState` 调度。
...
594. `ProxyErrorStatusKind` 到 `ProxyCoreError`/`ForwardFailureKind` 的分类规则和 forward failure message 选择策略已下沉到 `proxy-core`；host `error_mapper` 负责把 `ProxyError` 投影为 kind/raw-message/display-message/body 事实，通用 core-to-host error mapping、status projection 与 forward failure projection 不再经 `proxy_core_adapter` re-export。
...
599. 管理 API dry-run route 与 runtime `ProxyEngine::plan_materialized_route` 已新增同源排序测试：同一组 materialized channels 下，host route resolver 的 `management_route_response_from_router_source` 和 engine materialized plan 对 priority/weight 排序、模型匹配和首选 channel 选择保持一致，防止调试接口与真实转发路径漂移。
...
606a. channel key runtime 已接入失败回退闭环：`CcSwitchForwarderAttemptRuntimeSource::record_failure` 会把本次实际选中的 `channel_auth_key_ref` 写回 `proxy_channel_keys.last_failure_at`；`CcSwitchChannelKeyRuntimeSource` 读取同 channel 候选 key 时会结合 `healthPolicy.keyFailureCooldownMs`（兼容 `channelKeyFailureCooldownMs` / `key_failure_cooldown_ms`）、priority/weight 和通配 `channel-key:*` 选择下一个健康 key，失败 key 冷却结束后可重新参与选择。
607. legacy managed-auth command DTO 入口已从 `proxy_core_adapter` 二次出口改为 `commands/auth.rs`、`commands/copilot.rs`、`proxy/copilot_auth.rs` 与 `proxy/codex_oauth_auth.rs` 直接引用 `proxy_core::api::auth::{CopilotAuthStatus, GitHubAccount, GitHubDeviceCodeResponse, CodexOAuthStatus}`；命令层不再通过 `proxy::copilot_auth` 兼容路径消费这些 DTO，boundary 测试只允许这些精确 DTO direct-core 行。
608. ProviderRouter 的 channel failure-threshold 规则已由 `proxy/engine/routing.rs` 直接引用 `proxy_core::api::routing::effective_channel_health_failure_threshold`；`proxy_core_adapter` 不再 re-export `RoutePolicy`、failover provider id 提取、auto-failover config decision 或 failover config log line 等 route-policy helper，只在 adapter 内部私有消费仍需桥接宿主 DB/source 的投影规则。
609. ProviderRouter 的 channel reset fact 构造已由 `proxy/engine/routing.rs` 直接引用 `proxy_core::api::ports::channel_health_reset_from_parts`；`proxy_core_adapter` 不再 re-export 该 reset helper，只保留 adapter 内部 reset/stat projection 和仍需装配的 host service port 出口。
610. Channel breaker stats 的 core fact builder、`ChannelBreakerStats` 与 `ChannelHealthReset` fact 类型已从 `proxy_core_adapter` re-export 移出；host `CcSwitchChannelHealthStore` / `CcSwitchProviderRouterHealthStore` 继续直接引用 `proxy_core::api::ports` 的 health fact 类型。`CcSwitchChannelHealthStore` 已拥有 channel attempt DB update projection、channel reset app lookup/router reset wrapper 和 breaker stats projection；`CcSwitchProviderRouterHealthStore` 已拥有 ProviderRouter provider health DB update projection、router-facing channel health wrapper 和 channel reset wrapper。
...
613. `ForwardUpstreamUrlPlan` 已从 `proxy_core_adapter` re-export 降为 adapter/test 私有 import；默认 `ForwarderRequestSource` 与 owning forward pipeline contract 继续直接引用 `proxy_core::api::transport` 的 URL plan contract，adapter 不再作为 request-source contract 的中转出口。
...
615. 默认 `CcSwitchForwarderRuntimeStateSource` 已直接引用 `proxy_core::api::transport` 的 forward failure 分类、rectifier retry failover 判断和 terminal/no-available status 文案 helper；`proxy_core_adapter` 不再 re-export 这些纯 runtime failure helper，也不再保留 host `ProxyError -> ForwardFailureKind` 投影 bridge，该投影由 `proxy::error_mapper` 拥有。
616. Claude/Codex/Gemini provider adapter 的 upstream URL builder 已改为直接引用 `proxy_core::api::{transport,transforms}` 的 core helper；`proxy_core_adapter` 不再 re-export `build_claude_upstream_url`、`build_codex_upstream_url` 或 `build_gemini_upstream_url`，auth header wrapper 仍留在 adapter 承接 host 错误文案和 Copilot header 常量。
...
625. config service 的 Codex restored live settings 解析 helper 已改为直接引用 `proxy_core::api::ports`；`proxy_core_adapter` 不再 re-export `codex_restored_live_settings_parts`，仍保留接收 host `Provider` 的 Codex live settings/provider backfill wrapper。
626. config service 与 live provider service 使用的 Codex live settings/snapshot issue 类型已改为直接引用 `proxy_core::api::ports`；`proxy_core_adapter` 不再 re-export Codex live settings/snapshot/backfill parts 或 issue 类型，仅私有使用这些类型定义 Provider-shaped bridge 签名。
627. provider service、live provider service 与 live takeover host source 使用的 provider/live policy issue、scope、action 与 dispatch 类型已改为直接引用 `proxy_core::api::ports`；`proxy_core_adapter` 不再 re-export 这些纯 policy contract，仅私有引用它们定义 Provider-shaped wrapper 和 host error bridge。
628. provider service 使用的 provider settings validation spec、live config presence policy 和 key-change policy issue message helper 已改为直接引用 `proxy_core::api::ports`；`proxy_core_adapter` 不再 re-export 这些纯 helper，provider settings validation 仍通过 adapter wrapper 读取 host `Provider` settings 并把 `AppType` 投影到 core app kind。
629. lib startup/exit restore、provider service、live provider service 与 live takeover host source 使用的 takeover ownership、hot-switch、local proxy URL、runtime stopped、Gemini env takeover 和 placeholder removal helper 已改为直接引用 `proxy_core::api::ports`；`proxy_core_adapter` 不再 re-export 这些纯 proxy/live policy helper，仍保留需要 Codex TOML 与 host backup/provider 投影的 wrapper。
630. provider deletion current-provider predicate 与 Claude live settings sanitize helper 已改为由 provider/live 调用方直接引用 `proxy_core::api::ports`；`proxy_core_adapter` 不再 re-export 这些纯 helper，host 侧仍通过 provider module re-export 保持现有 service/config 调用面。
...
611. response build failure 的协议上下文和日志前缀已迁入 `proxy-core::ProxyResponseBuildFailureContext`；host `error_mapper` 只消费 core context 并映射为既有 `ProxyError`，不再维护 Claude/Codex 响应构建失败文案表。
...
640. stream check 的配置 DTO/默认值/camelCase 序列化契约已迁入 `proxy-core::StreamCheckConfig`：host 继续负责 reqwest 探测与 provider 覆盖合并；service 内部直接使用 core DTO，commands/DAO 不再通过 service re-export 获取 `StreamCheckConfig`。
641. stream check 的结果 DTO/历史字段兼容契约已迁入 `proxy-core::StreamCheckResult`：host service 不再定义重复 response struct，也不再 re-export 该 DTO；commands/DAO/handler 直接引用 `proxy-core::api::management` 的对外契约。
...
646. channel health 的 success/failure 状态推进规则已迁入 `proxy-core::channel_health_update_from_input`：DAO 直接消费 `proxy-core::api::management` helper，只读取当前失败数并写入 core 计算出的 healthy/degraded/unhealthy、时间戳和 disabled reason。
647. channel health 的初始/缺省 unknown 状态已收敛到 `proxy-core::CHANNEL_HEALTH_UNKNOWN_STATUS`：DAO 直接消费 `proxy-core::api::management` 常量，创建 health row 与缺省读取不再硬编码字符串。
...
654. provider health success/failure 状态推进规则已新增 `proxy-core::provider_health_update_from_input`：provider DAO 直接消费 `proxy-core::api::management` helper，只读取当前失败数并写入 core 计算出的健康布尔值、失败计数、时间字段和 last_error，保持 SQLite 查询与 UPSERT 仍属于 host adapter。
...
656. app 级 proxy_config 默认值策略已新增 `proxy-core::app_proxy_config_defaults_for_app`：DAO 直接消费 `proxy-core::api::config` helper，缺省读取、单行 ensure 和三行 init 不再手写 claude/codex/gemini 的重试、超时和熔断 seed 值，只保留 SQL upsert/insert。
...
659. response transform failure 的协议上下文和日志前缀已迁入 `proxy-core::ProxyResponseTransformFailureContext`；host `error_mapper` 只消费 core context 并映射为既有 `TransformError`，不再维护 Claude/Codex 转换失败文案表。
...
662a. `ProxyServer::start` 级 runtime smoke 已覆盖真实上游转发闭环：真实本机端口创建 materialized channel、写入 `channel-key:<keyRef>`、`headerOverrides` 与 `paramOverrides`，再通过 `/v1/messages` 客户端入口转发到本机临时 upstream，验证 channel key 生成的 `x-api-key`、channel header/query override、public model 到 upstream model 的改写、上游 JSON 响应透传和 active route target 归因，并通过 `/proxy/v1/events` 验证 `route_selected` SSE 携带 channel/provider/public model/upstream model 事实。
662b. `ProxyServer::start` 级 runtime smoke 已覆盖 materialized channel 失败重试闭环：同一 provider 下两个 channel 分别指向本机临时 upstream，第一路返回 503 后按 channel priority / retry policy 继续尝试第二路，验证失败 channel 与成功 channel 分别使用独立 `channel-key`、模型改写不串线、provider fallback secret 不泄漏，并把 active route target 归因到最终成功 channel。
662c. `ProxyServer::start` 级 runtime smoke 已覆盖 wildcard channel-key failure cooldown 闭环：真实本机端口通过管理 API 创建 `channel-key:*` channel 和 priority 策略 key 池，第一次请求用高优先级 key 命中 503 后写回 `last_failure_at`，第二次请求在 cooldown 窗口内改用 backup key 并成功返回，验证同一 channel 的 key failure 回退不依赖 DAO/unit-only 假设。
662c-1. `ProxyServer::start` 级 runtime smoke 已覆盖 wildcard channel-key round-robin 闭环：真实本机端口通过管理 API 创建 `channel-key:*` channel 和同优先级 key 池，连续两次 `/v1/messages` 转发分别使用 alpha/beta key 的 `x-api-key` 进入本机 upstream，验证轮询策略已经穿透到真实认证 header，而不是只停留在 DB source/unit selector。
662d. `proxy-core` public prelude 已覆盖外部宿主 route-visible catalog 多 channel 筛选：外部 relay services 可注入同 provider 下不同 base URL、interface、模型映射、group、priority/weight 的 channel，并通过 `ProxyEngine::list_model_catalog_for_request` 验证 premium/default group、Anthropic/OpenAI Chat/OpenAI Responses 兼容接口和 pricing model 归因均由 core 管理 API contract 输出，不依赖 CC Switch host adapter。
...
666. legacy channel migration 的 provider settings 投影已迁回 `database/dao/proxy_channels.rs`：DAO 直接解析 Codex TOML、env/modelCatalog 和 Claude Desktop model routes，装配 core legacy projection input，不再经 `proxy_core_adapter` 中转。
667. legacy channel projection 到 `ProxyChannelRecord` / `ProxyChannelModelRecord` 的 host 映射已迁回 DAO 私有 helper：DAO 拥有 core projection 到 DB row shape 的逐字段映射、source kind 映射和 preview 组装，`proxy_core_adapter` 不再暴露 legacy channel migration preview helper。
668. forward result 到 `ProxyResult` 的 selected route、metadata 和 outbound model 投影已移入 host-owned `proxy/host/cc_switch/forward_pipeline.rs::proxy_result_from_forward_parts`：`proxy_core_host` 只保留测试兼容壳，结果 contract 由 host forward pipeline 统一生成。
669. `CcSwitchChannelSource` 的 channel record 到 `ChannelSpec` 投影与 `ChannelQuery` 过滤已收敛到 `proxy/host/cc_switch/database_channel_source.rs`：host-owned source 负责选择 legacy/materialized 数据来源、读取 DB/router 并映射 host 错误，`proxy_core_host` 只装配 source。
670. `CcSwitchProviderSource` 的 DB provider 到 `ProviderSpec` 投影已收敛到 host-owned `proxy/host/cc_switch/provider_projection.rs`：当前默认 source 已迁到 host-owned `provider_source.rs`，负责 provider list/get/current 查询与 active-route runtime map 读取，metadata 脱敏和 provider kind 推断由 host projection 模块采集 CC Switch `Provider/ProviderMeta` 事实后交给 core 规则生成。
671. route policy 的 failover queue provider id 到 `RoutePolicy` raw contract 投影已新增 `proxy-core::route_policy_from_failover_provider_ids` 并由 host-owned `CcSwitchRoutePolicySource` 直接消费；adapter 不再保留 route policy DB/source/queue façade。
672. `CcSwitchConfigSource` 的 global/app/runtime config DTO 投影已新增 `proxy-core` helpers 并由 host-owned source wrapper 接入：config source 负责读取 DB/settings，`ProxyGlobalConfig`、`ProxyAppConfig`、optimizer specs 与 `ProxyRuntimeConfig` 的 raw contract 由 core 统一生成。
...
675. client model catalog 的空目录默认 raw contract 已新增 `proxy-core::client_model_catalog_from_optional_raw` 并经 host adapter 接入：host-owned `CcSwitchModelCatalogProvider` 通过 adapter helper 读取 Codex 本地 catalog raw，非 Codex 默认空模型列表由 core 统一生成。
676. channel health attempt 的默认 failure threshold 已收敛到 `proxy-core::DEFAULT_CHANNEL_HEALTH_FAILURE_THRESHOLD`；`CcSwitchChannelHealthStore` 不再维护独立阈值常量，只负责把 attempt fact 写入 DB。
677. usage request log 字段、成本计算和缺失定价告警文案已新增 `proxy-core::usage_request_log_projection` 并由 host-owned `CcSwitchUsageSink` 映射到 `RequestLog`：usage sink 负责读取计费配置/定价、提供 request_id fallback、执行 host 落库和输出 core 生成的 warning，adapter 不再参与 usage request-log projection。
...
680. forward runtime 的 timeout/retry 投影已统一由 host runtime `response_runtime_policy_from_app_proxy_config` 复用 `proxy-core::resolve_response_runtime_policy`：`CcSwitchProxyRuntime::forward_host` 和 `ProxyHandlerContext` 不再各自展开 auto_failover_enabled、max_retries 与三类 timeout 字段。
...
683. forward runtime 当前 provider 来源优先级已新增 `proxy-core::current_provider_id_from_sources` 并经 host runtime 接入：host 只负责读取 settings 与 DB 两个来源，settings 优先、DB 兜底、缺失时空串的选择策略由 core 统一锁定。
...
686. host adapter 的 context/error 拼接格式已新增 `proxy-core::error_message_with_context` 并经 adapter/host source 接入：`proxy_core_host` 的 `app_error` 只负责选择 `Config` variant，usage sink 等 owning module 直接调用 core helper 选择 `Internal` variant，不再维护 `{context}: {error}` 文案格式。
...
688. current provider 来源优先级的 `Option<String>` 形态已新增 `proxy-core::current_provider_id_option_from_sources`：host-owned `CcSwitchConfigSource::load_app` 负责读取 settings 当前 provider 事实，`ProxyAppConfig.raw.currentProviderId` 的 option contract 由 core helper 统一生成；forward runtime 的 string helper 继续复用同一来源选择规则。
...
691. host `ProxyResponse` 到 `ProxyCoreResponse` 的 buffered/streamed bridge 已移入 `proxy/host/cc_switch/forward_pipeline.rs::proxy_response_to_core_response`：`proxy_core_host` 不再维护响应体/stream guard 转换细节，adapter 也不再保留该 bridge façade。
691a. `ProxyCoreResponse` 到 Axum `Response` 的 transport bridge 已移入 owning `proxy/engine/response_pipeline.rs`：stream/non-stream passthrough 与 transformed response helper 共用同一 core-to-Axum bridge，`response_adapter` 只复用 pipeline 出口，不再被 engine 反向依赖。
691b. Codex Chat 上游错误响应与 Codex proxy error 的 Axum bridge 已移入 owning `proxy/engine/response_pipeline.rs`：错误体 normalization warning、上游错误 body read、Codex proxy error body build-error 映射和 `CodexResponsesError`/`CodexProxyError` Axum build context 选择不再由 `response_adapter` 承接。
...
693. host `ForwardResult` 到 `ProxyResult` 的 selected route、metadata、outbound model 和 response bridge 组合已移入 `proxy/host/cc_switch/forward_pipeline.rs::forward_result_to_proxy_result`：`proxy_core_host` 不再拆 host forward result，adapter 也只调用 host forward pipeline owning helper。
...
695. channel-key auth profile 的 missing-key 错误包装已移入 `proxy-core::provider_auth::channel_auth_profile_missing_key_error`，provider settings patch helper 已迁入 `proxy/host/cc_switch/auth_provider.rs::provider_with_channel_auth_key` 并经 adapter re-export：`proxy_core_host` 只保留 channel key DB 查询和 attempts 变更，不再 clone provider 或注入 auth key settings。
696. host `AppError` 到 `ProxyCoreError::Config` 的上下文包装仍由 `proxy_core_adapter::app_error` 承接；usage sink 等 host-owned module 对 `Internal` 错误直接调用 core error helper，`proxy_core_host` 不再直接维护 `error_message_with_context` 文案。
...
698. forward pipeline 缺少 runtime 时的 Unsupported 错误包装已直接使用 `proxy-core::forwarding_requires_runtime_error`：host forward pipeline 不再经 `proxy_core_adapter` 选择 `ProxyCoreError` variant 或复制 runtime-required 文案。
...
702. Codex client model catalog 的 active config 路径解析、文件读取与 stale guard fallback 已移入 host-owned `CcSwitchModelCatalogProvider`：`proxy_core_host` 只装配 model catalog provider，adapter 不再承接 client catalog raw helper。
...
704. app config source 的当前 provider settings 读取、DB 配置/优化器读取与 `ProxyAppConfig` parts 组装已收敛到 host-owned `CcSwitchConfigSource`；该 source 直接调用 core config builder，`proxy_core_adapter` 不再公开 config-source-only DB helper。
704a. `/status` 的基础 HTTP JSON bridge 已从 `proxy::response_adapter` 移回 `proxy/transport/http/handlers.rs`：handler 只构造 `ProxyStatusRequest`、调用 `ProxyState::proxy_engine().proxy_status_response` 并映射 core error，不再直接读取 runtime status lock；`response_adapter` 继续聚焦协议 dispatch、复杂管理 API bridge 和响应编排。
705. runtime config source 的 host 默认 privacy-filter flag 包装已收敛到 host-owned `CcSwitchConfigSource`；`proxy_core_host` 不再直接传入固定 `false` 构造 runtime config，`proxy_core_adapter` 也不再持有该 source 的 runtime config DB helper。
706. ProviderSource 的列表/单条 provider 到 `ProviderSpec` 投影已移入 host-owned `proxy/host/cc_switch/provider_projection.rs`：`proxy_core_host` 和 `proxy_core_adapter` 不再直接解析 app kind 或持有 provider spec conversion。
...
708. ChannelHealthStore 的 attempt 写库参数投影和 DB 写入调用已移入 `proxy/host/cc_switch/channel_health_store.rs` owning module；ProviderRouter health store 的 provider raw DB update 投影已移到 `proxy/host/cc_switch/provider_router_health_store.rs` owning module，adapter 不再保留 `record_provider_attempt_in_db_source` 或 channel health attempt/update 公开入口，`proxy_core_host` 只装配对应 store。
709. `ProxyCoreEvent` 到 host event bus name/payload 的 raw 投影已迁到 `proxy/host/cc_switch/event_sink.rs` owning module：`CcSwitchEventSink` 直接调用 core event 的 `event_name()` 与 `into_event_payload()` 后 emit 到 host bus，`proxy_core_adapter` 只保留私有 runtime-event helper 给自身内部 request/server/attempt event source 复用，不再公开 generic event bus dispatch facade。
710. RoutePolicySource 的 failover queue 到 optional `RoutePolicy` source 投影、DB queue 查询与错误映射已由 host-owned `CcSwitchRoutePolicySource` 包装；该 source 直接调用 core route-policy 与 config error helper，`proxy_core_adapter` 不再保留 route policy DB/source/queue facade。
711. ChannelHealthStore reset 的 app lookup 结果校验、router reset 调用、reset fact 投影和 DB/router 错误映射已由 host-owned `CcSwitchChannelHealthStore` 包装；`proxy_core_adapter` 不再保留该桥接 helper，`proxy_core_host` 不再保留该桥接实现。
...
713. UsageSink 的 provider/app 计费配置 lookup 输入已迁回 `proxy/host/cc_switch/database_usage_sink.rs`：host-owned `CcSwitchUsageSink` 负责调用 usage logger 读取配置、定价和落库，`proxy_core_host` 只装配 sink。
714. `ProxyCoreEvent` 的 event bus 投影与分发闭包入口不再作为 `proxy_core_adapter` 公开 API；`proxy/host/cc_switch/event_sink.rs` 拥有 host bus emit 副作用，adapter 内仅保留私有 helper 供自身 runtime event source 复用。
715. forward runtime 的 `RoutePlan` + host providers 到 `ForwardAttempt` 列表构造入口已迁入 `proxy/host/cc_switch/channel_auth_profile_attempts.rs`：`CcSwitchProxyRuntime` 直接消费 owning host helper，`proxy_core_adapter` 不再 re-export forward attempt helper。
716. forward runtime 的 auth profile provider/channel-key 应用循环已迁入 `proxy/host/cc_switch/channel_auth_profile_attempts.rs`：`CcSwitchProxyRuntime` 通过注入的 `ChannelKeyRuntimeSource` 调用该路径，`proxy_core_adapter` 不再作为兼容中转面。
717. forward runtime 的 current-provider settings/DB fallback 组合已移入 `proxy/host/cc_switch/proxy_runtime.rs`，并复用 core `current_provider_id_from_sources` / `current_provider_db_fallback_required`；`proxy_core_adapter` 不再保留该 helper。
718. forward runtime 的 required attempts 空结果错误判断已迁入 `proxy/host/cc_switch/channel_auth_profile_attempts.rs`：host runtime 只接收可执行 attempts 或 core error。
719. `ProxyRequest` 到 host forward runtime 输入的 app/body/method/header/session 投影已移入 `proxy/host/cc_switch/proxy_runtime.rs`：adapter 不再直接拆 `ProxyRequest`、调用 `ProxyBody::into_json` 或抽取 session id。
720. `AppProxyConfig` 到 `RequestForwarder` timeout/retry 选项的投影已移入 `proxy/host/cc_switch/proxy_runtime.rs`：host runtime 不再直接展开 `ResponseRuntimePolicy.timeout`。
...
721. forward runtime 的 `AppProxyConfig`、rectifier、optimizer 与 Copilot optimizer 组合已收敛为 `proxy/host/cc_switch/proxy_runtime.rs::forwarder_runtime_config_from_sources`：host runtime 读取 DB 配置并把 grouped `ForwarderRuntimeConfig` 交给 forwarder，不再把 runtime config 作为散落局部变量维护。
...
724. management handlers 的 provider summary/current route provider 投影已改为调用 host-owned provider projection helper：handler 不再直接依赖 provider projection trait，只保留 HTTP path、DB 查询和 response envelope 调用。
...
732. health/status/Claude Desktop model-list/app list/provider list/app model list/Codex client model catalog 基础管理路由已继续收敛：health 只在 HTTP handler 内消费 core `HealthCheckRequest` 并注入当前时间，status 的 request/response helper 与 runtime status engine 调用已由 HTTP handler 直接承接且仍禁止直接读取 runtime status lock，Claude Desktop model-list typed JSON bridge 也已由 HTTP handler 直接承接且仍保留 gateway auth，app list 的 `AppListRequest` 与 `ProxyEngine::app_list_response` 调用、provider list 的 `ManagementAppPathRequest` 与 `ProxyEngine::provider_list_response` 调用、app model list 的 `AppModelCatalogRequest` 与 `ProxyEngine::list_model_catalog_for_request` 调用、Codex client model catalog 的 `AppKind::Codex` 与 `ProxyEngine::client_model_catalog_response` 调用也已由 HTTP handler 直接承接；handler 不再直接读取 runtime/status/provider/model catalog 细节，只保留 Axum 提取、基础 typed JSON bridge 和 Claude Desktop gateway auth。
...
734. host 生产路径的 `ProxyEngine` 构造入口先前收敛到 `proxy_core_adapter::proxy_engine_from_services`，使 `ProxyState` 和 forwarder 的 materialized route planning 不再散落直连 `ProxyEngine::new`；后续已删除该一跳 facade，并由 host-owned `ProxyState::proxy_engine` 在 `proxy/host/cc_switch/proxy_state.rs` 直接构造 engine。
735. `proxy_core_boundary` 已新增生产代码 `ProxyEngine::new` 禁用扫描：除 `proxy/host/cc_switch/proxy_state.rs` 外，host 生产源码必须通过 `ProxyState::proxy_engine` 构造 core engine，测试模块中的直接实例化仍可用于验证 services。
...
739. `RequestContext` 的 app config raw 投影已迁回 `proxy/engine/context.rs` owning module：context 直接把 core `ProxyAppConfig.raw` 投影为 `AppProxyConfig` 并生成 `ResponseRuntimePolicy`，不再为了请求上下文配置读取依赖 `proxy_core_adapter`。
740. Claude Desktop model-list 的基础 HTTP JSON bridge 已从 `proxy::response_adapter` 移回 `proxy/transport/http/handlers.rs`：handler 只执行 gateway bearer 校验、调用 `ProxyState::proxy_engine().claude_desktop_model_list_response` 并映射 core error，provider selection 与 model-list response 构造仍留在 `ProxyEngine`/core source 边界。
...
741. `ProxyResult` 回填 `RequestContext` 的 outbound model、usage route context 与 selected provider hydration 已迁回 `proxy/engine/context.rs` owning module：context 直接消费 core route/usage/error helper 与 host `ForwardAttempt` 映射；`UsageSink` 的 pricing lookup/model source 选择、`UsageRecord` 到 `RequestLog` 的投影与 missing pricing warning emission 已由 host-owned usage sink 承接。
742. `/proxy/v1/apps` 的基础 HTTP JSON bridge 已从 `proxy::response_adapter` 移回 `proxy/transport/http/handlers.rs`：handler 只构造 `AppListRequest`、调用 `ProxyState::proxy_engine().app_list_response` 并映射 core error；app summary source 读取和 envelope 构造仍留在 `ProxyEngine`/core source 边界。
743. `/proxy/v1/apps/{app}/providers` 的 HTTP JSON bridge 已从 `proxy::response_adapter` 移回 `proxy/transport/http/handlers.rs`：handler 只校验 app path、调用 `ProxyState::proxy_engine().provider_list_response` 并映射 core/management error；provider 查询、current/failover/routeCandidate facts 和 envelope 构造仍留在 `ProxyEngine`/core source 边界。
744. `/proxy/v1/apps/{app}/models` 与 Codex `/v1/models` 的 HTTP JSON bridge 已从 `proxy::response_adapter` 移回 `proxy/transport/http/handlers.rs`：handler 只构造 `AppModelCatalogRequest` 或固定 `AppKind::Codex`、调用对应 `ProxyEngine` 模型目录方法并映射 core/management error；route-visible model facts、client catalog source selection 和 raw catalog envelope 仍留在 `ProxyEngine`/core source 边界。
745. route inspection 类 HTTP JSON bridge 已从 `proxy::response_adapter` 移回 `proxy/transport/http/handlers.rs`：`/proxy/v1/channels`、`/proxy/v1/apps/{app}/channels`、`/proxy/v1/groups`、`/proxy/v1/apps/{app}/routes/current` 与 `/proxy/v1/route/resolve` handler 只做 path/query/body request factory、`ProxyEngine` 调用、error mapping 和 `Json` 包装；channel source、route dry-run、current route facts 与 response envelope 仍留在 `ProxyEngine`/core source 边界。
746. channel migration 与 breaker 类 HTTP JSON bridge 已从 `proxy::response_adapter` 移回 `proxy/transport/http/handlers.rs`：preview/materialize handler 只构造 `ManagementAppPathRequest` 并调用 migration response 方法，breaker stats/reset handler 只构造 `ChannelPathRequest` 并调用 breaker response 方法；legacy migration planning、materialize side effect、breaker stats/reset source 和 envelope 仍留在 `ProxyEngine`/core source 边界。
747. channel mutation 类 HTTP JSON bridge 已从 `proxy::response_adapter` 移回 `proxy/transport/http/handlers.rs`：channel CRUD、channel key CRUD、channel model list/replace 和 channel test handler 只做 path/body request factory、`ProxyEngine` 调用、error mapping 和 `Json` 包装；DB mutation、key/model/test source、副作用和 response envelope 仍留在 `ProxyEngine`/core source 边界。至此 `response_adapter` 不再持有 management `dispatch_*_to_axum_json_response` bridge，只保留协议请求/响应 transport adapter 职责。
742. `RequestContext::new` 不再调用 `ProviderRouter` selection API 或提前保存 provider：provider 只在 `ProxyEngine` 成功返回 route result 后由 `apply_proxy_result` 回填；转发入口 handler 的 `ProxyRequest` app/interface/body/context bridge 已继续收敛到 `proxy::response_adapter::ParsedAxumJsonProxyRequest` 的协议专用 `into_*_proxy_request` 方法，forward error usage 与 Codex error envelope 在选路失败时使用 app/tag fallback。
743. `proxy_core_boundary` 已新增 `RequestContext` provider preselect 禁用扫描：`engine/context.rs` 生产代码不得重新调用 `provider_router`、`.select_providers(` 或 `.select_provider_ids(`，防止请求 context 重新承担 route planning。
744. channel reachability probe 已新增 `ChannelReachabilityProbe` core 端口，probe request 的 app 解析、provider 缺失、DB provider/config 读取、`StreamCheckService` 副作用和 probe 错误包装均由 `proxy/host/cc_switch/channel_reachability_probe.rs` 的 DB-backed `CcSwitchChannelReachabilityProbe` 包装；host probe 直接消费 core `ChannelReachabilityProbe` / `ChannelTestProbeRequest` / `ChannelReachabilityResult` / `ProxyCoreResult` contract，并直接使用 core config error helper 映射 DB 错误，`proxy_core_adapter` 不再 re-export probe port、请求/结果 DTO 或 DB error helper。
745. app namespace catalog 已收敛到 `ProxyConfigSource::list_apps` 端口，CC Switch 桌面宿主的当前 app catalog 事实由 host-owned `CcSwitchConfigSource` 提供；route candidate provider selection 的 host router 结果已经直接是 provider id 列表，错误兼容包装与 empty-selection policy 委托已由 host-owned `CcSwitchProviderSource` 本地承接；Claude Desktop model route 的 provider id selection 结果、Provider 回查、错误包装和 route input 转换已迁回 host-owned `CcSwitchModelCatalogProvider` 本地实现，完整 Provider 只在该 provider 需要模型路由时通过 DB 加载；ProviderRouter 的 core route error 到 `AppError` 通用映射不再作为 adapter 公开 helper 保留，provider selection failure 映射不再作为 crate-visible helper 暴露，ProviderRouter health store 的 core-to-host 错误映射由 owning host store 本地承接；materialized channel 空表时是否加载 legacy projection 以及最终 records/source 返回已收敛到 `proxy_core_adapter::channel_route_records_from_sources`，`proxy_core_boundary` 已钉住 `proxy_core_host.rs` 生产代码不得直接构造 `ProxyCoreError` variant；`/proxy/v1/apps` 与 `/proxy/v1/groups` 的 handler 不再把 `AppType::all()` 直接传给 engine，`CcSwitchProviderSource`、`CcSwitchConfigSource` 与 `CcSwitchModelCatalogProvider` 均已迁为 host-owned source。
...
757. server lifecycle 的 runtime status 更新策略已迁入 `proxy-core::{record_proxy_server_started_status,record_proxy_server_stopped_status,apply_proxy_runtime_uptime}`：HTTP transport 继续负责 socket 监听，host-owned `proxy/host/cc_switch/proxy_server.rs` 负责 Instant 计时、事件发送、监听端口记录和 server info 组合，running/address/port/uptime 字段变更由 core 维护。
...
777. `RequestContext` 的请求模型推断已接入并直接经 `proxy_core::api::transport::request_model_for_forward` 引用：context 只传入 app/body 或 Gemini path 事实，普通 body `model` trim/empty fallback 与 Gemini path 提取规则由 core request URL policy 维护。
778. `RequestContext` 的 selected-provider 未回填 lifecycle 错误、route result provider source 缺失文案与 `unselected:<app>` fallback id 已接入并直接经 `proxy_core::api::errors::{selected_provider_not_applied_message,selected_provider_missing_from_source_message,unselected_provider_fallback_id}` 引用；context 负责 route result 到 usage/provider 事实回填，DB provider hydration 已由 host-owned `RequestContextProviderSource` 默认实现承接。
779. `RequestContext` 的 Codex 错误体 provider display name fallback 已接入并直接经 `proxy_core::api::errors::selected_provider_display_name_for_error` 引用：context 只提供 selected provider name 与 tag fallback 事实，“选中 provider 用 name，否则用 app tag” 的错误展示 contract 由 core 维护。
...
787. forwarder 的 GitHub Copilot upstream 判定已随 provider URL facts 进入 host-owned `ForwarderAdapterContext`：provider_type、full URL 标记与 base URL 投影后直接调用 core request URL policy，forwarder 与 `proxy_core_adapter` 都不再暴露单独的一跳 Copilot upstream helper。
...
789. forwarder 的 Provider 级自定义 User-Agent 投影已从 `proxy_core_adapter` 移到 `proxy/host/cc_switch/forwarder_request_source.rs`：request source 本地读取 `ProviderMeta.custom_user_agent` raw 值并调用 `proxy_core::api::transport::provider_custom_user_agent_header`，Copilot 指纹 UA 不可覆盖语义仍由 core policy 维护；纯 `parse_custom_user_agent` 解析规则由 provider/model-fetch 直接引用 `proxy_core::api::transport::parse_custom_user_agent`。
...
791. forwarder 的 provider settings 模型映射已改为 host-owned `proxy/host/cc_switch/forwarder_request_source.rs` 本地投影，text-only media 预防投影已改为 `proxy-core::request_media::apply_forwarder_media_prevention_from_facts`：forwarder 只传入 `Provider` 与运行期开关，settings schema 读取集中在 request source/core policy 边界。
792. forwarder 的 Anthropic thinking rectifier provider 判定已改为 `provider_projection::provider_uses_anthropic_rectifiers`：forwarder 不再直接调用 provider kind 推断并 matches `ProviderKind`，只消费 host-owned provider projection 给出的是否可运行 rectifier 布尔事实，adapter 不再暴露该 façade。
793. stream_check Copilot 动态 endpoint 预解析的 target 判定已移回命令层本地投影：命令层读取 `Provider.meta.provider_type` 与 `settings_config.env.ANTHROPIC_BASE_URL` 后直接调用 `proxy-core::transport::is_github_copilot_upstream`，完整 URL guard 继续使用 `Provider::is_full_url()`；adapter 只保留 managed-account id 解析供 OAuth manager endpoint 副作用使用。
794. stream_check service 的 base URL 投影已从 `proxy_core_adapter` 移回服务本地：标准 provider 直接经 host-owned `ForwarderAdapterContext` 读取 URL facts，OpenCode/OpenClaw/Hermes 继续调用 `proxy-core::domain` 的 additive base URL policy；自定义 User-Agent 与 provider `testConfig` override 投影也由服务本地读取 Provider meta 并调用 core policy。服务层继续负责 reachability 探测与错误包装。
795. managed account auth 的 Copilot/Codex OAuth 账号 ID 投影已改为 host-owned `provider_projection` 读取 `ProviderMeta.authBinding`/legacy `githubAccountId` 后交给 core 账号选择规则：认证模块只负责调用 OAuth manager，不再通过 `proxy_core_adapter` 暴露账号 ID façade。
796. provider usage 查询命令里的 usage script 已回到 `Provider::usage_script()` 本地模型 accessor，Copilot account 投影继续走 host-owned `provider_projection::provider_github_copilot_managed_account_id`：命令层保留模板分支和外部查询副作用，不再通过 adapter 读取普通 provider meta 字段。
本轮补充：ProviderRouter 运行时熔断配置热更新、provider breaker reset 与 provider breaker stats 查询已迁到 `proxy/host/cc_switch/provider_router_circuit_runtime.rs`：HTTP server 继续保留原有公开方法，但直接调用 host-owned ProviderRouter runtime helper；`proxy_core_adapter` 不再承接这组运行时副作用 helper。
...
798. saved usage script 查询服务的 usage script 读取已回到 `Provider::usage_script()`，script/provider credential 覆盖策略已改为直接调用 `proxy_core::api::ports::usage_script_credentials_from_parts`；custom endpoints 服务的列表排序与 last-used mutation 已回到 `Provider::{custom_endpoint_list,mark_custom_endpoint_last_used}`，URL 归一化、删除/last-used key 和空 URL 本地化错误映射已改为直接调用 `proxy_core::api::management::{normalize_custom_endpoint_url,custom_endpoint_url_key,custom_endpoint_url_issue_spec}`：`services/provider` 继续负责执行脚本、credential fallback、DB 读写与结果格式化，普通 provider meta accessor 与 URL policy 不再经 adapter 中转。
799. stream check 服务的 provider-level testConfig 读取已回到 `Provider::enabled_test_config()`；service 本地把启用后的 host testConfig 投影为 core `StreamCheckConfigOverride`，并继续负责与全局 `StreamCheckConfig` 合并。
...
801. proxy service 的接管策略 provider 判定已改为消费高层 managed-account/Codex OAuth 事实；GitHub Copilot account id 与 binding context 投影已收进 host-owned `provider_projection`，stream-check-only target 判定已回到命令层本地投影，不再通过 adapter 暴露。
...
804. Gemini provider adapter 的 API key 与 base URL 单字段 getter 已删除，raw provider kind/auth strategy helper 也已收成 adapter 私有；Gemini adapter 继续通过 `proxy_core_adapter::{provider_gemini_auth_info,provider_gemini_auth_headers,required_gemini_provider_base_url}` 对外入口工作，adapter 内部直接调用 core settings extractor 读取 `Provider.settings_config`。
805. Codex provider adapter 的裸 API key/base URL getter 已收成 adapter 私有 helper，公开入口保留 `proxy_core_adapter::{provider_codex_auth_info,required_codex_provider_base_url}`：Codex adapter 继续负责 auth header 与 upstream URL 构造，不再在 `extract_auth`/`extract_base_url` 中直接穿透 `Provider.settings_config`。
806. Codex provider adapter 的 chat-completions 判定与 upstream model 已改为 `proxy_core_adapter::{provider_codex_uses_chat_completions,provider_codex_upstream_model,provider_apply_codex_chat_upstream_model}`；catalog model ids raw helper 已收成 adapter 私有，Codex adapter 保留 request body 改写时机，settings/meta/TOML 投影集中在 adapter。
...
808. Claude provider adapter 的裸 auth key/base URL getter 初期收成 adapter 私有 helper；后续 `provider_claude_auth_info` 与 `provider_claude_auth_headers` 已迁回 `proxy/provider/claude.rs` owning module，`proxy_core_adapter` 不再暴露这两个 provider auth façade，Claude adapter 继续通过 host provider projection 读取 provider settings/meta 事实并直接调用 core auth/header helper。
809. Claude provider kind 的 API format、Google OAuth key shape、meta provider type、base URL/settings 组合推断已收成 host provider projection：Claude adapter 继续通过 provider projection 和 adapter auth/transform 入口消费高层事实，不再自行拼装推断输入；`provider_needs_claude_transform` 已由 `provider_projection` 拥有，adapter 不再暴露该 façade。
810. Claude transform 的 Responses prompt cache provider facts、OpenAI Chat 显式 prompt cache key 与 Codex fast-mode provider fact 初期收成 adapter 私有 helper；后续 Claude request api_format wrapper 已迁回 `proxy/provider/claude.rs` owning module，Claude adapter 与 forwarder protocol state source 直接调用该 provider helper，`proxy_core_adapter` 不再暴露 request transform façade。
811. Claude transform 的 OpenAI Chat reasoning_content 保留判定初期收成 adapter 私有 helper；后续该判定随 request transform wrapper 回到 `proxy/provider/claude.rs`，provider helper 本地读取 `Provider.settings_config` 并直接调用 core `should_preserve_reasoning_content_for_openai_chat`。
...
813. `provider_kind_from_app_type_and_config` 的真实实现已收敛到 host-owned `proxy/host/cc_switch/provider_projection.rs`，Gemini/Claude kind 推断作为 CC Switch provider projection 事实保留，相关 provider kind inference fixture 也已迁到 owning module tests：`proxy/providers` 只保留兼容 facade，`proxy_core_adapter` 不再拥有该 helper 或 provider spec conversion。
814. provider terminal 启动环境变量投影已改为命令边界直接调用 `proxy-core::ports::launch_env_vars_from_provider_settings`：命令层只负责加载 provider、传入 `Provider.settings_config` 与 `AppKind`、启动终端，Claude/Codex/Gemini 的 settings schema、base URL env key 与 API key/env 转换规则集中在 core。
...
817. Claude Desktop proxy provider 的 credential shape 检测已收成 adapter 私有 helper，公开入口保留 `proxy_core_adapter::{provider_claude_desktop_proxy_provider_validation,provider_claude_desktop_provider_validation}`：`claude_desktop_config` 继续负责本地化错误和模式验证，base URL/API key/settings/meta schema 读取集中在 adapter，typed OAuth provider 的无静态 key 放行语义保持精确。
818. Claude Desktop 的 MiMo Anthropic thinking history normalize gate 已改为 `proxy-core::response_transform::should_normalize_mimo_anthropic_thinking_history`：`claude_desktop_config` 继续负责请求体改写，provider API format、MiMo 模型/endpoint 判定集中在 core response transform，host 只投影 provider settings/meta/upstream model facts。
819. Claude Desktop direct/proxy provider 的配置兼容性 validation issue helper 已收成 adapter 私有 helper，公开入口保留 `proxy_core_adapter::{provider_claude_desktop_direct_provider_validation,provider_claude_desktop_proxy_provider_validation,provider_claude_desktop_provider_validation}`：`claude_desktop_config` 继续负责本地化错误、route 校验与直连凭证提取，settings/meta schema 判定集中在 adapter。
...
821. OpenClaw live 写入 fallback 的 raw provider shape 检测、write plan 与 live import provider 投影已迁到 `services/provider/live.rs`：provider live 直接负责 typed parse/raw write 策略、空 ID/无模型校验和 display name 推导，`baseUrl`/`api`/`models` 字段存在性规则继续集中在 core/domain。
822. Codex live 默认导入的 provider category 推断已回到 `services/provider/live.rs` owning module，并直接调用 `proxy-core::ports::provider_default_live_import_category_from_parts`：provider live 继续负责导入与 DB 写入，`auth` 登录材料、`OPENAI_API_KEY` 与 TOML experimental bearer token 的官方/自定义分类规则集中在 core。
823. legacy config sync 的 Codex live settings 结构校验与 `auth`/`config` 提取已回到 `services/config.rs` owning module，并直接调用 `proxy-core::ports::codex_live_settings_parts_from_settings`：`services/config` 继续负责旧配置同步和中文错误消息，provider settings shape 策略集中在 core。
...
825. provider live snapshot 的 Codex `auth`/`config` 提取已回到 `services/provider/live.rs` owning module，并直接调用 `proxy-core::ports::codex_live_snapshot_parts_from_settings`：live 写入继续负责文件落盘和错误文案，snapshot 路径保留旧的 auth 存在性语义，字段 shape 策略集中在 core。
826. provider live snapshot 的 OpenCode provider fragment 投影、write plan 与 live import provider 投影已迁到 `services/provider/live.rs`：live 写入继续负责 typed/raw 写入和日志，整份 opencode config 到单 provider fragment 的兼容提取由 provider live service 本地承接并复用 core ports。
...
836. Gemini live 写入的 provider settings 到 `.env` map 投影已改为 `services/provider/live.rs` 直接调用 `proxy-core::ports::gemini_env_string_map_from_settings`，strict validation 入口暂留 `proxy_core_adapter::validate_provider_gemini_settings_strict` 做 `AppError` 映射：provider live 继续负责 auth mode 分支、原子写 `.env`、settings.json 合并与安全标记写入，Gemini env schema 转换集中在 core。
837. ProviderService Gemini credential 提取的 settings 到 `.env` map 投影不再依赖 `proxy_core_adapter::provider_gemini_env_map`；服务层继续负责缺失 API key 本地化错误与 base URL 默认值，Gemini env schema 转换集中在 core。
...
841. ProxyService Codex takeover 的 config.toml proxy base URL/wire_api/model 投影与 provider `modelCatalog` 注入已迁回 `proxy/host/cc_switch/live_takeover.rs`：服务层在 owning module 内读取/写入 live 文件、处理备份策略/OAuth auth 保留分支，并组合 core `codex_takeover_toml_config_patch` 与 host Codex provider helper 完成 TOML/catalog 注入；adapter 不再暴露 Codex takeover 高层接管入口。
...
843. ProxyService Codex takeover 的 `auth.OPENAI_API_KEY` 占位符注入已改为 live takeover 直接调用 `proxy-core::ports::{apply_codex_takeover_auth_placeholder_if_present,ensure_codex_takeover_auth_placeholder}`：服务层继续区分 live takeover 的“已有 auth 才写”和 provider sync 的“必要时创建 auth”两种控制流，Codex auth schema 写入规则集中在 core。
...
866. ProxyService Codex provider live 写入参数拆解不再经 `proxy_core_adapter` 包装；`live_takeover` 直接调用 `proxy-core::ports::codex_provider_live_write_parts_from_settings`：服务层继续负责调用 `codex_config` 落盘与保留不同业务上下文错误文案，provider category、auth 引用、config.toml 文本提取与缺 auth 分支集中在 core。
...
868. ProxyService Codex takeover live 字段投影已迁回 `proxy/host/cc_switch/live_takeover.rs`：服务层继续负责读取 live、查当前 provider 和写回，已有 live 接管只改已有 auth、provider 同步时确保 auth 占位符、config.toml base_url/wire_api/model 更新与 modelCatalog attachment 集中在 owning module，adapter 不再暴露 `CodexTakeoverAuthPolicy` 或 field applicator。
869. Host Codex live snapshot/sync 写入参数中的 provider category 投影已拆分到 owning modules：legacy `ConfigService` 本地调用 `proxy-core::ports::codex_live_settings_parts_from_settings` 读取 sync 所需 settings parts，`provider/live` 的 snapshot 路径本地调用 `proxy-core::ports::codex_live_snapshot_parts_from_settings`，两者都不再通过 adapter 中转。
870. Host Codex provider backfill 的 switch-away wrapper 已回到 `services/provider/live.rs` owning module，legacy live sync restore wrapper 已回到 `services/config.rs` owning module：两边各自直接组合 `proxy-core::ports::codex_provider_backfill_parts_from_settings` 与 `codex_config::restore_codex_settings_for_backfill`，provider live 额外负责 official unified-session bucket strip 和 DB `modelCatalog` overlay；`proxy_core_adapter` 不再暴露这组 backfill facade。
871. ProxyService Codex live backup 的统一会话 bucket 注入不再经 `proxy_core_adapter` 一跳 wrapper；`live_takeover` 在已持有 provider 的位置直接调用 `codex_config::apply_codex_unified_session_bucket_to_settings`：服务层继续负责构造 effective settings、读取现有备份和错误上下文，provider category 到 official-only unified-session 注入的参数投影留在 owning module。
872. ProxyService takeover 启用后的官方供应商风险告警判定已改为直接复用 `proxy-core::ports::should_emit_proxy_official_warning_for_provider_category`：host service 继续负责读取当前 provider 与 Tauri 事件发送，provider category 到 official warning 的纯策略集中在 core。
...
875. Host Codex switch-away backfill 的 restore/strip mutation 已拆回 owning modules：`ConfigService` 本地执行 legacy live sync 的 token restore，`provider/live` 本地执行 switch-away restore、official unified-session strip、日志和写回；provider category/settings_config 到 restore/strip 参数的选择继续委托 core backfill parts。
876. Provider live sync/save/switch 的 proxy-owned live 判定已改为直接引用 `proxy_core::api::ports::{proxy_live_config_owned_by_takeover,proxy_switch_should_hot_switch}`：服务层继续负责读取 backup、检测 live 占位符和执行写回，backup/live placeholder 到“由代理接管拥有”以及 takeover active 到 hot-switch 的布尔策略集中在 proxy-core ports。
877. ProxyService hot-switch 的 backup 更新触发判定已复用 `proxy_core::api::ports::proxy_live_config_owned_by_takeover`：服务层继续负责读取 backup、检测 live 占位符、更新 DB/settings 与执行 live/backup 写回，hot-switch 中 backup row/live placeholder 到“需要同步备份”的策略集中在 proxy-core ports。
878. app 启动异常恢复与退出清理的 takeover 残留恢复判定已复用 `proxy_core::api::ports::proxy_live_config_owned_by_takeover`：入口层继续负责读取全局 backup、检测 live 占位符和调用恢复流程，backup/live placeholder 到“需要恢复”的布尔策略集中在 proxy-core ports。
...
880. ProxyService 单 app takeover 已标记状态的可复用/重建前恢复判定已改为直接引用 `proxy_core::api::ports::{proxy_takeover_marked_state_is_reusable,proxy_takeover_should_restore_existing_backup_before_retakeover}`：服务层继续负责读取 backup、检测 live 是否指向当前代理、恢复旧备份和重新接管，backup/live match 到幂等返回或恢复旧 backup 的布尔策略集中在 proxy-core ports。
...
884. Host Claude live 写入前的内部字段清洗已改为 `proxy-core::api::ports::sanitize_claude_settings_for_live`：`ConfigService`、`ProviderService` 与 `ProxyService` 继续负责 live 文件读写和 provider 状态回填，`api_format`/`apiFormat`/OpenRouter compatibility 字段不写入 Claude live 的投影规则集中在 core ports。
885. Provider common-config 的 JSON 子集匹配、数组移除和 deep merge/remove 语义已由 `proxy-core::ports` 维护；`services/provider/live.rs` 直接消费 core ports，继续负责 snippet 解析、settings apply/remove 编排和 AppError 映射。
886. Provider common-config 的 Codex TOML 子集匹配、merge/remove 和数组差集算法已迁到 `services/provider/live.rs`：provider live service 直接负责 config.toml snippet 解析、变更编排和错误映射，adapter 不再承接这组 TOML helper。
887. Provider common-config 的命中判定已迁到 `services/provider/live.rs::{contains_common_config_snippet,provider_uses_common_config}`：provider live service 负责 DB snippet 读取、provider meta/legacy snippet 命中判断、apply/remove 编排和 warning 日志，纯判定仍复用 core ports。
888. Provider common-config 的 settings apply/remove 编排已迁到 `services/provider/live.rs`：Claude/Gemini mutation 直接复用 core ports，Codex TOML mutation 由 provider live service 本地承接，unsupported app 保持 no-op。
...
890. Provider common-config snippet 生成已迁到 `services/provider/live.rs::common_config_snippet_from_settings`：`ProviderService` 继续负责 current provider 查询和 AppError 文案映射，Claude/Gemini/OpenCode/OpenClaw/Hermes snippet 投影直接复用 core ports，Codex TOML snippet 清洗和 parse 分类由 provider live service 承接。
891. Claude provider 模型字段规范化已改为直接消费 `proxy-core::ports::normalize_claude_models_in_value`：`provider/mod` 继续负责 add/update 时机，`provider/live` 继续负责 live import 时机，旧 `ANTHROPIC_SMALL_FAST_MODEL` 到 `ANTHROPIC_DEFAULT_*` 的回填与清理策略及 fixture 覆盖集中在 core ports。
892. Provider settings 基础校验不再经 `proxy_core_adapter` 包装；`ProviderService` 直接调用 `proxy-core::ports::provider_settings_validation_parts_from_settings` 并消费 `ProviderSettingsValidationIssue`：`ProviderService` 继续负责 Claude Desktop/Gemini 专用校验、Codex TOML 语义校验和 AppError 本地化映射，Claude/Codex/OpenCode/OpenClaw/Hermes 的 settings object/auth/config 形态分类集中在 core。
893. Provider common-config 的存储前规范化已迁到 `services/provider/live.rs::normalize_provider_common_config_for_storage`：provider live service 负责 DB snippet 读取、日志和 AppError 映射，并通过 core ports 判断显式 meta 开关是否要求保存前剥离公共配置，避免 legacy subset 检测误影响新增/更新存储路径。
894. Provider live 写入前的 effective settings 构造已迁到 `services/provider/live.rs::{build_effective_settings_with_common_config,build_effective_settings_with_common_config_from_db,ProviderEffectiveSettingsWarning}`：provider live service 负责 DB snippet 读取、公共配置 apply、失败保留原 settings 和 warning 日志，host takeover 只调用 provider service 出口。
895. Provider switch-away backfill 的 common-config snippet 剥离编排已迁到 `services/provider/live.rs`：provider live service 负责 DB snippet 读取、provider meta/legacy snippet 命中、剥离失败降级为原 live settings，并串联后续 Codex backfill projection。
896. Provider switch-away backfill 的 Codex settings 恢复、统一会话 bucket 清理和 DB `modelCatalog` 优先保留已改为 `services/provider/live.rs` 本地 wrapper：`provider/live` 负责 DB snippet 读取、warning 日志、Codex-only backfill projection 和诊断事件；legacy `ConfigService` 的 restore mutation 也已本地化，adapter 不再承接该组 backfill API。
897. `proxy_core_adapter` 已不再保留 common-config snippet 生成、命中判定、settings apply/remove、存储规范化、effective settings 或 switch-away strip facade；边界测试禁止这组 helper 回流到 adapter。
898. `services/provider/live.rs` 现在是 common-config live 编排 owning module，后续迁移 OpenCode/OpenClaw/Hermes live import/write projection 时应继续把 DB/source 编排留在 provider service 或 host service 边界，而不是重新扩大 `proxy_core_adapter`。
899. OpenCode additive provider live 写入计划已迁到 `services/provider/live.rs`：provider live service 负责完整 config fragment 提取、typed parse、raw fallback、invalid reject 分类、full-config fragment warning 标记以及 `opencode_config` typed/raw 写入日志。
900. OpenClaw additive provider live 写入计划已迁到 `services/provider/live.rs`：provider live service 负责 typed parse、raw fallback、invalid reject 分类以及 `openclaw_config` typed/raw 写入日志。
901. channel-key runtime 选择入口已从单个 weighted helper 收敛为 `proxy-core::ChannelKeyRuntimeSelectionPolicy` / `ChannelKeyRuntimeSelectionInput`：CC Switch DB source 只读取候选 key、channel health policy 和 runtime roll，并把 priority/weighted 策略作为 core policy 传入；后续接入 round-robin、随机种子或外部中转策略时不需要再改变 host DB source 的调用形状。
901. OpenCode live provider 导入投影已迁到 `services/provider/live.rs`：provider live service 继续负责读取 live typed providers、重复 ID 过滤、DB 保存和日志，并本地完成 OpenCode typed config 到 `Provider`/`settings_config`/`live_config_managed` 的投影。
902. OpenClaw live provider 导入投影已迁到 `services/provider/live.rs`：provider live service 继续负责读取 live typed providers、重复 ID 过滤、DB 保存和日志，并本地完成空 ID/无模型校验、首模型名 display name 推导和 typed config 到 `Provider` 的投影。
903. Hermes live provider 导入投影已迁到 `services/provider/live.rs`：provider live service 继续负责读取 live providers、重复名称过滤、DB 保存和日志，并本地完成空名称校验与 Hermes config 到 `Provider`/`settings_config`/`live_config_managed` 的投影。
904. 通用 default live 配置导入投影已回到 `services/provider/live.rs` owning module：`provider/live` 继续负责 takeover 检测、live 文件读取、DB 保存和 current provider 写入，`settings_config` 到 default `Provider`、Codex official/custom category 推导和非 Codex custom category 归类直接复用 core default-import category policy。
...
906. `proxy_core_adapter` 已不再保留 OpenCode/OpenClaw/Hermes live import/write projection facade 或 core decision alias；边界测试禁止这组 live projection helper 回流到 adapter。
...
908. Codex live settings 的 model catalog 复原投影已回到 `services/provider/live.rs`：`provider/live` 继续负责读取 `auth.json`/`config.toml` 和 catalog projection 文件，backfill 继续负责 provider/live 数据来源选择，`modelCatalog` 附加到 live settings 的纯数据规则不再通过 adapter 暴露。
...
912. Default live import 的 settings 归一化已改为 `services/provider/live.rs` 直接调用 `proxy-core::ports::provider_default_live_import_settings`：`provider/live` 继续负责 live 文件读取、takeover gate、DB 保存和 current provider 写入，Claude 旧模型字段归一化等 default-import 前 settings shape 规则集中在 core。
...
914. Provider live 同步范围策略已改为 `proxy-core::api::ports::{provider_live_sync_scope_for_app,ProviderLiveSyncScope}`：`provider/live` 继续负责 DB 读取、current provider 解析、takeover-aware live/backup 写入、MCP/Skill sync，调用点只把宿主 app 投影为 core `AppKind`，additive app 同步全部 provider、switch app 只同步当前 provider 的策略集中在 core ports。
915. Provider live 同步包含策略已改为 `provider/live` 直接调用 `proxy_core::api::ports::provider_should_sync_to_live(provider.live_config_managed())`：`provider/live` 继续负责遍历 DB provider 与写 live 文件，`liveConfigManaged=false` 的 DB-only provider 跳过同步、未知/managed provider 参与同步的判定集中在 core，`Provider` 只负责暴露本地 metadata fact。
916. Provider live config 存在性检查的错误容忍策略已改为 `proxy-core::api::ports::{provider_live_config_presence_error_policy,ProviderLiveConfigPresenceErrorPolicy}`：`ProviderService` 继续负责调用 live config 解析/查询，DB-only provider 遇到 live 解析错误按缺失处理、未知/managed provider 严格传播错误的策略集中在 core ports。
917. Provider current-provider scope 已改为 `proxy-core::api::ports::provider_app_has_current_provider`：`ProviderService::current` 继续负责读取 settings/DB 的有效 current provider，additive app 不暴露 current provider 概念、switch app 使用 current provider 的策略集中在 core ports。
918. Provider key rename 策略已改为 `proxy-core::api::ports::{provider_key_change_policy_issue_for_app,provider_key_change_policy_issue_message,ProviderKeyChangePolicyIssue}`：`ProviderService::update` 继续负责原 provider/目标 provider DB 查询、live config 存在性检查和落库，只有 additive app 支持改 key、OpenCode OMO/OMO Slim provider 禁止改 key 以及对应错误文案集中在 core ports。
919. Additive provider 新增时的 live 写入策略已改为 `proxy-core::api::ports::{provider_additive_live_write_action_for_app,ProviderAdditiveLiveWriteAction}`：`ProviderService::add` 继续负责 DB 保存与 live 写入副作用，OpenCode OMO/OMO Slim 新增不自动启用、用户未选择写 live 时跳过、其他 additive provider 写入 live 的策略集中在 core ports。
920. Provider switch 分流策略已改为 `proxy-core::api::ports::{provider_switch_dispatch_for_app,ProviderSwitchDispatch}`：`ProviderService::switch` 继续负责 provider 查询、proxy takeover lock/hot-switch 和 normal switch 副作用，调用点只把宿主 `AppType` / `Provider.category` 投影为 core app/category facts；OpenCode OMO/OMO Slim 与 Claude Desktop 直接走 normal switch、其他 provider 进入 takeover-aware 分支的策略由 core ports 维护。
921. Provider normal switch 的状态推进策略已改为 `proxy-core::api::ports::{provider_switch_backfill_source_id,provider_switch_should_mark_live_config_managed}`：`ProviderService::switch_normal` 继续负责 live config 读取、common config strip、DB 保存、live 回滚和 MCP sync，exclusive app 切换到不同 current provider 时才 backfill、DB-only additive provider 写入 live 后才标记为 managed 的判定集中在 core ports。
922. OpenCode OMO/OMO Slim normal switch 互斥策略已改为 `proxy-core::api::ports::{provider_omo_switch_pair_for_app_category,ProviderOmoSwitchPair,ProviderOmoVariant}`：`ProviderService::switch_normal` 继续负责 current OMO DB 标记、OMO config 写入和旧 variant config 删除，调用点只传入 app/category facts；`omo` 启用 standard 并禁用 slim、`omo-slim` 启用 slim 并禁用 standard、非 OpenCode/非 OMO provider 不进入互斥分支的策略由 core ports 维护。
923. OpenCode OMO/OMO Slim category 解析策略已改为 `proxy-core::api::ports::provider_omo_variant_for_app_category`：`ProviderService::update/delete/remove_from_live_config/switch_normal` 继续负责 DB current 判定、provider 保存、live config 写删和回滚，只有 OpenCode 的 `omo`/`omo-slim` category 会映射到 OMO variant、其他 app/category 走普通 additive provider 分支的判定集中在 core ports。
924. Provider legacy common-config 迁移跳过策略已改为 `proxy-core::api::ports::should_skip_provider_legacy_common_config_migration`：`ProviderService::migrate_legacy_common_config_usage*` 继续负责 DB snippet/provider 查询、common config 使用检测、settings mutation 和 DB 保存，additive app 不参与 legacy common-config 迁移、空 snippet 直接跳过的判定集中在 core ports。
925. Provider 单 app live sync 分流已复用 `proxy-core::api::ports::{provider_live_sync_scope_for_app,ProviderLiveSyncScope}`：`ProviderService::sync_current_provider_for_app` 继续负责 current provider 查询、takeover backup/live 判定和 live 写入，additive app 直接同步全部 provider、switch app 进入 current-provider/takeover-aware 路径的分流与 `provider/live` 的全量同步策略保持同一 core ports 来源。
926. Provider settings 保存前旧模型字段归一化已改为 `proxy-core::api::ports::normalize_provider_settings_for_storage`：`ProviderService::add/update` 继续负责 provider validation、common config storage normalization 和 DB/live 写入，Claude 旧 `ANTHROPIC_MODEL`/`ANTHROPIC_SMALL_FAST_MODEL` 归一化到默认模型字段、非 Claude app 不改 settings 的判定和 mutation 集中在 core ports，并与 default live import 复用同一 helper。
927. Provider 新增时的 `live_config_managed` 初始 marker 策略已改为 `proxy-core::api::ports::provider_initial_live_config_managed_marker`：`ProviderService::add` 继续负责 provider meta mutation、DB 保存和 live 写入，只有 additive app 根据用户是否选择写入 live 设置初始 managed marker、switch app 不写该 marker 的判定集中在 core ports。
...
934. handler 请求体 Axum request parts 拆包、body collection、strict JSON 解析、Gemini empty-body null fallback、`stream` 标志组合、path/query 到 proxy endpoint 的 bridge、`RequestContext` app/tag/body/header/session/model 初始化，以及 app/interface/body/headers/extensions 到 `ProxyRequest` 的组装，已收敛到 `proxy::response_adapter::{collect_json_proxy_request,collect_json_or_null_proxy_request,endpoint_from_uri}` 和 `ParsedAxumJsonProxyRequest::{endpoint_from_request_uri_stripping_prefix,endpoint_for_path,request_context,codex_request_context,gemini_request_context,into_*_proxy_request}`；handler 不再直接调用 `BodyExt::collect`、`parse_json_proxy_request_body*`、`append_query_to_endpoint_path`、`strip_endpoint_prefix`、`RequestContext::new`、`with_model_from_uri`、`JsonProxyRequestInput`、`InterfaceKind::*` 或 `request_body_read_error_message`。
935. handler 的请求体读取错误文案、strict JSON parse contract 和 Gemini 空 body -> `Null` 语义仍由 `proxy-core`/`proxy_core_adapter` 维护；`response_adapter` 只负责把 Axum transport 拆包与 core parse helper 串成 handler 可消费的 transport DTO。
936. `RequestContext` 的 Claude API format provider fact 已改为直接消费 host-owned `provider_projection::provider_claude_api_format`：context 继续负责把 route metadata 和选中 provider 组合成响应格式判断，但不再通过 `proxy_core_adapter` 或 `proxy::provider::claude_provider_api_format` 兼容 helper 间接读取 Provider 字段。
937. handler 的 `ProxyEngine::handle` dispatch、forward core error usage 记录和 `ProxyResult -> ProxyResponse` bridge 已继续上移到 `proxy::response_adapter::{dispatch_proxy_request_to_proxy_response,dispatch_claude_proxy_request_to_proxy_response}`，Claude/Claude Desktop Messages 的 request collection、context/endpoint/proxy request 组装、dispatch、transform-or-passthrough response 分支与 app/tag/prefix metadata 已收敛到 `proxy::response_adapter::{dispatch_claude_request_to_axum_response,dispatch_claude_desktop_messages_request_to_axum_response}`，Codex Chat/Responses/Compact 的 request collection、context/endpoint/proxy request 组装、forward error envelope、dispatch outcome extraction、endpoint metadata 与 transform-or-passthrough response 分支已收敛到 `proxy::response_adapter::{dispatch_codex_chat_request_to_axum_response,dispatch_codex_responses_request_to_axum_response,dispatch_codex_responses_compact_request_to_axum_response}`，Gemini handler 的 request collection、context/endpoint/proxy request 组装、dispatch 和 passthrough response 全链路已收敛到 `dispatch_gemini_request_to_axum_response`；Claude transform gate 与 Codex Responses→Chat gate 已继续上移到 `proxy::response_adapter::{claude_response_needs_transform,codex_response_needs_chat_transform}`，Claude/Codex transform streaming decision 调用与 transformed response orchestration 也继续上移到 `proxy::response_adapter::{claude_transformed_response_to_axum_response,codex_chat_to_responses_transformed_response_to_axum_response}`，passthrough response parser-config 选择与 `process_response` 调用已上移到 `proxy::response_adapter::*_passthrough_response_to_axum_response`：handler 只负责按 helper 结果选择响应处理分支，不再直接调用 engine dispatch、ProxyResult bridge、provider/endpoint 或 streaming/aggregation 判定 helper，也不再本地构造 transformed SSE stream/JSON bridge 或绑定 parser config；Claude/Codex/Gemini protocol handlers 只保留 adapter 入口调用且不再携带 app/tag/prefix/endpoint route metadata。`/health` 的本地 JSON bridge 已回到 HTTP handler，response adapter 不再承接无 engine/runtime 的 health 响应。
    `/proxy/v1/events` 的 event bus subscription、connected/lagged envelope bridge 和 Axum SSE keep-alive 构造现在由 HTTP transport handler 拥有；handler 继续从 `proxy-core` 读取 `ProxyEventSseSpec` 与 keep-alive contract，但不再通过 `response_adapter` 中转事件 SSE。
    `/proxy/v1/apps/{app}/providers`、`/proxy/v1/apps/{app}/models` 与 Codex `/v1/models` 的基础 request factory、`ProxyEngine` 调用、固定 Codex app route metadata 和 typed `Json<T>` bridge 已进一步移回 HTTP handler，handler 仍只保留 Axum state/path/query 提取、基础 typed JSON bridge 与 error mapping。
    `/proxy/v1/channels`、`/proxy/v1/apps/{app}/channels`、`/proxy/v1/groups`、`/proxy/v1/apps/{app}/routes/current` 与 `/proxy/v1/route/resolve` dry-run 的 request factory、`ProxyEngine` 调用和 typed `Json<T>` bridge 已进一步移回 HTTP handler；handler 继续只保留 Axum path/query/body/state 提取、基础 typed JSON bridge 与 error mapping。
    channel CRUD、channel key、channel model 与 channel test endpoints 的 path/body request factory、`ProxyEngine` 调用和 typed `Json<T>` bridge 已进一步移回 HTTP handler；handler 只保留 Axum path/body/state 提取、基础 typed JSON bridge 与 error mapping。
    channel migration preview/materialize 与 channel breaker stats/reset endpoints 的 path request factory、`ProxyEngine` 调用和 typed `Json<T>` bridge 已进一步移回 HTTP handler；handler 只保留 Axum path/state 提取、基础 typed JSON bridge 与 error mapping。
...
939. 边界测试已对齐 source-owned runtime 结构：`RequestForwarder` 不再持有 managed-account runtime source，`ForwarderAuthSource` / `ForwarderRequestSource` 持有 managed-account runtime；`ActiveConnectionGuard` 的 lifecycle 校验也改为验证 forward-pipeline-owned guard 与 `ForwarderRuntimeStateSource`。
...
980. `RequestForwarder` 生产路径不再直接 import 或传递 provider adapter trait；provider adapter trait 被包进 `ForwarderAdapterContext`，forwarder 只消费 context/facts，底层 `ProviderAdapter` 调用继续集中在 `proxy/host/cc_switch/provider_adapter_context.rs`，forwarder/request source 直接引用 host context，adapter 不再 re-export 该 context/factory。
...
994. `record_forward_provider_failure_runtime_source`、`record_forward_provider_rectifier_retry_failure_runtime_source` 与 attempt failure runtime helper 已从裸 `provider_name`/`rectifier_label`/`error_message` 参数收束为 `Provider`、`ForwarderRectifierRetryKind`、`ForwardAttempt` 与 `ProxyError` facts；默认 source 只转交结构化事实，错误展示字符串继续留在 owning runtime-source helper 内部生成。
...
本轮继续收窄 `proxy_core_adapter` 的 config/circuit helper 暴露面：adapter-only 的 circuit key/config helper 改为私有导入，routing/runtime 调用方继续直接消费 owning core API 或宿主模块。
...
本轮继续把 Codex Chat Completions provider-local policy 从 `proxy_core_adapter` 迁到 `proxy/provider/codex.rs`：Responses->Chat gate、upstream model override、catalog model preservation 与 chat reasoning options 由 Codex provider adapter 拥有，并直接委托 `proxy-core::api::{ports,transport,transforms}`；forwarder request source、response adapter 与 Codex takeover TOML 只消费 `codex_provider_*` owning-module helper，边界测试禁止旧 `provider_*codex*` façade 回流到 adapter。
本轮继续把 Claude provider `api_format` 投影从 `proxy_core_adapter` 收敛到 host-owned `provider_projection`：Claude provider adapter 的 `claude_provider_api_format` 仅作为兼容薄壳委托 `provider_projection::provider_claude_api_format`，`RequestContext` 已直接消费 host projection，forwarder request source 与 adapter 内部 remaining bridge 可继续消费旧 helper 名称，`proxy_core_adapter` 不再定义 `provider_claude_api_format` façade；边界测试禁止 `provider_projection` 反向依赖 provider adapter 或旧 adapter façade 回流。
...
本轮继续收窄 app proxy_config 默认值入口：`database/dao/proxy.rs` 直接消费 `proxy-core::api::config::app_proxy_config_defaults_for_app`，`proxy_core_adapter` 不再二次 re-export 该纯 config helper。
...
forwarder provider URL facts 内部的 full URL 与 GitHub Copilot upstream 一跳 wrapper 已删除；adapter context 直接把 provider type、full URL flag 与 base URL 投影给 core URL facts helper，`forwarder.rs` 仍只消费注入后的 URL facts。
本轮继续把 forwarder 的 media prevention text-only provider 图片替换 fact 收敛到 `proxy-core::request_media::apply_forwarder_media_prevention_from_facts`；media 预防式降级不再直接消费 provider 级模型能力投影 helper，相关策略 fixture 由 `proxy-core::request_media` owning tests 覆盖，adapter 不再保留重复测试。Codex Chat streaming decision 的 SSE/fallback 策略、Codex tool context 构造和 Chat error body normalization fixture 也分别由 `proxy-core::response_transform` / `proxy-core::codex_error` owning tests 覆盖，adapter 不再重复直接调用 core policy。OpenAI stream include_usage 与 Claude thinking normalization fixture 也回到 `proxy-core::request_body` / `proxy-core::response_transform` owning tests，adapter 不再保留 test-only direct-core coverage。
...
本轮继续把 usage sink 的计费配置 lookup 输入收敛到 host-owned `database_usage_sink`，由该模块直接拆 `UsageRecord` 的 app/provider 字段并读取 DB 计费配置。
本轮曾把 core event 到 host event bus 的投影+分发入口收敛到 adapter；后续已继续迁到 owning module：event sink 端口实现由 host-owned `CcSwitchEventSink` 承接，`ProxyEventBus::emit_core_event` 统一负责 core event 到 host event envelope 的投影，adapter 不再保留 event bus helper。
此前曾把 `CcSwitchForwardPipeline` 本身迁为 adapter-owned optional runtime wrapper，runtime 缺失判断和 host forward runtime 调度都在 adapter wrapper 内完成；该方向已由后续 host forward pipeline/runtime split 修正，wrapper 数据结构、`ForwardPipeline` impl 与 optional-runtime dispatch helper 回到 `proxy/host/cc_switch/forward_pipeline.rs`，真正进入旧转发链的 host bridge 也已回到 `proxy/host/cc_switch/proxy_runtime.rs`。
...
本轮继续把 response processor 的 SSE usage collector、finish guard 和流式 first-byte/idle timeout loop 迁入 `proxy/engine/response_pipeline.rs::create_logged_passthrough_stream`；chunk-level SSE passthrough policy 仍委托 `proxy-core::SsePassthroughStreamState`，host response processor/handlers 只传入 stream、tag、collector、timeout config 与 active-connection guard。
...
后续已把 forward runtime 的 route plan attempt 构造入口、auth profile DB key 注入、current-provider settings/DB fallback、`ProxyRequest` 投影、runtime config 读取与 preplanned `RequestForwarder` 启动从 adapter bridge 迁回 `proxy/host/cc_switch/proxy_runtime.rs`；host runtime 直接调用 `channel_auth_profile_attempts` owning helper 和 core 选择策略，adapter 不再作为该转发链路的中转面。
本轮后续已把 `CcSwitchConfigSource` 的 app catalog、global/app/summary/runtime 配置 DB 读取、settings current-provider 事实读取和 core DTO 投影收敛到 host-owned source wrapper；该模块直接消费 core config/source/result contract，adapter 不再保留 config-source-only DB helper 或 re-export `CcSwitchConfigSource` host type。
本轮后续已把 `CcSwitchProviderSource` 的 provider list/get/current-provider DB 读取、active route runtime map 读取和 `ProviderSpec` 投影收敛到 host-owned source/projection wrapper；该模块直接消费 core `AppKind` / `ProviderSpec` / `ProviderSource` / `CurrentRouteTarget` / `ProxyCoreResult` contract，adapter 不再为该 source re-export `ProviderSource` port facade，也不再持有 provider source projection helper。
本轮继续把 `CcSwitchChannelSource` 的 channel spec list/get 读取与 `ChannelSpec` 投影收敛到 host-owned DB channel source wrapper；该模块直接消费 core channel domain/routing/management/port/result contract。
本轮继续把 `CcSwitchChannelSource` 的 channel record create/get/update/delete DB 操作与 `ChannelRecord` 投影收敛到 host-owned DB channel source wrapper；adapter 只保留宿主错误包装和旧 HTTP/response 层仍需的兼容别名。
本轮继续把 `CcSwitchChannelSource` 的 channel key/model 子资源 DB 操作与 `ChannelKeyRecord`/`ChannelModelRecord` 投影收敛到 host-owned DB channel source wrapper；channel source 专用 DTO/helper facade 不再经 adapter 导入。
...
本轮删除 adapter 内部 `proxy_engine_from_services` 一跳构造 facade；`ProxyState::proxy_engine` 在 host state 所有权边界内直接调用 `ProxyEngine::new`，保留“生产 host 只能经 host `ProxyState` 构造 engine”的边界测试。
...
本轮继续把 `CcSwitchChannelSource` 的 route/materialized channel record list 读取与 `ChannelRecord` 投影收敛到 host-owned source wrapper，并直接引用 core `ChannelRouteSource` / `ChannelRecord` / `ChannelSource` contract。
本轮继续把 `CcSwitchChannelSource` 的 legacy channel migration preview/materialize DB 操作与 response input 投影收敛到 host-owned source wrapper；该模块直接引用 core migration input DTO，host services 只装配 channel source。
本轮后续已把 `CcSwitchRoutePolicySource` 的 failover queue DB 读取与 `RoutePolicy` 投影迁入 host-owned source；该模块直接消费 core route-policy/source/result contract，adapter 只保留 helper，host services 只装配 source。
本轮后续已把 channel health attempt DB 更新、reset app lookup、breaker reset、breaker stats 查询与 `ChannelHealthReset`/`ChannelBreakerStats` 投影迁入 host-owned `CcSwitchChannelHealthStore`，adapter 不再保留这组 helper，host services 只装配 store。
本轮继续把 `CcSwitchChannelReachabilityProbe` 的 probe request app/provider 投影、provider/config DB 读取、stream-check 调用与 reachability 结果投影迁入 `proxy/host/cc_switch/channel_reachability_probe.rs`；该模块直接消费 core reachability probe/request/result/error contract，adapter 只保留 stream-check 投影/错误 helper 和默认 probe type re-export，不再 re-export `ChannelReachabilityProbe` port 或 probe DTO，host services 只装配 probe。
...
本轮继续把 `CcSwitchUsageSink` 的 usage pricing lookup、pricing model 解析、request log 投影、缺价告警与 usage log 写入收敛到 host-owned source wrapper，adapter 不再保留 usage request-log projection helper，host services 只装配 sink。
...
本轮后续已把 `CcSwitchProviderSource` 的 active route runtime map lookup 收敛到 host-owned source wrapper，`proxy_core_host` 不再直接读取 `current_providers`。
...
本轮继续把 `RequestForwarder` 的 attempt allow、provider/channel 成功失败健康记录和 neutral half-open permit 释放收敛到 `ForwarderAttemptRuntimeSource`，forwarder 不再直接分叉调用 ProviderRouter 的 provider/channel runtime API。
...
本轮曾把 health/status/app list 的基础 source 组装收敛到 adapter；后续已继续收窄，health/status/Claude Desktop model-list/app list 的简单 HTTP JSON bridge 均由 HTTP handler 直接调用 core request/engine surface，adapter 不再承接这些基础端点。
...
本轮把 `ProxyResult` 到 `RequestContext` route/provider update 的投影迁回 `proxy/engine/context.rs` owning module，context 直接调用 core usage/error helper 与 host route attempt 映射 helper，adapter 不再承接该 request-context 专用 façade。
...
本轮继续把 `RequestContext::apply_proxy_result` 的 selected-route provider lookup、缺失文案和 route update 组装收敛到 context owning module；adapter 删除 `RequestContextRouteUpdate` DTO、错误类型和 source wrapper。
...
本轮继续把 Claude provider adapter 的 auth key source 日志、占位 token、Gemini OAuth 降级和 `ProviderAuthInfo` 构造从 `proxy_core_adapter` 移到 `proxy/provider/claude.rs` owning helper，Claude adapter 的 `extract_auth` 只保留本地 helper 调用。
...
本轮继续把 Claude provider adapter 的 auth strategy 到 header kind 映射、Copilot 指纹 header 构造和 `AuthError` 文案包装移到 `proxy/provider/claude.rs` owning helper，Claude adapter 的 `get_auth_headers` 只保留本地 helper 调用，`proxy_core_adapter` 不再承担 Claude provider auth header façade。
...
本轮还把 Codex client model catalog raw JSON 解析失败 fallback 收敛到 core，host-owned model catalog provider 通过 adapter helper 复用路径选择与文件读取。
...
本轮继续把 channel-key auth profile 的错误包装收敛到 core，并把 provider settings patch helper 收敛到 `proxy/host/cc_switch/auth_provider.rs`；host auth-profile 应用逻辑只保留 DB key 查询和 attempt auth provider 替换。
...
本轮继续把通用 passthrough streaming 的 upstream response receive log、usage runtime source 和 `ProxyCoreResponse` stream response 构造迁入 `proxy/engine/response_pipeline.rs`；host `response_processor` 只拆出 upstream status、headers 和 `bytes_stream`，再统一交给 response adapter 桥接 Axum。
本轮继续把非流式 passthrough 的 body log、usage runtime source 和 `ProxyCoreResponse` bytes response 构造迁入 `proxy/engine/response_pipeline.rs`；host `response_processor` 只保留 decoded response 读取和 Axum bridge。
本轮继续把 Claude transformed SSE 的 provider stream transform、Gemini shadow/provider/session/tool hints 注入和 transformed logged stream 包装迁入 `proxy/engine/response_pipeline.rs::claude_transformed_sse_stream_from_context`；`handlers` 只保留 streaming branch 选择与 Axum response bridge。
本轮继续把 Codex Chat→Responses transformed SSE 的 history-recording stream transform 和 transformed logged stream 包装迁入 `proxy/engine/response_pipeline.rs::codex_auto_transformed_sse_stream_from_context`；`handlers` 只保留 streaming branch 选择与 Axum response bridge。
本轮继续把 Claude transformed JSON 的 provider response transform、Gemini shadow/provider/session/tool hints 注入和 transformed usage runtime source 包装迁入 `proxy/engine/response_pipeline.rs::claude_transformed_json_response_from_context`；非流式 upstream body read、JSON/SSE fallback parse、transform error 映射和 Axum bridge 由 `proxy::response_adapter::claude_transformed_upstream_json_response_to_axum_response` 承接。
本轮继续把 Claude transformed JSON/SSE 的 Anthropic tool schema hints 提取迁入 `proxy/engine/response_pipeline.rs::claude_transform_tool_schema_hints`；`handlers` 只把原始请求 body 作为转换上下文传入，不再直接调用 `extract_anthropic_tool_schema_hints`。
本轮继续把 Codex Chat→Responses transformed JSON 的 history-recording response transform 和 transformed usage runtime source 包装迁入 `proxy/engine/response_pipeline.rs::codex_auto_transformed_json_response_from_context`；非流式 upstream body read、Chat JSON/SSE fallback parse、transform error 映射和 Axum bridge 由 `proxy::response_adapter::codex_transformed_upstream_json_response_to_axum_response` 承接。
本轮继续把 Claude/Codex/Gemini 的协议 `ProxyRequest` 构造收敛到 `ParsedAxumJsonProxyRequest::into_*_proxy_request`；Codex Responses/Compact 的 `CodexToolContext` 提取仍复用 `proxy_core_adapter::codex_responses_proxy_request_from_input`，但 handler 不再直接调用 `JsonProxyRequestInput`、`InterfaceKind::*` 或 `codex_responses_proxy_request_from_input`。
本轮继续把 `ProxyResult` 到 `RequestContext` 的回填、Claude api_format 投影、Codex forward error response outcome 和 `ProxyCoreResponse -> hyper_client::ProxyResponse` transport bridge 收敛到 `proxy::response_adapter::{dispatch_proxy_request_to_proxy_response,dispatch_claude_proxy_request_to_proxy_response,dispatch_codex_proxy_request_to_proxy_response}`；`handlers` 不再直接调用 `apply_proxy_result`、`claude_api_format_for_proxy_result`、`proxy_result_to_proxy_response`、`claude_proxy_result_to_proxy_response`、`codex_proxy_error_to_axum_response` 或 `proxy_core_response_to_proxy_response`。
本轮继续把 Codex Chat/Responses/Compact 的 dispatch outcome match、forward error response early-return、Chat passthrough 和 Responses transform-or-passthrough 分支收敛到 `proxy::response_adapter::{dispatch_codex_chat_request_to_axum_response,dispatch_codex_responses_request_to_axum_response}`；`handlers` 不再直接引用 `CodexProxyDispatchResponse`、`dispatch_codex_proxy_request_to_proxy_response`、`codex_response_needs_chat_transform` 或 Codex passthrough/transform response helper。
本轮继续把 Codex Chat/Responses/Compact 的 Axum request collection、Codex `RequestContext` 初始化、endpoint path/query bridge、`ProxyRequest`/`CodexToolContext` 组装收敛到 `proxy::response_adapter::{dispatch_codex_chat_request_to_axum_response,dispatch_codex_responses_request_to_axum_response}`；三个 Codex HTTP handlers 只保留 adapter 入口调用。
本轮继续把 Claude/Claude Desktop Messages 的 Axum request collection、`RequestContext` 初始化、strip-prefix endpoint bridge、Anthropic Messages `ProxyRequest` 组装、dispatch、Claude transform branch 和 passthrough response 处理收敛到 `proxy::response_adapter::dispatch_claude_messages_request_to_axum_response`；共享 handler 只保留 app/tag/prefix 路由元数据转发。
本轮继续把 Claude/Claude Desktop 的 app/tag/prefix route metadata 与 Codex Responses/Compact endpoint path metadata 收敛到协议专用 `response_adapter` 入口；HTTP handlers 不再导入 `AppType`、保留 `handle_messages_for_app`，也不再传递 `"/responses"`/`"/responses/compact"` 字符串。
本轮继续把协议 handler 的 Axum request parts 拆包、body collection、strict JSON/Gemini null JSON 解析、stream 标志读取、endpoint path/query bridge、`RequestContext` 初始化与 Gemini URI 模型回填收敛到 `proxy::response_adapter::{collect_json_proxy_request,collect_json_or_null_proxy_request,endpoint_from_uri}` 与 `ParsedAxumJsonProxyRequest` endpoint/context helper；`handlers` 只保留 parsed request 用于少量上下文选择与协议 request 构造，不再直接持有 body collection、JSON parse helper、endpoint bridge、context bridge 或 proxy request DTO 组装。
本轮继续把 Gemini 协议 handler 的 request collection、Gemini context/URI model attribution、endpoint bridge、`ProxyRequest` 构造、`ProxyEngine` dispatch 和 passthrough response 处理收敛到 `proxy::response_adapter::dispatch_gemini_request_to_axum_response`；`handle_gemini` 只保留 Axum 入参转发。
本轮继续把 Claude transform gate 与 Codex Responses→Chat gate 收敛到 `proxy::response_adapter::{claude_response_needs_transform,codex_response_needs_chat_transform}`；`handlers` 不再直接调用 `provider_needs_claude_transform` 或 `provider_should_convert_codex_responses_to_chat`，只根据 response adapter 的 branch gate 结果进入 passthrough/transform 分支；Claude provider fact 投影由 `provider_projection` 拥有。
本轮继续把 Claude/Codex transform streaming decision 调用收敛到 `proxy::response_adapter::{claude_transform_streaming_decision_for_response,codex_chat_transform_streaming_decision_for_response}`；`handlers` 不再直接调用 `provider_claude_transform_streaming_decision` 或 `codex_chat_transform_streaming_decision`，stream/non-stream 分支仍保持原有行为；Claude streaming decision 的 provider 事实投影由 `provider_projection` 拥有。
...
本轮也把 app config source 的当前 provider settings 查询和 `ProxyAppConfig` parts 组装收敛到 host-owned source wrapper。
...
本轮后续已把 ChannelHealthStore 的 attempt 写库参数投影和 DB 写入调用收敛到 host-owned health store，adapter 不再保留该 helper，host services 只装配 store。
本轮继续把 `ProxyCoreEvent` 到 host event bus 的 name/payload 投影收敛到 adapter。
本轮继续把 `CcSwitchEventSink` 的 `ProxyEventBus` 分发副作用和 optional event bus 判断收敛到 host-owned source，host services 只负责注入事件总线；后续 `ProxyEventBus::emit_core_event` 接管 core event envelope 投影，adapter 不再保留 event bus helper。
...
本轮后续已把 ChannelHealthStore reset 的 app lookup 校验、router reset 和 reset fact 投影收敛到 host-owned health store，adapter 不再保留 reset 桥接 helper，host services 不再保留 reset 桥接实现。
...
本轮继续把 Codex Chat->Responses transform 的 streaming/错标 SSE 聚合决策收敛到 `proxy-core::response_transform::codex_chat_transform_streaming_decision`，`proxy::response_adapter` 直接传入请求 stream flag 和响应 header，handler 不再直接判断上游 SSE header 或指定 ChatCompletions 聚合策略，`proxy_core_adapter` 也不再保留一跳 facade。
...
本轮继续把 Codex Chat 错误体归一化后的非 JSON warning 输出与 Responses 错误体 neutral response 构造收敛到 `response_adapter`，`handlers` 不再直接调用 `normalize_codex_chat_error_body` 或 `non_json_body_log_message`，`proxy_core_adapter` 不再保留 `codex_chat_error_proxy_response` 中转 helper。
...
本轮继续把 Codex Chat 上游错误响应的 body read、Responses 错误体 bridge 和 Axum bridge 包装收敛到 `proxy::response_adapter::codex_chat_upstream_error_response_to_axum_response`；`handlers` 只保留非成功 Chat 响应的转换分支调度。
...
本轮继续把协议入口的 `ProxyEngine::handle` dispatch、`ProxyCoreError -> ProxyError` 映射、forward error usage 记录和 Codex forward-error response 构造收敛到 `proxy::response_adapter` dispatch helper；Claude/Codex/Gemini handler 不再直接持有 `state.proxy_engine().handle`、`proxy_core_error_to_proxy_error`、`record_forward_core_error_usage` 或 `codex_proxy_error_to_axum_response` 调用。
本轮继续把 `response_adapter` 使用的 management request/response DTO 改为直接经 `proxy_core::api::management` 引用；`proxy_core_adapter` 不再作为 response adapter 的 channel/list/group/status/route/channel-key/model 管理 DTO 中转入口，HTTP handlers 也直接经 core API 声明 typed JSON signature。
后续已把 app model list 与 Codex client model catalog 的 typed JSON bridge 移回 HTTP handler；`response_adapter` 不再导入 `ClientModelCatalogResponse` / `RoutableModelList`，模型目录 response DTO 只作为 HTTP handler signature 与 `ProxyEngine` 调用边界的 direct-core contract。
本轮继续把 `response_adapter` 使用的 `AppKind`、`InterfaceKind`、`CurrentRouteTarget`、Claude/Codex transform decision、Codex tool context 与 response build failure context 改为直接经 `proxy_core::api::{domain,ports,routing,transforms,transport}` 引用；Claude Desktop model-list response 已进一步从 `response_adapter` 迁到 HTTP handler 直接引用 core auth DTO，`proxy_core_adapter` 不再向 response adapter 暴露这些纯 DTO/context，且已移除未使用的 response build failure context 兼容别名。
...
本轮继续把 ProviderRouter 的 channel route input/source fallback 决策收敛到 host-owned ProviderRouter channel source，router 只消费该 source 提供的 core route input 与 source，不再暴露 route record 命名；批量 channel record 到 route input 的投影 helper 已从 adapter 移到 `provider_router_channel_source.rs`。
...
本轮继续把 management dry-run route resolution 收敛到 host route resolver：`ProviderRouter` 不再持有 `RouteResolveRequest`/`RouteResolveResponse`、不再直接调用 `resolve_channel_route` 或 response mutation helper，只暴露 channel route facts 和 candidate circuit availability 查询。
本轮继续把固定 app catalog 从 proxy-core 默认端口移出，`ProxyConfigSource::list_apps` 改为宿主必填能力，CC Switch 的 `AppType::all()` 只保留在 host-owned `CcSwitchConfigSource` 经 adapter helper 暴露的 app catalog source 中。
本轮继续把 RoutePolicy raw 中 `failoverProviderIds` 的读取 contract 收敛到 domain/routing helper，`ProxyEngine` 不再直接读取 raw JSON 字段。
后续已把 forward runtime 的 auth profile DB key 注入路径收敛到 host-owned `channel_auth_profile_attempts` + `ChannelKeyRuntimeSource`，`proxy_core_host`/`proxy_core_adapter` 不再维护本地 wrapper 或直接查询 channel-key。
后续已把 forward runtime 的 current-provider settings/DB fallback 读取入口迁回 `CcSwitchProxyRuntime`，该 runtime 直接读取 settings/DB facts 并调用 core 来源选择 helper。
...
本轮继续把 forward failure 的 `ProxyError -> ForwardFailureKind` 投影收敛到 `proxy::error_mapper`，并把 raw message 与 display message 的选择策略下移到 `proxy-core::forward_failure_message_from_proxy_status`；`proxy_core_adapter` 不再直接引用 `ForwardFailureKind` 或调用 core forward-failure 分类入口。
本轮继续把 forwarder 的 Claude Desktop route 模型映射收敛到 host-owned request source，forwarder 不再直接调用 `claude_desktop_config`，`proxy_core_adapter` 也不再暴露 forward request model mapping façade。
...
forwarder 的 Claude 默认 api_format 一跳 wrapper `forwarder_claude_api_format` 已删除；Copilot vendor 分流入口已下沉到 host-owned `managed_account_runtime_source`，runtime source 直接调用 `proxy-core::resolve_claude_forward_api_format` 并复用 provider 级 adapter API，forwarder 仍不直接读取 provider Claude api format。
...
本轮继续把 `ProviderRouterSource` 拆成 router 端的 provider/channel/config/health 四个 focused port；host adapter 侧拆出对应 DB-backed source/store，并把 `ProviderRouter::new(Arc<Database>)` 迁到 `proxy_core_adapter::provider_router_from_database` factory，生产代码不再直连 router 的 DB 构造入口。后续该 DB-backed source assembly 已迁入 `proxy/host/cc_switch/provider_router_sources.rs`，adapter 只 re-export `provider_router_from_database`。
...
本轮继续把 stream_check 服务的标准 provider adapter base URL 投影从 `proxy_core_adapter::stream_check_provider_base_url` 移回 `StreamCheckService::resolve_base_url`：服务层直接经 host-owned `ForwarderAdapterContext` 读取标准 provider URL facts，并继续只把 OpenCode/OpenClaw/Hermes 的纯 base URL 解析和缺失错误规格委托给 `proxy-core::domain`。
本轮继续把 `FailoverSwitchManager` 的 proxy_config enabled 读取收敛到 host manager 边界：manager 直接读取 `Database::get_proxy_config_for_app`，配置读取失败时继续复用 `proxy-core::provider_selection::failover_config_read_error_log_line` 发出 FO-002 warning 并跳过切换；adapter 不再持有 `failover_switch_app_enabled_from_db` 或 config-result helper。
...
本轮继续把 `reset_circuit_breaker` 恢复后切回判断收敛到命令边界 + core 纯策略：命令层读取 proxy_config、当前 provider、故障转移队列和 provider name，并继续保留健康状态重置、运行时熔断器重置和 `FailoverSwitchManager` 的 Tauri 副作用；切回目标 DTO 与 sort-index 比较策略已迁入 `proxy-core::management::reset_circuit_breaker_switchback_target_from_sources`，adapter 不再持有 `reset_circuit_breaker_switchback_target_from_db` 或私有 source helper。
本轮继续把 `set_auto_failover_enabled` 的开关 plan source 读取收敛到命令边界 + core 纯策略：命令层读取 proxy_config、故障转移队列和空队列时的当前 provider，并继续保留队列写入、目标切换、proxy_config 写回、事件和托盘刷新；开关 plan outcome 和“开启时队列为空则加入当前 provider”的策略已迁入 `proxy-core::management::auto_failover_toggle_plan_from_sources`，adapter 不再持有 `auto_failover_toggle_plan_from_db` 或私有 source helper。
本轮继续把批量 stream check 的 `proxy_targets_only` 过滤收敛到命令边界 + core 纯策略：命令层读取当前 provider 与故障转移队列，集合归并改为直接调用 `proxy-core::management::stream_check_proxy_target_ids_from_sources`；adapter 不再持有 `stream_check_proxy_target_ids_from_db` 或 `stream_check_proxy_target_ids_from_sources`。
本轮继续把 `ProxyService::get_takeover_status` 的 per-app proxy_config enabled 读取收敛到 host service 边界：`live_takeover.rs` 直接读取 Claude/Codex/Gemini 三个配置行并把缺失/失败投影为 `Option<bool>`，DTO 矩阵仍由 `proxy-core::ports::proxy_takeover_status_from_enabled_options` 生成；adapter 不再持有 `proxy_takeover_status_from_db`。
...
本轮继续把接管开启后的官方供应商 warning source 和事件投影收敛到 host service 边界：`live_takeover.rs` 本地 helper 直接读取当前 provider、加载 Provider 实体并复用 `proxy-core` 官方供应商判定与 warning event payload contract；adapter 不再持有 official warning DB wrapper，`ProxyService::set_takeover_for_app` 只调用 host helper 并保留 Tauri `AppHandle` emit。
本轮继续把 Live 接管流程里的当前 provider source 读取收敛到 host service 边界：`live_takeover.rs` 本地 helper 直接读取 settings 当前 provider 和 provider 表实体，并保留 required-provider 错误文案；adapter 不再持有 current-provider source wrapper。
本轮继续把 Live token 同步的当前 provider source 和 provider settings 写回收敛到 host service 边界：`ProxyService::sync_live_config_to_provider` 继续复用 `proxy-core::ports::live_token_sync_app_label` 作为支持 app 纯策略，本地 helper 直接读取 settings/provider 表并执行 provider settings 写回；adapter 不再持有 live-token sync DB wrapper。
本轮继续把 `ProxyService::set_takeover_for_app` 的 proxy_config enabled 状态读取和 read-modify-write 收敛到 host service 边界：`live_takeover.rs` 本地 helper 直接读取/更新 app proxy config，并复用 core `app_proxy_config_with_enabled` 维护纯配置 mutation；adapter 不再持有 `proxy_app_enabled_from_db` / `set_proxy_app_enabled_in_db`。
本轮继续把手动 `stop_with_restore` 的批量 enabled 清理收敛到 host service 边界：`live_takeover.rs` 本地 helper 复用 `live_takeover_app_types()` 与 core `app_proxy_config_with_enabled`，读取失败忽略、写入失败 warning 的 best-effort 语义仍保持；adapter 不再持有 `clear_live_takeover_enabled_flags_in_db`。
本轮继续把简单 Live 备份恢复的备份读取和 JSON 解析错误投影收敛到 host service 边界：`live_takeover.rs` 本地 helper 直接读取 `get_live_backup`、保持读备份失败静默跳过和备份 JSON 损坏时报错的旧语义，adapter 不再持有 simple restore backup wrapper。
本轮继续把 keep-state 关闭流程里的 legacy `live_takeover_active` 标志清理收敛到 host service 边界：`live_takeover.rs` 本地 helper 直接读取/更新全局 proxy config，并复用 core `proxy_config_with_live_takeover_active` 清理兼容标志；`ProxyService::stop_with_restore_keep_state` 仍保留 per-app enabled 状态用于下次启动自动恢复，adapter 不再持有 `clear_legacy_live_takeover_active_flag_in_db`。
...
本轮继续把 `set_takeover_for_app` 的 per-app Live 备份删除收敛到 host service 边界：`live_takeover.rs` 本地 helper 直接调用 `delete_live_backup`，同步/写入失败回滚继续静默清理，关闭接管时删除失败继续作为错误返回；adapter 不再持有 `delete_live_backup_best_effort_in_db` / `delete_live_backup_in_db`。
本轮继续把 `set_takeover_for_app` 的 legacy 接管 active flag 读写收敛到 host service 边界：`live_takeover.rs` 本地 helper 直接调用 `is_live_takeover_active` / `set_live_takeover_active`，维护“检查接管状态失败”错误投影，best-effort 写入继续忽略错误；adapter 不再持有 active-state read/write wrapper。
...
本轮继续把 `start_with_takeover` 的全量 Live 备份清理收敛到 host service 边界：`live_takeover.rs` 本地 helper 区分 warning best-effort 与 silent best-effort，启动前失败继续输出清理 warning，恢复成功后的清理继续静默 best-effort；adapter 不再持有全量备份清理 wrapper。
本轮继续把 `start_with_takeover` 的 legacy 接管 active flag 写入收敛到 host service 边界：`live_takeover.rs` 本地 strict/best-effort helper 分别维护首次写入失败中止启动和恢复成功后回滚写入忽略错误的语义；adapter 不再持有 legacy active flag 写入 wrapper。
本轮继续把 Live token 回填后的 provider settings 持久化收敛到 host service 边界：`ProxyService::sync_live_config_to_provider` 只负责判断 token 投影是否产生变更，本地 helper 保持 DB 写回失败 warning-only、不阻断接管的旧语义。
本轮继续把 `ProxyService::{start,stop}` 的全局 `proxy_enabled` 持久化收敛到 host service 边界：`live_takeover.rs` 本地 helper 直接读取/更新 global proxy config，启动时启用失败仍返回错误，停止时禁用失败继续只输出 warning；adapter 不再持有全局开关写入 wrapper。
本轮继续把 `ProxyService` 的 `proxy_config` 读取、动态端口持久化和 `update_config` 保存规则收敛到 host service 边界：`live_takeover.rs` 本地 helper 直接持有 `get_proxy_config`/`update_proxy_config` DB source 和错误文案，动态端口写回与 `live_takeover_active` 保留仍复用 `proxy-core::ports` 纯策略；adapter 不再持有这组生命周期配置 wrapper。
本轮继续把 provider common-config 的 snippet 生成、命中判定、settings apply/remove、存储规范化、effective settings 构造和 switch-away strip 编排从 `proxy_core_adapter` 迁到 `services/provider/live.rs`：`live_takeover.rs` 继续只通过 provider service 出口构建 effective settings，provider live service 直接读取 DB snippet、调用 core ports 并维护 AppError/warning 文案；adapter 不再持有这组 common-config facade。
本轮继续把 `stop_with_restore` / `stop_with_restore_keep_state` 的 legacy active flag 清理、全量 Live 备份删除和全量 provider health reset 收敛到 host service 边界：`live_takeover.rs` 本地 strict/non-strict helper 分别维护手动停止和 keep-state 退出语义，本地 strict 备份删除 helper 与 health reset helper 保留“删除备份失败”与“重置健康状态失败”错误文案；adapter 不再持有全量 health reset wrapper。
本轮继续把 `recover_from_crash` 的 legacy active flag 清理和全量 Live 备份删除收敛到 host service 边界：`live_takeover.rs` 本地 strict helper 直接清理旧版接管状态并保留“清除接管状态失败”/“删除备份失败”错误文案，异常恢复路径继续按旧语义中断返回。
本轮继续把 `backup_live_configs` / `backup_live_config_strict` 的 Live 备份 JSON 序列化和 `save_live_backup` 错误投影收敛到 host service 边界：service 本地 helper 持有备份持久化 source 和错误文案，读取 live 文件、判断占位符接管状态并决定是否保存备份仍留在业务流程中。
本轮继续把 `restore_live_config_for_app_with_fallback_inner` 的 Live 备份读取与 JSON 解析错误投影收敛到 host service 边界：service 保留“备份是代理占位符则跳过并走 SSOT/清理兜底”的恢复策略，DB backup source 细节由本地 helper 维护。
本轮继续把 `restore_live_from_ssot_for_app` 的当前供应商读取、provider 列表读取和代理占位符供应商保护收敛到 host service 边界：`live_takeover.rs` 本地 helper 直接读取 settings 当前 provider、provider 列表并复用 takeover placeholder 判定，adapter 不再持有 SSOT restore provider DB wrapper；`write_live_with_common_config` 文件写入边界仍留作后续切片。
本轮继续把 `update_live_backup_from_provider_inner` 中用于保留 Codex MCP/OAuth 的现有 Live 备份读取与 JSON 解析错误投影收敛到 host service 边界：service 保留备份内容合并和统一会话路由注入，existing backup source 细节由本地 helper 维护。
本轮继续把 `update_live_backup_from_provider_inner` 的 provider-derived Live 备份序列化和 `save_live_backup` 更新错误投影收敛到 host service 边界：Claude/Codex 保留完整 effective settings，Gemini 继续复用 `proxy-core::ports::gemini_live_backup_from_effective_settings` 生成 env 备份，adapter 不再持有 provider-derived backup persistence wrapper。
本轮继续把 `hot_switch_provider_inner` 的 target-state source 和 current provider 双写收敛到 host service 边界：`live_takeover.rs` 本地 helper 直接读取 provider、执行官方供应商拦截、比较当前目标、检查 Live 备份存在性，并执行 DB current provider 与 settings current provider 双写；adapter 不再持有 hot-switch target-state DTO/source wrapper 或 persistence wrapper，阻断策略仍复用既有 provider category 纯规则。
本轮继续把 `ProxyService::{start,update_config}` 中的 `ProxyServer::new` 构造入口收敛到 host-owned `proxy/host/cc_switch/proxy_server.rs::proxy_server_from_runtime_config`：service 仍持有运行中 server 并负责生命周期锁、启动、停止和重启编排，但不再直接绑定 runtime server 构造签名，也不再经 `proxy_core_adapter` 取得 server factory。
本轮继续把 `restore_live_from_ssot_for_app` 的 SSOT provider live 写入调用和错误投影收敛到 host service 边界：`live_takeover.rs` 本地 helper 直接调用 provider common-config live writer 并保留 `写入 {app_type:?} Live 配置失败` 文案；adapter 不再持有 SSOT live writer wrapper。
本轮继续把 `ProxyServer::new` 的运行态装配收敛到 host-owned `proxy_state_from_runtime_sources`：server 只保留 listener/router/shutdown 生命周期编排，ProviderRouter、事件总线、failover manager、shadow/history store 与 `CcSwitchProxyServices` 装配集中在 `proxy/host/cc_switch/proxy_state.rs`。
本轮继续把 `ProxyService::write_claude_live` 的 Claude live settings 清洗入口改为直接调用 `proxy-core::api::ports::sanitize_claude_settings_for_live`：service 不再绕行 `services::provider` façade，Live 写入仍保留原有 host 文件落盘职责。
本轮继续把 `ConfigService` 的 Claude live settings 清洗入口改为直接调用 `proxy-core::api::ports::sanitize_claude_settings_for_live`；`services::provider` 不再 re-export 这个纯 core helper，live/config 调用方分别在 owning module 内导入 core ports contract。
后续已把 `CcSwitchProxyRuntime` 到 `ForwarderRuntimeHostResources` 的资源包构造迁回 `proxy/host/cc_switch/proxy_runtime.rs`：`HostForwardRuntime for CcSwitchProxyRuntime` 直接取 DB、构造 runtime resources 并进入 preplanned forwarding bridge，不再委托 `proxy_core_adapter`。
此前曾把 `ProxyServiceRuntimeResources` 与 `HostForwardRuntime` for `CcSwitchProxyRuntime` 两个 runtime trait impl 移入 `proxy_core_adapter`，让 adapter 负责把 host runtime 投影成 core service/forwarding 能力；该方向已由后续 host runtime split 修正，runtime trait impl 回到 `proxy/host/cc_switch/proxy_runtime.rs`。
此前曾把 `CcSwitchProxyRuntime` 数据结构和具体 `CcSwitchProxyServices<CcSwitchProxyRuntime>` alias 移入 `proxy_core_adapter`，让生产版 host 文件不再定义 runtime 数据形状；该方向已由后续 host runtime split 修正，runtime 数据结构回到 host 模块，adapter 不再保留 concrete runtime-services type alias。
本轮继续把 `CcSwitchProxyRuntime` 数据结构及其 `ProxyServiceRuntimeResources` / `HostForwardRuntime` contract 与 impl 从 `proxy_core_adapter` 迁回 `proxy/host/cc_switch/proxy_runtime.rs`：host 模块重新拥有 CC Switch DB、router、status、event、history、failover、current-target、runtime-source 与 Tauri handle 资源字段；后续 forward bridge 也已迁回该 host module，adapter 不再保留 runtime type re-export 或 forwarding bridge helper。
本轮继续把 `ProxyState.proxy_core_services` 字段改为直接持有 `CcSwitchProxyServices<CcSwitchProxyRuntime>`：运行中 state 的 core services 类型不再依赖 `proxy_core_adapter::CcSwitchProxyRuntimeServices` alias，`proxy_core_host` 的旧测试兼容 alias 也改为本地 type alias。
本轮继续把 `src/lib.rs` 中的 `proxy_core_host` 模块声明限制为 `#[cfg(test)]`：生产 crate 编译图不再包含该兼容模块，相关历史边界测试仍可通过 test-only module 读取旧测试辅助路径。
此前曾把 `ProxyState` 运行态数据结构与 `proxy_engine` helper 移入 `proxy_core_adapter`，让 `proxy/server.rs` 只保留旧路径 re-export；该方向已由后续 host state split 修正，运行态 state shape、`proxy_engine` helper 与 runtime assembly 回到 `proxy/host/cc_switch/proxy_state.rs`，adapter 不再拥有 `ProxyState` impl 或 `ProxyEngine::new` 边界。
...
本轮继续把生产 `ProxyServer` 构造入口改为 `from_runtime_state(config, state)`：host-owned `proxy_server_from_runtime_config` 负责把 `Database/AppHandle` 装配成 `ProxyState` 并创建 HTTP server，`proxy_core_adapter` 不再保留该一跳 factory；旧 `ProxyServer::new(config, db, app_handle)` 仅保留为 server 单元测试兼容入口。
...
本轮继续把 failover 切换调度封装为 `FailoverSwitchScheduler` runtime source：`RequestForwarder` 不再持有 `FailoverSwitchManager` 或 `AppHandle`，只在成功记录后调用注入 scheduler；CC Switch 默认 scheduler 位于 `proxy/host/cc_switch/failover_switch.rs`，负责把调度投影到现有 manager、托盘/UI 与 Live 切换副作用。
...
本轮继续移除 `proxy_core_adapter` 对 Copilot GitHub OAuth URL、token URL、usage URL、API base 和 usage endpoint fallback helper 的 re-export：`proxy::copilot_auth` 直接消费 `proxy_core::api::model_catalog` 的 URL/endpoint 契约，Copilot URL/model parsing fixtures 由 `proxy-core::copilot_model_map` owning tests 覆盖，adapter 不再作为 Copilot URL 构造或测试壳的二次出口。
本轮继续移除 `proxy_core_adapter` 对 Copilot public GitHub domain 常量的 const facade：`proxy::copilot_auth` 直接引用 `proxy_core::api::model_catalog::COPILOT_PUBLIC_GITHUB_DOMAIN`，Copilot domain/composite account fixtures 由 `proxy-core::copilot_model_map` owning tests 覆盖，adapter 不再作为 Copilot 模型目录常量或测试壳出口。
本轮继续移除 `proxy_core_adapter` 对 Codex/provider/client raw 模型目录 helper 的 re-export：`codex_config` 直接从 `proxy_core::api::model_catalog` 导入 Codex 模型目录构造、spec 检测和简化 helper，Codex catalog build/simplify 与 provider/client raw catalog fixtures 均由 `proxy-core::model_fetch` owning tests 覆盖，adapter 仅保留使用这些 helper 组合 DB/provider/client catalog 的宿主投影函数。
本轮继续把 client model catalog source selection 下沉到 host-owned model catalog provider：`client_model_catalog_from_app_source` 只在 `proxy/host/cc_switch/model_catalog_provider.rs` 内部调用 `proxy_core::api::model_catalog::{client_model_catalog_source_for_app, ClientModelCatalogSource}`，adapter 不再持有这层 client catalog source wrapper。
本轮继续移除 `proxy_core_adapter` 对 Claude Desktop provider validation issue enum 的 re-export：`claude_desktop_config` 直接从 `proxy_core::api::auth` 导入 direct/proxy provider validation issue 类型，adapter 只保留 provider facts 投影和 validation policy 调用。
本轮继续移除 `proxy_core_adapter` 对 proxy live URL formatting helper 的 re-export：`claude_desktop_config` 与 `live_takeover` 直接从 `proxy_core::api::ports` 导入 `proxy_live_urls_from_listen_parts`，adapter 不再作为该纯 URL 规则的二次出口。
本轮继续移除 `proxy_core_adapter` 对 proxy takeover status enabled-options helper 的 re-export：`ProxyService::get_takeover_status` 的 host-local source helper 直接调用 `proxy_core::api::ports::proxy_takeover_status_from_enabled_options`，adapter 不再持有接管状态矩阵 helper 出口或 DB wrapper。
本轮继续把 server lifecycle 的 stopped/status/info helper 下沉到 host-owned `proxy/host/cc_switch/proxy_server.rs`：`live_takeover` 直接从 `proxy_core::api::ports` 导入已运行场景的 `proxy_server_info_from_parts`，HTTP transport 通过 host server helper 执行启动/停止状态写入、事件发送、监听端口记录和 `ProxyServerInfo` 组合；adapter 不再私有调用或导出这些 server runtime helper。
本轮继续把 active connection、request-started、success/failure、current-provider 和 active-route target 等 forward runtime helper 下沉到 host-owned `proxy/host/cc_switch/forwarder_runtime_state_source.rs`：`ActiveConnectionGuard` 仍通过 `ForwarderRuntimeStateSource` 执行 acquire/release 生命周期，adapter 生产路径不再私有调用 `ProxyRuntimeStatus` mutation helper 或 request/attempt event helper。
本轮继续移除 `proxy_core_adapter` 对 takeover marked-state helper 的 re-export：`live_takeover` 直接从 `proxy_core::api::ports` 导入 `proxy_takeover_marked_state_is_reusable` 与 `proxy_takeover_should_restore_existing_backup_before_retakeover`，takeover/switch policy fixture 由 `proxy-core::ports` owning test 覆盖，不再通过 adapter 测试壳验证该接管状态策略。
本轮继续移除 `proxy_core_adapter` 对 common-config issue contract 的 re-export：`services/provider::{mod,live}` 直接从 `proxy_core::api::ports` 导入 `CommonConfigSnippetIssue`、`CommonConfigSettingsMutationIssue` 及对应 message helper，adapter 仅保留自身业务函数需要的私有 core import。
本轮继续移除 `proxy_core_adapter` 对 Claude Desktop gateway token error helper 的 re-export：`CcSwitchClaudeDesktopGatewayAuthSource` 直接从 `proxy_core::api::auth` 导入 `claude_desktop_gateway_token_error`，adapter 只保留 provider selection/unavailable policy helper 的私有 core import。
本轮继续移除 `proxy_core_adapter` 对 management channel-not-found helper 的 re-export：adapter 内部私有导入 `proxy_core::api::management::channel_not_found_error` 并用于 channel key/model 查询错误映射，不再把该 management helper 暴露为 crate-wide 出口。
本轮继续移除 `proxy_core_adapter` 对 Codex upstream model resolution helper 的 re-export：`provider_codex_upstream_model` 仍在 adapter 内部组合 provider settings 与 config TOML，但私有导入 `proxy_core::api::transport::resolve_codex_provider_upstream_model`，不再把该 transport helper 暴露为 crate-wide 出口。
本轮继续移除 `proxy_core_adapter` 对 Gemini settings extraction helper 的 re-export：adapter 内部仍用 `extract_gemini_api_key_from_settings` 与 `extract_gemini_base_url_from_settings` 组装 provider auth/base URL 事实，但改为私有导入 `proxy_core::api::auth`，不再暴露为 crate-wide 出口。
本轮继续修复独立 `proxy-core` 外部宿主示例：`external_relay_host` 的 runtime status active target 改为通过 `api::ports::current_route_target_from_input` 构造，并补齐 `pricing_model` channel 事实，保持外部 Rust API 示例跟随 current-route contract 演进。
本轮继续移除 `proxy_core_adapter` 对 Gemini live settings write helper 的 re-export：`services/provider/live` 直接从 `proxy_core::api::ports` 导入 `gemini_live_settings_to_write`，adapter 只保留 Gemini live env/config projection helper 出口和自身测试作用域的 core helper 引用。
本轮继续移除 `proxy_core_adapter` 对 response runtime policy resolver 的 re-export：`proxy/engine/context` 已直接从 `proxy_core::api::transport` 导入 `resolve_response_runtime_policy`，adapter 仅在 `response_runtime_policy_from_app_proxy_config` 包装内私有调用该 helper。
本轮继续移除 `proxy_core_adapter` 对 Claude provider kind inference helper 的 re-export：`provider_claude_kind` 仍在 adapter 内组合 provider meta/api_format/base URL/settings 事实，但 `infer_claude_provider_kind` 只作为 adapter 私有 core import 使用，不再暴露为 crate-wide 出口。
本轮继续移除 `proxy_core_adapter` 对 Claude api_format resolver helper 的 re-export：`provider_claude_api_format` 仍作为 host `Provider` 到 api_format 的 adapter 包装，`resolve_claude_api_format_from_settings` 只在 adapter 内私有引用并由测试作用域直接从 core 导入验证。
本轮继续移除 `proxy_core_adapter` 对 Claude/Copilot prompt-cache helper 的 re-export：prompt-cache provider fact 与 responses cache key projection 已随 Claude request transform wrapper 回到 `proxy/provider/claude.rs`，底层 `is_copilot_prompt_cache_provider` 与 `resolve_claude_responses_prompt_cache_key` 由 Claude provider owning module 私有引用。
...
本轮继续收窄 `proxy_core_adapter` 的 forward runtime facade；后续 `ForwardRuntimeRequest`、`ForwarderRuntimeHostResources`、`forward_runtime_request_from_proxy_request`、`response_runtime_policy_from_app_proxy_config`、`forward_with_preplanned_host_runtime`、`forward_proxy_request_with_host_runtime` 和 forwarder runtime config/resource projection helper 均已迁到 `proxy/host/cc_switch/proxy_runtime.rs`，`proxy_core_adapter` 不再参与该 host forward runtime 入口。
...
本轮继续收窄 `proxy_core_adapter` 的 Claude Desktop model route source facade：`claude_desktop_model_routes_from_router_source`、provider selection 结果映射和 route input 转换已迁回 `proxy/host/cc_switch/model_catalog_provider.rs`，`CcSwitchModelCatalogProvider` 直接读取 router/provider 并复用 adapter 内仍保留的 `provider_claude_desktop_proxy_model_routes` route projection helper；Claude Desktop model-list route input 合同由 model catalog provider 单测覆盖。
...
本轮继续把 Codex backup projection 策略迁回 host live takeover：`CodexBackupProjectionIssue`、MCP server 合并、OAuth auth 保留和错误消息 helper 都改为 `proxy/host/cc_switch/live_takeover.rs` 私有实现，adapter 不再为 live backup 迁移策略暴露 crate-wide 出口。
本轮继续把 Codex live write projection 分类迁回 host live takeover：`CodexLiveWriteProjection` 与 `codex_live_write_projection` 改为 `proxy/host/cc_switch/live_takeover.rs` 私有实现，adapter 不再为 Codex live 文件写入动作分类暴露 crate-wide 出口。
本轮继续把 Codex takeover config cleanup 与 preserved-auth live config projection 迁回 host live takeover：`remove_codex_takeover_config_placeholders_if_present` 和 `codex_preserved_auth_live_config_text_for_configured_policy` 改为 `proxy/host/cc_switch/live_takeover.rs` 私有实现，adapter 不再为 Codex live takeover 清理/写入策略暴露 crate-wide 出口。
本轮继续把 live token sync policy 迁回 host live takeover：`provider_settings_with_live_token_sync` 和 `sync_provider_settings_with_live_token` 改为 `proxy/host/cc_switch/live_takeover.rs` 私有实现；host live takeover 直接调用 core `provider_settings_with_live_token_sync`，adapter 不再为 live token 同步策略暴露 crate-wide 出口。
本轮继续收窄 `proxy_core_adapter` 的 provider/Claude Desktop 内层 projection facade：provider model mapping 已迁到 host-owned request source，`provider_claude_models_are_claude_safe`、`provider_claude_desktop_direct_inference_model_specs` 和 `provider_claude_desktop_proxy_provider_validation` 都改为 adapter 私有；host/command 调用方仍只消费 import decision、direct validation 和 gateway/profile 等高层入口。
本轮继续收窄 `proxy_core_adapter` 的 legacy channel migration facade：`legacy_provider_projection_input`、`proxy_channel_record_from_legacy_projection` 与 `legacy_channel_migration_preview_from_providers` 都已迁回 `database/dao/proxy_channels.rs` 私有实现；DAO 直接装配 provider facts、调用 core plan 并生成完整 preview payload。
本轮继续把 Codex takeover TOML 与 field applicator 迁回 host live takeover：`codex_takeover_toml_config_for_provider`、`CodexTakeoverAuthPolicy` 和 `apply_codex_takeover_fields_for_provider` 均为 `proxy/host/cc_switch/live_takeover.rs` 私有实现，adapter 不再保留 Codex takeover 公开入口；live takeover 测试覆盖 base_url、wire_api、upstream model、auth policy 和 modelCatalog attachment。
...
本轮继续收窄 `proxy_core_adapter` 的 session 提取 helper：host UUID 注入入口已归到 `proxy/host/cc_switch/proxy_runtime.rs` 并由 owning test 覆盖，session 策略仍由 `proxy-core::session::extract_session_id_with_generator` 维护，adapter 不再承载 session UUID fixture。
本轮继续把 stream-check reachability fixture 迁回 owning host module：`proxy/host/cc_switch/channel_reachability_probe.rs` 直接覆盖 StreamCheckResult 到 channel reachability、probe error 与 status 字符串 contract，adapter 不再承载 reachability 投影测试。
本轮继续删除 `proxy_core_adapter` 中重复的 usage record/logging fixture：selected-provider 缺失日志、usage 写入失败 warning、debug log 与 logging 开关 fallback 已由 `proxy-core::usage` owning tests 覆盖，adapter 不再承载这组纯 core usage contract 测试。
本轮继续删除 `proxy_core_adapter` 中重复的 Claude takeover model/env fixture：`proxy-core::model_mapping` 覆盖 1M suffix 与 display name，`proxy-core::ports` 覆盖 takeover model fields 与 env rewrite policy，adapter 不再承载这组纯 core takeover contract 测试。
本轮继续删除 `proxy_core_adapter` 中重复的 provider credential fixture：Claude/Codex/Gemini/OpenCode/OpenClaw/Hermes 凭据来源、Codex base_url issue 与本地化 spec 已由 `proxy-core::ports` owning tests 覆盖，adapter 不再承载这组纯 core credential contract 测试。
本轮继续删除 `proxy_core_adapter` 中重复的 default live import skip fixture：manual/startup seed 跳过规则已由 `proxy-core::ports` owning test 覆盖，adapter 不再承载这组纯 core live-policy 测试。
本轮继续删除 `proxy_core_adapter` 中重复的 provider live sync scope fixture：additive app 全量同步与其它 app 当前 provider 同步规则已由 `proxy-core::ports` owning test 覆盖，adapter 不再承载这组纯 core live-policy 测试。
本轮继续删除 `proxy_core_adapter` 中重复的 provider current/initial-marker/legacy common-config migration fixtures：当前 provider scope、初始 live config managed 标记和 legacy common config 迁移跳过规则已由 `proxy-core::ports` owning tests 覆盖，adapter 不再承载这组纯 core live-policy 测试。
本轮继续删除 `proxy_core_adapter` 中重复的 provider live-config presence/key-change/additive-write fixtures：DB-only live config 容错、key change 阻断和 additive live write action 规则已由 `proxy-core::ports` owning tests 覆盖，adapter 不再承载这组纯 core live-policy 测试。
本轮继续删除 `proxy_core_adapter` 中重复的 provider OMO/takeover/removal/additive-update fixtures：OMO variant/switch pair、takeover live sync target、additive live removal 和 additive update route 规则已由 `proxy-core::ports` owning tests 覆盖，adapter 不再承载这组纯 core live-policy 测试。
本轮继续删除 `proxy_core_adapter` 中重复的 provider live-config managed mark fixture：只有 unmanaged additive provider switch 需要标记 live config managed 的规则已由 `proxy-core::ports` owning test 覆盖，adapter 不再承载这组纯 core live-policy 测试。
本轮继续删除 `proxy_core_adapter` 中的 live takeover app kind 兼容 fixture：dispatch、takeover lock 与 live token label 纯策略已由 `proxy-core::ports` owning tests 覆盖，core live takeover app kind 到 CC Switch `AppType` 的兼容解析断言已迁到 host-owned `live_takeover.rs`，adapter 不再承载该 live takeover app catalog 测试。
本轮继续删除 `proxy_core_adapter` 中重复的 Claude live sanitize 与 JSON subset/merge/remove helper fixtures：host-only Claude live 字段清理、数组子集删除和 JSON 深合并/删除规则已由 `proxy-core::ports` owning tests 覆盖，adapter 不再承载这组纯 core helper 测试。
本轮继续把 `proxy_core_adapter` 中 managed-account runtime 的 no-AppHandle / non-managed passthrough fixtures 下沉到 `proxy/host/cc_switch/managed_account_runtime_source.rs`：默认 runtime source、Copilot live model/vendor/endpoint helper 和 `PROXY_MANAGED` 缺少 AppHandle 的错误语义由 host runtime source owning tests 覆盖，adapter 不再承载这组 runtime-source 行为测试。
本轮继续把 `proxy_core_adapter` 中 AppKind/AppType 与 `ProxyRequest` 到 host forward request 的转换 fixture 下沉到 `proxy/host/cc_switch/proxy_runtime.rs`：host runtime 拥有 unsupported app kind 错误包装、JSON body 解包和 session id 生成断言，adapter 不再承载这组 runtime bridge 行为测试。
本轮继续收窄 `proxy_core_adapter` 的 Copilot fingerprint 常量：`COPILOT_INTEGRATION_ID` 仅由 adapter 内部构造 GitHub Copilot auth headers 使用，Claude provider 测试改为断言实际 header 值，不再把该常量作为 crate-wide 出口。
本轮继续把 Copilot fingerprint 版本/UA 常量移回 owning module：`COPILOT_EDITOR_VERSION`、`COPILOT_PLUGIN_VERSION`、`COPILOT_USER_AGENT` 和 `COPILOT_API_VERSION` 改由 `proxy::copilot_auth` 定义，`proxy_core_adapter` 只作为 GitHub Copilot auth header 构造调用方消费这些常量，不再作为 Copilot 常量出口。
...
本轮继续移除 `proxy_core_adapter` 对 Claude forward api_format helper 与 metadata key 的 re-export：`resolve_forwarder_claude_api_format` 已迁到 host-owned managed-account runtime source 私有组合中，response metadata 投影继续由 adapter 包装，底层 `resolve_claude_forward_api_format` 由 runtime source 直接调用，`CLAUDE_API_FORMAT_METADATA_KEY` 只在 adapter 内私有引用。
...
本轮继续把 provider/channel attempt started/succeeded/failed 事件发射接入 `ForwarderRuntimeStateSource`：`RequestForwarder` 只决定 attempt 阶段与时机，并通过 `ForwarderAttemptStartedInput` / `ForwarderSuccessfulAttemptInput` / `ForwarderAttemptFailedInput` 传递 lifecycle 事实，不再直接拆出 `ProxyEventBus` 调用 attempt event helper。
本轮继续删除 `proxy_core_adapter` 中重复的 Claude Desktop profile-safe model id fixture：`[1m]` 后缀拒绝、旧/不完整 Claude alias 拒绝和安全 route prefix 白名单已由 `proxy-core::claude_desktop_gateway_auth` owning test 覆盖，adapter 不再承载这组纯 core gateway auth policy 测试。
本轮继续删除 `proxy_core_adapter` 中重复的 proxy server/runtime contract fixture：server log code、stopped runtime status、server info、takeover status、live URL 与 current route target 字段 contract 已由 `proxy-core::ports` owning tests 覆盖，provider-only active target projection 由 host-owned `forwarder_runtime_state_source` 自测覆盖，adapter 不再承载这组运行态纯 contract 测试。
本轮继续把 active route target 写入与 route-selected 事件发射接入 `ForwarderRuntimeStateSource`：`RequestForwarder` 不再同时拆出 `current_providers` 与 `ProxyEventBus` 调用 active target helper，成功 attempt lifecycle input 由 source 统一消费，后续外部宿主可替换当前路由目标存储/事件桥接。
...
本轮继续把 forwarder 的 provider/channel attempt runtime 包装为 `ForwarderAttemptRuntimeSource`：`RequestForwarder` 不再直持 `ProviderRouter`，attempt 放行、成功/失败健康记录和 neutral permit 释放都通过注入 source 进入 host-owned helper，后续可把该 source 替换为独立中转模块的 routing/circuit runtime。
...
本轮继续收窄 `ForwarderRuntimeStateSource` 的日志接口：media/signature/budget rectifier retry 的成功/失败日志行拼接从 trait surface 收进默认 source 内部，`RequestForwarder` 只触发 `log_rectifier_retry_success(ForwarderRectifierRetrySuccessLogInput)` / `log_rectifier_retry_failure(ForwarderRectifierRetryFailureLogInput)` 行为方法，外部中转实现不再需要返回 CC Switch 格式化日志字符串，也不需要接收散落的 app/kind/error 参数。
本轮继续把 terminal forward failure warning 的日志行拼接从 `ForwarderRuntimeStateSource` trait surface 收进默认 source 内部：`RequestForwarder` 只触发 `log_terminal_forward_failure(ForwarderTerminalFailureLogInput)`，外部中转实现不再需要返回 `[FWD-002]` 格式化日志字符串，也不需要接收散落的 app/attempt/error 参数。
本轮继续把 retryable forward failure warning 的日志行拼接和错误消息 payload 从 `ForwarderFailureDecision::Retryable` 返回值中移出：`RequestForwarder` 只消费 retryable 控制流并触发 `log_retryable_forward_failure(ForwarderRetryableFailureLogInput)`，外部中转实现不再需要返回 `[FWD-001]` 格式化日志字符串或格式化错误消息，也不需要接收散落的 app/provider/attempt/error 参数。
...
本轮继续把上游 URL planning 的 endpoint rewrite、full-endpoint/query 透传和 channel param override 策略收敛到 `proxy-core::request_url::forward_upstream_url_plan`：`ForwarderRequestSource` 只投影 base URL、endpoint、Claude/Codex/Gemini transform facts 与 host adapter URL builder，不再维护本地 plan 策略；Codex Responses→Chat query/override 与 Gemini Native full-url fixture 已由 `proxy-core::request_url` owning tests 承接，`proxy_core_adapter` 不再保留 Codex/Gemini URL planning 聚合 fixture。
...
Claude Desktop gateway token DB helper 已迁入 `proxy/host/cc_switch/claude_desktop_gateway_auth_source.rs`，由 `CcSwitchClaudeDesktopGatewayAuthSource` 与 profile apply/status/server smoke 共同复用；`proxy_core_adapter` 不再保留 gateway token setting key、DB read/write 或 token 生成 helper，边界测试防止该宿主持久化职责回流到 adapter 或 `claude_desktop_config`。
...
本轮继续把 failover switch 配置读取失败的 FO-002 warning 日志行收敛到 `proxy-core::provider_selection::failover_config_read_error_log_line`：host `FailoverSwitchManager` 负责读取 config、默认返回 false 和发 warning，不再维护 `[FO-002] 无法读取...跳过切换` payload；adapter 不再保留该 manager 专属配置 wrapper。
...
本轮继续把接管状态的 enabled/缺失配置矩阵收敛到 `proxy-core::ports::proxy_takeover_status_from_enabled_options`：host service 只把 DB config 读取结果投影成 `Option<bool>`，不再维护 Claude/Codex/Gemini 缺失时默认 false 和 OpenCode/OpenClaw 默认 false 的状态组装规则；adapter 不再保留该 source wrapper。
...
本轮继续把 Live 接管支持的 switch-mode app catalog 收敛到 `proxy-core::ports::live_takeover_app_kinds`：core 统一维护 Claude/Codex/Gemini 清单与顺序，host-owned `live_takeover.rs` 映射为宿主 `AppType` 并保留 DB/live 文件副作用，adapter 不再承载该 app catalog 映射。
本轮继续把 Provider switch 的 normal/takeover-aware 分流和 takeover lock 适用范围收敛到 `proxy-core::ports::{provider_switch_dispatch_for_app,provider_switch_requires_takeover_lock}`：core 维护 OpenCode OMO/Claude Desktop 直通 normal、Claude/Codex/Gemini 需要 lock 的纯策略，`ProviderService` 调用点直接投影 `AppType` 与 provider category，adapter 不再保留 provider switch 一跳 facade。
...
本轮继续把默认 live import 跳过策略、provider 是否同步 live 和 DB-only provider live-config presence 错误策略收敛到 `proxy-core::ports`：core 消费 `AppKind` 与 `live_config_managed` 事实，`Provider` 暴露本地 metadata fact，host service 继续负责实际 DB/live 读写。
...
本轮继续把 provider launch env 投影收敛到 `proxy-core::ports::launch_env_vars_from_provider_settings`：core 维护 Claude/Codex/Gemini 从 provider settings JSON 到启动环境变量的纯映射，命令边界直接传入宿主 `Provider.settings_config` 与 `AppKind`，adapter 不再保留 `provider_launch_env_vars_for_app` 一跳 facade。
...
本轮继续把 provider 默认 live import 与存储前 settings 归一化收敛到 `proxy-core::ports::{provider_default_live_import_settings,normalize_provider_settings_for_storage}`：core 维护 Claude 旧模型字段兼容和非 Claude no-op 规则，provider live / ProviderService 只负责把 host `AppType` 投影为 core `AppKind` 并执行 DB/文件副作用。
...
本轮继续把 additive app 的 stream-check base URL 纯策略收敛到 `proxy-core::domain`，并让 `StreamCheckService::resolve_base_url` 直接消费该策略：core 维护 OpenCode npm fallback、OpenClaw `baseUrl`、Hermes `base_url` 纯解析和缺失 base URL 错误规格，service 负责 app 分支、标准 provider URL facts 投影和 reachability 探测。
本轮继续把 Gemini provider settings 基础结构校验收敛到 `proxy-core::ports::validate_gemini_settings_basic`：core 维护 `env`/`config` 字段形状和本地化错误规格，`gemini_config` 与 `ProviderService` 只通过 adapter 复用同一校验入口；结构化 issue 到 `AppError` 的映射已收为 adapter 私有。
...
本轮继续把 Gemini settings 严格切换校验收敛到 `proxy-core::ports::validate_gemini_settings_strict`：core 维护 OAuth 空 env 放行、非空 env 必须含 `GEMINI_API_KEY` 和本地化错误规格，host 只保留 adapter 私有的 AppError 映射。
本轮继续把 Gemini provider 鉴权类型判定收敛到 `proxy-core::ports::detect_gemini_auth_type`：core 维护 partner key 优先级、Google official 名称匹配与 PackyCode 关键词规则，`services::provider::gemini_auth` 直接从 `Provider` 投影 `GeminiAuthTypeInput` 并执行 settings 文件写入；`provider/live.rs` 直接从 `proxy_core::api::ports` 引用 `GeminiAuthType`，`services::provider::gemini_auth` 不再作为该 enum 的二次出口，adapter 也不再作为该 enum 或 detector 的二次出口。
本轮继续把 Gemini `.env` 解析和序列化收敛到 `proxy-core::ports`：core 维护宽松解析、严格行号错误分类和稳定排序输出，adapter 只通过 strict parse wrapper 暴露宿主 `AppError` 结果，结构化 parse issue 映射 helper 已收为私有。
...
本轮继续把 usage script 凭据覆盖策略的 service 入口收敛到 `proxy_core::api::ports::usage_script_credentials_from_parts`：service 层直接使用纯 core 策略，adapter 只保留 `Provider.meta.usage_script` 宿主投影，不再保留 provider-scoped credential wrapper。
本轮继续把 usage pricing source 常量收敛到 `proxy-core::api::usage` 直接入口：`database/dao/proxy.rs` 只直接引用实际用于默认值的 `PRICING_SOURCE_RESPONSE`，host-owned `database_usage_sink.rs` 直接引用 `PRICING_SOURCE_REQUEST` / `PRICING_SOURCE_RESPONSE`，`database` 模块不再作为该 core 常量的二次出口。
本轮继续把 HTTP handler typed JSON 签名 DTO 收敛到 `proxy-core::api` 直接入口：`proxy/transport/http/handlers.rs` 直接从 `auth`、`management`、`model_catalog` 与 `ports` 分组导入 response/query/body DTO，`proxy::response_adapter` 只保留协议 dispatch/bridge 函数，不再作为这些 core DTO 的二次出口。
本轮继续把 Copilot/Codex OAuth 命令 DTO 收敛到 `proxy-core::api` 直接入口：`commands/auth.rs`、`commands/copilot.rs`、`proxy/copilot_auth.rs` 与 `proxy/codex_oauth_auth.rs` 直接引用 auth/model_catalog DTO，`proxy::copilot_auth` 不再作为 `CopilotAuthStatus`、`GitHubAccount`、`GitHubDeviceCodeResponse` 或 `CopilotUsageResponse` 的二次出口。
本轮继续把 Claude takeover 的 provider facts 决策收敛到 `proxy-core::ports::apply_claude_takeover_fields_for_provider_facts`：core 维护 managed-account/Copilot auth policy、provider-vs-live 模型字段来源和 env 写入，host-owned `live_takeover.rs` 直接投影宿主 Provider facts 并调用 core helper，adapter 不再保留 Claude takeover provider wrapper。
本轮继续移除 `proxy_core_adapter` 对 Claude takeover 纯策略 helper 的 re-export：`proxy/host/cc_switch/live_takeover.rs` 直接引用 `proxy-core::ports::{apply_claude_takeover_fields_with_policy,ClaudeTakeoverAuthPolicy,apply_claude_takeover_fields_for_provider_facts,ClaudeTakeoverProviderFacts}`，adapter 不再保留 `Provider` 到 `ClaudeTakeoverProviderFacts` 的宿主投影入口。
本轮继续把 Codex takeover TOML 字段计划收敛到 `proxy-core::ports::codex_takeover_toml_config_patch`：core 维护接管时强制 local proxy `base_url`、`wire_api = responses` 和可选 upstream model 写回规则，host live takeover 负责 `toml_edit` 语法保留写入并组合 provider model catalog attachment。
...
本轮继续把 channel-key 覆盖 provider settings 的 app 选择入口从字符串推进到 `proxy-core::auth::settings_config_with_channel_auth_key_for_app`：core 接收 `AppKind` 维护 Claude/Gemini/Codex/自定义 app 的 key 注入字段规则，`proxy/host/cc_switch/auth_provider.rs` 负责克隆宿主 `Provider` 并投影 `AppType`，adapter 只 re-export helper。
本轮继续把 `ProxyError` 的用户展示文案策略收敛到 `proxy-core::errors::proxy_error_display_message_from_status`：core 维护上游错误、超时、转发失败、不可用 provider、数据库和转换错误的稳定中文 contract，`error_mapper` 把 host `ProxyError` 投影为状态 kind、原始 message、上游 body 与 fallback display message，adapter 只保留兼容 re-export。
本轮继续把显式上游代理 URL 的解析失败文案和 scheme allowlist 收敛到 `proxy-core::transport::{validate_explicit_proxy_url,invalid_explicit_proxy_url_message}`：core 维护 `http/https/socks5/socks5h` 支持矩阵、URL 脱敏和错误文本，host-owned `global_http_client` 仍负责实际构造 `reqwest::Proxy`。
本轮继续移除 `proxy_core_adapter` 对自定义 User-Agent 纯 parser/header helper 的 re-export：`provider.rs` 仍通过 adapter 边界调用显式 parser wrapper，adapter 私有导入 `proxy-core::transport` parser/header helper 并只保留读取宿主 `Provider.meta.custom_user_agent` 的 provider-scoped wrapper。
本轮继续把非流式上游响应 JSON/SSE 解析失败的日志上下文与 body lossy 投影收敛到 `proxy-core::response_parse::upstream_response_parse_failure_log_message`：core 维护 Claude/Codex 前缀、Chat 专用文案和 body 展示策略，`error_mapper` 直接消费 core transport API 并把 fallback 日志级别分发给宿主 logger。
本轮继续把 Axum/transport response 构造失败的上下文文案收敛到 `proxy-core::response_build::ProxyResponseBuildErrorContext`：core 维护 tag、Claude、Codex SSE/JSON/错误响应的稳定日志文本，host response adapter 与 response pipeline 直接引用 core context 并执行 Axum bridge。
本轮继续删除 root-level legacy proxy facade：`proxy/forwarder.rs`、`proxy/response_processor.rs`、`proxy/server.rs`、`proxy/handlers.rs`、`proxy/hyper_client.rs`、`proxy/http_client.rs`、`proxy/provider_router.rs`、`proxy/failover_switch.rs`、`proxy/providers/mod.rs`、`proxy/usage/mod.rs` 与 `proxy/usage/logger.rs` 已移除，`proxy/mod.rs` 不再声明这些旧模块，`proxy_core_boundary` 固定旧文件和旧模块声明不得回流。
...
987. channel test/reachability probe 的 provider-not-found 与 probe failure `ProxyCoreError` 构造已迁入 `proxy-core::ports`；DB-backed host probe source 只负责查询宿主 provider 与执行 stream check。
...
990. Claude Desktop gateway token 加载失败的 auth error 构造已迁入 `proxy-core::claude_desktop_gateway_auth`；host gateway auth source 负责调用宿主 token store 并把错误交给 core constructor。
...
992a. 本轮继续移除 `proxy_core_adapter` 对 adapter-only core error helper 的 re-export：`config_error_with_context`、`internal_error_with_context`、`invalid_request_error` 与 `selected_provider_missing_from_source_message` 只在 adapter/context owning modules 内私有导入，host-facing adapter 出口仍是 `app_error`；`database_channel_source` 已自行保留 write-path invalid-request 映射，不再需要 `app_write_error`，`database_usage_sink` 也已自行保留 usage internal-error 映射，不再需要 `usage_error`。
...
1013. 自定义 User-Agent 的 trim、空值忽略和 `HeaderValue` 校验规则已迁入 `proxy-core::request_headers::parse_custom_user_agent`，provider 与 model-fetch 路径已直接引用 core parser；stream-check/forwarder 中需要 `Provider` 形状和 Copilot upstream fact 的路径已由 owning modules 本地投影 raw 值并直接调用 `proxy_core::api::transport::provider_custom_user_agent_header`。
1014. Stream check 的 provider override 配置合并与 probe result 到 `StreamCheckResult` 的成功/失败/degraded envelope 构造已迁入 `proxy-core::api::management`，host `StreamCheckService` 只保留 reqwest 探测、timestamp 注入和 provider `testConfig` override 投影；该投影已由 service 本地拥有，不再经 `proxy_core_adapter::provider_stream_check_config_override`。
...
1016a. Provider switched failover 事件构造已改为 `proxy-core::events::{provider_switched_failover_event,provider_switched_failover_enabled_event}`：failover command 与 host failover scheduler 继续负责 Tauri emit、托盘刷新和错误日志，`provider-switched` 事件名、payload shape 与 failover/failoverEnabled source 选择集中在 core，adapter 不再暴露 provider-switched 专用消息 facade。
...
1032. provider-router config source 删除 `circuit_breaker_config_from_router_config_result`、`circuit_failure_threshold_from_router_config_result` 和 `proxy_takeover_status_from_config_results` 三个 Result-fallback facade；host config source 只负责把 host `AppError` 投影为缺省事实，core helper 继续负责 circuit/takeover DTO 策略。
...
1033a. 本轮继续移除 `proxy_core_adapter` 对 adapter-only config projection helper 的 re-export：`CcSwitchConfigSource` 直接导入 `proxy_app_config_from_parts`、`proxy_global_config_from_global_config` 与 `proxy_runtime_config_from_proxy_config`，adapter 不再提供 config-source-aware DB wrapper。
1033b. 本轮继续移除 `proxy_core_adapter` 对 adapter-only provider metadata/domain helper 的 re-export：`provider_metadata_from_input`、`provider_account_ref` 与 `unsupported_app_kind_config_error` 只在 adapter 内私有导入，host-facing 出口仍是 provider/domain projection wrapper。
1033c. Request/server lifecycle 纯事件构造已改为 `proxy-core::events::{request_started_event,server_started_event,server_stopped_event}`：forward runtime state source 与 host proxy server source 直接触发 event bus 副作用，`request_started`、`server_started` 和 `server_stopped` 的事件名与 payload shape 集中在 core，adapter 不再暴露对应 message facade。
1033d. Proxy official warning 纯事件构造已改为 `proxy-core::events::proxy_official_warning_event`：live takeover host helper 负责查询当前 provider、复用 core event contract 并触发 Tauri emit，`proxy-official-warning` 事件名与 payload shape 集中在 core，adapter 不再暴露 warning message facade 或 DB wrapper。
...
1035. client model catalog source 删除 `client_model_catalog_raw_from_source` wrapper；host-owned `CcSwitchModelCatalogProvider` 按 core `ClientModelCatalogSource` 选择是否读取 Codex active catalog，不再通过 adapter 暴露一次 match 的额外 host API。
1036. Codex takeover model catalog 注入删除 `attach_codex_model_catalog_from_provider` 单调用点 mutation wrapper；takeover field applicator 直接读取并合并 provider `modelCatalog`，不再为值级合并暴露额外 adapter API。
...
1040. Claude forwarder api_format 删除 `forwarder_claude_api_format` 一跳 wrapper；request source 和 managed-account runtime source 直接复用 provider 级 adapter API，runtime source 直接调用 `proxy-core::resolve_claude_forward_api_format` 完成 Copilot vendor 分流，`proxy_core_adapter::resolve_forwarder_claude_api_format` façade 已删除。
...
1065. 删除 `proxy/error_mapper.rs` 的 `map_proxy_error_to_status` 与 `get_error_message` dead-code facade；状态码和展示文案行为由 `proxy::error_mapper` 拥有并委托 core contract，新增 boundary marker 防止旧 public facade 回流。
...
1083. `/claude-desktop/v1/models` 已补充 `ProxyServer::start` 级 runtime smoke：内存 DB 配置 Claude Desktop gateway provider，并通过 adapter-owned token source 创建 gateway token 后，真实本机监听端口验证无 bearer 返回 401、有效 bearer 返回模型列表和 `supports1m` contract。
...
1096. crate 外 public prelude smoke 已覆盖 `/proxy/v1/channels/{channel_id}/models` 管理 contract：外部宿主只依赖 prelude 即可构造 `ChannelModelsSource`、`ChannelModelRecordInput` 与 replace request，调用 `ProxyEngine` 的 channel model list/replace response 入口，并完整命名 `ChannelModelsResponse` 与 `ChannelModelRecord` 字段类型。
1097. crate 外 public prelude smoke 已覆盖 `/proxy/v1/channels/{channel_id}/keys` 管理 contract：外部宿主只依赖 prelude 即可构造 `ChannelKeyPathRequest`、`ChannelKeysSource`、`ChannelKeyRecordSource`、`ChannelKeyRecordInput` 与 key 写入/更新 request，调用 `ProxyEngine` 的 channel key list/upsert/update/delete response 入口，并完整命名 key 列表、记录和删除响应字段类型。
...
1104a. `external_relay_host` example 已补充 managed-account token cache 演示：外部中转宿主只通过 public prelude 构造 `ManagedAccountTokenSnapshotStore`、refresh success/failure input 和 runtime/account key，即可复用 core 的 retryable 短窗口缓存回退、terminal reject 与 Codex OAuth account resolution 语义。
...
1116. Claude transform 与 Codex Chat->Responses transform 的 streaming/非流 SSE 聚合决策已迁入 `proxy-core::response_transform::{claude_transform_streaming_decision,codex_chat_transform_streaming_decision}`；Claude 决策仍由 host adapter 投影 `Provider` 是否 Codex OAuth 等宿主事实，Codex Chat 决策则由 `proxy::response_adapter` 直接传入 stream flag 与响应 header。
...
1119. Claude Anthropic 消息兼容 normalization 的组合规则已迁入 `proxy-core::response_transform::normalize_claude_anthropic_messages`：core 统一维护 Anthropic format gate、tool-thinking history 修复和 DeepSeek thinking-disabled effort 清理，host request source 只传 `Provider.settings_config` 与请求体。
...
1121. App 到 provider/protocol adapter family 的选择策略已迁入 `proxy-core::provider_adapter_kind_for_app`；host `proxy/provider::get_adapter` 直接消费 `proxy_core::api::domain::{provider_adapter_kind_for_app,AppKind,AppProviderAdapterKind}` 并实例化具体 `Claude/Codex/Gemini` adapter，custom/additive app 继续按 Codex-compatible fallback 处理，边界测试防止 host 重新手写 `AppType` 分支或恢复 adapter facade。
...
1124. Forwarder provider URL facts 已迁入 `proxy-core::request_url::{ForwarderProviderUrlFacts,ForwarderProviderUrlFactsInput,forwarder_provider_url_facts}`；host adapter 只从具体 provider adapter 提取 base_url、从 Provider meta 投影 is_full_url/provider_type，core 统一生成 `base_url/is_full_url/is_copilot` facts 供 forwarder URL planning、Copilot dynamic endpoint 和 full-url 处理复用，边界测试防止 `ForwarderAdapterContext::provider_url_facts` 重新本地构造 DTO 或手写 Copilot upstream 判定。
1125. Managed provider classification 已迁入 `proxy-core::managed_account_auth::{ProviderManagedAuthFacts,ProviderManagedAuthClassification,classify_provider_managed_auth}`；host adapter 只投影 provider kind 与 `ANTHROPIC_BASE_URL`，core 统一判定 GitHub Copilot、Codex OAuth 和 managed-account auth 使用关系，边界测试防止 `proxy_core_adapter` 重新分散调用 provider_kind_* wrapper 或手写上游域名判定。
1125a. 本轮继续移除 `proxy_core_adapter` 对 adapter-only managed-auth core helper 的 re-export：`classify_provider_managed_auth`、`managed_account_id_for_auth_provider` 与 `managed_provider_auth_info_for_provider_kind` 只在 adapter 内私有导入，host-facing 出口仍是 provider-shaped managed-auth wrapper。
1126. Provider custom User-Agent policy 已迁入 `proxy-core::request_headers::provider_custom_user_agent_header`；forwarder request source 与 stream-check 本地投影 `ProviderMeta.custom_user_agent` raw 值和 Copilot upstream fact，core 统一执行空值 trim、`HeaderValue` 校验和 GitHub Copilot 禁用自定义 UA 规则，边界测试防止 `proxy_core_adapter` 重新持有 provider-shaped façade 或调用 host `ProviderMeta::custom_user_agent_header` 解析入口。
1127. Claude Desktop profile-safe model policy 已迁入 `proxy-core::claude_desktop_gateway_auth::{claude_desktop_model_id_is_profile_safe,claude_desktop_provider_models_are_profile_safe}`；host `claude_desktop_config::is_claude_safe_model_id` 只保留经 `proxy_core_adapter` 的兼容 wrapper，adapter 只传 `Provider.settings_config`，core 统一维护 `claude-`/`anthropic/claude-` route 前缀、`[1m]` 后缀拒绝和 sonnet/opus/haiku/fable 角色白名单，边界测试防止 adapter 重新枚举 `ANTHROPIC_*MODEL` env 字段或回调 host 判定函数。
1128. Claude Desktop proxy route projection 已迁入 `proxy-core::claude_desktop_gateway_auth::{ClaudeDesktopProxyRouteInput,ClaudeDesktopResolvedProxyRoute,claude_desktop_proxy_model_routes}`；host `claude_desktop_config::proxy_model_routes` 只投影 `ProviderMeta.claude_desktop_model_routes` 并保留缺失/空结果的本地化错误，core 统一维护 unsafe route 修复、默认安全 route 分配、label fallback、排序与 route_id 去重，边界测试防止 host 重新持有 `next_catalog_safe_route_id` 或 `DEFAULT_PROXY_ROUTES` 投影策略。
1129. Claude Desktop proxy request route lookup 已迁入 `proxy-core::claude_desktop_gateway_auth::claude_desktop_proxy_request_upstream_model`；host `map_proxy_request_model` 只读取请求 `model`、投影 resolved/raw route facts、映射 route_unknown 本地化错误并保留 MiMo body normalization，core 统一维护 `[1m]` 后缀剥离、raw legacy route 兼容、Opus 4.7/4.8 rollout alias、官方角色关键词 fallback 与 fable->opus 兜底，边界测试防止 `claude_desktop_config` 重新持有这些 lookup helper。
1130. Claude Desktop direct inference model specs 已迁入 `proxy-core::claude_desktop_gateway_auth::{ClaudeDesktopDirectInferenceModelSpec,ClaudeDesktopDirectModelRouteIssue,claude_desktop_direct_inference_model_specs}`；host `direct_inference_model_specs` 只投影 route facts 并把 core issue 映射为本地化 `AppError`，core 统一维护直连 safe route 校验、禁止 route->upstream 映射、label trim、supports1m 排序优先与 route name 去重，边界测试防止 host 重新持有 direct route policy。
1131. Claude Desktop direct gateway credentials 已迁入 `proxy-core::claude_desktop_gateway_auth::{ClaudeDesktopDirectGatewayCredentials,ClaudeDesktopDirectGatewayCredentialIssue,claude_desktop_direct_gateway_credentials}`；host `direct_gateway_credentials` 只传 `Provider.settings_config` 并把缺 env/base URL/auth token issue 映射为本地化 `AppError`，core 统一维护 `ANTHROPIC_BASE_URL` 与 `ANTHROPIC_AUTH_TOKEN` 的 env 提取、trim 和缺失分类，边界测试防止 host 重新解析这些直连 credential key。
1132. Claude Desktop proxy gateway base URL 的 listen-address origin 格式化已复用 `proxy-core::ports::proxy_live_urls_from_listen_parts`；host `proxy_gateway_base_url_from_db` 只读取 DB proxy_config、保留零端口本地化错误并追加 `/claude-desktop` 前缀，core 统一维护 `0.0.0.0`/`::` client connect 地址替换、IPv6 bracket 和 live Codex base URL contract，边界测试防止 host 重新持有 `proxy_origin_from_parts`。
1133. Claude Desktop status 的 profile stale model detection 已迁入 `proxy-core::claude_desktop_gateway_auth::claude_desktop_profile_has_unsafe_model_ids`；host `get_status` 只负责读取 profile JSON 与组装 `ClaudeDesktopStatus`，core 统一遍历 `inferenceModels` 字符串/对象形态并复用 profile-safe model policy 判定 raw/unsafe 模型，边界测试防止 host 重新解析 `inferenceModels` 安全性。
1134. Claude Desktop default proxy route catalog 已迁入 `proxy-core::claude_desktop_gateway_auth::{ClaudeDesktopDefaultProxyRouteSpec,claude_desktop_default_proxy_routes}`；host `default_proxy_routes` 只把 core catalog 投影成 Tauri DTO，`commands/provider` 的 Claude import route 推导也改为走函数 surface，core 统一维护默认 route_id/env_key/supports1m、unsafe route repair catalog 顺序和 fable 末位兼容策略，边界测试防止 host 重新定义 `DEFAULT_PROXY_ROUTES`。
1135. Claude Desktop import 的 proxy route suggestion policy 已迁入 `proxy-core::claude_desktop_gateway_auth::{ClaudeDesktopSuggestedProxyRoute,claude_desktop_suggested_proxy_routes}`；`commands/provider::suggested_claude_desktop_routes` 只经 `proxy_core_adapter` 投影成现有 host route DTO，core 统一维护默认 env key 扫描、`[1M]` 后缀到 supports1m 的翻译、`*_NAME` label override 优先级、相同 upstream model 合并和 `ANTHROPIC_MODEL` fallback，边界测试防止 command 层重新解析 env key 或手写 route merge 策略。
1136. Claude Desktop gateway profile JSON schema 已迁入 `proxy-core::claude_desktop_gateway_auth::{ClaudeDesktopGatewayProfileModelSpec,claude_desktop_gateway_profile}`；host `apply_provider_to_paths_inner` 只把 direct/proxy route facts 投影成 core profile spec 并继续负责文件写入、rollback 与 token/provider 读取，core 统一维护 `inferenceProvider`、gateway base URL/auth scheme/api key、`coworkEgressAllowedHosts`、`disableDeploymentModeChooser` 以及 `inferenceModels` 中 string/object、`labelOverride`、`supports1m` 的序列化形状，边界测试防止 host 重新定义 `build_gateway_profile` 或 `inference_model_json`。
1137. Claude Desktop 本地 config JSON mutation policy 已迁入 `proxy-core::claude_desktop_gateway_auth::{claude_desktop_config_with_deployment_mode,claude_desktop_config_without_gateway_enterprise_config}`；host `write_deployment_mode` 和 `remove_cc_switch_enterprise_config` 只负责文件存在判断、读写 JSON 与错误映射，core 统一维护非 object 配置归一化、`deploymentMode` 写入、restore 时 enterprise gateway key 清理以及空 `enterpriseConfig` 删除规则，边界测试防止 host 重新持有这些 JSON key 集合或 object mutation 策略。
1138. Claude Desktop configLibrary meta JSON policy 已迁入 `proxy-core::claude_desktop_gateway_auth::{claude_desktop_meta_with_profile_entry,claude_desktop_meta_applied_id,claude_desktop_meta_has_profile_entry}`；host `write_meta`、`read_applied_id`、`meta_has_profile_entry` 只负责 meta 文件读写和 profile id/name 常量注入，core 统一维护非 object meta 归一化、`entries` 去重/替换、`appliedId` 设置与 restore 时切换到剩余 profile 或清空的规则，边界测试防止 host 重新解析 `appliedId`/`entries`。
1139. Claude Desktop proxy gateway endpoint shape 已迁入 `proxy-core::claude_desktop_gateway_auth::claude_desktop_proxy_gateway_base_url`；host `proxy_gateway_base_url_from_db` 只负责读取 DB 中的 listen address/port 并经 `proxy_live_urls_from_listen_parts` 得到 proxy origin，core 统一维护对外 gateway base URL 的 `/claude-desktop` endpoint 拼接规则，边界测试防止 host 重新持有 `CLAUDE_DESKTOP_PROXY_PREFIX` 或直接 `format!` endpoint。
1140. Claude Desktop MiMo Anthropic tool-thinking history mutation 已复用 `proxy-core::response_transform::normalize_anthropic_tool_thinking_history` 并由 core request-body helper 消费；core 统一维护 assistant `tool_use` turn 的 missing/empty thinking 补齐、`redacted_thinking` 转换和 `signature` 清理规则，边界测试防止 host 重新持有 MiMo placeholder 常量或 JSON mutation 细节。
1141. Claude Desktop proxy request body mapping 已迁入 `proxy-core::claude_desktop_gateway_auth::claude_desktop_proxy_request_body_with_upstream_model`；host `map_proxy_request_model` 只负责把 `Provider` route/meta facts 投影成 `ClaudeDesktopProxyRouteInput`/`ClaudeDesktopResolvedProxyRoute` 并把 `ClaudeDesktopProxyRequestBodyIssue` 映射成现有 localized `AppError`，core 统一维护请求 `model` trim/缺失判断、route lookup、upstream model 写回和 MiMo normalization gate，边界测试防止 host 重新读取/写入 `body["model"]` 或直接调用 route lookup/normalizer。
...
1152. Claude Desktop Proxy provider validation assembly 已收敛到 `proxy_core_adapter::provider_claude_desktop_proxy_provider_validation`；host `validate_proxy_provider` 只保留官方 provider 快速通过和 config/model-routes/credentials issue 到本地化 `AppError` 的映射，adapter 统一维护 proxy config validation、proxy model route projection validation 和 Base URL/API Key 存在性检查顺序，边界测试防止 host config 重新展开三段 Proxy 校验。
1153. Claude Desktop provider validation mode dispatch 已收敛到 `proxy_core_adapter::provider_claude_desktop_provider_validation`；host `validate_provider` 只保留官方 provider 快速通过和 Direct/Proxy issue 到本地化 `AppError` 的映射，adapter 统一维护 provider mode 到 Direct/Proxy validation 的分发，旧的 host `validate_direct_provider` / `validate_proxy_provider` wrapper 已删除，边界测试防止 host config 重新持有 validation mode dispatch。
1154. Claude Desktop model catalog route source 已迁回 host-owned `CcSwitchModelCatalogProvider`：该 provider 直接调用 router 选择 provider id、从 DB 回查 Provider、映射 selection/load 错误并把 resolved route 转成 core `ClaudeDesktopModelRouteInput`；adapter 仅保留 `provider_claude_desktop_proxy_model_routes` 共享 route projection helper，且不再通过 host `claude_desktop_config::proxy_model_routes` 或 `ResolvedModelRoute` DTO 做 route projection，边界测试防止 model route source 回流到 adapter 或重新形成 adapter 到 host config 的反向依赖。
1155. Claude Desktop forward request body mapping 已改为在 `proxy/host/cc_switch/forwarder_request_source.rs` 内直接调用 `provider_claude_desktop_proxy_request_body`；production forward request path 不再回调 host `claude_desktop_config::map_proxy_request_model`，request source 负责把 route/body issue 投影为 `ProxyError::InvalidRequest` 文案，边界测试防止 forward mapping 重新形成 adapter 到 host config 的反向依赖。
1156. 通用 passthrough 响应 usage 的运行态 source 选择已迁入 `proxy/engine/response_pipeline.rs`，调用方和 adapter 单测均直接引用 owning module；host `response_processor` 不再直接读取 `ProxyState.config` 或拼装 `StreamingUsageCollectorContext` / `NonStreamingUsageRecordContext`，只负责传递 transport body、status 和 parser config。
1157. 非流式 upstream response 的整包读取、body timeout 包装和 decode bridge 已迁入 `proxy/engine/response_pipeline.rs`，调用方直接从 response pipeline import `read_decoded_proxy_response_body`；host `handlers` 与 `response_processor` 不再直接调用 `ProxyResponse::bytes()` 或维护 `read_decoded_body` 兼容函数。
1158. Claude/Codex transformed streaming 的 usage collector 选择、timeout config 和 logged passthrough wrapper 已迁入 `proxy/engine/response_pipeline.rs`，调用方和 adapter 单测不再经 `proxy_core_adapter` re-export；host `handlers` 只生成协议转换后的 SSE stream 并交给 response adapter。
1159. 通用 passthrough streaming 的 usage collector 选择、timeout config 和 logged passthrough wrapper 已迁入 `proxy/engine/response_pipeline.rs`，`proxy_core_adapter` 不再保留 test-only response pipeline helper re-export；host `response_processor` 只保留 upstream `bytes_stream` 提取、headers/status 保存和 Axum response bridge。
1160. ChannelSource 的 spec/list/get 与 materialized-vs-legacy fallback helper 已从 `proxy_core_adapter.rs` 迁入 `proxy/host/cc_switch/database_channel_source.rs`：host source 模块现在拥有 DB channel 查询、legacy projection 回退、`ChannelSpec` 投影和 `ChannelQuery` 过滤，对应 channel conversion/source fallback fixture 也已迁到 owning module tests，并直接引用 core domain/routing/management/port/result contract；`proxy_core_adapter` 不再提供 channel source 专用 DTO/helper facade。
1161. ChannelSource 的 record CRUD/list、key/model 管理和 migration preview/materialize DB helper 已继续迁入 `proxy/host/cc_switch/database_channel_source.rs`：该模块现在完整拥有 DB-backed `ChannelSource` 管理面、record/key/model DTO 投影和 DB write/read 错误映射，`proxy_core_adapter` 只保留旧 HTTP/response 层仍需的少量兼容别名。
1162. DB-backed channel-key runtime source 已迁入 `proxy/host/cc_switch/channel_key_runtime_source.rs`：该 host 模块拥有 raw key record 查询、runtime candidate 投影、enabled-key core selection 和 `ChannelKeyRuntimeSource` 实现，并直接引用 core `ChannelKeyRuntimeSource` / `ProxyCoreResult` contract；`proxy_core_adapter` 不再 re-export `ChannelKeyRuntimeSource` port trait，只 re-export 默认 source factory/type。
1163. DB-backed channel reachability probe 已迁入 `proxy/host/cc_switch/channel_reachability_probe.rs`：该 host 模块拥有 app type parse、provider/config DB 读取、`StreamCheckService` 副作用和 core reachability 投影，并直接引用 core `ChannelReachabilityProbe` / probe request/result / `ProxyCoreResult` contract；`proxy_core_adapter` 只 re-export probe 类型并继续在 `CcSwitchProxyServices` 中装配，不再 re-export probe port 或 probe DTO。
...
1165a. `CcSwitchAuthProvider` 的 provider-config 与 route-context auth info 行为测试已从 `proxy_core_adapter` 迁到 owning `proxy/host/cc_switch/auth_provider.rs`；边界测试继续约束这两个 auth-provider fixture 不再回流到 adapter。
1166. `CcSwitchManagedAccountRuntimeSource`、`ManagedAccountRuntimeSource` extension trait、Copilot dynamic endpoint/live model/model vendor runtime lookup 和 Copilot/Codex OAuth token AppHandle 读取已迁入 `proxy/host/cc_switch/managed_account_runtime_source.rs`：该 host 模块拥有 Tauri state 读取与 token runtime 副作用，`proxy_core_adapter` 只 re-export runtime source/type/test helper。
1167. `ForwarderAdapterContext`、provider adapter trait object、provider auth info/header fallback 调用、base URL facts、transform gate/action 和 upstream URL assembly 已迁入 `proxy/host/cc_switch/provider_adapter_context.rs`：该 host 模块拥有 CC Switch provider adapter registry/trait 调用，forward pipeline/request source 直接引用 host context，`proxy_core_adapter` 只在自身 runtime 装配中私有消费 context/factory，剩余鉴权迁移焦点转向 token 缓存/刷新、channel-key 轮询/随机和失败回退行为。
1168. `ForwarderAuthSource`、ref type 与 auth headers input 已迁入 `proxy/engine/forward_pipeline.rs`，由 forward pipeline 拥有上游 auth 端口 contract；`CcSwitchForwarderAuthSource` 默认实现保留在 `proxy/host/cc_switch/forwarder_auth_source.rs`，负责上游 auth header finalization、AuthProvider 显式 header/fallback 分支、managed-account auth resolution 和 Copilot auth optimization 准备；`proxy_core_adapter` 仅保留 host Provider/error 投影，并在 runtime 装配和 adapter-local 测试中消费 owning module contract，不再拥有该 source trait/input/type alias。
1169. `ForwarderTransportSource`、`ForwarderUpstreamTransportRequest` 与 `ForwarderTransportSourceRef` 已迁入 `proxy/engine/forward_pipeline.rs`：forward pipeline 拥有上游发送执行端口契约，`CcSwitchForwarderTransportSource` 作为默认 host 实现保留在 `proxy/host/cc_switch/forwarder_transport_source.rs` 并只委托 `proxy::transport::upstream::send_request`，`proxy_core_adapter` 不再作为该 contract 的中转出口。
1170. `ForwarderResponseSource`、`ForwarderChannelResponseStatusInput`、`ForwarderResponseFinalizationInput` 与 ref type 已迁入 `proxy/engine/forward_pipeline.rs`：forward pipeline 拥有上游响应 finalization/status mapping 端口契约，`CcSwitchForwarderResponseSource` 默认实现保留在 `proxy/host/cc_switch/forwarder_response_source.rs` 并继续承接响应成功就绪、流式首包预读、非成功 upstream error 投影和 channel response status mapping，`proxy_core_adapter` 不再作为该 contract 的中转出口。
1171. `ForwarderProtocolStateSource`、ref type 与 protocol input DTO 已迁入 `proxy/engine/forward_pipeline.rs`，由 forward pipeline 拥有协议状态端口 contract；`CcSwitchForwarderProtocolStateSource` 默认实现保留在 `proxy/host/cc_switch/forwarder_protocol_state_source.rs`，负责 Gemini shadow store、Codex Chat history store、Codex Chat request enrich 和 Claude/Gemini transform replay 的默认协议状态副作用；`proxy_core_adapter` 仅在 runtime 装配和 adapter-local 测试中消费 owning module contract，不再拥有该 trait/input/type alias。
1172. `ForwarderAttemptRuntimeSource`、ref type、allow/success/failure/neutral-release input DTO 与 allow decision 已迁入 `proxy/engine/forward_pipeline.rs`，由 forward pipeline 拥有 attempt runtime 端口 contract；`CcSwitchForwarderAttemptRuntimeSource` 默认实现保留在 `proxy/host/cc_switch/forwarder_attempt_runtime_source.rs`，负责 ProviderRouter-backed attempt allow、max-attempt/circuit-bypass 默认兼容决策、成功/失败健康记录和 neutral permit 释放；`proxy_core_adapter` 不再保留该 source trait/input/type alias 或 ProviderRouter permit/health 副作用 helper。
1173. `ForwarderRuntimeStateSource`、ref type、failure decision enums 与 `ActiveConnectionGuard` 已迁入 `proxy/engine/forward_pipeline.rs`，由 forward pipeline 拥有运行态状态端口 contract；`CcSwitchForwarderRuntimeStateSource` 默认实现保留在 `proxy/host/cc_switch/forwarder_runtime_state_source.rs`，负责 `ProxyRuntimeStatus`、current route target map、`ProxyEventBus`、request/attempt event、active connection 计数、forward success/failure 状态和 rectifier/terminal failure 日志副作用；`proxy_core_adapter` 仅在 runtime 装配和 adapter-local 测试中消费 ref type/decision enum，不再拥有该 contract。
1174. `ForwarderRequestSource`、ref type、请求准备/parts/log input/output DTO 与 rectifier/optimizer request-source contract 已迁入 `proxy/engine/forward_pipeline.rs`，由 forward pipeline 拥有上游请求组装端口；`CcSwitchForwarderRequestSource` 默认实现保留在 `proxy/host/cc_switch/forwarder_request_source.rs`，负责请求体预处理、provider transform/Codex Responses→Chat 转换、Copilot request optimizer、upstream URL planning、media fallback/rectifier plan、上游 body/header 组装和请求日志副作用；`proxy_core_adapter` 仅在 adapter-local 测试中消费 owning module contract，不再保留 request-source trait/input/type alias 或 re-export factory/test helper。
1175. `FailoverSwitchScheduler`、`FailoverSwitchSchedulerRef` 与 `ForwarderFailoverSwitchTarget` 已迁入 `proxy/engine/forward_pipeline.rs`：forward pipeline 拥有成功路径切换调度端口契约，`CcSwitchFailoverSwitchScheduler` 默认实现保留在 `proxy/host/cc_switch/failover_switch.rs`，继续拥有 `FailoverSwitchManager`、Tauri AppHandle、托盘/UI 更新和 Live hot-switch 副作用；`proxy_core_adapter` 不再作为该 scheduler contract 的中转出口，noop 测试 source 也由 owning host failover 模块提供。
1176. `CcSwitchEventSink` 已迁入 `proxy/host/cc_switch/event_sink.rs`：该 host 模块拥有 core event 到 `ProxyEventBus` 的分发副作用，并直接引用 core `ProxyEventSink` / `ProxyCoreEvent` contract；`ProxyEventBus::emit_core_event` 统一负责 event name/payload envelope 投影，`proxy_core_adapter` 不再 re-export `ProxyEventSink`，也不再保留 event bus 投影/分发 helper 或默认 event sink source re-export。
1177. `CcSwitchUsageSink` 已迁入 `proxy/host/cc_switch/database_usage_sink.rs`：该 host 模块拥有 `UsageLogger`、pricing lookup/model source 选择、`UsageRecord` 到 `RequestLog` 投影、缺失定价 warning、`RequestLog` 写入和 DB 持久化副作用，并直接引用 core `UsageSink` / `ProxyCoreResult` / usage DTO contract；`proxy_core_adapter` 不再 re-export `UsageSink`，也不再保留 request-log projection helper。
...
1180. `CcSwitchClaudeDesktopGatewayAuthSource` 已迁入 `proxy/host/cc_switch/claude_desktop_gateway_auth_source.rs`：该 host 模块拥有默认 gateway token source 的 `ClaudeDesktopGatewayAuthSource` 实现、gateway token setting key、DB read/write 和 token 生成 helper，并直接引用 core gateway auth contract；`proxy_core_adapter` 不再 re-export `ClaudeDesktopGatewayAuthSource`，也不再保留 token DB helper。
1181. `CcSwitchProviderRouterChannelSource` 已迁入 `proxy/host/cc_switch/provider_router_channel_source.rs`：该 host 模块拥有 ProviderRouter 的 channel route input source，直接引用 core `ChannelSource`、`ChannelRouteSource`、`RouteResolveChannelInput` 与 record-to-route-input helper，包装 `CcSwitchChannelSource` 并在 host 模块内完成 route-input projection；`proxy_core_adapter` 不再保留这组 channel route input helper，也不再作为该 source 的 route DTO facade。
1182. `CcSwitchProviderRouterConfigSource` 已迁入 `proxy/host/cc_switch/provider_router_config_source.rs`：该 host 模块拥有 ProviderRouter 的 auto-failover、circuit breaker config 和 failure threshold source，直接引用 host `CcSwitchConfigSource`、core `ProxyConfigSource`、`CircuitBreakerConfig` 与 provider selection contract，并在 host 模块内完成 app config raw 解析与缺省事实投影；`proxy_core_adapter` 不再保留这组 router config source helper，也不再 re-export `CcSwitchConfigSource` host type 给该模块。
1183. `CcSwitchProviderRouterHealthStore` 已迁入 `proxy/host/cc_switch/provider_router_health_store.rs`：该 host 模块拥有 ProviderRouter 的 provider/channel health write 和 channel reset side effect，并在 owning module 内完成 provider health DB update projection、router-facing channel health wrapper 和 channel reset wrapper；`proxy_core_adapter` 只保留 `CcSwitchChannelHealthStore` 仍需复用的 channel health/reset/stats projection helper。
1184. `CcSwitchProviderRouterProviderSource` 已迁入 `proxy/host/cc_switch/provider_router_provider_source.rs`：该 host 模块拥有 ProviderRouter 的 provider id/current provider/failover source，直接引用 host `CcSwitchRoutePolicySource` 与 core `AppKind` / `ProviderSpec` / `ProviderSource` / `RoutePolicySource` / `ProviderSelectionInput` contract，并在 host 模块内完成 provider ids、current provider ids、failover queue ids 与 failover lookup 投影；`proxy_core_adapter` 不再为该模块 re-export `ProviderSource` 或 `CcSwitchRoutePolicySource`，也不再保留这组 ProviderRouter provider source helper。
1185. `CcSwitchRoutePolicySource` 已迁入 `proxy/host/cc_switch/route_policy_source.rs`：该 host 模块拥有 failover queue 到 `RoutePolicySource` 的默认 DB-backed source，直接引用 core `AppKind` / `ProxyCoreResult` / `RoutePolicySource` / `RoutePolicy` contract，并继续复用 adapter 的 queue-to-policy projection helper；`proxy_core_adapter` 不再 re-export `RoutePolicySource` port trait 或 `CcSwitchRoutePolicySource` host type，只保留 helper，默认装配由 host services / ProviderRouter sources 直接引用 host 模块。
1186. `CcSwitchRouteResolver` 已迁入 `proxy/host/cc_switch/route_resolver.rs`：该 host 模块拥有默认 `RouteResolver` 实现，直接导入 core route contract 并调用 `proxy-core::build_route_plan`，同时包装 `ProviderRouter` 复用 adapter 的 management dry-run projection helper；`proxy_core_adapter` 不再保留 `route_plan_from_request`、`RouteRequest` 或 `RouteResolver` facade，只保留 management helper 和默认 resolver re-export/装配。`RouteRequest` 只作为 resolver 内部 import 使用，不再经 host resolver re-export 给 `proxy_core_host` 测试兼容层。
1187. `CcSwitchChannelHealthStore` 已迁入 `proxy/host/cc_switch/channel_health_store.rs`：该 host 模块拥有默认 `ChannelHealthStore` 实现，包装 DB 与 `ProviderRouter`，并在 owning module 内完成 channel attempt DB update、reset app lookup/router reset 和 breaker stats projection；`proxy_core_adapter` 不再保留 channel health store helper 或默认 store re-export/装配。
1188. `CcSwitchProviderSource` 已迁入 `proxy/host/cc_switch/provider_source.rs`：该 host 模块拥有默认 `ProviderSource` 实现，直接引用 core `AppKind` / `ProviderSpec` / `ProviderSource` / `CurrentRouteTarget` / `ProxyCoreResult` contract，包装 DB、`ProviderRouter` 与 active-route runtime map，并复用 host-owned `provider_projection.rs` 的 provider list/get 投影；`proxy_core_adapter` 不再 re-export `ProviderSource` port trait，也不再保留该 source 的 projection helper 或默认 source type re-export/装配。
...
1190. `CcSwitchConfigSource` 已迁入 `proxy/host/cc_switch/config_source.rs`：该 host 模块拥有默认 `ProxyConfigSource` 实现，直接引用 core `AppKind` / `ProxyCoreResult` / `ProxyConfigSource` / `AppSummaryConfig` / config DTO contract，包装 DB/settings app catalog、global/app/app-summary/runtime config 读取，并直接调用 core config builder；`proxy_core_adapter` 不再 re-export `ProxyConfigSource`、`AppSummaryConfig` 或 `CcSwitchConfigSource` host type，也不再保留 config-source-only DB helper。
...
1192. `CcSwitchForwarderRequestSource` 的请求准备、上游 URL planning、媒体/rectifier plan、Codex Responses->Chat 转换、请求 body/header 构造与 transport policy helper 已改为直接引用 `proxy_core::api::{transport,transforms}`；request-source trait/input/output contract 已由 `proxy/engine/forward_pipeline.rs` 拥有，`proxy_core_adapter` 不再为这些纯 request helper/type、source contract 或 host request source factory 提供生产 re-export，仅保留 adapter-local 测试私有导入。
1193. `CcSwitchForwarderResponseSource` 的响应 status mapping、非流式 body timeout 和流式首包 priming 错误文案 helper 已改为直接引用 `proxy_core::api::transport`；`proxy_core_adapter` 不再为这些纯 response helper 提供生产 re-export，response source 通过 `proxy/engine/forward_pipeline.rs` 拥有的 trait/input/type alias 与 forwarder 交互。
1194. `CcSwitchForwarderAuthSource` 的 route-context auth channel/request 构造、Copilot auth override 准备、AuthInfo header resolution 和上游 auth header finalization 已改为直接引用 `proxy_core::api::{domain,routing,transport}`；`proxy_core_adapter` 继续保留 host Provider/error 投影，但不再为这些纯 auth helper/type 提供生产 re-export。
1195. `proxy/transport/upstream` 的 SOCKS 判定、上游 send policy、raw-hyper URL parse 错误文案和 reqwest streaming header timeout 文案已改为直接引用 `proxy_core::api::transport`；`proxy_core_adapter` 不再为这些纯 upstream transport helper/type 提供生产 re-export，`ForwarderUpstreamTransportRequest` 也已改由 `proxy/engine/forward_pipeline.rs` 直接提供给 upstream transport 模块。
1196. `CcSwitchChannelKeyRuntimeSource` 的 runtime candidate 投影、key-ref selection、test-only enabled-key selection、core port trait 和 result contract 已改为直接引用 `proxy_core::api::{management,ports,errors}`；`proxy_core_adapter` 不再为这些 channel-key runtime contract/helper/input 提供生产 re-export，仅保留默认 source factory/type 和 DAO 测试便利函数仍需的 test-only candidate alias。
1197. `channel_auth_profile_attempts` 的 auth-profile provider application plan、missing channel-key error、`ChannelKeyRuntimeSource` port trait 与 `ProxyCoreResult` 已改为直接引用 `proxy_core::api::{domain,auth,ports,errors}`；`proxy_core_adapter` 不再为这些 auth-profile rule/port/result helper/type 提供生产 re-export，host source 继续负责具体 `Provider` mutation 与 channel-key 值注入。
1198. `CcSwitchAuthProvider` 的 route-context/profile auth info helper、core `AuthProvider` port/type 和 channel-key settings policy 已改为直接引用 `proxy_core::api::{ports,domain,auth}`；`proxy_core_adapter` 不再为这些纯 auth provider helper 提供生产 re-export，仅继续 re-export host provider/helper。
1199. `ForwarderAdapterContext::provider_url_facts` 的 provider URL facts projection helper/input 已改为直接引用 `proxy_core::api::transport`；`proxy_core_adapter` 不再为这些纯 URL facts helper/input 提供生产 re-export，`ForwarderProviderUrlFacts` type alias 暂留给 forward pipeline 兼容边界。
1200. `CcSwitchManagedAccountRuntimeSource` 的 token runtime diagnostics、managed auth resolution、Copilot dynamic endpoint/live model/model vendor helper、runtime source trait/type 和 `CopilotModel` 已改为直接引用 `proxy_core::api::{auth,model_catalog}`；`proxy_core_adapter` 不再为这些 runtime-source helper/type 提供生产 re-export，仍保留 host `ProviderMeta` 到 managed-account binding 的投影。
1201. `CcSwitchForwarderAttemptRuntimeSource` 的 max-attempt/circuit-bypass 决策 helper/input 已改为直接引用 `proxy_core::api::transport`，attempt runtime source contract 已由 `proxy/engine/forward_pipeline.rs` 拥有；`proxy_core_adapter` 不再为这些纯 attempt runtime decision helper/input、source trait/input 或 ProviderRouter permit/health 副作用 helper 提供生产 re-export。
1202. `proxy_core_adapter` 已停止对 `ForwarderAdapterContext` 和 provider adapter context factory 做 host re-export：forward pipeline 与 host request source 直接引用 `proxy/host/cc_switch/provider_adapter_context.rs`，adapter 仅在自身组装逻辑中私有消费该 host context。
...
1204. forwarder request/transport/response/failover/protocol/attempt/auth/runtime source 的默认 factory 与测试 helper 已从 adapter re-export 收口为私有 use 或直接 host 模块引用：request/transport/response/failover scheduler/protocol state/attempt runtime/auth source/runtime state contract 已归入 `proxy/engine/forward_pipeline.rs`，`proxy_core_adapter` 不再暴露这些 source contract 或 cc-switch host source factory。
...
1217. `proxy_core_adapter` 的 test-only `proxy_takeover_status_from_parts` re-export 已删除；adapter 单测直接调用 `proxy_core::api::ports` owning helper；后续 `proxy_takeover_status_from_enabled_options` 也已收窄为 adapter 内部私有 core 调用，不再作为 adapter 出口。
...
1232. proxy channel DAO 的 test-only `get_enabled_proxy_channel_key` convenience selector 已删除，`proxy_core_adapter` 的 test-only `ChannelKeyRuntimeCandidate` alias 同步删除；DAO 测试改为检查 raw persistence record，enabled/runtime key selection 只保留在 `CcSwitchChannelKeyRuntimeSource` 和 proxy-core selector，边界测试防止 DAO 重新承载 key-pool 选择逻辑。
1233. forward failure 到 channel-key failure timestamp 的写回闭环已接入 host runtime：channel-key auth profile 会把实际选中的 `key_ref` 保存在 `ForwardAttempt`，`CcSwitchForwarderAttemptRuntimeSource::record_failure` 在记录 channel/provider runtime failure 后把对应 `(channel_id,key_ref)` 的 `proxy_channel_keys.last_failure_at` 写回 DB；普通 provider/channel attempt 不写 key failure，wildcard key 失败后下一次 runtime selection 会回退到未失败 key。剩余 channel auth 迁移重点转为 managed-account token cache/refresh 语义收口。
1234. channel-key failure cooldown 语义已收敛到 proxy-core selector：新增默认 60 秒冷却窗口和 `select_channel_key_runtime_candidate_with_failure_cooldown`，CC Switch DB-backed source 只注入当前时间并继续加载候选集合；近期失败的 wildcard 高优先级 key 会临时降级，超过窗口后重新进入 priority/weight/key_ref 竞争，显式 key ref 仍按指定 channel/key 隔离选择。后续 key runtime 策略扩展应继续复用同一 core policy 入口。
...
1239. per-channel health policy 已继续接入 channel 熔断器 runtime 配置：ProviderRouter 现在从 app 级 `CircuitBreakerConfig` 出发，通过 channel source 读取当前 channel `healthPolicy.failureThreshold`/`failure_threshold`，并用 core helper 得到 channel 专属 failure threshold；`allow_channel_request` 与 `record_channel_result` 会用该配置创建或热更新 channel circuit breaker，provider 级 breaker 配置保持原有 app 配置。DB-backed 回归测试覆盖 failureThreshold=2 时第一次失败仍保持 Closed/degraded，第二次失败才 Open/unhealthy。
1240. channel-key failure cooldown 已支持 per-channel health policy：core 新增 `effective_channel_key_failure_cooldown_ms`，从 `healthPolicy.keyFailureCooldownMs`/`channelKeyFailureCooldownMs`/`key_failure_cooldown_ms` 解析正数毫秒值；CC Switch DB-backed `ChannelKeyRuntimeSource` 选择 key 前读取当前 channel policy，把默认 60 秒冷却窗口替换为 channel 专属窗口后再调用 core weighted selector。这样同一组 key pool 可以按每个中转地址独立配置失败回退恢复速度，不需要新增 schema 或让 DAO 解析策略 JSON。
1241. channel auto-ban 已开始进入 channel health policy：core 新增 `channel_status_after_health_attempt` / `channel_status_after_health_reset`，从 `healthPolicy.autoDisable`/`auto_disable`/`autoBan`/`auto_ban` 解析显式布尔开关；CC Switch DAO 直接消费 `proxy-core::api::routing` 的状态规则，在 channel health 达到 unhealthy 阈值后只把当前 `enabled` channel 改为 `auto_disabled`，reset 只把 `auto_disabled` 恢复为 `enabled`，不会覆盖手动禁用。ProviderRouter DB-backed 回归测试覆盖 auto-disabled channel 会在 dry-run 中以 `status:auto_disabled` 被拒绝，reset 后恢复可路由。
...
1248. 全局代理显式 URL 校验和系统代理 loopback 判定的 pure helper 已继续从 `proxy_core_adapter` facade 移除：host-owned `global_http_client` 直接引用 `proxy-core::api::transport::{validate_explicit_proxy_url,invalid_explicit_proxy_url_message,proxy_values_point_to_loopback_port,SYSTEM_PROXY_ENV_KEYS}` 与 `proxy-core::api::ports::DEFAULT_PROXY_LISTEN_PORT`；adapter 不再 re-export 这些全局代理 URL 校验常量/函数，也不再承载 masking/loopback/显式 URL 校验 contract fixture，相关测试由 `proxy-core` owning modules 维护。
...
1249a. managed-account token 失败回退策略已开始收敛到 core policy + host runtime source：`proxy-core::managed_account_auth` 维护 30 秒最近成功 token 快照回退窗口和诊断文案，`CcSwitchManagedAccountRuntimeSource` 按 runtime/account 隔离保存 Copilot/Codex 最近成功 token，并且只对宿主判定为可重试的刷新/网络类失败启用回退；账号不存在、refresh token 失效等不可重试错误仍直接暴露，避免不同中转地址或账号串用旧 token。
1249b. channel-key runtime selection 已把 `random` 从 weighted alias 提升为独立策略：`proxy-core::ports::ChannelKeyRuntimeSelectionStrategy::Random` 在健康/最高优先级候选层内按候选数量等概率选择，不消费 key weight；`weightedRandom` 继续走按权重分配，`roundRobin` 继续走 cursor offset。CC Switch DB source 仍只读取 channel policy 并注入 runtime roll，不在 DAO/forwarder 里实现随机 key 选择。
...
1251. custom endpoint URL policy facade 已从 `proxy_core_adapter` 移除：`services/provider/endpoints.rs` 作为受控 direct-core 消费者直接引用 `proxy-core::api::management::{normalize_custom_endpoint_url,custom_endpoint_url_key,custom_endpoint_url_issue_spec}` 处理新增校验、删除 key、last-used lookup key 与空 URL 本地化错误映射；provider meta 列表排序和 last-used mutation 继续通过 `Provider::{custom_endpoint_list,mark_custom_endpoint_last_used}`，避免服务层重新持有 host Provider 投影细节。
1252. usage placeholder pricing model 判定已从 `proxy_core_adapter` facade 移除：`services/usage_stats.rs` 在既有 `proxy-core::api::usage` direct-core 边界内直接引用 `is_placeholder_pricing_model`，`RequestContext` route update 直接引用 `usage_route_context_from_selection`；边界测试防止 pure usage predicate 或 route context helper 再经 adapter re-export。
1253. adapter 内部 wrapper 的 pure core delegate 出口继续收窄：`forward_failure_kind_from_proxy_status`、`normalize_claude_anthropic_messages`、`apply_provider_model_mapping` 不再通过 `proxy_core_adapter` re-export；forward failure 投影由 error mapper 持有，Claude normalization 与 model mapping 由 request source 直接调用 core helper；边界测试防止为了实现便利重新暴露这些纯 delegate。
1254. test-only pure helper facade 继续移除：已批准 direct-core 的 `forward_pipeline` / `response_pipeline` 测试直接引用 `proxy-core` 的 Gemini native URL 构造/解析、streaming request 判定、response decompression、SSE field parsing helper，adapter 自测用 fully-qualified core 调用 `inject_openai_stream_include_usage`；`proxy_core_adapter` 不再保留这些 `#[cfg(test)]` re-export，尚未批准 direct-core 的 `provider/codex.rs` user-agent 测试 helper 暂不扩边界。
...
1256. `route_plan_provider_ids` 的 test-only adapter re-export 已删除：adapter 自测直接调用 `proxy-core::api::routing::route_plan_provider_ids`，边界测试防止为测试便利重新把纯 routing helper 暴露在 adapter；生产侧 `route_candidate_provider_ids_from_router_source` 已迁入 host-owned `CcSwitchProviderSource`，在 owning module 内保留 host `ProviderRouter`/`AppError` 投影 wrapper。
...
1262. Codex Responses->Chat gate 的 test-only transport helper facade 已删除：adapter 自测直接调用 `proxy-core::api::transport::{resolve_codex_provider_uses_chat_completions,should_convert_codex_responses_endpoint_to_chat}`，生产 adapter 继续只保留 host provider facts 投影并通过私有 `core_codex_*` helper 委托，`codex_provider_uses_chat_completions` 与 `codex_responses_to_chat_conversion_required` 不再经 `proxy_core_adapter` re-export；边界测试防止这些 pure gate helper 重新暴露。
...
1265. Claude takeover model/display helper 的 test-only adapter re-export 已删除：adapter 自测直接调用 `proxy-core::api::model_catalog` 与 `proxy-core::api::ports::apply_claude_takeover_fields_with_policy_and_models` owning helper；live takeover 生产路径直接引用 `proxy-core::ports::{apply_claude_takeover_fields_with_policy,ClaudeTakeoverAuthPolicy,apply_claude_takeover_fields_for_provider_facts,ClaudeTakeoverProviderFacts}` 并本地投影 `Provider` facts，`proxy_core_adapter` 不再保留 Claude takeover provider facts wrapper；边界测试防止 pure model/policy/helper 重新暴露。
...
1275. Codex 官方客户端 User-Agent 检测的 test-only transport helper facade 已删除：User-Agent fixture 由 `proxy-core::request_headers` owning tests 覆盖，provider 测试不再绕过 host/core 边界；边界测试防止该 pure transport helper 重新通过 `proxy_core_adapter` re-export 或测试壳回流。
1275a. Claude/Codex/Gemini live takeover placeholder mutation 的 adapter fixture 已删除：正反例覆盖迁入 `proxy-core::ports::takeover_placeholder_mutations_update_app_specific_live_config`，边界测试防止 app-specific placeholder JSON mutation fixture 回流到 `proxy_core_adapter`。
1275b. Claude default model normalization 的 adapter fixture 已删除：`ANTHROPIC_MODEL`/`ANTHROPIC_SMALL_FAST_MODEL` 到 `ANTHROPIC_DEFAULT_*` 的回填、旧 key 清理和非 Claude no-op 覆盖由 `proxy-core::ports` owning tests 维护，边界测试防止该 pure ports fixture 回流到 adapter。
...
1277. Claude message normalization 的 test-only transform helper facade 已删除，后续生产 `provider_claude_normalize_anthropic_messages` wrapper 也已删除：request source 直接从 `proxy-core::api::transforms` 引用 pure helper，Claude provider 测试通过 request source 的 test helper 验证 host-shaped 行为；边界测试防止这组三个 normalization helper 或 host-shaped normalization façade 重新 re-export。
...
1284. hot-switch takeover 的三项 pure policy facade 已从 `proxy_core_adapter` 删除：`live_takeover.rs` 与 `services/provider/mod.rs` 直接引用 `proxy-core::api::ports::{proxy_hot_switch_should_refresh_codex_live_from_backup,proxy_hot_switch_should_sync_codex_live_while_proxy_active,proxy_hot_switch_should_sync_claude_live_while_proxy_active}` 并在调用点把 `AppType` 投影为 `AppKind`；adapter 不再持有只做 AppType/AppKind 转换的一跳 wrapper，边界测试防止这些 hot-switch policy facade 回流。
1285. live takeover 的 live placeholder 检测、backup snapshot 策略和当前 proxy 匹配投影已从 `proxy_core_adapter` 收回 host service：`live_takeover.rs` 本地 helper 负责 Codex `config.toml` bearer token/base_url fact 提取，再调用 `proxy-core::api::ports` 的中立策略；adapter 不再暴露 `live_config_has_proxy_placeholder_for_app`、`live_backup_snapshot_from_live_config` 或 `live_takeover_config_matches_proxy_for_app` wrapper，边界测试改为验证 host ownership 与 adapter facade 禁止项。
1284. router current-provider 与 DB channel record 投影的 crate-visible test-only adapter facade 已删除：`select_current_provider_ids_from_router_source` 降为 adapter 自测私有 helper，channel conversion 由 host-owned `provider_router_channel_source.rs` 生产 helper 承接；边界测试防止这类 test helper 重新出现在 adapter 顶层。
1285. MiMo thinking normalization 与 Claude takeover model-field 的 crate-visible test-only adapter facade 已删除：MiMo gate fixture 已迁到 `proxy-core::response_transform` owning tests，Claude takeover model-field fixture 直接调用 `proxy-core::api::ports::claude_takeover_model_fields_from_settings`；`proxy_core_adapter` 不再为这两类纯 core 策略提供测试用二次出口，边界测试防止对应 helper 或 fixture 回到 adapter。
1286. Codex 官方 provider live reapply 判定已从 `proxy_core_adapter` 收回 ProviderService：`services/provider/mod.rs` 本地 helper 直接把 `Provider.category` 投影给 `proxy-core::api::ports::should_reapply_codex_official_live_for_provider_category`；adapter 不再暴露 `should_reapply_codex_official_live_for_provider` Provider-shaped wrapper，边界测试防止该 facade 回流。
1287. Claude provider 测试用 thinking placeholder 常量不再经 `proxy_core_adapter` 二次出口：provider 测试只校验既有输出契约，adapter 删除 `anthropic_tool_thinking_placeholder` / `anthropic_redacted_thinking_placeholder` test-only facade，core 常量继续由 `proxy-core` 自测维护，边界测试防止这类纯 core 常量 wrapper 回流。
1288. Circuit breaker 日志码不再经 `proxy_core_adapter::circuit_breaker_log_codes` 二次出口：`proxy/circuit_breaker.rs` 直接引用 `proxy-core::api::logging::cb`，adapter 删除 CB 日志码 re-export，边界测试防止日志码常量重新通过 adapter 回流。
1289. HTTP server 日志码不再经 `proxy_core_adapter::server_log_codes` 二次出口：`proxy/transport/http/server.rs` 直接引用 `proxy-core::api::logging::srv`，adapter 删除 SRV 日志码 re-export，边界测试防止服务器日志码常量重新通过 adapter 回流。
1290. channel health 默认失败阈值不再经 `proxy_core_adapter::DEFAULT_CHANNEL_HEALTH_FAILURE_THRESHOLD` 二次出口：adapter 内部 channel attempt DB projection 直接引用 `proxy-core::api::ports::DEFAULT_CHANNEL_HEALTH_FAILURE_THRESHOLD`，边界测试防止该纯 core 常量 re-export 回流。
1291. 外部中转宿主示例的 current-route target 构造入口已补入 `proxy-core::api::prelude`：`external_relay_host` 不再额外导入 `api::ports`，边界测试禁止 public example/prelude smoke 通过 `api::ports` 旁路集成面，确保独立中转宿主只依赖稳定 prelude。
1292. failover switch 的 noop 测试 scheduler 已从 `proxy_core_adapter` 移到 `proxy/host/cc_switch/failover_switch.rs`：forward pipeline 与 proxy-core host 测试 harness 直接使用 owning host 模块的 test source，边界测试防止 adapter 再承载该 test-only helper。
1286. host-owned `forwarder_request_source` 已解除 `proxy_core_adapter::*` wildcard：request source 直接引用 `proxy-core::api::{auth,config,domain,model_catalog,transport}` 的请求预处理、media fallback、rectifier、header/auth guard 和模型归一化 helper，并从 `proxy/engine/forward_pipeline.rs` 消费 request-source contract；adapter 仅保留仍需的 provider projection/helper；同时删除不再需要的 adapter re-export/test facade，边界测试防止 wildcard、source contract 和纯 request helper 经 adapter 回流。
...
1288. provider adapter trait 与 Claude/Codex/Gemini provider adapters 已停止通过 `proxy_core_adapter` 获取 provider auth DTO/strategy：`ProviderAuthInfo`/`ProviderAuthStrategy` 直接来自 `proxy-core::api::auth`；Codex/Gemini 的 host-shaped auth extraction/header helper 仍由 adapter 承接，Claude auth info/header helper 已迁回 `proxy/provider/claude.rs` owning module；边界测试防止 provider adapter auth contract alias 与 Claude auth façade 回流。
...
1297. `host/cc_switch/forwarder_runtime_state_source` 不再通过 `proxy_core_adapter` 获取事件/运行态 DTO、低层状态 helper 或 forward failure 投影：`AttemptEventPhase`/`ProxyCoreEvent` 直接来自 `proxy-core::api::events`，`CurrentRouteTarget`、`ProxyRuntimeStatus` 和 `record_forward_*_status` 直接来自 `proxy-core::api::ports`，`ProxyError -> ForwardFailureKind` 投影来自 `proxy::error_mapper`；边界测试防止 runtime state source contracts 经 adapter alias 回流。
...
1301. `database/dao/proxy` 不再通过 `proxy_core_adapter` 获取 proxy 配置 DAO contract：`AppProxyConfig`、`CircuitBreakerConfig` 与 `app_proxy_config_defaults_for_app` 直接来自 `proxy-core::api::config`，`ProxyConfig` 直接来自 `proxy-core::api::ports`；adapter 仍保留需要宿主投影的校验和 provider health helper，边界测试防止 DAO 重新通过 adapter alias 获取这些纯 DTO/helper。
...
1304. `proxy/provider/mod` 不再通过 `proxy_core_adapter` 获取 provider adapter kind contract：`AppProviderAdapterKind`、`AppKind` 与 `provider_adapter_kind_for_app` 直接来自 `proxy-core::api::domain`，`provider_adapter_kind_for_app_type` 宿主 AppType 转换 helper 已删除；边界测试防止 provider adapter registry 重新经 adapter alias 读取纯 domain contract。
1305. `proxy_core_adapter` 不再暴露 `ProviderKind` 纯 domain alias：adapter 与 `proxy_core_host` 测试壳均直接 import `proxy-core::api::domain::ProviderKind`，外部仍通过具体 provider-kind projection/helper 获取宿主兼容结果；边界测试防止 `ProviderKind` 重新以 adapter type alias 或 host test import 回流。
...
1310. `proxy/engine/forward_pipeline` 与 forwarder runtime state source 不再通过 `proxy_core_adapter` 获取 `ForwarderRectifierRetryKind`：两者直接从 `proxy-core::api::transport` 读取 rectifier retry kind，adapter 内部也改为普通 core import；边界测试防止 rectifier retry kind 重新以 adapter alias 回流。
1311. `proxy_core_adapter` 不再暴露或内部消费 `ForwardFailureKind` 纯 transport alias：forward failure 类型由 `proxy::error_mapper` 与 forward runtime state source 直接从 `proxy-core::api::transport` 引用；边界测试防止 forward failure kind 重新以 adapter type alias 回流。
...
1318. `proxy_core_adapter` 不再暴露 response transport contract alias：`ProxyCoreResponse`、`ProxyResult` 与 `ProxyResponseBody` 均改为 adapter 内部普通 `proxy-core::api::transport` import，test-only `proxy_core_host` 兼容壳也直接从 core transport API 获取 `ProxyResponseBody`；JSON response 到 transport body 的 contract fixture 由 `proxy-core::domain` owning tests 覆盖，adapter 不再保留重复测试；边界测试防止 response transport DTO 或重复 fixture 重新经 adapter 回流。
...
1327. `proxy_core_adapter` 不再暴露 `ResponseRuntimePolicy` / `AppProxyConfig` / `AllowResult` / `CircuitBreakerConfig` / `CircuitBreakerStats` config contract alias：adapter 内部直接从 `proxy_core::api::config` 获取 response runtime、app config 与熔断配置 DTO，commands、DAO、HTTP server、router 与 circuit breaker 路径继续直接引用 core config；边界测试防止这些 config contract 重新经 adapter alias 回流。
1328. `proxy_core_adapter` 不再暴露 forwarder transport DTO alias：`ForwarderAuthHeaders`、`PreparedCopilotAuthOptimization`、`OptionalCopilotAuthOptimizationPreparationInput`、`ForwarderProtocolPreparation`、`ForwarderProtocolPreparationInput` 与 `ForwarderTransformPlan` 均由 adapter、forward pipeline 与 host forwarder source 直接从 `proxy_core::api::transport` 获取；边界测试防止 forwarder transport contract 重新经 adapter alias 回流。
...
1332. `proxy_core_adapter` 不再暴露 provider selection/failover routing DTO alias：`ProviderFailoverCircuitLookup`、`ProviderSelectionFailure`、`ProviderSelectionInput` 与 `FailoverQueuePosition` 均由 adapter 内部和 routing source 直接从 `proxy_core::api::routing` 获取；`AutoFailoverToggleInput`/`AutoFailoverTogglePlan` 已进一步被 `proxy-core::management::auto_failover_toggle_plan_from_sources` 包装为管理命令可复用的 plan outcome，边界测试防止 provider-selection routing contract 重新经 adapter alias 回流。
...
1335. `proxy_core_adapter` 不再暴露 legacy channel/provider projection DTO alias：`ChannelRecord`、`ChannelRouteSource`、`LegacyChannelModelProjection`、`LegacyChannelProjection`、`LegacyChannelMigrationPlanInput`、`LegacyEndpointInput`、`LegacyModelRouteInput`、`LegacyProviderChannelMigrationInput` 与 `LegacyProviderProjectionInput` 均由 DAO、adapter 内部和 host channel/router source 直接从 `proxy_core::api::{management,routing}` 获取；边界测试防止 legacy projection contract 或 migration preview helper 重新经 adapter 回流。
1336. `proxy_core_adapter` 不再暴露 `ModelMappingProjection` model catalog DTO alias，也不再保留 provider-shaped model mapping wrapper：host-owned request source 直接从 `proxy_core::api::model_catalog` 获取 projection DTO 与 mapping helper；边界测试防止 model mapping contract 重新经 adapter alias 或 façade 回流。
...
1338. `proxy_core_adapter` 不再 re-export `usage_route_context_from_selection` usage helper：`RequestContext` route update 投影已迁到 `proxy/engine/context.rs` 并直接从 `proxy_core::api::usage` 导入该 helper，adapter 不再保留宿主 `Provider` hydration 包装；边界测试防止 usage route context helper 重新经 adapter 公开回流。
...
1340. provider additive/OMO 的 category-only 策略 helper 已由 `services/provider` 直接从 `proxy_core::api::ports` 引用：`provider_omo_variant_for_app_category`、`provider_omo_switch_pair_for_app_category`、`provider_additive_update_route_for_app` 与 `provider_switch_dispatch_for_app` 均不再经 `proxy_core_adapter` re-export；生产调用点只传入 app/category facts，adapter 只保留真正需要 DB/runtime/host DTO 组合的 provider 侧投影。
...
1347. stream check 的 provider-level testConfig 读取不再保留 `proxy_core_adapter::provider_stream_check_test_config` 普通 meta accessor；host `ProviderTestConfig` 到 core `StreamCheckConfigOverride` 的投影入口已移入 `services/stream_check.rs`，adapter 不再保留该 bridge。
...
1349. custom endpoint 列表读取不再保留 `proxy_core_adapter::provider_custom_endpoint_list` 普通 accessor；endpoint service 直接调用 `Provider::custom_endpoint_list()`，Provider owning module 自测覆盖排序 contract，adapter 不再承接该 fixture。
1350. custom endpoint last-used 更新不再保留 `proxy_core_adapter::mark_custom_endpoint_last_used` 普通 mutation helper；endpoint service 直接调用 `Provider::mark_custom_endpoint_last_used()`，Provider owning module 自测覆盖 mutation contract，避免把 provider meta 写入细节挂在代理 core adapter 面上。
1351. Attempt/route-selected 事件 envelope 已收敛到 `proxy-core::events::{attempt_event,route_selected_event}`：默认 runtime state source 本地拥有 `ForwardAttempt -> AttemptEventPayloadInput` 的宿主投影与 event bus 副作用，不再经 adapter-local message/payload wrapper 组装事件名与 payload。
...
1358. `live_takeover_app_types` 不再作为 `proxy_core_adapter` 的 crate-visible host facade 提供给 live takeover service：`proxy/host/cc_switch/live_takeover.rs` 直接消费 core `live_takeover_app_kinds()` 并在 host 层投影为 `AppType`，adapter 不再保留该 app catalog helper。
...
1359a. ProviderService 的 current-provider delete 判定与 normal switch backfill source id 行为测试已从 `proxy_core_adapter` 迁到 owning `services/provider/mod.rs`；边界测试继续约束这两个 provider service policy fixture 不再回流到 adapter。
...
1363. Route policy source 的 DB failover queue 读取与 provider id 到 `RoutePolicy` 投影已归入 `proxy/host/cc_switch/route_policy_source.rs`：host source 直接调用 core `route_policy_from_failover_provider_ids`，`proxy_core_adapter` 不再暴露 `route_policy_from_db_source` / `route_policy_from_source` / `route_policy_from_failover_queue`。
1364. Management dry-run route response helper 已从 `proxy_core_adapter` 移到 `proxy/host/cc_switch/route_resolver.rs`：route resolver 直接消费 core `resolve_channel_route`、`route_candidate_channel_circuit_keys` 与 `apply_route_candidate_circuit_availability`，routing/server 测试夹具也改为从 owning module 引用该 helper。
1365. Forward result bridge 已从 `proxy_core_adapter` 移到 `proxy/host/cc_switch/forward_pipeline.rs`：host forward pipeline 直接拥有 `proxy_response_to_core_response`、`forward_result_to_proxy_result` 与 `proxy_result_from_forward_parts`，并在本地调用 core `select_route_for_forward_result` 生成 selected route。
1366. RequestContext route update bridge 已从 `proxy_core_adapter` 移到 `proxy/engine/context.rs`：context 直接拥有 `RequestContextRouteUpdate`、source lookup error、`usage_route_context_from_selection` 和 selected-provider missing 文案投影，adapter 不再中转 `ProxyResult` 到请求生命周期状态回填。
...
1373a. ProviderAuthInfo strategy/masking 行为测试已由 `proxy-core::provider_auth` 拥有，Codex legacy `apiKey` 提取、缺失 auth 和 Bearer header 行为测试已由 `proxy/provider/codex.rs` 拥有；`proxy_core_adapter` 不再承载 provider auth 策略/header 聚合 fixture。
...
1376. forward runtime 的 retryable/terminal failure warning、`ProxyError -> ForwardFailureKind` 投影与 rectifier retry success/failure log-line 投影已迁入 host-owned runtime/error 边界：默认 runtime state source 本地消费 core forward failure log builders 与 rectifier message helpers，`proxy::error_mapper` 拥有宿主错误事实投影，`proxy_core_adapter` 不再保留 forward failure façade。
...
1379. client model catalog source selection 与 Codex active-config catalog stale guard 已迁入 host-owned `proxy/host/cc_switch/model_catalog_provider.rs`：`CcSwitchModelCatalogProvider::load_client_catalog` 本地消费 core catalog planning/parsing helper，`proxy_core_adapter` 不再暴露 `client_model_catalog_from_app_source` 或持有 Codex client catalog raw helper。
1380. provider model catalog DB source 已迁入 host-owned `proxy/host/cc_switch/model_catalog_provider.rs`：`CcSwitchModelCatalogProvider::load_catalog` 本地读取 provider settings 并调用 core `provider_model_catalog_from_settings`，`proxy_core_adapter` 不再暴露 `provider_model_catalog_from_db_source`。
1381. ProviderSource 的 current-provider DB source、active-route runtime source 和 route-candidate provider ids router source 已迁入 host-owned `proxy/host/cc_switch/provider_source.rs`：`CcSwitchProviderSource` 本地读取 current provider/runtime target、包装 `ProviderRouter::select_provider_ids` 的 `AppError` 兼容错误，并继续把 empty-selection policy 委托给 `proxy-core`。
1382. CC Switch `Provider` 到 core `ProviderSpec` 的 metadata/accountRef/provider kind 投影、provider list/get DB source 和 `ProviderRouterProviderSource` 复用的 provider source projection 已迁入 host-owned `proxy/host/cc_switch/provider_projection.rs`：ProviderSpec redaction / managed-fact projection fixture 也已迁到 owning module tests，`proxy_core_adapter` 不再暴露 `proxy_provider_to_core_spec`、`provider_specs_from_db_source` 或 `provider_spec_from_db_source` façade，forwarder auth source 也直接引用 host projection。
1383. CC Switch `Provider.meta` 到 managed-account binding context 与 GitHub Copilot account id 的投影已迁入 host-owned `proxy/host/cc_switch/provider_projection.rs`：managed account runtime source、stream check command 和 provider usage command 直接消费 host projection，相关 projection fixture 也已迁到 owning module tests，`proxy_core_adapter` 不再暴露 `ProviderManagedAccountBindingContext`、`provider_managed_account_binding_context` 或 `provider_github_copilot_managed_account_id` façade。
1384. Claude forwarder api_format 的 Copilot vendor 分流已迁入 host-owned `proxy/host/cc_switch/managed_account_runtime_source.rs`：runtime source 在拿到 Copilot model vendor 后直接调用 `proxy-core::resolve_claude_forward_api_format`，`proxy_core_adapter` 不再暴露 `resolve_forwarder_claude_api_format` façade，边界测试防止 runtime source 回引 adapter wrapper。
1385. Claude api_format、transform gate 与 streaming decision 的 provider 事实投影已迁入 host-owned `proxy/host/cc_switch/provider_projection.rs`：response adapter 与 Claude provider adapter 直接消费 `provider_claude_api_format` / `provider_needs_claude_transform` / `provider_claude_transform_streaming_decision`，api_format/transform-gate fixture 与 Codex OAuth aggregation streaming fixture 也已迁到 owning module tests，`proxy_core_adapter` 不再暴露这些 façade，边界测试防止 adapter 回填 provider api_format/transform gate/streaming decision wrapper。
1386. Anthropic thinking rectifier provider fact 已迁入 host-owned `proxy/host/cc_switch/provider_projection.rs`：forwarder request source 直接消费 `provider_uses_anthropic_rectifiers`，`proxy_core_adapter` 不再暴露 rectifier provider fact façade，边界测试防止 adapter 回填该 wrapper。
1387. Forward failure 的 `ProxyError -> ForwardFailureKind` 宿主事实投影已迁入 `proxy::error_mapper`：forwarder runtime state source 直接消费 error mapper，`proxy_core_adapter` 不再暴露 `forward_failure_kind_from_proxy_error` 或私有 message-selection wrapper，边界测试防止该 forward failure façade 回流 adapter。
1387a. error mapper 的 Codex proxy error host facts/response 投影与 `ProxyError -> ForwardFailureKind` 行为测试已迁入 `proxy/error_mapper.rs` owning tests；core error context、Codex envelope 和 rectifier message contract 已由各自 core owner tests 覆盖，`proxy_core_adapter` 不再承载 error mapper 聚合 fixture。
1388. Channel route candidate 到 host `Provider` settings/meta 的 override 应用已迁入 `proxy/route_attempt.rs` owning module：route attempt 在构造 `ForwardAttempt` 时直接调用 core override plan/settings apply helper 并写回 `ProviderMeta.api_format`，`proxy_core_adapter` 不再暴露 `apply_channel_provider_overrides` façade。
1388a. RoutePlan provider id、selected route、forward route error 文案与 route policy helper 的纯 contract 已由 `proxy-core::domain` owning tests 覆盖；RoutePlan 到 host `ForwardAttempt` 的同 provider 多 channel 过滤与 required attempts 缺 host provider 错误已由 `proxy/route_attempt.rs` 与 `host/cc_switch/channel_auth_profile_attempts.rs` owning tests 覆盖，`proxy_core_adapter` 不再承载 route plan 聚合 fixture。
1388b. RequestContext 相关 response runtime policy、request model/Gemini path model inference 与 Claude api_format metadata fallback fixture 已分别由 `proxy-core::response_timeout`、`proxy-core::request_url` 和 `proxy-core::response_transform` owning tests 覆盖；`proxy_core_adapter` 不再承载 handler context 纯 helper 聚合 fixture。
1388c. Gemini provider auth/base-url/header/url fixture 已由 `proxy/provider/gemini.rs` owning tests 覆盖，Gemini live settings/env/config/validation fixture 已由 `proxy-core::ports` 与 host `gemini_config.rs` owning tests 覆盖；`proxy_core_adapter` 不再承载 Gemini provider/live 聚合 fixture。
...
1399a. managed-account runtime source 的调用面已进一步从 provider-aware 收敛为 binding-facts 输入：`ForwarderAuthSource` / `ForwarderRequestSource` 在 host 层通过 `provider_managed_account_binding_context` 投影 CC Switch `Provider`，runtime source 只消费 `ManagedAccountRuntimeBindingFacts`、auth/base_url/body/api_format 等已归一化事实；token 获取、最近成功快照、短窗口失败回退和不可重试失败直出策略保持不变，仍由 core token-cache contract 与 host Tauri runtime source 协作完成。
...
1404. `/proxy/v1/events` SSE keep-alive interval/text contract 已迁入 `proxy-core::event_payload::proxy_events_sse_keep_alive_spec`：HTTP transport handler 负责 Axum `KeepAlive` 构造和 broadcast stream 桥接，但对外事件流心跳语义由 core 维护。
...
1409. `ProxyCoreEvent` 到 host `ProxyEventBus` envelope 的投影已收敛到 `proxy::events::ProxyEventBus::emit_core_event`：`CcSwitchEventSink`、forwarder runtime state source、proxy server lifecycle 和 `proxy/events.rs` owning 回归测试都复用同一入口，不再各自展开 event name/payload；test-only adapter 不再承载 event stream contract fixture。
...
1414. test-only `proxy_core_adapter` 中 attempt event payload 与 live takeover app-type list 的私有 helper 已删除；事件兼容测试直接构造 `proxy-core::events::AttemptEventPayloadInput`，live takeover app catalog 兼容测试迁入 host-owned `live_takeover.rs` 并直接消费 `live_takeover_app_kinds()`，边界测试防止事件 payload / live app-kind helper 重新沉回 adapter 测试壳。
...
1426a. managed-account runtime source 已补齐外部宿主集成证据：`proxy-core::api::prelude` 现在暴露 `ManagedAccountRuntimeSource` 实现所需的 `CopilotModel` 与 managed-auth resolver helper；`public_prelude` 覆盖外部 `AuthProvider` 通过 runtime source 解析 Copilot token 并生成显式 Authorization header，`external_relay_host` 示例也用同一 prelude 路径把 demo Copilot runtime token 接入 `engine.handle()` 的 auth header 记录。
1426b. channel-key runtime selection contract 已补齐外部宿主 prelude 面：`proxy-core::api::prelude` 现在暴露 `ChannelKeyRuntimeCandidateInput`、runtime candidate 构造 helper、health policy 到 selection policy 的解析、random/round-robin 选择 helper 与 round-robin cursor key helper；`public_prelude` 覆盖外部中转宿主只导入 prelude 即可构造 key runtime candidate、读取健康策略中的 key 选择规则并执行 wildcard/显式 key runtime selection。
1426c. provider health 聚合状态推进也补齐到外部宿主 prelude 面：`proxy-core::api::prelude` 现在暴露 `provider_health_update_from_input`、`ProviderHealthUpdateInput` 与 `ProviderHealthUpdate`；`public_prelude` 覆盖外部中转宿主只导入 prelude 即可复用 core 的 provider 级连续失败阈值、恢复清零和时间戳字段 contract。
1426d. channel health 状态推进也补齐到外部宿主 prelude 面：`proxy-core::api::prelude` 现在暴露 `channel_health_update_from_input`、`ChannelHealthUpdateInput`、`ChannelHealthUpdate` 与 `CHANNEL_HEALTH_UNKNOWN_STATUS`；`public_prelude` 覆盖外部中转宿主只导入 prelude 即可复用 channel 级 degraded/unhealthy/healthy 状态、失败毫秒时间戳和恢复清零 contract。
1426e. route policy / failover queue contract 已补齐到外部宿主 prelude 面：`proxy-core::api::prelude` 现在暴露 `route_policy_from_failover_provider_ids` 与 `route_policy_failover_provider_ids`；`public_prelude` 覆盖外部中转宿主只导入 prelude 即可把自己的 failover provider id 顺序投影成 core-facing `RoutePolicy` 并按原顺序读回。
1426f. channel-key runtime selection tick contract 已补齐到外部宿主 prelude 面：`proxy-core::api::prelude` 现在暴露 `ChannelKeyRuntimeSelectionTick` 与 `channel_key_runtime_selection_tick_from_time_parts`；CC Switch DB-backed source 只负责读取真实时间和 DB key records，`now_ms`/`weighted_roll` 的输入形状与 timestamp nanos fallback 规则由 core contract 固定，外部中转宿主可复用同一 tick 驱动 weighted/random selection。
1426g. forward response finalization contract 已补齐到外部宿主 prelude 面：`proxy-core::api::prelude` 现在暴露 `upstream_success_response_finalization_plan`、`UpstreamSuccessResponseFinalizationPlan`、upstream error response projection、channel response policy application 和 streaming/body timeout 文案 helper；`public_prelude` 覆盖外部中转宿主只导入 prelude 即可复用 CC Switch 默认 `ForwarderResponseSource` 正在消费的纯响应决策与投影规则。
...
| 区域 | 代表文件 | 当前职责 |
| --- | --- | --- |
| HTTP server | `transport/http/server.rs`, `transport/http/handlers.rs` | Axum/hyper 服务、路由表、HTTP 请求转为代理请求；旧 `server.rs`/`handlers.rs` 已删除 |
| 请求上下文 | `engine/context.rs` | 从 config source/route result 维护请求生命周期事实、session、模型名；旧 `handler_context.rs` 已删除 |
| 转发核心 | `engine/forward_pipeline.rs` | provider 重试、熔断记录、请求体处理、认证、上游请求、状态统计；旧 `forwarder.rs` 已删除 |
| provider 适配 | `provider/*` | Claude/Codex/Gemini URL、认证、请求/响应转换、SSE 转换；旧 `providers/mod.rs` 已删除 |
| 响应处理 | `engine/response_pipeline.rs` | 流式/非流式响应透传、解压、用量收集、日志落库；旧 `response_processor.rs` 已删除 |
| 故障转移 | `engine/routing.rs`, `circuit_breaker.rs`, `host/cc_switch/failover_switch.rs` | provider/channel 选择、熔断状态、故障转移后切换当前 provider；旧 `provider_router.rs`/`failover_switch.rs` 已删除 |
| 用量 | `host/cc_switch/database_usage_sink.rs`, `proxy-core::api::usage` | token 解析、计价和中立 usage contract 由 core/adapter 提供，DB 请求日志写入由 host usage sink 承接；旧 `usage/*` facade 已删除 |
...
当前分支已先把 `ProviderRouter` 的 DB source 读取和健康持久化收进 `proxy_core_adapter` / host source 边界：router 生产代码不再直接调用 provider/channel/config/health 表的读写 API，也不再暴露 `Arc<Database>` 构造入口；provider selection 生产 API 只返回 provider id，完整 Provider record 的加载留在 host adapter。router 通过 `ProviderRouterProviderSource`、`ProviderRouterChannelSource`、`ProviderRouterConfigSource` 和 `ProviderRouterHealthStore` 四个可注入端口取得 provider failover id sources、current provider id source、channel route source、router config 和 health persistence 入口；其中 provider/channel/config source 与 health store 默认实现已迁入 `proxy/host/cc_switch/provider_router_provider_source.rs`、`proxy/host/cc_switch/provider_router_channel_source.rs`、`proxy/host/cc_switch/provider_router_config_source.rs` 和 `proxy/host/cc_switch/provider_router_health_store.rs`。后续真正拆 crate 时，应把这些 router 端口进一步映射到 core-facing `ProviderSource`/`ChannelSource`/`HealthStore` trait 实现，而不是让 core 持有 CC Switch `Database`。
...
provider adapter context 仍负责 CC Switch 默认 fallback 的 provider settings 到 header 转换，但 token 刷新和宿主账号状态读取不在 provider adapter 内完成。channel 可以指向同一个 provider 的不同 key/auth profile，必须避免把一个 channel 的 key 泄漏到另一个 channel。当前 managed-account token 短窗口回退、外部 AuthProvider runtime token 接入、channel-key weighted/random/round-robin 与 key failure cooldown 都已收敛到 core policy + host source；外部中转 AuthProvider 显式 header、示例 runtime smoke、managed-account runtime AuthProvider smoke、真实 HTTP listener/upstream 转发 smoke、同 provider 多 channel 失败重试 smoke、wildcard key cooldown smoke 和 wildcard key round-robin smoke 已补齐；`ForwarderAuthSource` 也不再展开 provider settings auth 或 managed-account binding，而是只消费 `ForwarderAdapterContext::resolve_provider_fallback_auth_headers` 的结构化结果。request-side 的 Copilot live model、动态 base URL 和 Claude api_format 分流同样不再由 `ForwarderRequestSource` 直接读取 `ProviderMeta` binding；默认实现只把 provider/body/base_url 事实交给 `ForwarderAdapterContext`，由 adapter context 投影 binding facts 后调用 `ManagedAccountRuntimeSource`。

本轮继续把 managed-account token cache 与 runtime source 的结构化 contract 暴露到 `proxy-core::api::prelude`：外部中转宿主只经 public prelude 即可构造 `ManagedAccountTokenCacheKey`、refresh success/failure input、`ManagedAccountTokenSnapshotStore` 与 fallback/reject 结果，也可以实现 `ManagedAccountRuntimeSource` 并调用 managed-auth resolver helper 把 Copilot/Codex OAuth runtime token 接入自己的 `AuthProvider`。`external_relay_host` 示例已经用 demo Copilot runtime source 生成真实 Authorization header，不需要导入内部模块或复制 CC Switch 默认 runtime source 的 cache/refresh 规则。
...
本轮继续把 channel-key runtime selection 的外部宿主 contract 补齐到 `proxy-core::api::prelude`：外部中转实现可以只通过 prelude 构造 `ChannelKeyRuntimeCandidate`、读取 channel health policy 中的 `channelKeySelectionStrategy` / `channelKeyFailureCooldownMs`、执行 weighted/random/round-robin 或显式 key selection，并为 wildcard round-robin 生成稳定 cursor key；不再需要导入 `api::management` 或内部 `ports` 才能实现不同地址不同 key 池的中转选择策略。

本轮继续把 channel-key runtime selection tick 的外部宿主 contract 补齐到 `proxy-core::api::prelude`：外部中转实现可以只通过 prelude 构造 `ChannelKeyRuntimeSelectionTick`，把真实时间投影成 core selection 需要的 `now_ms` 与 `weighted_roll`，并复用 timestamp nanos 不可用时退回毫秒值的规则；这让自定义 `ChannelKeyRuntimeSource` 不需要复制 CC Switch DB source 的私有 selection clock。
...
随后又把 `ChannelKeyRuntimeSource` 接入 `ProxyServices` 容器：core service catalog 现在显式包含 channel-key runtime source，CC Switch services 持有 DB-backed source 实现，并且 host channel-key/auth-profile 模块已直接引用 core port/result contract。外部中转宿主在组装独立 proxy module 时可以和 `AuthProvider`、`ProviderSource`、`ChannelSource` 一样注入自己的 key runtime，而不是依赖 CC Switch adapter 的 port facade。
...
本轮继续把 CC Switch `Provider.meta` 到 managed-account binding 的投影收敛为 host-owned `provider_projection::ProviderManagedAccountBindingContext`：host runtime source 不再在认证、动态 endpoint、live model 和 Claude API format 路径分别展开 `auth_binding`/legacy `github_account_id`，而是统一消费结构化 binding context 后交给 core runtime helper。当前这些 binding facts 的生产投影入口继续向 `ForwarderAdapterContext` 集中，auth fallback、Copilot live model、动态 endpoint 和 Claude api_format runtime 决策都由 adapter context 统一调用 `ManagedAccountRuntimeSource`；`ForwarderRequestSource` 不再直接组装 `ManagedAccountRuntimeBindingFacts`。后续外部中转宿主替换 Provider 形状时，只需要提供等价 binding facts，而不需要复制 CC Switch `ProviderMeta` 字段读取。

本轮继续收缩 `proxy_core_adapter` 的纯 helper 中转面积：`CcSwitchForwarderRequestSource` 的 Copilot optimizer sequencing 直接经 `proxy_core::api::transport` 引用分类、orphan tool_result sanitize、tool_result merge、thinking strip 和 warmup model override helper，request-source trait/input/output contract 由 `proxy/engine/forward_pipeline.rs` 拥有，adapter 不再 re-export 这些只被默认 request source 使用的 core 函数或 source contract。这样外部中转宿主可以按 `proxy_core::api` 与 forward pipeline contract 集成请求优化策略，而不是经过 CC Switch adapter facade。
...
旧 provider 主地址和 `provider_endpoints` 到 channel 的兼容迁移规划也已进入 core：`build_legacy_channel_migration_plan` 负责 primary/endpoint channel 投影、endpoint 稳定排序、normalized base URL 去重、priority/interface/model route 推导和 review 计数。CC Switch DAO 把本地 `Provider`、`custom_endpoints`、当前 provider 事实装配成 core input，并把 core projection 映射回 `proxy_channels`/models/health 写入形状；adapter 不再承接该 preview/projection 中转。
...
`CcSwitchChannelSource` 的 core-facing `ChannelSource` wrapper 已从 `proxy_core_adapter.rs` 下沉到 `proxy/host/cc_switch/database_channel_source.rs`；该 host 模块已直接引用 core channel domain/routing/management/port/result contract，接收 spec/list/get helper、`ChannelQuery` 过滤、materialized-vs-legacy fallback 选择，以及 record CRUD/list、key/model 管理和 migration preview/materialize DB helper。DB-backed provider source 已迁入 `proxy/host/cc_switch/provider_source.rs`，DB-backed channel-key runtime source 已迁入 `proxy/host/cc_switch/channel_key_runtime_source.rs`，DB-backed channel reachability probe 已迁入 `proxy/host/cc_switch/channel_reachability_probe.rs`，auth-profile attempt source 已迁入 `proxy/host/cc_switch/channel_auth_profile_attempts.rs`，channel health store 已迁入 `proxy/host/cc_switch/channel_health_store.rs`，默认 `CcSwitchAuthProvider` 已迁入 `proxy/host/cc_switch/auth_provider.rs`，managed-account runtime source 已迁入 `proxy/host/cc_switch/managed_account_runtime_source.rs`，provider adapter context 已迁入 `proxy/host/cc_switch/provider_adapter_context.rs`，forwarder auth source 已迁入 `proxy/host/cc_switch/forwarder_auth_source.rs`，forwarder transport source 已迁入 `proxy/host/cc_switch/forwarder_transport_source.rs`，forwarder response source 已迁入 `proxy/host/cc_switch/forwarder_response_source.rs`，forwarder protocol state source 已迁入 `proxy/host/cc_switch/forwarder_protocol_state_source.rs`，forwarder attempt runtime source 已迁入 `proxy/host/cc_switch/forwarder_attempt_runtime_source.rs`，forwarder runtime state source 已迁入 `proxy/host/cc_switch/forwarder_runtime_state_source.rs`，forwarder request source 已迁入 `proxy/host/cc_switch/forwarder_request_source.rs`，forward pipeline wrapper 已迁入 `proxy/host/cc_switch/forward_pipeline.rs`，proxy services container 已迁入 `proxy/host/cc_switch/proxy_services.rs`，failover switch scheduler 已迁入 `proxy/host/cc_switch/failover_switch.rs`，event sink 已迁入 `proxy/host/cc_switch/event_sink.rs`，usage sink 已迁入 `proxy/host/cc_switch/database_usage_sink.rs`，management auth source 已迁入 `proxy/host/cc_switch/management_auth_source.rs`，runtime status source 已迁入 `proxy/host/cc_switch/runtime_status_source.rs`，Claude Desktop gateway auth source 已迁入 `proxy/host/cc_switch/claude_desktop_gateway_auth_source.rs`，route policy source 已迁入 `proxy/host/cc_switch/route_policy_source.rs`，ProviderRouter channel source 已迁入 `proxy/host/cc_switch/provider_router_channel_source.rs`，ProviderRouter config source 已迁入 `proxy/host/cc_switch/provider_router_config_source.rs`，ProviderRouter health store 已迁入 `proxy/host/cc_switch/provider_router_health_store.rs`，ProviderRouter provider source 已迁入 `proxy/host/cc_switch/provider_router_provider_source.rs`，ProviderRouter DB-backed source assembly 已迁入 `proxy/host/cc_switch/provider_router_sources.rs`，ProviderRouter circuit runtime helper 已迁入 `proxy/host/cc_switch/provider_router_circuit_runtime.rs`，HTTP `ProxyState` 数据形状已迁入 `proxy/host/cc_switch/proxy_state.rs`；剩余 channel/auth 相关迁移应聚焦 token 缓存/刷新、channel-key 轮询/随机和失败回退策略。

dry-run route 的 circuit-open 识别也继续收敛：`route_candidate_channel_circuit_keys` 负责把 `RouteResolveResponse` 的候选投影成 channel circuit lookup facts，host route resolver 的 `management_route_response_from_router_source` 负责调用 core resolver、向 `ProviderRouter` 查询已有 breaker 可用性并把 availability facts 交给 `apply_route_candidate_circuit_availability`，由 host resolver/core 生成 rejected:circuit_open response mutation。

provider failover 的 circuit lookup 也继续收敛：`provider_failover_circuit_lookups` 负责保留 failover queue 顺序、标记 missing provider 并生成已配置 provider 的 circuit key；`CcSwitchProviderRouterProviderSource` 通过 core-facing `ProviderSource::list_providers` 读取 provider id facts，failover queue 通过 `RoutePolicySource::load_policy` 的 `RoutePolicy` raw contract 提取 failover provider ids，并在 host source 内投影为 lookup facts；`ProviderRouter` 只查询 live breaker 可用性，随后把 lookup availability facts 交给 `select_failover_provider_ids_from_router_lookup_availability` 投影为 selection candidates 并执行 core selection 策略，最终仍只返回 provider id 列表。

默认 `CcSwitchRoutePolicySource` 已迁入 `proxy/host/cc_switch/route_policy_source.rs`：host source 持有 DB，直接消费 core route-policy/source/result contract，自行读取 failover queue、调用 core `route_policy_from_failover_provider_ids` 生成 `RoutePolicy`，并直接使用 core config error helper 映射 DB 错误；adapter 不再保留 route policy DB/source/queue façade，也不再 re-export `RoutePolicySource` port trait 或 `CcSwitchRoutePolicySource` host type，默认装配由 host services / ProviderRouter sources 直接引用 host 模块。默认 `CcSwitchRouteResolver` 也已迁入 `proxy/host/cc_switch/route_resolver.rs`：host resolver 持有 `ProviderRouter`，直接导入 core `RouteResolver` / `RouteRequest` / `RoutePlan` contract 并调用 `build_route_plan`，并本地拥有 ProviderRouter-backed management dry-run response helper；adapter 不再保留 route plan 或 management dry-run route facade。

默认 `CcSwitchChannelHealthStore` 已迁入 `proxy/host/cc_switch/channel_health_store.rs`：host store 持有 DB 与 `ProviderRouter`，直接引用 core `ChannelHealthStore` / channel health fact 类型，并在 owning module 内完成 `record_channel_attempt_in_db_source`、`reset_channel_health_with_router_source`、`channel_breaker_stats_with_router_source` 和 DB/router 错误映射；adapter 不再保留 channel health attempt/reset/stats projection helper、error helper，也不再提供 store 兼容 re-export 装配入口。

本轮继续把 channel health 状态推进的外部宿主 contract 补齐到 `proxy-core::api::prelude`：外部中转实现可以只通过 prelude 复用 `channel_health_update_from_input` 的 degraded/unhealthy/healthy 状态推进、`unknown` 缺省状态、失败毫秒时间戳和成功恢复清零规则；这让自定义 `ChannelHealthStore` 不需要复制 CC Switch channel DAO 的健康状态写入策略。

本轮继续把 route policy / failover queue 的外部宿主 contract 补齐到 `proxy-core::api::prelude`：外部中转实现可以只通过 prelude 把自己的 failover provider id 顺序投影成 core-facing `RoutePolicy`，并用同一组 helper 按原顺序读回队列；这让自定义 `RoutePolicySource` 不需要了解 core 内部 routing/domain 模块拆分。

默认 `CcSwitchProviderSource` 已迁入 `proxy/host/cc_switch/provider_source.rs`：host source 持有 DB、`ProviderRouter` 与 active-route runtime map，直接消费 core provider/source/result contract，并在 owning module 内拥有 current-provider DB source、active-route runtime source 与 route-candidate provider ids router source；provider list/get 的 `Provider -> ProviderSpec` 投影已由 host-owned `provider_projection.rs` 统一提供，adapter 不再 re-export `ProviderSource` port trait，也不再保留 provider source type、provider spec projection 或 current/active-route/route-candidate source façade。

`ProviderRouter` 的配置源、provider 读取、健康持久化和 management dry-run route resolution 也已进一步收口：auto failover gate、circuit breaker config 和 failure threshold 读取已由 `CcSwitchProviderRouterConfigSource` 通过 core `ProxyConfigSource::load_app` 在 host 模块内投影；provider id/current provider id 读取已由 `CcSwitchProviderRouterProviderSource` 通过 core `ProviderSource` 投影，failover queue ids 读取通过 core `RoutePolicySource` 投影，router provider source trait 改为 async 以消费 core source；channel route 输入已由 `CcSwitchProviderRouterChannelSource` 通过 core `ChannelSource` 在 host 模块内投影，并直接引用 core `ChannelRouteSource` / `RouteResolveChannelInput` contract；channel health 写入由 router 组装 core `ChannelAttemptResult` fact，`CcSwitchChannelHealthStore` 与 `CcSwitchProviderRouterHealthStore` 直接引用 core health fact/port 类型并各自直接使用 core config error helper 映射 DB/router 错误；provider health 写入已落到 core `ProviderAttemptResult` / `ProviderHealthStore` attempt 端口，`CcSwitchProviderRouterHealthStore` 直接引用 core `ProviderAttemptResult` / `ProviderHealthStore` 并拥有 provider raw DB update projection，adapter 已移除 `ChannelHealthStore` / `ProviderHealthStore` port facade，也不再保留 ProviderRouter provider health DB update 公开入口，状态推进仍由 DB 层调用 core `provider_health_update_from_input` 完成；channel health reset 由 router 在清 live circuit 后组装 app-scoped `ChannelHealthReset` fact，再由 `CcSwitchProviderRouterHealthStore` 的 owning wrapper 写入宿主持久层，管理端只有 channel id 的 reset 路径仍由 `CcSwitchChannelHealthStore` 查询 app 并复用 router reset；management route response 由 adapter 组合 core resolver 与 router circuit availability；DB-backed router 构造由 host-owned `provider_router_sources::provider_router_from_database` 完成，并经 adapter re-export 保持既有调用面。router provider/channel/config source 和 health store 默认实现已迁入 `proxy/host/cc_switch/provider_router_provider_source.rs`、`proxy/host/cc_switch/provider_router_channel_source.rs`、`proxy/host/cc_switch/provider_router_config_source.rs` 与 `proxy/host/cc_switch/provider_router_health_store.rs`，再由 `proxy/host/cc_switch/provider_router_sources.rs` 统一装配，继续分别持有 `Database` + `CcSwitchRoutePolicySource` / `CcSwitchChannelSource` / `CcSwitchConfigSource` / `Database` 并通过 core source/attempt facts 生成 provider/route/config/health 输入；live circuit breaker map、permit/half-open 状态机、breaker availability 查询和热更新已由 router 内的 `ProviderRoutingCircuitRuntime` 承接；`ProviderRouter` 保留现有公开方法与 `AppError` 兼容，但自身不再直接持有 breaker map。

本轮继续把 provider health 聚合状态推进的外部宿主 contract 补齐到 `proxy-core::api::prelude`：外部中转实现可以只通过 prelude 复用 `provider_health_update_from_input` 的连续失败阈值、失败时间戳、恢复成功清零和 last_error 规则；这让自定义 `ProviderHealthStore` 或 provider 级聚合视图不需要复制 CC Switch DAO 中的健康状态推进逻辑。
...
管理 API 查询类入口已基本收敛到 `ProxyEngine`：`list_proxy_providers`、`list_proxy_channels` 的 route-aware 分支、`list_proxy_groups` 和 `test_proxy_channel` 都只保留 HTTP path/query/body 提取与错误映射；固定 app catalog 已从 `proxy-core` 的端口默认实现移出，改由 host-owned `CcSwitchConfigSource` 显式提供；RoutePolicy 的 failover provider id 读取也已由 domain/routing helper 维护，engine 不再直接读 raw JSON 字段；managed-auth 的单测入口也已切到 runtime source surface；真实 HTTP listener/upstream 转发 smoke 已覆盖 materialized channel、channel key auth、模型改写、响应透传、active route 归因、同 provider 多 channel 失败重试、wildcard key cooldown 和 wildcard key round-robin，下一步应继续减少 managed-account token refresh/cache 对 CC Switch 默认 runtime source 的隐性依赖。

默认 `CcSwitchModelCatalogProvider` 已迁入 `proxy/host/cc_switch/model_catalog_provider.rs`：host provider 持有 DB 与 `ProviderRouter`，直接消费 core `ModelCatalogProvider` / `ModelCatalog` / `ClaudeDesktopModelRouteInput` / `AppKind` contract；provider catalog DB 读取、client catalog source selection、Codex active-config stale guard、Claude Desktop route source、provider selection 错误映射和 route input 转换已由该 host provider 本地拥有。adapter 仅保留 Claude Desktop proxy model route 的 shared projection helper 和默认 provider 装配入口，但不再 re-export `ModelCatalogProvider` port trait。

默认 `CcSwitchConfigSource` 已迁入 `proxy/host/cc_switch/config_source.rs`：host source 持有 DB，直接消费 core config/source/result contract，自身负责 app catalog、global/app/app-summary/runtime config 读取和 settings current-provider 事实读取，并直接调用 core config builder；adapter 不再保留 `cc_switch_app_kinds`、`proxy_global_config_from_db_source`、`proxy_app_config_from_db_source`、`app_summary_config_from_db_source` 或 `proxy_runtime_config_from_db_source`。

默认 `CcSwitchProxyRuntime` 已迁入 `proxy/host/cc_switch/proxy_runtime.rs`：host runtime 持有 DB、`ProviderRouter`、`ProxyStatus`、current target map、event bus、shadow/history store、failover manager、runtime source 与 `AppHandle`，并在 host 模块内定义和实现 `ProxyServiceRuntimeResources` / `HostForwardRuntime`；forward bridge、runtime config/current-provider 读取、`ProxyRequest` 到 host forward 输入投影、`ForwarderRuntimeHostResources` 装配和 preplanned `RequestForwarder` 启动也已由 `proxy_runtime.rs` 拥有，adapter 不再保留该运行时转发入口或 runtime type re-export。

默认 `ProxyState` 数据形状已迁入 `proxy/host/cc_switch/proxy_state.rs`：host state 持有 HTTP server 实际读写的 DB、config/status/start_time、current target map、ProviderRouter、`CcSwitchProxyServices<CcSwitchProxyRuntime>` core services、Gemini shadow、Codex history 和 event bus；`proxy_engine()` impl 与 runtime assembly 也由 host state 拥有，adapter 不再保留 `ProxyState` re-export 或 `ProxyEngine::new` 边界。

默认 `CcSwitchForwardPipeline` wrapper 已迁入 `proxy/host/cc_switch/forward_pipeline.rs`：host pipeline 持有可选 `CcSwitchProxyRuntime` 与 DB-backed `CcSwitchChannelKeyRuntimeSource`，实现 core `ForwardPipeline` 端口，并在 host module 内完成缺 runtime 错误包装与 optional-runtime dispatch；adapter 不再拥有 `HostForwardRuntime` trait 或 `forward_with_optional_host_runtime` helper。
...
当前已迁移到 core 的 response pipeline 子能力包括响应头清理、响应体诊断、非流式 body 解压、timeout 选择、Claude/Codex transform streaming 决策、Claude 响应 api_format 转换分发、Codex 代理错误 envelope 和 SSE 文本/聚合工具：`strip_hop_by_hop_response_headers` 移除 hop-by-hop 头和 `Connection` 点名扩展头，`strip_entity_headers_for_rebuilt_body` 移除重建 body 后失真的实体头，`prepare_rebuilt_json_response_headers` 负责转换后 JSON body 的实体/hop-by-hop/content-type 头重建，`json_proxy_response`/`rebuilt_json_proxy_response` 负责 JSON 响应的 header 重建、body 序列化与 `ProxyCoreResponse` 构造，`ProxyResponseBuildFailureContext` 负责响应构建失败的协议上下文与日志前缀，`ProxyResponseBuildErrorContext` 负责 Axum response build 失败的 tag/Claude/Codex 文案策略，`ProxyResponseTransformFailureContext` 负责响应转换失败的协议上下文与日志前缀，`transformed_sse_response_headers` 负责转换后 SSE 固定响应头，`transformed_sse_proxy_response` 负责转换后 SSE 响应的固定 header、OK status 和 stream body `ProxyCoreResponse` 构造，`body_looks_like_sse`/`body_diagnostics_suffix`/`body_snippet` 负责未标记 SSE body 嗅探与错误现场摘要，`get_content_encoding`/`decompress_body`/`decode_response_body` 负责 gzip/x-gzip/deflate/br 的 content-encoding 判定与解压、未知编码/失败解码原样透传和成功解码后的 header 一致性处理，`resolve_response_timeout_config` 负责 failover-gated 非流式 body timeout 与流式 first-byte/idle timeout 选择规则，`claude_transform_streaming_decision`/`codex_chat_transform_streaming_decision` 提供 Claude/Codex transform 的 streaming 与非流 SSE 聚合 core 策略，`claude_response_to_anthropic_message_for_api_format`/`create_claude_to_anthropic_sse_stream_for_api_format` 负责 Claude OpenAI Chat/OpenAI Responses/Gemini Native 非流与流式响应转换分发，`codex_proxy_error_json`/`codex_upstream_error_to_response_error`/`normalize_codex_chat_error_body`/`codex_chat_error_proxy_response` 负责 Codex 转发层代理错误和 Chat 上游错误响应的 Responses 风格 envelope、上游错误体归一化、非 JSON 预览截断和 413 上游体积限制提示，`strip_sse_field`/`take_sse_block`/`append_utf8_safe` 负责 SSE 字段提取、事件分帧和跨 chunk UTF-8 拼接，`SseEventScanner` 负责流式 data 行扫描、`[DONE]` 判定和可选 JSON parse，`SseUsageAccumulator` 负责 usage 事件缓存、首个被收集事件计时和 finish-once 防重入，`SsePassthroughStreamState` 负责 passthrough chunk 的 timeout phase、首包日志、SSE 事件日志和可收集 usage JSON 投影，`claude_stream_usage_event_filter`/`openai_stream_usage_event_filter`/`codex_stream_usage_event_filter`/`gemini_stream_usage_event_filter` 负责热路径 usage 事件预过滤，`TokenUsage` 及其 Claude/OpenAI/Codex/Gemini usage JSON parser 与 stream model extractor 负责协议用量解析和模型归因，`usage_tokens_from_token_usage`/`normalize_usage_models`/`normalize_error_usage_models`/`token_usage_from_usage_record`/`resolve_usage_record_pricing_models`/`transformed_response_usage` 负责 `UsageRecord` 的 token bucket 映射、模型归因、pricing model 选择和转换后非流响应 usage 归因规则，`StreamingResponseUsageRecord::missing_usage_log_message` 与 `NonStreamingResponseUsageRecord::log_event` 负责 usage 缺失/非 JSON body 的诊断消息投影，`chat_sse_to_response_value`/`responses_sse_to_response_value` 负责错标 SSE 非流式兜底聚合。流式 async transport 和 app-specific 响应转换仍在 host 层；非流式 decoded body bridge、response 日志 event dispatch、passthrough streaming usage runtime source selection、非流式 usage runtime source selection、transformed streaming usage runtime source selection、transformed non-streaming usage runtime source selection、passthrough/transformed usage collector 构造、usage record 调度调用点、streaming/non-streaming/transformed response usage record builder、forward-error usage record helper、reusable sink-spawn helper、streaming/non-streaming passthrough response construction、Claude/Codex transformed SSE stream wrappers、Claude/Codex transformed JSON wrappers、shared logged-stream runtime loop、provider usage facts projection 与 Anthropic tool schema hints helper 已由 `proxy/engine/response_pipeline.rs` 拥有，调用方和 adapter 单测均直接 import owning module，不再通过 `proxy_core_adapter` 兼容 re-export；`proxy/engine/response_pipeline.rs` 直接经 `proxy_core::api::transport/usage` 消费 response header predicate、core response DTO、Axum build context、usage parser/config/format/failure context、route context 与 selected-provider-missing 文案；`proxy::response_adapter` 集中承接 Axum/旧 `hyper_client::ProxyResponse` transport 桥接，并直接经 `proxy_core::api::transport/events/management/model_catalog/transforms/usage` 消费 pure transport DTO、response builder helper、endpoint path/query helper、event envelope、management request/response DTO、model catalog response DTO、Codex Chat streaming decision、Codex Chat upstream error normalization 与 parser config；`proxy::error_mapper` 集中承接 core/host error 映射、response parse 专用错误适配、parse failure 日志投影、fallback event 日志分发、response build/transform failure context 消费、response build error 映射、response transform error 映射和 Codex proxy error body 分类，`proxy_core_adapter` 不再中转 response build context、Codex Chat streaming decision 或 Codex Chat upstream error response 构造；`proxy_core_adapter::provider_claude_transform_streaming_decision` 只投影 provider 是否 Codex OAuth 并调用 core streaming decision，`proxy_core_adapter::provider_claude_transform_response_for_api_format`/`proxy_core_adapter::provider_claude_transform_sse_for_api_format` 只注入 Gemini tool-call UUID、shadow store 参数和 rectifier 日志回调，`proxy_core_adapter::transform_codex_chat_response_with_history` 集中承接 Codex Chat 非流式 Chat->Responses 转换和 history 记录，`proxy_core_adapter::transform_codex_chat_sse_with_history` 集中承接 Codex Chat 流式 Chat->Responses SSE 转换和 history 记录；后续应按纯逻辑先行、transport adapter 后置的顺序继续抽离。
...
更新：forward response source 需要的纯响应 finalization contract 已补齐到 `proxy-core::api::prelude`：外部中转实现可以只通过 prelude 复用 success response 的 passthrough/buffer/prime 计划、streaming first-chunk/body timeout 文案、upstream error status/body 投影，以及 channel response status/header policy application；host `CcSwitchForwarderResponseSource` 仍只负责具体 `ProxyResponse` transport 读取和 stream priming。

同一 provider 下多个 channel 的行为必须互相隔离：

更新：`proxy::response_adapter` 现在直接经 `proxy_core::api::{domain,transport,events,management,model_catalog,ports,routing,transforms,usage}` 消费纯 core DTO/context，并只作为 Axum/legacy `ProxyResponse` transport bridge、协议 dispatch 和转换响应编排入口；上方 response pipeline 长段中提到的 `proxy_core_adapter::CoreResponseBuildFailureContext` 与 `proxy_core_adapter::AxumResponseBuildErrorContext` 均已过期，response pipeline、response adapter 和 error mapper 直接从 core transport API 引用 response build context。HTTP handler typed JSON 签名 DTO 与 `ProxyState` 均直接来自 owning core/host module，不再经 `proxy_core_adapter` 或 `response_adapter` 二次出口。
...
| 现文件 | 目标位置 | 处理方式 |
| --- | --- | --- |
| `server.rs` | `transport/http/server.rs` + `host/cc_switch/proxy_server.rs` | `ProxyServer` 实现已迁到 `proxy/transport/http/server.rs`，旧 `proxy/server.rs` 已删除；listener binding、route assembly、Hyper accept loop/header-case capture、stop wait/error mapping、shutdown sender/server handle storage、running-state gate、start 编排与 stop 编排已由 `proxy/transport/http/server.rs` 拥有；server started/stopped event、runtime status、listen port 和 `ProxyServerInfo` 组装副作用由 `host/cc_switch/proxy_server.rs` 拥有，`proxy_core_adapter` 不再保留 HTTP lifecycle 或 server runtime side-effect re-export |
| `handlers.rs` | `transport/http/handlers.rs` + `engine` | HTTP handler 实现已迁到 `proxy/transport/http/handlers.rs`，route builder 已导入新路径；typed JSON request/response 签名 DTO 直接来自 `proxy_core::api::{auth,management,model_catalog,ports}`，运行态 `ProxyState` 直接来自 `proxy/host/cc_switch/proxy_state.rs`；旧 `proxy/handlers.rs` 已删除，后续继续把非 HTTP 解析/鉴权/桥接的业务处理下沉到 engine/response adapter owning 边界 |
| `handler_context.rs` | `engine/context.rs` + `host/cc_switch/request_context_provider_source.rs` | `RequestContext` 实现已迁到 `proxy/engine/context.rs`，旧 `proxy/handler_context.rs` 已删除；route result 后的 Provider hydration 已改为 `RequestContextProviderSource` source 注入，CC Switch 默认 DB 实现位于 host 模块，context 只负责消费 source 后回填 usage/provider 事实 |
| `forwarder.rs` | `engine/forward_pipeline.rs` | `RequestForwarder`/`ForwardResult`/`ForwardError` 实现已迁到 `proxy/engine/forward_pipeline.rs`，旧 `proxy/forwarder.rs` 已删除，`proxy/mod.rs` 只保留现有调用需要的精确出口；后续继续切掉残余 Tauri/AppHandle/Database 依赖并收敛为可替换 source/port |
| `provider_router.rs` | `engine/routing.rs` | `ProviderRouter` 与 routing source/store traits 已迁到 `proxy/engine/routing.rs`，旧 `proxy/provider_router.rs` 已删除；当前已通过 router 端 provider/channel/config/health 四个 focused port 注入 source/store，channel route 输入已切到 core `RouteResolveChannelInput`，provider/config/channel source 与 health store 默认实现已迁到 `host/cc_switch/provider_router_*`，并分别通过 `ProviderSource` + `RoutePolicySource` / `CcSwitchConfigSource` / `CcSwitchChannelSource` / core attempt facts 对齐 core `ProviderSource` / `RoutePolicySource` / `ProxyConfigSource` / `ChannelSource` / `ProviderHealthStore`；live breaker map 已由 `ProviderRoutingCircuitRuntime` 承接，下一步评估是否可在不引入 router/store 环的前提下完全实现 core `ChannelHealthStore` |
| `failover_switch.rs` | `host/cc_switch` | `FailoverSwitchManager` 实现已迁到 `proxy/host/cc_switch/failover_switch.rs`，旧 `proxy/failover_switch.rs` 已删除；核心/engine 继续只通过 failover scheduler resource 触发宿主副作用 |
| `response_processor.rs` | `engine/response_pipeline.rs` | response pipeline 实现已迁到 `proxy/engine/response_pipeline.rs`，非流式 decoded body bridge、response log event dispatch、passthrough usage runtime source、transformed streaming/non-streaming usage runtime source、passthrough/transformed usage runtime orchestration、streaming/non-streaming/transformed response usage record builder、forward-error usage helper、reusable sink-spawn helper、streaming/non-streaming passthrough response construction、Claude/Codex transformed SSE stream wrappers、Claude/Codex transformed JSON wrappers、shared logged-stream runtime loop 与 provider usage facts projection 已归入该模块，旧 `proxy/response_processor.rs` 已删除；后续继续收敛 response adapter/error mapper 中仍依赖 adapter 投影的响应错误与 transport 边界 |
| `usage/logger.rs` | `host/cc_switch/database_usage_sink.rs` | DB-backed `UsageLogger`/`RequestLog` 已迁到 `proxy/host/cc_switch/database_usage_sink.rs`，旧 `proxy/usage/logger.rs` 与 `proxy::usage` re-export 已删除；token/cost DTO 与纯计价策略继续通过 core/adapter 入口 |
| `providers/*` | `provider/*` | provider adapter 实现已迁到 `proxy/provider/*`，`proxy_core_adapter` 已引用 `proxy::provider`，旧 `proxy/providers/mod.rs` 已删除；后续继续把 adapter 内仍依赖 host `Provider` 存储形态的策略下沉到 adapter/core 边界 |
...
| `provider_endpoints` / `proxy_channels` 相关 DB 访问 | `host/cc_switch/database_channel_source.rs` | `CcSwitchChannelSource` wrapper 已迁到 `proxy/host/cc_switch/database_channel_source.rs` 并包装 provider 主 URL、`provider_endpoints` 与 materialized channel 的 core-facing `ChannelSource` 入口；spec/list/get、record CRUD/list、key/model 管理、migration preview/materialize helper 和 DB 错误映射已迁入该 host 模块，并直接引用 core channel DTO/trait/helper/error contract；adapter 仅保留旧兼容别名 |
| `proxy_channel_keys` runtime lookup | `host/cc_switch/channel_key_runtime_source.rs` | DB-backed `CcSwitchChannelKeyRuntimeSource`、raw key record 查询、runtime candidate 投影、key-ref/enabled-key core selection 和 DB 错误映射已迁到 `proxy/host/cc_switch/channel_key_runtime_source.rs`；runtime source 已直接引用 `proxy_core::api::{management,ports,errors}` 的 candidate projection/selection helper、`ChannelKeyRuntimeSource` port、config error helper 和 result contract，并通过 core `ChannelKeyRuntimeLookupInput` 承接 channel/key lookup 事实；adapter 不再参与该 source 的 DB error mapping |
| channel test reachability probe | `host/cc_switch/channel_reachability_probe.rs` | DB-backed `CcSwitchChannelReachabilityProbe`、app type parse、provider/config DB 读取、`StreamCheckService` 执行、DB error mapping 和 reachability result 投影已迁到 `proxy/host/cc_switch/channel_reachability_probe.rs`；host probe 直接引用 core reachability probe/request/result/error contract，adapter 不再 re-export `ChannelReachabilityProbe` port、probe DTO 或 DB error helper |
| forward attempt auth-profile source | `host/cc_switch/channel_auth_profile_attempts.rs` | route plan host-provider 过滤、ForwardAttempt 构造、provider auth-profile 替换、channel-key runtime lookup 与 settings patch 调用已迁到 `proxy/host/cc_switch/channel_auth_profile_attempts.rs`；auth-profile application plan、missing-key error、`ChannelKeyRuntimeSource` port、`ChannelKeyRuntimeLookupInput` 与 result contract 已直接引用 `proxy_core::api::{domain,auth,ports,errors}`，forward runtime 与测试直接引用 owning host module，adapter 不再 re-export 该 source helper |
| default auth provider source | `host/cc_switch/auth_provider.rs` | `CcSwitchAuthProvider`、provider-config auth profile helper、route-context auth info helper 与 `provider_with_channel_auth_key` settings patch helper 已迁到 `proxy/host/cc_switch/auth_provider.rs`；auth provider source 已直接引用 `proxy_core::api::{ports,domain,auth}` 的 AuthProvider/AuthInfo、route/profile metadata 和 channel-key settings policy，adapter 仅 re-export host provider/helper |
| managed-account runtime source | `host/cc_switch/managed_account_runtime_source.rs` | `CcSwitchManagedAccountRuntimeSource`、`ManagedAccountRuntimeSource` extension trait、Copilot dynamic endpoint/live model/model vendor lookup、Claude forward api_format Copilot vendor 分流和 Copilot/Codex OAuth token AppHandle 读取已迁到 `proxy/host/cc_switch/managed_account_runtime_source.rs`；runtime source 已直接引用 `proxy_core::api::{auth,model_catalog,transforms}` 的 runtime trait/type、token diagnostics、managed-auth/Copilot helper、`CopilotModel` 和 `resolve_claude_forward_api_format`，并持有按 runtime/account 隔离的最近成功 token 快照；刷新/网络类失败只在 core 30 秒窗口内回退，不可重试账号错误直接暴露，adapter 仅 re-export runtime source/type/test helper，host `ProviderMeta` binding 投影由 `provider_projection` 拥有；`api::prelude` 已暴露外部宿主实现该 trait 所需的 `CopilotModel` 与 managed-auth resolver helper，`public_prelude` / `external_relay_host` 覆盖 runtime token 接入 AuthProvider header |
| forwarder protocol state source | `engine/forward_pipeline.rs` + `host/cc_switch/forwarder_protocol_state_source.rs` | `ForwarderProtocolStateSource`、ref type 与 protocol input DTO 由 `proxy/engine/forward_pipeline.rs` 拥有；`CcSwitchForwarderProtocolStateSource` 默认实现保留在 `proxy/host/cc_switch/forwarder_protocol_state_source.rs`，负责 Gemini shadow store、Codex Chat history enrich 与 Claude/Gemini transform replay；adapter 不再保留 protocol state contract alias，生产/测试构造器直接引用 owning module 或由 adapter 私有组装 |
| forwarder attempt runtime source | `engine/forward_pipeline.rs` + `host/cc_switch/forwarder_attempt_runtime_source.rs` | `ForwarderAttemptRuntimeSource`、ref type、allow/success/failure/neutral-release input DTO 与 allow decision 由 `proxy/engine/forward_pipeline.rs` 拥有；`CcSwitchForwarderAttemptRuntimeSource` 默认实现保留在 `proxy/host/cc_switch/forwarder_attempt_runtime_source.rs`，负责 ProviderRouter-backed attempt allow、max-attempt/circuit-bypass 默认决策、成功/失败健康记录和 neutral permit 释放；max-attempt/circuit-bypass decision helper/input 已直接引用 `proxy_core::api::transport`，ProviderRouter permit/health 副作用 helper 也由该 host source 私有持有，adapter 不再保留 attempt runtime contract alias 或副作用 façade |
| forwarder runtime state source | `engine/forward_pipeline.rs` + `host/cc_switch/forwarder_runtime_state_source.rs` | `ForwarderRuntimeStateSource`、ref type、failure decision enums 与 `ActiveConnectionGuard` 由 `proxy/engine/forward_pipeline.rs` 拥有；`CcSwitchForwarderRuntimeStateSource` 默认实现保留在 `proxy/host/cc_switch/forwarder_runtime_state_source.rs`，负责 runtime status、active route target、event bus、active connection lifecycle、forward success/failure 状态和日志副作用；adapter 不再保留 runtime state contract alias，factory 由 host 模块直接提供给构造器 |
| forwarder request source | `engine/forward_pipeline.rs` + `host/cc_switch/forwarder_request_source.rs` | `ForwarderRequestSource`、ref type 与 request-source input/output contract 由 `proxy/engine/forward_pipeline.rs` 拥有；`CcSwitchForwarderRequestSource` 默认实现保留在 `proxy/host/cc_switch/forwarder_request_source.rs`，负责请求体预处理、provider/Codex/Copilot 转换、URL planning、media/rectifier plan、upstream body/header 组装和请求日志；request source 已直接引用 `proxy_core::api::{transport,transforms}` 的纯 helper/type，Copilot live model、动态 base URL 和 Claude api_format 只委托 `ForwarderAdapterContext` 执行 binding facts 投影与 managed-account runtime 调用，adapter 不再保留 request-source contract alias 或 re-export factory/test helper |
| failover switch scheduler | `engine/forward_pipeline.rs` + `host/cc_switch/failover_switch.rs` | `FailoverSwitchScheduler`、ref type 与 `ForwarderFailoverSwitchTarget` 由 `proxy/engine/forward_pipeline.rs` 拥有；`CcSwitchFailoverSwitchScheduler` 默认实现保留在 `proxy/host/cc_switch/failover_switch.rs`，负责把 forwarder 的切换目标投影到 `FailoverSwitchManager`、Tauri AppHandle、托盘/UI 和 Live hot-switch 副作用；adapter 不再保留 scheduler contract alias，noop 测试 source 也留在 owning host 模块 |
| event sink | `host/cc_switch/event_sink.rs` + `proxy/events.rs` | `CcSwitchEventSink` 默认实现已迁到 `proxy/host/cc_switch/event_sink.rs`，负责直接引用 core event sink contract 并把 `ProxyCoreEvent` 分发到可选 `ProxyEventBus`；`ProxyEventBus::emit_core_event` 拥有 core event 到 host event envelope 的投影，adapter 不再保留 event bus 投影/分发 helper 或 source re-export |
| usage sink | `host/cc_switch/database_usage_sink.rs` | `CcSwitchUsageSink` 默认实现已迁到 `proxy/host/cc_switch/database_usage_sink.rs`，直接引用 core `UsageSink` / result / usage DTO contract，负责 `UsageLogger`、pricing lookup、`UsageRecord` 到 `RequestLog` 投影、缺失定价 warning、`RequestLog` 写入和 DB 持久化；adapter 不再保留 usage projection helper，也不再 re-export `UsageSink` port trait |
| config source | `host/cc_switch/config_source.rs` | `CcSwitchConfigSource` 默认实现已迁到 `proxy/host/cc_switch/config_source.rs`，直接引用 core config/source/result contract，负责 global/app/app-summary/runtime config、settings current-provider 和 app catalog 端口实现；adapter 不再保留 config-source-only DB helper，也不再 re-export `ProxyConfigSource` / `AppSummaryConfig` / `CcSwitchConfigSource` |
...
| Claude Desktop gateway auth source | `host/cc_switch/claude_desktop_gateway_auth_source.rs` | `CcSwitchClaudeDesktopGatewayAuthSource` 默认实现已迁到 `proxy/host/cc_switch/claude_desktop_gateway_auth_source.rs`，负责直接引用 core gateway auth contract，并通过 host-owned DB token helper 加载 gateway bearer token、映射 core auth 错误；adapter 不再保留 token helper、默认 source re-export 或 bearer validation fixture |
| Claude Desktop provider projection | `host/cc_switch/claude_desktop_provider.rs` | Claude Desktop provider 导入决策、直连/本地路由校验、gateway profile provider 投影、proxy request body 映射、status facts 和 provider mode 默认值已迁到 `proxy/host/cc_switch/claude_desktop_provider.rs`，直接引用 core auth/profile/model-route contract；adapter 不再保留 Claude Desktop provider projection facade |
| provider source / projection | `host/cc_switch/provider_source.rs` + `host/cc_switch/provider_projection.rs` | `CcSwitchProviderSource` 默认实现已迁到 `proxy/host/cc_switch/provider_source.rs`，直接引用 core provider/source/result contract，负责 provider list/get/current-provider、active route target 和 route candidate provider ids；CC Switch `Provider` 到 `ProviderSpec` 的 metadata/accountRef/provider kind 投影、provider list/get DB source、managed-account binding context、GitHub Copilot account id 投影、Claude api_format/transform gate/streaming decision provider fact 投影、Codex Responses→Chat gate provider fact 投影和 Anthropic rectifier provider fact 投影由 `proxy/host/cc_switch/provider_projection.rs` 拥有；adapter 不再保留 provider source type、provider spec projection、managed-account provider projection、Claude api_format/transform gate/streaming decision projection、Codex response conversion projection、rectifier provider fact projection、current/active-route/route-candidate projection helper 或 `ProviderSource` port trait re-export |
| model catalog provider | `host/cc_switch/model_catalog_provider.rs` | `CcSwitchModelCatalogProvider` 默认实现已迁到 `proxy/host/cc_switch/model_catalog_provider.rs`，直接引用 core model catalog port/DTO/request/result contract，负责 provider/client/Claude Desktop model catalog 端口实现、Claude Desktop provider selection 错误映射和 route input 转换；Claude Desktop proxy model route projection 复用 host-owned `claude_desktop_provider`，adapter 不再 re-export `ModelCatalogProvider` port trait 或保留该投影 facade |
| channel health store | `host/cc_switch/channel_health_store.rs` | `CcSwitchChannelHealthStore` 默认实现已迁到 `proxy/host/cc_switch/channel_health_store.rs`，负责 channel health attempt 写入、channel breaker reset、stats 查询和 DB/router error mapping，并拥有 attempt/reset/stats projection helper；reset/stats 端口已通过 core `ChannelHealthLookupInput` 承接 channel lookup 事实，adapter 不再保留 channel health store helper、error helper、factory 装配或 re-export |
| route policy source | `host/cc_switch/route_policy_source.rs` | `CcSwitchRoutePolicySource` 默认实现已迁到 `proxy/host/cc_switch/route_policy_source.rs`，直接引用 core route-policy/source/result/error contract，负责读取 failover queue 并生成 core-facing `RoutePolicySource`；adapter 不再保留 queue-to-policy projection helper、error helper，也不再 re-export `RoutePolicySource` port trait 或 `CcSwitchRoutePolicySource` host type，默认装配由 host services / ProviderRouter sources 直接引用 host 模块 |
...
| ProviderRouter provider source | `host/cc_switch/provider_router_provider_source.rs` | `CcSwitchProviderRouterProviderSource` 默认实现已迁到 `proxy/host/cc_switch/provider_router_provider_source.rs`，直接引用 core provider/source/result、route-policy/source 和 provider-selection contract 与 host `CcSwitchRoutePolicySource`，负责包装 DB provider source 并在 host 模块内生成 ProviderRouter provider/current/failover inputs；adapter 不再保留 ProviderRouter provider source helper，也不再 re-export `ProviderSource` 或 `CcSwitchRoutePolicySource`，默认装配由 `provider_router_sources.rs` 完成 |
| ProviderRouter channel source | `host/cc_switch/provider_router_channel_source.rs` | `CcSwitchProviderRouterChannelSource` 默认实现已迁到 `proxy/host/cc_switch/provider_router_channel_source.rs`，直接引用 core `ChannelSource`、`ChannelRouteSource`、`RouteResolveChannelInput` 与 record-to-route-input helper，负责包装 `CcSwitchChannelSource` 并生成 ProviderRouter route inputs；adapter 不再保留该 route-input projection helper，也不再作为该 source 的 route DTO facade，默认装配由 `provider_router_sources.rs` 完成 |
| ProviderRouter config source | `host/cc_switch/provider_router_config_source.rs` | `CcSwitchProviderRouterConfigSource` 默认实现已迁到 `proxy/host/cc_switch/provider_router_config_source.rs`，直接引用 host `CcSwitchConfigSource`、core `ProxyConfigSource`、`CircuitBreakerConfig` 与 provider selection contract，负责包装 config source 并生成 ProviderRouter auto-failover、circuit breaker 和 failure threshold config；adapter 不再保留 router config source helper，也不再 re-export `CcSwitchConfigSource`，默认装配由 `provider_router_sources.rs` 完成 |
| ProviderRouter health store | `host/cc_switch/provider_router_health_store.rs` | `CcSwitchProviderRouterHealthStore` 默认实现已迁到 `proxy/host/cc_switch/provider_router_health_store.rs`，负责 provider/channel health write、channel reset side effect 和 DB error mapping，并拥有 provider DB update projection、router-facing channel health wrapper 与 channel reset wrapper；router-facing channel health wrapper 复用 host `channel_health_store` 的 DB projection helper，默认装配由 `provider_router_sources.rs` 完成 |
| ProviderRouter source assembly | `host/cc_switch/provider_router_sources.rs` | `CcSwitchProviderRouterSources` 与 `provider_router_from_database` 默认工厂已迁到 `proxy/host/cc_switch/provider_router_sources.rs`，负责组装 ProviderRouter provider/channel/config/health 四个端口；adapter 仅 re-export `provider_router_from_database` |
| ProviderRouter circuit runtime | `host/cc_switch/provider_router_circuit_runtime.rs` | ProviderRouter 运行时熔断配置热更新、provider breaker reset 与 provider breaker stats 查询已迁到 `proxy/host/cc_switch/provider_router_circuit_runtime.rs`；HTTP server 公开方法直接调用 host helper，adapter 不再保留这组运行时副作用 helper |
| provider adapter context | `host/cc_switch/provider_adapter_context.rs` | `ForwarderAdapterContext`、provider adapter trait object/factory、provider auth info/header fallback 调用、managed-account binding projection、base URL facts、transform gate/action 和 upstream URL assembly 已迁到 `proxy/host/cc_switch/provider_adapter_context.rs`；provider URL facts projection helper/input 已直接引用 `proxy_core::api::transport`，fallback auth、Copilot live model、动态 base URL 和 Claude api_format 入口都消费 `ManagedAccountRuntimeSource` 并由 context 统一投影 binding facts，fallback auth 返回结构化 headers/session/account facts，forward pipeline/request/auth source 直接引用 host context，adapter 仅在自身装配中私有引用 context/factory 并保留当前仍需的 facts type alias；token runtime、channel-key weighted/random/round-robin 和失败冷却已由对应 runtime source/core policy 承接；`external_relay_host` 与 `public_prelude` 已覆盖外部宿主通过 prelude 实现 per-channel 显式 AuthProvider header 和 managed-account runtime token header，`cargo test --example external_relay_host` 已覆盖外部示例 runtime smoke，`ProxyServer::start` 级 smoke 已覆盖真实 HTTP listener 到本机 upstream 的 materialized channel 转发、同 provider 多 channel 失败重试、wildcard key cooldown 与 wildcard key round-robin 闭环 |
| forwarder auth source | `engine/forward_pipeline.rs` + `host/cc_switch/forwarder_auth_source.rs` | `ForwarderAuthSource`、ref type 与 auth headers input 由 `proxy/engine/forward_pipeline.rs` 拥有；`CcSwitchForwarderAuthSource` 默认实现保留在 `proxy/host/cc_switch/forwarder_auth_source.rs`，负责 AuthProvider 显式 header/fallback 分支选择、Copilot auth optimization 准备和上游 auth header finalization；provider settings auth、managed-account auth resolution 和 Codex OAuth account/session facts 已下沉到 `ForwarderAdapterContext::resolve_provider_fallback_auth_headers`；auth source 已直接引用 `proxy_core::api::{domain,routing,transport}` 的纯 auth/header helper，adapter 不再保留 auth source contract alias，仅保留 host Provider/error 投影；外部宿主 AuthProvider 显式 header 示例、示例 runtime smoke、真实端到端转发 smoke、channel 失败重试 smoke、wildcard key cooldown smoke 和 wildcard key round-robin smoke 已固化 |
| forwarder transport source | `engine/forward_pipeline.rs` + `host/cc_switch/forwarder_transport_source.rs` | `ForwarderTransportSource`、`ForwarderUpstreamTransportRequest` 与 ref type 由 `proxy/engine/forward_pipeline.rs` 拥有；`CcSwitchForwarderTransportSource` 默认实现保留在 `proxy/host/cc_switch/forwarder_transport_source.rs` 并委托 `proxy::transport::upstream::send_request`；upstream transport module 已直接引用 `proxy_core::api::transport` 的 send-policy/diagnostic helper，adapter 不再保留 transport contract alias |
| forwarder response source | `engine/forward_pipeline.rs` + `host/cc_switch/forwarder_response_source.rs` | `ForwarderResponseSource`、status mapping input、finalization input 与 ref type 由 `proxy/engine/forward_pipeline.rs` 拥有；`CcSwitchForwarderResponseSource` 默认实现保留在 `proxy/host/cc_switch/forwarder_response_source.rs`，负责响应成功就绪、流式首包预读、非成功 upstream error 投影和 channel response status mapping；response source 已直接引用 `proxy_core::api::transport` 的纯响应 helper，adapter 不再保留 response contract alias |
...
1. `CcSwitchConfigSource` 作为 host-owned 端口直接消费 core config/source/result contract，包装现有 DB/settings 读取和 app catalog 事实；adapter 不再保留 config-source-only DB helper，也不再 re-export 默认 source host type。
2. `CcSwitchProviderSource` 作为 host-owned source 直接消费 core provider/source/result contract，包装 provider/current provider、active route target 和 route candidate provider ids；adapter 只保留投影 helper 与默认 source type 装配，不再 re-export `ProviderSource` port trait。
3. `CcSwitchChannelSource` 作为 host-owned 端口直接消费 core channel contract，包装 provider 主 URL、`provider_endpoints` 和未来 channel 表读取，并自行处理 DB read/write 错误到 core error 的映射；adapter 只保留旧兼容别名。
4. `CcSwitchRoutePolicySource` 作为 host-owned source 直接消费 core route-policy/source/result contract，包装 failover queue 到 core `RoutePolicy` 的默认桥接；adapter 只保留投影 helper，不再 re-export `RoutePolicySource` port trait 或 `CcSwitchRoutePolicySource` host type；`CcSwitchRouteResolver` 作为 host-owned resolver 直接调用 core `build_route_plan` 并仅通过 adapter helper 包装 management dry-run route response，由 adapter re-export/装配。
5. `CcSwitchChannelHealthStore` 作为 host-owned store 包装 channel health attempt 写入、breaker reset 和 stats 查询，并由 host services 直接装配；兼容期可同时写 provider health 聚合。
6. `CcSwitchUsageSink` 作为 host-owned source 直接消费 core usage sink/result/DTO contract，包装 `UsageLogger`、pricing lookup、host `RequestLog` 投影和日志落库副作用；写入必须使用完整 `UsageRecord`，不能用简化 hint 直接写账单。
...
10. `CcSwitchClaudeDesktopGatewayAuthSource` 作为 host-owned source 包装默认 Claude Desktop gateway bearer token source，直接消费 core gateway auth contract，并拥有 gateway token DB helper；profile apply/status/server smoke 直接复用该 host source，adapter 不再保留 token helper。
11. `CcSwitchProviderRouterProviderSource` 作为 host-owned source 直接消费 core provider/source/result、route-policy/source 和 provider-selection contract，并直接引用 host `CcSwitchRoutePolicySource`，在 host 模块内包装 DB provider source 到 ProviderRouter provider/current/failover inputs 的默认桥接，由 host-owned `CcSwitchProviderRouterSources` 装配。
12. `CcSwitchProviderRouterChannelSource` 作为 host-owned source 直接消费 core route input/source contract，包装 `CcSwitchChannelSource` 到 ProviderRouter route inputs 的默认桥接，并由 host-owned `CcSwitchProviderRouterSources` 装配。
13. `CcSwitchProviderRouterConfigSource` 作为 host-owned source 直接引用 host `CcSwitchConfigSource` 与 core `CircuitBreakerConfig`，包装 config source 到 ProviderRouter config inputs 的默认桥接，并由 host-owned `CcSwitchProviderRouterSources` 装配。
...
16. `CcSwitchAuthProvider` 作为 host-owned source 包装 provider config auth profile 到 `AuthInfo` 的默认投影，并由 adapter re-export；`CcSwitchManagedAccountRuntimeSource` 作为 host-owned source 包装 Codex/Copilot OAuth token runtime 和短窗口失败回退；`CcSwitchForwarderProtocolStateSource` 作为 host-owned source 包装默认协议状态副作用；`CcSwitchForwarderAttemptRuntimeSource` 作为 host-owned source 包装默认 attempt allow/health/circuit runtime；`CcSwitchForwarderRuntimeStateSource` 作为 host-owned source 包装默认 runtime status/current-target/event 副作用；`ForwarderAdapterContext` 作为 host-owned context 包装 provider adapter trait 调用；`CcSwitchForwarderAuthSource` 作为 host-owned source 包装默认上游 auth header 组装；`CcSwitchForwarderTransportSource` 作为 host-owned source 包装默认上游 transport delegate；`CcSwitchForwarderResponseSource` 作为 host-owned source 包装默认上游 response finalization；`CcSwitchFailoverSwitchScheduler` 作为 host-owned source 包装默认 failover switch 调度副作用；host forward bridge 已由 `CcSwitchProxyRuntime` 拥有，channel-key weighted/random/round-robin、失败冷却和 token runtime 策略已由 core policy + host runtime source 渐进承接。
...
1. HTTP handler 只负责触发 transport adapter 读取 body、鉴权入口、事件流入口、本地 health/status/Claude Desktop model-list/app-list/provider-list/app-models/client-model-catalog JSON、route inspection JSON、migration/breaker JSON、channel mutation JSON 和管理 API path/query/body/state 转发；管理 API auth validation 由 `auth_adapter` 承接，协议入口的 app/tag/prefix/endpoint metadata、endpoint path/query bridge、`RequestContext` 初始化、`ProxyRequest` 构造、`ProxyEngine::handle` dispatch 与 `ProxyResult` 到 HTTP response 的宿主 bridge 由 adapter 承接；Claude/Codex/Gemini protocol handlers 已先降为 `response_adapter` 编排入口调用，`/proxy/v1/events` 的 SSE transport bridge 由 HTTP handler 拥有。
...
| OAuth token 刷新被移出 forwarder 后时序改变 | Copilot/Codex OAuth 请求失败 | `managed_account_auth` 覆盖非托管透传、缺少 AppHandle、provider account 绑定、Codex account 回传、session header gate、短窗口 token 快照失败回退和不可重试错误不回退；host runtime source 测试覆盖最近成功 token 回退与过期/不可重试失败直出 |
...
- host-owned `CcSwitchProviderSource` 与现有 DB provider 表兼容，可以直接依赖 core provider/source/result contract，并通过 adapter helper 暴露 current provider、active route target 和 route candidate provider ids；adapter 不再 re-export `ProviderSource` port trait。
- host-owned `CcSwitchConfigSource` 与现有 DB/settings 配置兼容，直接依赖 core config/source/result contract，自身暴露 app catalog、global/app/app-summary/runtime config 和 settings current-provider 事实；adapter 不再 re-export config source host type，也不再保留 config-source-only DB helper。
- host-owned `CcSwitchChannelSource` 可以从 provider 主 URL、`provider_endpoints` 和新 channel 表生成一致候选，并直接依赖 core channel/error contract；adapter 只保留旧兼容别名。
- 旧配置迁移 dry-run 可以输出新增、重复、需人工确认的 channel。
- host-owned `CcSwitchUsageSink` 直接依赖 core usage sink/result/DTO contract，写入 `proxy_request_logs` 字段完整；adapter 不再保留 usage projection helper，也不再 re-export `UsageSink` port trait。
- host-owned `CcSwitchEventSink` 可以直接消费 core event sink contract 并驱动托盘和前端事件，`ProxyEventBus::emit_core_event` 统一处理 core event 到 host event envelope 的投影，adapter 不再保留 event bus helper 或 sink re-export。
...
- host-owned `CcSwitchClaudeDesktopGatewayAuthSource` 可以直接消费 core gateway auth contract，并通过自身 DB helper 加载/创建 gateway bearer token，adapter 不再保留 gateway token source helper。
- host-owned Claude Desktop provider projection 可以直接消费 core auth/profile/model-route contract，并为 config、command、forwarder request source 和 model catalog provider 提供 Provider-shaped 导入、校验、profile、request body 与 status facts；adapter 不再暴露 Claude Desktop provider projection facade。
- host-owned `CcSwitchChannelHealthStore` 可以在 owning module 内写入 channel attempt、reset breaker 并读取 breaker stats，adapter 不再保留 channel health projection helper 或装配入口。
- host-owned `CcSwitchRoutePolicySource` 可以直接依赖 core route-policy/source/result/error contract，读取 failover queue 并通过 core helper 生成 core-facing `RoutePolicy`；adapter 不再保留 projection/error helper，也不再 re-export `RoutePolicySource` port trait 或 `CcSwitchRoutePolicySource` host type。
- host-owned `CcSwitchRouteResolver` 可以直接通过 core `build_route_plan` 生成 runtime route plan，并通过 adapter helper 生成 management dry-run route response；adapter 只保留 management projection helper 和装配。
- host-owned `CcSwitchProviderRouterProviderSource` 可以通过 core-facing `ProviderSource` 与 `RoutePolicySource` 生成 ProviderRouter provider/current/failover inputs，并直接引用 host `CcSwitchRoutePolicySource`；adapter 不再保留 ProviderRouter provider source helper，也不再 re-export `ProviderSource` 或 `CcSwitchRoutePolicySource`。
- host-owned `CcSwitchProviderRouterChannelSource` 可以通过 core-facing `ChannelSource` 生成 ProviderRouter route inputs，并直接引用 core `ChannelRouteSource` / `RouteResolveChannelInput`；adapter 不再保留 route-input projection helper，也不再作为该 source 的 route DTO facade。
- host-owned `CcSwitchProviderRouterConfigSource` 可以通过 core-facing `ProxyConfigSource` 生成 ProviderRouter config inputs，并直接引用 host `CcSwitchConfigSource` 与 core `CircuitBreakerConfig`；adapter 只保留 projection helper，不再 re-export config source host type。
- host-owned `CcSwitchProviderRouterHealthStore` 可以通过 core attempt/reset facts 写入 ProviderRouter health side effect，并在 owning module 内拥有 provider DB update projection、router-facing channel health wrapper、channel reset wrapper 与 DB error mapping；adapter 不再保留 ProviderRouter 或 ChannelHealthStore health projection/error helper。
...
- host-owned `CcSwitchModelCatalogProvider` 可以直接依赖 core model catalog contract，并通过 host Claude Desktop provider projection 加载 provider/client/Claude Desktop model catalog；adapter 不再 re-export `ModelCatalogProvider` port trait 或 Claude Desktop model route projection facade。
