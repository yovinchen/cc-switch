...
本轮继续把 Codex Responses→Chat 上游请求体转换收敛到 `ForwarderRequestSource`：Codex Chat 上游模型覆写、reasoning options 投影、OpenAI o-series/max token 分流和 Responses body 到 Chat Completions body 的纯转换都由 request source 完成；`RequestForwarder` 只保留 protocol source 的 history enrich 和转换分支选择。
