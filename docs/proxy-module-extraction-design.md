...
669. channel test 管理路由的 preflight 规划入口已新增 `proxy_core_adapter::channel_test_plan_from_record`：handler 不再直接执行 `ProxyChannelRecord -> ChannelSpec -> plan_channel_test` 投影，只保留 provider/config 查询和 reachability probe 副作用。
...
672. health/status/app list 基础管理路由的 source 组装已新增 `proxy_core_adapter::health_check_source_from_timestamp`、`proxy_status_source_from_status` 与 `app_list_source_from_summaries`：handler 不再直接调用这些 response source DTO。
