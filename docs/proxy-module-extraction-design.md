...
本轮继续把 Claude provider `api_format` 投影从 `proxy_core_adapter` 收敛到 host-owned `provider_projection`：Claude provider adapter 的 `claude_provider_api_format` 仅作为兼容薄壳委托 `provider_projection::provider_claude_api_format`，RequestContext、forwarder request source 与 adapter 内部 remaining bridge 可继续消费旧 helper 名称，`proxy_core_adapter` 不再定义 `provider_claude_api_format` façade；边界测试禁止 `provider_projection` 反向依赖 provider adapter 或旧 adapter façade 回流。
