...
708. ChannelHealthStore 的 attempt 写库参数投影已移入 `proxy_core_adapter::channel_health_attempt_db_update`，host-owned `CcSwitchChannelHealthStore` 负责调用 DB 写入；ProviderHealthStore 的 raw DB update 投影已收成 adapter 私有 helper，公开入口保留 `record_provider_attempt_in_db_source`，`proxy_core_host` 只装配对应 store。
...
1183. `CcSwitchProviderRouterHealthStore` 已迁入 `proxy/host/cc_switch/provider_router_health_store.rs`：该 host 模块拥有 ProviderRouter 的 provider/channel health write 和 channel reset side effect，继续复用 adapter 的 provider/channel attempt projection helper，`proxy_core_adapter` 只保留 helper。
