...
693. active route target DTO 构造已迁入 `proxy-core::current_route_target_from_input`：host adapter 只把 `ForwardAttempt` 投影为 core input，forwarder 不再手写 `CurrentRouteTarget` 的 provider/channel/model 字段拷贝。
