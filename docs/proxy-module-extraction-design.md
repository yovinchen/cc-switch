...
704. app config source 的当前 provider settings 读取与 `ProxyAppConfig` parts 组装已收成 `proxy_core_adapter` 私有 helper，公开入口保留 `proxy_app_config_from_config_source` / `proxy_app_config_from_db_source`：host-owned `CcSwitchConfigSource` 保留 DB 配置和优化器配置读取，`proxy_core_host` 只装配 source。
...
本轮后续已把 `CcSwitchConfigSource` 的 global/app/summary/runtime 配置 DB 读取和 core DTO 投影收敛到 host-owned source wrapper；该模块直接消费 core config/source/result contract，adapter 只保留 config helper 和装配入口，不再 re-export `CcSwitchConfigSource` host type。
...
1033a. 本轮继续移除 `proxy_core_adapter` 对 adapter-only config projection helper 的 re-export：`proxy_app_config_from_parts` 与 `proxy_global_config_from_global_config` 只在 adapter 内私有导入，host-facing 出口仍是 DB/config-source aware wrapper。
...
1190. `CcSwitchConfigSource` 已迁入 `proxy/host/cc_switch/config_source.rs`：该 host 模块拥有默认 `ProxyConfigSource` 实现，直接引用 core `AppKind` / `ProxyCoreResult` / `ProxyConfigSource` / `AppSummaryConfig` / config DTO contract，包装 DB 并复用 adapter 的 app catalog、global/app/app-summary/runtime config helper；`proxy_core_adapter` 不再 re-export `ProxyConfigSource`、`AppSummaryConfig` 或 `CcSwitchConfigSource` host type，只保留 helper 和装配入口。
...
默认 `CcSwitchConfigSource` 已迁入 `proxy/host/cc_switch/config_source.rs`：host source 持有 DB，直接消费 core config/source/result contract，继续调用 adapter-owned `cc_switch_app_kinds`、`proxy_global_config_from_db_source`、`proxy_app_config_from_db_source`、`app_summary_config_from_db_source` 和 `proxy_runtime_config_from_db_source`；adapter 保留 config projection helper 和装配入口，但不再 re-export `ProxyConfigSource`、`AppSummaryConfig` 或 `CcSwitchConfigSource` host type。
...
| config source | `host/cc_switch/config_source.rs` | `CcSwitchConfigSource` 默认实现已迁到 `proxy/host/cc_switch/config_source.rs`，直接引用 core config/source/result contract，负责 global/app/app-summary/runtime config 和 app catalog 端口实现；adapter 仅保留 config projection helper 和 factory 装配，不再 re-export `ProxyConfigSource` / `AppSummaryConfig` / `CcSwitchConfigSource` |
...
1. `CcSwitchConfigSource` 作为 host-owned 端口直接消费 core config/source/result contract，包装现有 DB/settings 读取；adapter 只保留 config projection helper 和装配入口，不再 re-export 默认 source host type。
...
- host-owned `CcSwitchConfigSource` 与现有 DB/settings 配置兼容，直接依赖 core config/source/result contract，并通过 adapter helper 暴露 app catalog、global/app/app-summary/runtime config；adapter 不再 re-export config source host type。
