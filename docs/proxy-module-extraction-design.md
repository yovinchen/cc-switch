...
615. 默认 `CcSwitchForwarderRuntimeStateSource` 已直接引用 `proxy_core::api::transport` 的 forward failure 分类、rectifier retry failover 判断和 terminal/no-available status 文案 helper；`proxy_core_adapter` 不再 re-export 这些纯 runtime failure helper，只保留需要 host `ProxyError` 投影与日志前缀的 bridge helper。
...
1297. `host/cc_switch/forwarder_runtime_state_source` 不再通过 `proxy_core_adapter` 获取事件/运行态 DTO 或低层状态 helper：`AttemptEventPhase`/`ProxyCoreEvent` 直接来自 `proxy-core::api::events`，`CurrentRouteTarget`、`ProxyRuntimeStatus` 和 `record_forward_*_status` 直接来自 `proxy-core::api::ports`；adapter 仅保留 `ProxyError -> ForwardFailureKind` 投影与 CC Switch 日志行 bridge，边界测试防止 runtime state source contracts 经 adapter alias 回流。
