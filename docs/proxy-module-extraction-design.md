...
1168. `CcSwitchForwarderAuthSource`、上游 auth header finalization、AuthProvider 显式 header/fallback 分支、managed-account auth resolution 和 Copilot auth optimization 准备已迁入 `proxy/host/cc_switch/forwarder_auth_source.rs`：该 host 模块拥有 CC Switch 默认 forwarder auth source，`proxy_core_adapter` 只保留 `ForwarderAuthSource` trait/input/type alias 与 host Provider/error 投影，并在 runtime 装配和 adapter-local 测试中私有引用 host factory/helper。
...
1204. forwarder transport/response/failover/protocol/attempt/runtime source 的默认 factory 与测试 helper 已从 adapter re-export 收口为私有 use 或直接 host 模块引用：transport/response/failover scheduler/protocol state/attempt runtime/runtime state contract 已归入 `proxy/engine/forward_pipeline.rs`，`proxy_core_adapter` 仅继续保留尚未迁出的 auth/request source trait/input/type alias，且不再暴露 cc-switch host source factory。
...
| forwarder auth source | `host/cc_switch/forwarder_auth_source.rs` | `CcSwitchForwarderAuthSource`、AuthProvider 显式 header/fallback 分支、managed-account auth resolution、Copilot auth optimization 准备和上游 auth header finalization 已迁到 `proxy/host/cc_switch/forwarder_auth_source.rs`；auth source 已直接引用 `proxy_core::api::{domain,routing,transport}` 的纯 auth/header helper，adapter 仅保留 trait/input/type alias 与 host Provider/error 投影，并在 runtime 装配和 adapter-local 测试中私有引用 host factory/helper，后续继续补 token 缓存/刷新、channel-key 轮询/随机和失败回退策略 |
