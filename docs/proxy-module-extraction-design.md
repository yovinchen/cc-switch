...
1181. `CcSwitchProviderRouterChannelSource` 已迁入 `proxy/host/cc_switch/provider_router_channel_source.rs`：该 host 模块拥有 ProviderRouter 的 channel route input source，包装 `CcSwitchChannelSource` 并复用 adapter 的 route-input projection helper，`proxy_core_adapter` 只保留 helper 和默认 source re-export/装配。
1182. `CcSwitchProviderRouterConfigSource` 已迁入 `proxy/host/cc_switch/provider_router_config_source.rs`：该 host 模块拥有 ProviderRouter 的 auto-failover、circuit breaker config 和 failure threshold source，包装 `CcSwitchConfigSource` 并复用 adapter 的 config projection helper，`proxy_core_adapter` 只保留 helper 和默认 source re-export/装配。
1183. `CcSwitchProviderRouterHealthStore` 已迁入 `proxy/host/cc_switch/provider_router_health_store.rs`：该 host 模块拥有 ProviderRouter 的 provider/channel health write 和 channel reset side effect，继续复用 adapter 的 provider/channel attempt projection helper，`proxy_core_adapter` 只保留 helper 和默认 store re-export/装配。
...
| ProviderRouter channel source | `host/cc_switch/provider_router_channel_source.rs` | `CcSwitchProviderRouterChannelSource` 默认实现已迁到 `proxy/host/cc_switch/provider_router_channel_source.rs`，负责包装 `CcSwitchChannelSource` 并生成 ProviderRouter route inputs；adapter 仅保留 route-input projection helper、factory 装配和 re-export |
...
| ProviderRouter health store | `host/cc_switch/provider_router_health_store.rs` | `CcSwitchProviderRouterHealthStore` 默认实现已迁到 `proxy/host/cc_switch/provider_router_health_store.rs`，负责 provider/channel health write 与 channel reset side effect；adapter 仅保留 attempt/reset projection helper、factory 装配和 re-export |
...
11. `CcSwitchProviderRouterProviderSource` 作为 host-owned source 包装 DB provider source 与 `CcSwitchRoutePolicySource` 到 ProviderRouter provider/current/failover inputs 的默认桥接，并由 adapter re-export/装配。
12. `CcSwitchProviderRouterChannelSource` 作为 host-owned source 包装 `CcSwitchChannelSource` 到 ProviderRouter route inputs 的默认桥接，并由 adapter re-export/装配。
13. `CcSwitchProviderRouterConfigSource` 作为 host-owned source 包装 `CcSwitchConfigSource` 到 ProviderRouter config inputs 的默认桥接，并由 adapter re-export/装配。
14. `CcSwitchProviderRouterHealthStore` 作为 host-owned store 包装 ProviderRouter provider/channel health side effect，并由 adapter re-export/装配。
...
- host-owned `CcSwitchProviderRouterProviderSource` 可以通过 core-facing `ProviderSource` 与 `RoutePolicySource` 生成 ProviderRouter provider/current/failover inputs，adapter 只保留 projection helper 和装配。
- host-owned `CcSwitchProviderRouterChannelSource` 可以通过 core-facing `ChannelSource` 生成 ProviderRouter route inputs，adapter 只保留 projection helper 和装配。
- host-owned `CcSwitchProviderRouterConfigSource` 可以通过 core-facing `ProxyConfigSource` 生成 ProviderRouter config inputs，adapter 只保留 projection helper 和装配。
- host-owned `CcSwitchProviderRouterHealthStore` 可以通过 core attempt/reset facts 写入 ProviderRouter health side effect，adapter 只保留 projection helper 和装配。
