...
1154. Claude Desktop model catalog route source 已改为在 `proxy_core_adapter::claude_desktop_model_routes_from_router_source` 内直接调用 `provider_claude_desktop_proxy_model_routes`；adapter 不再通过 host `claude_desktop_config::proxy_model_routes` 或 `ResolvedModelRoute` DTO 做 route projection，host `proxy_model_routes` wrapper 已删除，边界测试防止 model route source 重新形成 adapter 到 host config 的反向依赖。
