...
1126. Provider custom User-Agent policy 已迁入 `proxy-core::request_headers::provider_custom_user_agent_header`；host adapter 只投影 `ProviderMeta.custom_user_agent` raw 值与 Copilot upstream fact，core 统一执行空值 trim、`HeaderValue` 校验和 GitHub Copilot 禁用自定义 UA 规则，边界测试防止 `proxy_core_adapter` 重新写 `is_copilot` 分支或调用 host `ProviderMeta::custom_user_agent_header` 解析入口。
