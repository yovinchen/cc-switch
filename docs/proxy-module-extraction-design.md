...
本轮继续把 Claude transformed JSON 的 provider response transform、Gemini shadow/provider/session/tool hints 注入和 transformed usage record 包装收敛到 `proxy_core_adapter::claude_transformed_json_response_from_context`；非流式 upstream body read、JSON/SSE fallback parse、transform error 映射和 Axum bridge 由 `proxy::response_adapter::claude_transformed_upstream_json_response_to_axum_response` 承接。
...
本轮继续把 Codex Chat→Responses transformed JSON 的 history-recording response transform 和 transformed usage record 包装收敛到 `proxy_core_adapter::codex_auto_transformed_json_response_from_context`；非流式 upstream body read、Chat JSON/SSE fallback parse、transform error 映射和 Axum bridge 由 `proxy::response_adapter::codex_transformed_upstream_json_response_to_axum_response` 承接。
...
| `response_processor.rs` | `engine/response_pipeline.rs` | response pipeline 实现已迁到 `proxy/engine/response_pipeline.rs`，非流式 decoded body bridge、response log event dispatch、passthrough usage runtime source、transformed streaming usage runtime source、streaming/non-streaming passthrough response construction 与 Claude/Codex transformed SSE stream wrappers 已归入该模块，旧 `proxy/response_processor.rs` 仅保留兼容 re-export；后续继续把 transformed JSON wrappers 与 shared logged-stream internals 收敛到 pipeline 端口 |
