...
1399a. managed-account runtime source 的 provider-aware 调用面已进一步收敛为结构化输入：`resolve_auth_for_provider`、Copilot dynamic base URL、Copilot live model 和 Claude API format 解析都消费 `ManagedAccount*Input`；`ForwarderAuthSource` / `ForwarderRequestSource` 不再把 provider、body/base_url 与 gating flags 作为散参转手给 runtime source。token 获取、最近成功快照、短窗口失败回退和不可重试失败直出策略保持不变，仍由 core token-cache contract 与 host Tauri runtime source 协作完成。
