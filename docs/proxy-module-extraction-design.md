...
145. Codex Responses 到 Chat 的 role 映射、pending reasoning 拼接/去重、assistant reasoning 回填和 tool_call reasoning_content 占位兜底已迁入 `proxy-core::response_transform`；host `transform_codex_chat` 只保留转换编排与兼容 re-export。
...
150. Codex Responses function_call 到 Chat tool_call 的 id/arguments JSON 组装已迁入 `proxy-core::response_transform::responses_function_call_to_chat_tool_call`；host wrapper 只负责通过 `CodexToolContext` 计算最终 chat tool name。
151. Codex Responses function/custom/tool_search tool_choice 到 Chat function selector 的 JSON 组装已迁入 `proxy-core::response_transform::responses_tool_choice_to_chat_function_selector`；host `transform_codex_chat` 只负责选择或解析最终 chat tool name，并保留未知 tool_choice 原样透传。
