...
1140. Claude Desktop MiMo Anthropic tool-thinking history mutation 已复用 `proxy-core::response_transform::normalize_anthropic_tool_thinking_history`；host `map_proxy_request_model` 只负责在 `provider_should_normalize_mimo_anthropic_thinking_history` 命中后调用 adapter normalizer，core 统一维护 assistant `tool_use` turn 的 missing/empty thinking 补齐、`redacted_thinking` 转换和 `signature` 清理规则，边界测试防止 host 重新持有 MiMo placeholder 常量或 JSON mutation 细节。
