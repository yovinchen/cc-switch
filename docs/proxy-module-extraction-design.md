...
794. stream_check service 的 OpenCode/OpenClaw/Hermes base URL 继续经 `proxy_core_adapter` 的 Provider-aware helper 投影；自定义 User-Agent 与 provider `testConfig` override 投影已改为服务本地读取 Provider meta 并调用 core policy。服务层继续负责 reachability 探测与错误包装。
