...
328. `CircuitBreakerConfig` DTO、默认值和 `AppProxyConfig` 到熔断器配置/失败阈值的投影已迁入 `proxy-core::circuit_breaker_config`；host `proxy::circuit_breaker` 只保留状态机实现并 re-export core 配置类型。
...
348. Codex 官方客户端 User-Agent 前缀判定已迁入 `proxy-core::is_official_codex_client_user_agent`；host `CodexAdapter::is_official_client` 仅保留兼容委托并删除本地 regex 状态。
