...
1182. `CcSwitchProviderRouterConfigSource` 已迁入 `proxy/host/cc_switch/provider_router_config_source.rs`：该 host 模块拥有 ProviderRouter 的 auto-failover、circuit breaker config 和 failure threshold source，直接引用 host `CcSwitchConfigSource` 与 core `CircuitBreakerConfig` contract，并复用 adapter 的 config projection helper；`proxy_core_adapter` 只保留 helper，不再 re-export `CcSwitchConfigSource` host type 给该模块。
...
| ProviderRouter config source | `host/cc_switch/provider_router_config_source.rs` | `CcSwitchProviderRouterConfigSource` 默认实现已迁到 `proxy/host/cc_switch/provider_router_config_source.rs`，直接引用 host `CcSwitchConfigSource` 与 core `CircuitBreakerConfig` contract，负责包装 config source 并生成 ProviderRouter auto-failover、circuit breaker 和 failure threshold config；adapter 仅保留 config projection helper，不再 re-export `CcSwitchConfigSource`，默认装配由 `provider_router_sources.rs` 完成 |
