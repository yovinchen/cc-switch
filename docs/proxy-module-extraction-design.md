...
17. `/proxy/v1/channels/{channel_id}/breakers/reset` 已从 handler 直连 DB/router 改为 `ProxyEngine::reset_channel_health`；host-owned `CcSwitchChannelHealthStore` 负责同时清内存 circuit breaker 和持久化健康状态，adapter 保留 reset/stat/attempt projection helper。
...
610. Channel breaker stats 的 core fact builder、`ChannelBreakerStats` 与 `ChannelHealthReset` fact 类型已从 `proxy_core_adapter` re-export 降为 adapter 私有 import；host `CcSwitchChannelHealthStore` / `CcSwitchProviderRouterHealthStore` 继续直接引用 `proxy_core::api::ports` 的 health fact 类型。`CcSwitchChannelHealthStore` 仍通过 adapter helper 获取 channel DB/router projection 结果；`CcSwitchProviderRouterHealthStore` 已拥有 ProviderRouter provider health DB update projection、router-facing channel health wrapper 和 channel reset wrapper。
...
708. ChannelHealthStore 的 attempt 写库参数投影已移入 `proxy_core_adapter::channel_health_attempt_db_update`，host-owned `CcSwitchChannelHealthStore` 负责调用 DB 写入；ProviderRouter health store 的 provider raw DB update 投影已移到 `proxy/host/cc_switch/provider_router_health_store.rs` owning module，adapter 不再保留 `record_provider_attempt_in_db_source` 公开入口，`proxy_core_host` 只装配对应 store。
...
默认 `CcSwitchChannelHealthStore` 已迁入 `proxy/host/cc_switch/channel_health_store.rs`：host store 持有 DB 与 `ProviderRouter`，直接引用 core `ChannelHealthStore` / channel health fact 类型，继续调用 adapter-owned `record_channel_attempt_in_db_source`、`reset_channel_health_with_router_source` 和 `channel_breaker_stats_with_router_source`，adapter 保留 channel health attempt/reset/stats fact 的 shared projection helper 和 store 兼容 re-export 装配入口。
...
| ProviderRouter health store | `host/cc_switch/provider_router_health_store.rs` | `CcSwitchProviderRouterHealthStore` 默认实现已迁到 `proxy/host/cc_switch/provider_router_health_store.rs`，负责 provider/channel health write 与 channel reset side effect，并拥有 provider DB update projection、router-facing channel health wrapper 与 channel reset wrapper；adapter 仅保留 `CcSwitchChannelHealthStore` 仍需的 channel attempt/reset/stats projection helper，默认装配由 `provider_router_sources.rs` 完成 |
...
- host-owned `CcSwitchChannelHealthStore` 可以通过 adapter helper 写入 channel attempt、reset breaker 并读取 breaker stats，adapter 只保留 projection helper 和装配。
...
- host-owned `CcSwitchProviderRouterHealthStore` 可以通过 core attempt/reset facts 写入 ProviderRouter health side effect，并在 owning module 内拥有 provider DB update projection、router-facing channel health wrapper 与 channel reset wrapper；adapter 只保留 `CcSwitchChannelHealthStore` 仍需的 channel health/reset/stats projection helper。
