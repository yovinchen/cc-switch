...
本轮继续移除 `proxy_core_adapter` 对 takeover marked-state helper 的 re-export：`live_takeover` 直接从 `proxy_core::api::ports` 导入 `proxy_takeover_marked_state_is_reusable` 与 `proxy_takeover_should_restore_existing_backup_before_retakeover`，adapter 测试模块也只在测试作用域内直接验证 core helper，不再通过 adapter 暴露该接管状态策略。
