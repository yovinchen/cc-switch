...
1171. `CcSwitchForwarderProtocolStateSource` 已迁入 `proxy/host/cc_switch/forwarder_protocol_state_source.rs`：该 host 模块拥有 Gemini shadow store、Codex Chat history store、Codex Chat request enrich 和 Claude/Gemini transform replay 的默认协议状态副作用，`proxy_core_adapter` 只保留 `ForwarderProtocolStateSource` trait/input/type alias，runtime 装配私有引用 host factory。
...
1204. forwarder transport/response/failover/runtime source 的默认 factory 与测试 helper 已从 adapter re-export 收口为私有 use 或直接 host 模块引用：transport/response/failover scheduler/runtime state contract 已归入 `proxy/engine/forward_pipeline.rs`，`proxy_core_adapter` 仅继续保留尚未迁出的 protocol/attempt/auth/request source trait/input/type alias，且不再暴露 cc-switch host source factory。
...
| forwarder protocol state source | `host/cc_switch/forwarder_protocol_state_source.rs` | `CcSwitchForwarderProtocolStateSource` 默认实现已迁到 `proxy/host/cc_switch/forwarder_protocol_state_source.rs`，负责 Gemini shadow store、Codex Chat history enrich 与 Claude/Gemini transform replay；adapter 仅保留 trait/input/type alias，生产/测试构造器直接引用 host factory 或由 adapter 私有组装 |
