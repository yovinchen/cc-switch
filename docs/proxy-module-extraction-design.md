...
298. OpenAI Chat Completions SSE -> Anthropic SSE 的状态机已迁入 `proxy-core::OpenAiChatToAnthropicSseState`；host `streaming` 只保留 async byte transport、UTF-8 安全拼接和 SSE data 行分派。
