...
810. Claude transform 的 Responses prompt cache provider facts、OpenAI Chat 显式 prompt cache key 与 Codex fast-mode provider fact 已收成 adapter 私有 helper，公开入口保留 `proxy_core_adapter::provider_claude_transform_request_for_api_format`：Claude adapter 继续负责请求体转换与日志输出，不再直接读取这些 `Provider.meta/settings_config` 字段。
811. Claude transform 的 OpenAI Chat reasoning_content 保留判定已收成 adapter 私有 helper，公开入口保留 `proxy_core_adapter::provider_claude_transform_request_for_api_format`：Claude adapter 继续负责转换时机，只把 provider 与请求体交给 adapter 解析 settings/model facts。
...
本轮继续移除 `proxy_core_adapter` 对 Claude/Copilot prompt-cache helper 的 re-export：`provider_is_copilot_prompt_cache_provider` 与 `provider_claude_responses_prompt_cache_key` 继续作为 host `Provider` 包装入口，底层 `is_copilot_prompt_cache_provider` 与 `resolve_claude_responses_prompt_cache_key` 只在 adapter 内私有引用。
