...
本轮继续收窄 `proxy_core_adapter` 的 legacy channel 内层 projection facade：`legacy_provider_projection_input` 与 `proxy_channel_record_from_legacy_projection` 都改为 adapter 私有；DAO/host 仍只通过 `legacy_channel_migration_preview_from_providers` 获取完整 preview payload。
