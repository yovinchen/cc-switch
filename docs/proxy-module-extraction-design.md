...
264b. `src/lib.rs` 已将 `proxy_core_adapter` 限制为 `#[cfg(test)]` 模块：生产 crate 编译图不再包含 adapter 壳，剩余 `proxy_core_adapter.rs` 仅用于历史边界/单元测试兼容，生产集成点应继续落在 `proxy-core::api`、`proxy/engine/*`、`proxy/host/cc_switch/*` 或 `proxy::response_adapter` 的 owning module。
