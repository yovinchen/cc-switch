...
711. ChannelHealthStore reset 的 app lookup 结果校验、router reset 调用与 reset fact 投影已由 host-owned `CcSwitchChannelHealthStore` 包装；`proxy_core_adapter` 不再保留该桥接 helper，`proxy_core_host` 不再保留该桥接实现。
...
| channel health store | `host/cc_switch/channel_health_store.rs` | `CcSwitchChannelHealthStore` 默认实现已迁到 `proxy/host/cc_switch/channel_health_store.rs`，负责 channel health attempt 写入、channel breaker reset 和 stats 查询，并拥有 attempt/reset/stats projection helper；adapter 不再保留 channel health store helper、factory 装配或 re-export |
| route policy source | `host/cc_switch/route_policy_source.rs` | `CcSwitchRoutePolicySource` 默认实现已迁到 `proxy/host/cc_switch/route_policy_source.rs`，直接引用 core route-policy/source/result contract，负责读取 failover queue 并生成 core-facing `RoutePolicySource`；adapter 仅保留 queue-to-policy projection helper，不再 re-export `RoutePolicySource` port trait 或 `CcSwitchRoutePolicySource` host type，默认装配由 host services / ProviderRouter sources 直接引用 host 模块 |
...
| ProviderRouter health store | `host/cc_switch/provider_router_health_store.rs` | `CcSwitchProviderRouterHealthStore` 默认实现已迁到 `proxy/host/cc_switch/provider_router_health_store.rs`，负责 provider/channel health write 与 channel reset side effect，并拥有 provider DB update projection、router-facing channel health wrapper 与 channel reset wrapper；router-facing channel health wrapper 复用 host `channel_health_store` 的 DB projection helper，默认装配由 `provider_router_sources.rs` 完成 |
...
- host-owned `CcSwitchRoutePolicySource` 可以直接依赖 core route-policy/source/result contract，读取 failover queue 并通过 adapter helper 生成 core-facing `RoutePolicy`；adapter 只保留 projection helper，不再 re-export `RoutePolicySource` port trait 或 `CcSwitchRoutePolicySource` host type。
...
- host-owned `CcSwitchProviderRouterHealthStore` 可以通过 core attempt/reset facts 写入 ProviderRouter health side effect，并在 owning module 内拥有 provider DB update projection、router-facing channel health wrapper 与 channel reset wrapper；adapter 不再保留 ProviderRouter 或 ChannelHealthStore health projection helper。
