...
1033d. Proxy official warning 纯事件构造已改为 `proxy-core::events::proxy_official_warning_event`：live takeover source 继续通过 adapter 查询当前 provider 并触发 Tauri emit，`proxy-official-warning` 事件名与 payload shape 集中在 core，adapter 不再暴露 warning message facade。
