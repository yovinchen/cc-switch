...
778. `RequestContext` 的 selected-provider 未回填 lifecycle 错误与 `unselected:<app>` fallback id 已接入并直接经 `proxy_core::api::errors::{selected_provider_not_applied_message,unselected_provider_fallback_id}` 引用；route result provider source 缺失文案仍由 `proxy_core_adapter::request_context_route_update_from_proxy_result_source` 承接。context 仍负责 DB provider hydration，但错误/fallback contract 不再手写在 host。
...
992a. 本轮继续移除 `proxy_core_adapter` 对 adapter-only core error helper 的 re-export：`config_error_with_context`、`internal_error_with_context`、`invalid_request_error` 与 `selected_provider_missing_from_source_message` 只在 adapter 内私有导入，host-facing 出口仍是 `app_error` / `app_write_error` / `usage_error` 与 route-update projection wrapper。
