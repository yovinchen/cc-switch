...
937. handler 的 Claude transform gate 与 Codex Responses→Chat gate 已继续上移到 `proxy::response_adapter::{claude_response_needs_transform,codex_response_needs_chat_transform}`：handler 只负责按 helper 结果选择响应处理分支，不再直接调用 provider/endpoint 判定 helper。
