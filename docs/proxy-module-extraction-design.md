...
1. HTTP handler 只负责触发 transport adapter 读取 body、鉴权入口、事件流入口、本地 health/status/Claude Desktop model-list/app-list/provider-list/app-models/client-model-catalog JSON 和管理 API path/query/body/state 转发；管理 API auth validation 由 `auth_adapter` 承接，协议入口的 app/tag/prefix/endpoint metadata、endpoint path/query bridge、`RequestContext` 初始化、`ProxyRequest` 构造、`ProxyEngine::handle` dispatch 与 `ProxyResult` 到 HTTP response 的宿主 bridge 由 adapter 承接；Claude/Codex/Gemini protocol handlers 已先降为 `response_adapter` 编排入口调用，`/proxy/v1/events` 的 SSE transport bridge 由 HTTP handler 拥有，channel/group/current-route/route-resolve dry-run，channel CRUD/key/model/test mutation，以及 channel migration/breaker endpoints 的 typed JSON bridge 仍由 `response_adapter` 承接。
