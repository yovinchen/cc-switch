...
29. `UsageRecord` 到 `TokenUsage` 的 host 回填转换、pricing model override/request/response 选择规则已迁入 `proxy-core::usage`；host-owned `CcSwitchUsageSink` 负责读取 DB 计价配置、查询定价、执行 Decimal 成本计算并写入 `UsageLogger`，adapter 仅保留 usage projection helper 与默认 sink re-export。
...
536. core `UsageRecord` 到 host `RequestLog` 的 usage/cost/request-id/missing-pricing 投影已迁入 `proxy_core_adapter::usage_record_to_request_log`；host-owned `CcSwitchUsageSink` 负责 pricing DB 查询、pricing lookup/model source 选择、warning 输出和日志落库，adapter 仅保留 request-log 投影 helper。
...
563. session usage 与 usage stats 服务使用的 cost calculator、pricing、token usage 与 session request id prefix 已改为直接经 `proxy_core::api::usage` 引用；usage sink/database bridge 仍通过 `proxy_core_adapter` 承接 `UsageRecord` 到 host request log/cost breakdown 的投影，host 继续负责 DB 查询、日志文件解析、dedup 与 request log 落库。
...
677. usage request log 字段、成本计算和缺失定价告警文案已新增 `proxy-core::usage_request_log_projection` 并经 host adapter 映射到 `RequestLog`：host-owned `CcSwitchUsageSink` 负责读取计费配置/定价、提供 request_id fallback、执行 host 落库和输出 core 生成的 warning。
...
741. `ProxyResult` 回填 `RequestContext` 的 outbound model、usage route context 与 selected provider hydration 已迁回 `proxy/engine/context.rs` owning module：context 直接消费 core route/usage/error helper 与 host `ForwardAttempt` 映射；`UsageSink` 的 pricing lookup/model source 选择与 missing pricing warning emission 已由 host-owned usage sink 承接，adapter 仅保留 `UsageRecord` 到 `RequestLog` 的投影与 warning message 输出。
...
本轮继续把 `CcSwitchUsageSink` 的 usage pricing lookup、pricing model 解析、request log 投影、缺价告警与 usage log 写入收敛到 host-owned source wrapper，adapter 只保留 projection helper，host services 只装配 sink。
...
1177. `CcSwitchUsageSink` 已迁入 `proxy/host/cc_switch/database_usage_sink.rs`：该 host 模块拥有 `UsageLogger`、pricing lookup/model source 选择、缺失定价 warning、`RequestLog` 写入和 DB 持久化副作用，并直接引用 core `UsageSink` / `ProxyCoreResult` / usage DTO contract；`proxy_core_adapter` 不再 re-export `UsageSink`，只保留 request-log projection helper。
...
| usage sink | `host/cc_switch/database_usage_sink.rs` | `CcSwitchUsageSink` 默认实现已迁到 `proxy/host/cc_switch/database_usage_sink.rs`，直接引用 core `UsageSink` / result / usage DTO contract，负责 `UsageLogger`、pricing lookup、缺失定价 warning、`RequestLog` 写入和 DB 持久化；adapter 仅保留 usage projection helper 和默认 source re-export，不再 re-export `UsageSink` port trait |
...
6. `CcSwitchUsageSink` 作为 host-owned source 直接消费 core usage sink/result/DTO contract，包装 `UsageLogger`、pricing lookup 和日志落库副作用，并由 adapter re-export 默认 sink；写入必须使用完整 `UsageRecord`，不能用简化 hint 直接写账单。
...
- host-owned `CcSwitchUsageSink` 直接依赖 core usage sink/result/DTO contract，写入 `proxy_request_logs` 字段完整；adapter 只保留 projection helper 和默认 sink re-export，不再 re-export `UsageSink` port trait。
