...
本轮继续把 Claude provider `api_format` 投影从 `proxy_core_adapter` 收敛到 `proxy/provider/claude.rs`：Claude provider adapter、RequestContext、forwarder request source 与 adapter 内部 remaining bridge 统一消费 `claude_provider_api_format` owning-module helper，`proxy_core_adapter` 不再定义 `provider_claude_api_format` façade；边界测试允许 Claude provider 直接引用 core API-format resolver，并禁止旧 adapter façade 回流。
