...
10. `CcSwitchForwardPipeline` 已迁回 host-owned optional-runtime wrapper，`CcSwitchProxyServices<R>` 仍是 adapter-owned generic `ProxyServices` 容器，并接入运行中 `ProxyServer` 的共享 router/status/event/history/failover 运行态；`CcSwitchProxyRuntime` 及其 `ProxyServiceRuntimeResources` / `HostForwardRuntime` impl 已迁回 host-owned `proxy/host/cc_switch/proxy_runtime.rs`，`ProxyState` 数据形状也已迁回 host-owned `proxy/host/cc_switch/proxy_state.rs`，负责装配 DB、router、Tauri handle 等不可移植资源，把 `ProxyEngine::handle` 的 `RoutePlan` 映射为 host `ForwardAttempt` 并复用现有 HTTP 转发链。后续重点转为迁移 response pipeline、模型列表接口和外部管理 API。
...
本轮继续把 `CcSwitchProxyServices` 迁为 adapter-owned generic `ProxyServices` 容器；runtime 具体资源字段与 `ProxyServiceRuntimeResources` 实现由 host-owned `CcSwitchProxyRuntime` 维护，adapter 只保留 service container、runtime trait contract 和 `CcSwitchProxyRuntimeServices` alias。
...
本轮继续把 `CcSwitchProxyRuntime` 数据结构及其 `ProxyServiceRuntimeResources` / `HostForwardRuntime` impl 从 `proxy_core_adapter` 迁回 `proxy/host/cc_switch/proxy_runtime.rs`：host 模块重新拥有 CC Switch DB、router、status、event、history、failover、current-target、runtime-source 与 Tauri handle 资源字段，adapter 只保留 runtime/service traits、forward helper、`CcSwitchProxyServices<R>` 容器、`CcSwitchProxyRuntimeServices` alias 和 runtime type re-export。
