...
91b. `response_adapter` 中 Codex Responses→Chat 响应分支 gate 的 Provider-shaped facts 已从 `proxy::provider::codex` 直接调用改为通过 host-owned `provider_projection` 获取；`provider_projection` 负责从 CC Switch `Provider`/TOML/settings 投影 api_format、wire_api、base_url 和 config_base_url facts，并继续委托 `proxy-core::request_url` 的纯 conversion policy。
