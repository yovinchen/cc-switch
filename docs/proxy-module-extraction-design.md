...
本轮继续把 `stop_with_restore` / `stop_with_restore_keep_state` 的 legacy active flag 清理和全量 Live 备份删除收敛到 host service 边界：`live_takeover.rs` 本地 strict/non-strict helper 分别维护手动停止和 keep-state 退出语义，本地 strict 备份删除 helper 保留“删除备份失败”错误文案；全量 provider health reset 仍复用 `proxy_core_adapter::clear_all_provider_health_in_db`。
