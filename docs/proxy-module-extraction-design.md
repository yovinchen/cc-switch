...
561. response adapter 使用的 pure core response/transport DTO、response builder helper、endpoint path/query bridge、Gemini path model extraction、event envelope、management request/response DTO、model catalog response DTO、auth/domain/ports/routing/transforms DTO 与 parser config 已改为直接经 `proxy_core::api::{auth,domain,transport,events,management,model_catalog,ports,routing,transforms,usage}` 引用；`proxy_core_adapter` 只保留其它宿主仍需的 request/result/core response alias、runtime/host 投影 helper、仍被 response pipeline 兼容面使用的 Axum build context 投影和 HTTP handler typed signature 兼容别名，不再作为 response adapter 的 pure transport/parser-config/management/model catalog/auth/domain/ports/routing/transforms DTO 中转入口。host response adapter 继续只负责映射到内部 `ProxyResponse` 与 Axum response/SSE event，response pipeline 实现已迁到 `proxy/engine/response_pipeline.rs`，旧 `proxy/response_processor.rs` 仅保留兼容 re-export。
...
更新：`proxy::response_adapter` 现在直接经 `proxy_core::api::{auth,domain,transport,events,management,model_catalog,ports,routing,transforms,usage}` 消费纯 core DTO/context；上方 response pipeline 长段中提到的 `proxy_core_adapter::CoreResponseBuildFailureContext` 已过期，当前只保留仍被 response pipeline 兼容面使用的 `proxy_core_adapter::AxumResponseBuildErrorContext`，response build failure context 由 response adapter/error mapper 直接从 core transport API 引用。
